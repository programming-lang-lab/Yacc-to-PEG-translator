...
struct vtm { VALUE year; VALUE subsecx; VALUE utc_offset; const char *zone; uint16_t yday:9; uint8_t mon:4; uint8_t mday:5; uint8_t hour:5; uint8_t min:6; uint8_t sec:6; uint8_t wday:3; uint8_t isdst:2;} ;
...
vm_check_ints_blocking(rb_execution_context_t *ec)
{
    ...
    if ((__builtin_expect(!!(rb_threadptr_pending_interrupt_empty_p(th)), 1))) {
 if ((__builtin_expect(!!(!((ec)->interrupt_flag & ~(ec)->interrupt_mask)), 1))) return;
    }
    else {
 th->pending_interrupt_queue_checked = 0;
 __atomic_fetch_or(&((ec)->interrupt_flag), (PENDING_INTERRUPT_MASK), 5);
    }
...
vm_living_thread_num(rb_vm_t *vm)
{
 ...
 static int thread_start_func_2(rb_thread_t *th, VALUE *stack_start);
...
gvl_yield(rb_vm_t *vm, rb_thread_t *th)
{
    ...
    if ((__builtin_expect(!!(vm->gvl.wait_yield), 0))) {
...
gvl_init(rb_vm_t *vm)
{
    ...
    native_cond_initialize(&vm->gvl.cond, 1);
    native_cond_initialize(&vm->gvl.switch_cond, 1);
    native_cond_initialize(&vm->gvl.switch_wait_cond, 1);
    vm->gvl.acquired = 0;
    vm->gvl.waiting = 0;
    vm->gvl.need_yield = 0;
    vm->gvl.wait_yield = 0;
...
{
    gvl_init(vm);
    gvl_acquire(vm, rb_current_thread());
}
...
mutex_debug(const char *msg, void *lock)
{
    if (0) {
 int r;
 static pthread_mutex_t dbglock = { { 0, 0, 0, 0, 0, 0, 0, { 0, 0 } } };
...
native_cond_initialize(rb_nativethread_cond_t *cond, int flags)
{
    ...
    cond->clockid = 0;
    if (flags & 1) {
 r = pthread_condattr_setclock(&attr, 1);
 if (r == 0) {
     cond->clockid = 1;
 }
    }
...
native_cond_timeout(rb_nativethread_cond_t *cond, struct timespec timeout_rel)
{
 ...
 goto out;
    ...
    if (cond->clockid != 0)
 rb_bug("unsupported clockid %ld", (long)cond->clockid);
 ...
 timeout.tv_sec = (time_t) ((((time_t)1) << (sizeof(time_t) * 8 - 2)) | ((((time_t)1) << (sizeof(time_t) * 8 - 2)) - 1));
...
{
    pthread_key_create(&ruby_native_thread_key, ((void*)0));
    ...
    ruby_posix_signal(26, null_func);
}
...
native_thread_init(rb_thread_t *th)
{
    ...
    native_cond_initialize(&nd->sleep_cond, 1);
...
get_stack(void **addr, size_t *size)
{
    ...
    size_t guard = 0;
                            ;
    {int err = (pthread_getattr_np(pthread_self(), &attr)); if (err) return err;};

    {int err = (pthread_attr_getstack(&attr, addr, size)); if (err) return err;};
    (((void)(*addr = (char *)*addr + *size)));




    {int err = (pthread_attr_getguardsize(&attr, &guard)); if (err) return err;};
    *size -= guard;
...
reserve_stack(volatile char *limit, size_t size)
{
    ...
    if (!getrlimit(RLIMIT_STACK, &rl) && rl.rlim_cur == ((__rlim_t) -1))
 return;
    ...
    )
{
 ...
 if (get_stack(&stackaddr, &size) == 0) {
    ...
    native_main_thread.stack_start = __libc_stack_end;
    {
 ...
 size_t size = ( 128 * 1024 * sizeof(VALUE));
...
native_thread_create(rb_thread_t *th)
{
 ...
 if(0)printf("create (use cached thread): %p\n", (void *)th);
    }
    else {
 ...
 {int err = (pthread_attr_init(&attr)); if (err) {rb_bug_errno("pthread_attr_init(&attr)", err);}};


 if(0)printf("create - stack size: %lu\n", (unsigned long)stack_size);
 {int err = (pthread_attr_setstacksize(&attr, stack_size)); if (err) {rb_bug_errno("pthread_attr_setstacksize(&attr, stack_size)", err);}};



 {int err = (pthread_attr_setinheritsched(&attr, PTHREAD_INHERIT_SCHED)); if (err) {rb_bug_errno("pthread_attr_setinheritsched(&attr, PTHREAD_INHERIT_SCHED)", err);}};

 {int err = (pthread_attr_setdetachstate(&attr, PTHREAD_CREATE_DETACHED)); if (err) {rb_bug_errno("pthread_attr_setdetachstate(&attr, PTHREAD_CREATE_DETACHED)", err);}};




 err = pthread_create(&th->thread_id, attrp, thread_start_func_1, th);
 if(0)printf("create: %p (%d)\n", (void *)th, err);

 (void)0;

 {int err = (pthread_attr_destroy(&attr)); if (err) {rb_bug_errno("pthread_attr_destroy(&attr)", err);}};
...
ubf_pthread_cond_signal(void *ptr)
{
    ...
    if(0)printf("ubf_pthread_cond_signal (%p)\n", (void *)th);
...
native_sleep(rb_thread_t *th, struct timeval *timeout_tv)
{
 ...
 timeout = native_cond_timeout(cond, timeout_rel);
    ...
    do { rb_thread_t *_th_stored = rb_current_thread(); do { ((void)0); ; _setjmp ((_th_stored)->ec->machine.regs); ; } while (0); gvl_release(_th_stored->vm);;
    {
 native_mutex_lock(lock);
 th->unblock.func = ubf_pthread_cond_signal;
 th->unblock.arg = th;

 if (((th->ec)->interrupt_flag & ~(th->ec)->interrupt_mask & (PENDING_INTERRUPT_MASK|TRAP_INTERRUPT_MASK))) {

     if(0)printf("native_sleep: interrupted before sleep\n");
 }
 else {
     if (!timeout_tv)
 ...
 native_mutex_unlock(lock);
    ...
    gvl_acquire(_th_stored->vm, _th_stored); do { if ((_th_stored)->vm->running_thread != (_th_stored)) { (_th_stored)->running_time_us = 0; } (void)(ruby_current_execution_context_ptr = (_th_stored)->ec); (_th_stored)->vm->running_thread = (_th_stored); } while (0); } while(0);

    if(0)printf("native_sleep done\n");
}


static struct list_head ubf_list_head = { { &ubf_list_head.n, &ubf_list_head.n } };
...
register_ubf_list(rb_thread_t *th)
{
    ...
    if (list_empty_((struct list_head*)node, "./thread_pthread.c:1164")) {
 native_mutex_lock(&ubf_list_lock);
 list_add_(&ubf_list_head, node, "./thread_pthread.c:1166");
...
unregister_ubf_list(rb_thread_t *th)
{
    ...
    if (!list_empty_((struct list_head*)node, "./thread_pthread.c:1177")) {
 native_mutex_lock(&ubf_list_lock);
 list_del_init_(node, "./thread_pthread.c:1179");
 native_mutex_unlock(&ubf_list_lock);
    }
}
...
{
    if(0)printf("thread_wait_queue_wakeup (%p)\n", ((void *)(th)->thread_id));
    if (th)
 pthread_kill(th->thread_id, 26);
}
...
{
    return list_empty_(&ubf_list_head, "./thread_pthread.c:1217");
}
...
ubf_wakeup_all_threads(void)
{
 ...
 native_mutex_lock(&ubf_list_lock);
 for ((th) = list_node_to_off_(((void)"./thread_pthread.c:1228", (&ubf_list_head))->n.next, (((__builtin_offsetof() + ((int *)0 != (struct list_node *)0))))); list_node_from_off_((void *)(th), (((__builtin_offsetof() + ((int *)0 != (struct list_node *)0))))) != &((&ubf_list_head))->n; (th) = list_node_to_off_(list_node_from_off_((void *)(th), (((__builtin_offsetof() + ((int *)0 != (struct list_node *)0)))))->next, (((__builtin_offsetof() + ((int *)0 != (struct list_node *)0)))))) {
...
static struct {
 ...
 static void async_bug_fd(const char *mesg, int errno_arg, int fd);
static void
...
rb_thread_wakeup_timer_thread_fd(volatile int *fdp)
{
 ...
 if (0) (void)(write((2),("rb_thread_wakeup_timer_thread: write\n"),sizeof("rb_thread_wakeup_timer_thread: write\n")-1)<0);
    }
    else {
...
rb_thread_wakeup_timer_thread(void)
{
 ...
 __atomic_fetch_add(&(timer_thread_pipe.writing), 1, 5);
 rb_thread_wakeup_timer_thread_fd(&timer_thread_pipe.normal[1]);
 __atomic_fetch_sub(&(timer_thread_pipe.writing), 1, 5);
    }
}
...
close_invalidate(volatile int *fdp, const char *msg)
{
    ...
    *fdp = -1;
    if (close(fd) < 0) {
 async_bug_fd(msg, (*__errno_location ()), fd);
    }
}
...
setup_communication_pipe(void)
{
 ...
 int e = (*__errno_location ());
 close_invalidate(&timer_thread_pipe.normal[0],"close_invalidate: normal[0]");
 close_invalidate(&timer_thread_pipe.normal[1],"close_invalidate: normal[1]");
 return e;
    }
...
timer_thread_sleep(rb_global_vm_lock_t* gvl)
{
    ...
    pollfds[0].events = 0x001;
    pollfds[1].fd = timer_thread_pipe.low[0];
    pollfds[1].events = 0x001;

    need_polling = !ubf_threads_empty();

    if (gvl->waiting > 0 || need_polling) {

 result = poll(pollfds, 1, (100 * 1000)/1000);
    }
    else {

 result = poll(pollfds, ((int)(sizeof(pollfds) / sizeof((pollfds)[0]))), -1);
    }
...
native_set_thread_name(rb_thread_t *th)
{
 ...
 VALUE loc;
 if (!!((VALUE)(loc = th->name) != ((VALUE)RUBY_Qnil))) {
     pthread_setname_np(pthread_self(), (!(((struct RBasic*)(loc))->flags & RSTRING_NOEMBED) ? ((struct RString*)(loc))->as.ary : ((struct RString*)(loc))->as.heap.ptr));
 }
 else if (!!((VALUE)(loc = rb_proc_location(th->first_proc)) != ((VALUE)RUBY_Qnil))) {
     const VALUE *ptr = rb_array_const_ptr(loc);
     char *name, *p;
     char buf[16];
     size_t len;
     int n;

     name = (!(((struct RBasic*)(ptr[0]))->flags & RSTRING_NOEMBED) ? ((struct RString*)(ptr[0]))->as.ary : ((struct RString*)(ptr[0]))->as.heap.ptr);
     p = strrchr(name, '/');
     if (p && p[1])
  name = p + 1;

     n = ruby_snprintf(buf, sizeof(buf), "%s:%d", name, rb_num2int_inline(ptr[1]));
...
native_set_another_thread_name(rb_nativethread_id_t thread_id, VALUE name)
{
    ...
    const char *s = "";
    if (!!((VALUE)(name) != ((VALUE)RUBY_Qnil))) s = (!(((struct RBasic*)(name))->flags & RSTRING_NOEMBED) ? ((struct RString*)(name))->as.ary : ((struct RString*)(name))->as.heap.ptr);
    pthread_setname_np(thread_id, s);
...
thread_timer(void *p)
{
    ...
    if (0) (void)(write((2),("start timer thread\n"),sizeof("start timer thread\n")-1)<0);


    pthread_setname_np(pthread_self(), "ruby-timer-thr");
 ...
 timer_thread_sleep(gvl);
    ...
    close_invalidate(&timer_thread_pipe.normal[0],"close_invalidate: normal[0]");
    close_invalidate(&timer_thread_pipe.low[0],"close_invalidate: low[0]");
    ...
    if (0) (void)(write((2),("finish timer thread\n"),sizeof("finish timer thread\n")-1)<0);
    return ((void*)0);
}
...
rb_thread_create_timer_thread(void)
{
...

 {
  ...
  0 != 0
     ...
     if (needs_more_stack) stack_size += 8192;
 ...
 if (err == 22) {
     ...
     err = pthread_create(&timer_thread.id, ((void*)0), thread_timer, &vm->gvl);
 }
 ...
 if (err != 0) {
     ...
     close_invalidate(&timer_thread_pipe.normal[1],"close_invalidate: normal[1]");
     close_invalidate(&timer_thread_pipe.low[0],"close_invalidate: low[0]");
     close_invalidate(&timer_thread_pipe.low[1],"close_invalidate: low[1]");
...
native_stop_timer_thread(void)
{
    ...
    if (0) fprintf(stderr, "stop timer thread\n");
 ...
 while (({ int oldvaldup = ((rb_atomic_t)0); __atomic_compare_exchange_n(&(timer_thread_pipe.writing), &oldvaldup, (0), 0, 5, 5); oldvaldup; })) {
     (void)sched_yield();
 }
 ...
 if (0) fprintf(stderr, "joined timer thread\n");
...
{
    if (0) fprintf(stderr, "reset timer thread\n");
}
...
ruby_stack_overflowed_p(const rb_thread_t *th, const void *addr)
{
 ...
 base = (char *)th->ec->machine.stack_start - ((size));
    }
    else {
 return 0;
    }
...
wakeup_one(struct list_head *head)
{
    ...
    for ((cur) = list_node_to_off_(((void)"./thread_sync.c:20", (head))->n.next, (((__builtin_offsetof() + ((int *)0 != (struct list_node *)0))))), (next) = list_node_to_off_(list_node_from_off_((cur), (((__builtin_offsetof() + ((int *)0 != (struct list_node *)0)))))->next, (((__builtin_offsetof() + ((int *)0 != (struct list_node *)0))))); list_node_from_off_((cur), (((__builtin_offsetof() + ((int *)0 != (struct list_node *)0))))) != &((head))->n; (cur) = (next), (next) = list_node_to_off_(list_node_from_off_((cur), (((__builtin_offsetof() + ((int *)0 != (struct list_node *)0)))))->next, (((__builtin_offsetof() + ((int *)0 != (struct list_node *)0)))))) {
 list_del_init_(&cur->node, "./thread_sync.c:21");
...
wakeup_all(struct list_head *head)
{
    ...
    for ((cur) = list_node_to_off_(((void)"./thread_sync.c:36", (head))->n.next, (((__builtin_offsetof() + ((int *)0 != (struct list_node *)0))))), (next) = list_node_to_off_(list_node_from_off_((cur), (((__builtin_offsetof() + ((int *)0 != (struct list_node *)0)))))->next, (((__builtin_offsetof() + ((int *)0 != (struct list_node *)0))))); list_node_from_off_((cur), (((__builtin_offsetof() + ((int *)0 != (struct list_node *)0))))) != &((head))->n; (cur) = (next), (next) = list_node_to_off_(list_node_from_off_((cur), (((__builtin_offsetof() + ((int *)0 != (struct list_node *)0)))))->next, (((__builtin_offsetof() + ((int *)0 != (struct list_node *)0)))))) {
 list_del_init_(&cur->node, "./thread_sync.c:37");
 if (cur->th->status != THREAD_KILLED) {
     rb_threadptr_interrupt(cur->th);
     cur->th->status = THREAD_RUNNABLE;
 }
...
rb_mutex_num_waiting(rb_mutex_t *mutex)
{
    ...
    for ((w) = list_node_to_off_(((void)"./thread_sync.c:95", (&mutex->waitq))->n.next, (((__builtin_offsetof() + ((int *)0 != (struct list_node *)0))))); list_node_from_off_((void *)(w), (((__builtin_offsetof() + ((int *)0 != (struct list_node *)0))))) != &((&mutex->waitq))->n; (w) = list_node_to_off_(list_node_from_off_((void *)(w), (((__builtin_offsetof() + ((int *)0 != (struct list_node *)0)))))->next, (((__builtin_offsetof() + ((int *)0 != (struct list_node *)0)))))) {
 n++;
    }
...
static const rb_data_type_t mutex_data_type = {
    "mutex",
    {((void*)0), mutex_free, mutex_memsize,},
    0, 0, 1
};
...
mutex_alloc(VALUE klass)
{
    ...
    obj = ({ VALUE data_struct_obj = rb_data_typed_object_zalloc(klass, sizeof(rb_mutex_t), &mutex_data_type); (void)((mutex) = (rb_mutex_t *)(((struct RData*)(data_struct_obj))->data));; data_struct_obj; });
...
{
    rb_mutex_t *mutex;
    (((mutex)) = (rb_mutex_t*)rb_check_typeddata(((self)), (&mutex_data_type)));
    return mutex->th ? ((VALUE)RUBY_Qtrue) : ((VALUE)RUBY_Qfalse);
}
...
{
    rb_mutex_t *mutex;
    VALUE locked = ((VALUE)RUBY_Qfalse);
    (((mutex)) = (rb_mutex_t*)rb_check_typeddata(((self)), (&mutex_data_type)));

    if (mutex->th == 0) {
 rb_thread_t *th = rb_current_thread();
 mutex->th = th;
 locked = ((VALUE)RUBY_Qtrue);
...
static const rb_thread_t *patrol_thread = ((void*)0);
VALUE
rb_mutex_lock(VALUE self)
{
    ...
    if (!(((struct RBasic*)(self))->flags&(((VALUE)RUBY_FL_USER1))) &&
 th->ec->interrupt_mask & TRAP_INTERRUPT_MASK) {
 rb_raise(rb_eThreadError, "can't be called from trap context");
    }

    if (rb_mutex_trylock(self) == ((VALUE)RUBY_Qfalse)) {
 ...
 while (mutex->th != th) {
  ...
  patrol_thread = th;
     ...
     list_add_tail_(&mutex->waitq, &w.node, "./thread_sync.c:272");
     native_sleep(th, timeout);
     list_del_(&w.node, "./thread_sync.c:274");
     if (!mutex->th) {
  mutex->th = th;
     }

     if (patrol_thread == th)
  patrol_thread = ((void*)0);

     th->locking_mutex = ((VALUE)RUBY_Qfalse);
     if (mutex->th && timeout && !((th->ec)->interrupt_flag & ~(th->ec)->interrupt_mask & (PENDING_INTERRUPT_MASK|TRAP_INTERRUPT_MASK))) {
...
rb_mutex_unlock_th(rb_mutex_t *mutex, rb_thread_t volatile *th)
{
 ...
 mutex->th = 0;
 for ((cur) = list_node_to_off_(((void)"./thread_sync.c:336", (&mutex->waitq))->n.next, (((__builtin_offsetof() + ((int *)0 != (struct list_node *)0))))), (next) = list_node_to_off_(list_node_from_off_((cur), (((__builtin_offsetof() + ((int *)0 != (struct list_node *)0)))))->next, (((__builtin_offsetof() + ((int *)0 != (struct list_node *)0))))); list_node_from_off_((cur), (((__builtin_offsetof() + ((int *)0 != (struct list_node *)0))))) != &((&mutex->waitq))->n; (cur) = (next), (next) = list_node_to_off_(list_node_from_off_((cur), (((__builtin_offsetof() + ((int *)0 != (struct list_node *)0)))))->next, (((__builtin_offsetof() + ((int *)0 != (struct list_node *)0)))))) {
     list_del_init_(&cur->node, "./thread_sync.c:337");
...
rb_mutex_unlock(VALUE self)
{
    ...
    err = rb_mutex_unlock_th(mutex, rb_current_thread());
...
rb_mutex_abandon_locking_mutex(rb_thread_t *th)
{
    ...
    (((mutex)) = (rb_mutex_t*)rb_check_typeddata(((th->locking_mutex)), (&mutex_data_type)));
    if (mutex->th == th)
...
rb_mutex_wait_for(VALUE time)
{
    ...
    sleep_timeval(rb_current_thread(), *t, 0);
    return ((VALUE)RUBY_Qnil);
}
...
rb_mutex_sleep(VALUE self, VALUE timeout)
{
    ...
    beg = time(0);
    if (!((VALUE)(timeout) != ((VALUE)RUBY_Qnil))) {
 rb_ensure(rb_mutex_sleep_forever, ((VALUE)RUBY_Qnil), rb_mutex_lock, self);
    }
    else {
 rb_ensure(rb_mutex_wait_for, (VALUE)&t, rb_mutex_lock, self);
    }
    end = time(0) - beg;
    return (((VALUE)(end))<<1 | RUBY_FIXNUM_FLAG);
}
static VALUE
...
rb_mutex_synchronize_m(VALUE self, VALUE args)
{
 ...
 rb_raise(rb_eThreadError, "must be called with a block");
    ...
    return rb_mutex_synchronize(self, rb_yield, ((VALUE)RUBY_Qundef));
}

void rb_mutex_allow_trap(VALUE self, int val)
{
    rb_check_typeddata((VALUE)(self),(&mutex_data_type));

    if (val)
 (void)(((struct RBasic*)(self))->flags |= (((VALUE)RUBY_FL_USER1)));
    else
 (void)(((struct RBasic*)(self))->flags &= ~(VALUE)(((VALUE)RUBY_FL_USER1)));
}




struct rb_queue { struct list_head waitq; const VALUE que; int num_waiting;} ;
...
struct rb_szqueue { struct rb_queue q; int num_waiting_push; struct list_head pushq; long max;} ;
...
static const rb_data_type_t queue_data_type = {
    "queue",
    {queue_mark, ((RUBY_DATA_FUNC)-1), queue_memsize,},
...
queue_alloc(VALUE klass)
{
    ...
    obj = ({ VALUE data_struct_obj = rb_data_typed_object_zalloc(klass, sizeof(struct rb_queue), &queue_data_type); (void)((q) = (struct rb_queue *)(((struct RData*)(data_struct_obj))->data));; data_struct_obj; });
    list_head_init(({
;
; int unaligned_member_access_result = (&(q)->waitq);
; unaligned_member_access_result; }));
    return obj;
}
...
queue_ptr(VALUE obj)
{
    ...
    ((q) = (struct rb_queue*)rb_check_typeddata((obj), (&queue_data_type)));
    return q;
}
...
static const rb_data_type_t szqueue_data_type = {
    "sized_queue",
    {szqueue_mark, ((RUBY_DATA_FUNC)-1), szqueue_memsize,},
    0, 0, 1|RUBY_FL_WB_PROTECTED
};
...
szqueue_alloc(VALUE klass)
{
    ...
    VALUE obj = ({ VALUE data_struct_obj = rb_data_typed_object_zalloc(klass, sizeof(struct rb_szqueue), &szqueue_data_type); (void)((sq) = (struct rb_szqueue *)(((struct RData*)(data_struct_obj))->data));; data_struct_obj; });

    list_head_init(({
;
; int unaligned_member_access_result = (&(sq)->q.waitq);
; unaligned_member_access_result; }));
    list_head_init(({
;
; int unaligned_member_access_result = (&(sq)->pushq);
; unaligned_member_access_result; }));
    return obj;
}
...
szqueue_ptr(VALUE obj)
{
    ...
    ((sq) = (struct rb_szqueue*)rb_check_typeddata((obj), (&szqueue_data_type)));
    return sq;
}
...
{
    if (!( ((RUBY_T_ARRAY) == RUBY_T_FIXNUM) ? (((int)(long)(ary))&RUBY_FIXNUM_FLAG) : ((RUBY_T_ARRAY) == RUBY_T_TRUE) ? ((ary) == ((VALUE)RUBY_Qtrue)) : ((RUBY_T_ARRAY) == RUBY_T_FALSE) ? ((ary) == ((VALUE)RUBY_Qfalse)) : ((RUBY_T_ARRAY) == RUBY_T_NIL) ? ((ary) == ((VALUE)RUBY_Qnil)) : ((RUBY_T_ARRAY) == RUBY_T_UNDEF) ? ((ary) == ((VALUE)RUBY_Qundef)) : ((RUBY_T_ARRAY) == RUBY_T_SYMBOL) ? ((((VALUE)(ary)&~((~(VALUE)0)<<RUBY_SPECIAL_SHIFT)) == RUBY_SYMBOL_FLAG)||(!(((VALUE)(ary) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(ary) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(ary))->flags & RUBY_T_MASK) == (RUBY_T_SYMBOL))) : ((RUBY_T_ARRAY) == RUBY_T_FLOAT) ? ( ((((int)(long)(ary))&RUBY_FLONUM_MASK) == RUBY_FLONUM_FLAG) || (!(((VALUE)(ary) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(ary) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(ary))->flags & RUBY_T_MASK) == RUBY_T_FLOAT)) : (!(((VALUE)(ary) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(ary) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(ary))->flags & RUBY_T_MASK) == (RUBY_T_ARRAY)))) {
 rb_raise(rb_eTypeError, "%+li\v not initialized", obj);
    }
    return ary;
}
...
{
    return rb_array_len(check_array(self, q->que));
}

static int
queue_closed_p(VALUE self)
{
    return (((struct RBasic*)(self))->flags&(((VALUE)RUBY_FL_USER5))) != 0;
}
...
{
    ((queue_length(self, q) == 0) ? (void) (0) : __assert_fail ("queue_length(self, q) == 0", "./thread_sync.c", 672,  __PRETTY_FUNCTION__));
    return ((VALUE)RUBY_Qnil);
}
static VALUE
rb_queue_initialize(VALUE self)
{
    ...
    ({
;
; int unaligned_member_access_result = (rb_obj_write((VALUE)(self), (VALUE *)(&q->que), (VALUE)(ary_buf_new()), "./thread_sync.c", 719));
; unaligned_member_access_result; });
    list_head_init(({
;
; int unaligned_member_access_result = (&(q)->waitq);
...
rb_queue_close(VALUE self)
{
    ...
    if (!queue_closed_p(self)) {
 ((!(((VALUE)(self) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(self) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(self))->flags & RUBY_T_MASK) != RUBY_T_NODE) ? (void)(((struct RBasic*)(self))->flags |= (((VALUE)RUBY_FL_USER5))) : (void)0);
...
{
    return queue_closed_p(self) ? ((VALUE)RUBY_Qtrue) : ((VALUE)RUBY_Qfalse);
}
static VALUE
...
queue_sleep_done(VALUE p)
{
    ...
    list_del_(&qw->w.node, "./thread_sync.c:829");
    qw->as.q->num_waiting--;
...
szqueue_sleep_done(VALUE p)
{
    ...
    list_del_(&qw->w.node, "./thread_sync.c:840");
...
queue_do_pop(VALUE self, struct rb_queue *q, int should_block)
{
 ...
 if (!should_block) {
     ...
     ((rb_array_len(q->que) == 0) ? (void) (0) : __assert_fail ("RARRAY_LEN(q->que) == 0", "./thread_sync.c", 861,  __PRETTY_FUNCTION__));
     ((queue_closed_p(self) == 0) ? (void) (0) : __assert_fail ("queue_closed_p(self) == 0", "./thread_sync.c", 862,  __PRETTY_FUNCTION__));

     qw.w.th = rb_current_thread();
     qw.as.q = q;
     list_add_tail_(&qw.as.q->waitq, &qw.w.node, "./thread_sync.c:866");
     qw.as.q->num_waiting++;

     rb_ensure(queue_sleep, ((VALUE)RUBY_Qfalse), queue_sleep_done, (VALUE)&qw);
 }
    }
...
queue_pop_should_block(int argc, const VALUE *argv)
{
    ...
    if (argc > 0) {
 should_block = !!(((VALUE)(argv[0]) & (VALUE)~((VALUE)RUBY_Qnil)) == 0);
    }
    return should_block;
}
static VALUE
...
{
    return queue_length(self, queue_ptr(self)) == 0 ? ((VALUE)RUBY_Qtrue) : ((VALUE)RUBY_Qfalse);
}
...
{
    return rb_long2num_inline(queue_length(self, queue_ptr(self)));
}
...
rb_queue_num_waiting(VALUE self)
{
    ...
    return (((VALUE)((int)(q->num_waiting)))<<1 | RUBY_FIXNUM_FLAG);
}
static VALUE
rb_szqueue_initialize(VALUE self, VALUE vmax)
{
 ...
 rb_raise(rb_eArgError, "queue size must be positive");
    ...
    ({
;
; int unaligned_member_access_result = (rb_obj_write((VALUE)(self), (VALUE *)(&sq->q.que), (VALUE)(ary_buf_new()), "./thread_sync.c", 992));
; unaligned_member_access_result; });
    list_head_init(({
;
; int unaligned_member_access_result = (&(sq)->q.waitq);
...
{
    return rb_long2num_inline(szqueue_ptr(self)->max);
}
static VALUE
rb_szqueue_max_set(VALUE self, VALUE vmax)
{
    long max = rb_num2long_inline(vmax);
    long diff = 0;
    struct rb_szqueue *sq = szqueue_ptr(self);
...
szqueue_push_should_block(int argc, const VALUE *argv)
{
    ...
    if (argc > 1) {
 should_block = !!(((VALUE)(argv[1]) & (VALUE)~((VALUE)RUBY_Qnil)) == 0);
    }
    return should_block;
}
static VALUE
...
; unaligned_member_access_result; });
     ...
     qw.w.th = rb_current_thread();
     qw.as.sq = sq;
     list_add_tail_(pushq, &qw.w.node, "./thread_sync.c:1108");
     sq->num_waiting_push++;

     rb_ensure(queue_sleep, ((VALUE)RUBY_Qfalse), szqueue_sleep_done, (VALUE)&qw);
 }
    }
...
rb_szqueue_length(VALUE self)
{
    ...
    return rb_long2num_inline(queue_length(self, &sq->q));
}
...
rb_szqueue_num_waiting(VALUE self)
{
    ...
    return (((VALUE)((int)(sq->q.num_waiting + sq->num_waiting_push)))<<1 | RUBY_FIXNUM_FLAG);
}
static VALUE
rb_szqueue_empty_p(VALUE self)
{
    ...
    return queue_length(self, &sq->q) == 0 ? ((VALUE)RUBY_Qtrue) : ((VALUE)RUBY_Qfalse);
}
...
static const rb_data_type_t cv_data_type = {
    "condvar",
    {0, ((RUBY_DATA_FUNC)-1), condvar_memsize,},
    0, 0, 1|RUBY_FL_WB_PROTECTED
};
...
condvar_ptr(VALUE self)
{
    ...
    ((cv) = (struct rb_condvar*)rb_check_typeddata((self), (&cv_data_type)));
...
condvar_alloc(VALUE klass)
{
    ...
    obj = ({ VALUE data_struct_obj = rb_data_typed_object_zalloc(klass, sizeof(struct rb_condvar), &cv_data_type); (void)((cv) = (struct rb_condvar *)(((struct RData*)(data_struct_obj))->data));; data_struct_obj; });
...
{
    list_del_(&w->node, "./thread_sync.c:1310");
...
rb_condvar_wait(int argc, VALUE *argv, VALUE self)
{
    ...
    w.th = rb_current_thread();
    list_add_tail_(&cv->waitq, &w.node, "./thread_sync.c:1338");
...
{
    rb_raise(rb_eTypeError, "can't dump %li\v", rb_obj_class(obj));
    __builtin_unreachable();
}
...
{
    rb_cMutex = define_thread_class(rb_cThread, "Mutex", rb_cObject);
    ...
    rb_cQueue = define_thread_class(rb_cThread, "Queue", rb_cObject);
    ...
    rb_cSizedQueue = define_thread_class(rb_cThread, "SizedQueue", rb_cQueue);
    ...
    rb_cConditionVariable = define_thread_class(rb_cThread, "ConditionVariable", rb_cObject);
    rb_define_alloc_func(rb_cConditionVariable, condvar_alloc);

    id_sleep = (__builtin_constant_p("sleep") ?  ({ static ID rb_intern_id_cache; if (!rb_intern_id_cache) rb_intern_id_cache = rb_intern2((("sleep")), (long)strlen(("sleep"))); (ID) rb_intern_id_cache; }) : rb_intern("sleep"));
...
unblock_function_set(rb_thread_t *th, rb_unblock_function_t *func, void *arg, int fail_if_interrupted)
{
 ...
 if (fail_if_interrupted) {
     if (((th->ec)->interrupt_flag & ~(th->ec)->interrupt_mask)) {
  return FALSE;
     }
 }
 else {
     rb_vm_check_ints(th->ec);
 }

 native_mutex_lock(&th->interrupt_lock);
    } while (!th->ec->raised_flag && ((th->ec)->interrupt_flag & ~(th->ec)->interrupt_mask) &&
...
rb_threadptr_interrupt_common(rb_thread_t *th, int trap)
{
    native_mutex_lock(&th->interrupt_lock);
    if (trap) {
 __atomic_fetch_or(&((th->ec)->interrupt_flag), (TRAP_INTERRUPT_MASK), 5);
    }
    else {
 __atomic_fetch_or(&((th->ec)->interrupt_flag), (PENDING_INTERRUPT_MASK), 5);
    }
    if (th->unblock.func != ((void*)0)) {
...
terminate_all(rb_vm_t *vm, const rb_thread_t *main_thread)
{
    ...
    for ((th) = list_node_to_off_(((void)"thread.c:451", (&vm->living_threads))->n.next, (((__builtin_offsetof() + ((int *)0 != (struct list_node *)0))))); list_node_from_off_((void *)(th), (((__builtin_offsetof() + ((int *)0 != (struct list_node *)0))))) != &((&vm->living_threads))->n; (th) = list_node_to_off_(list_node_from_off_((void *)(th), (((__builtin_offsetof() + ((int *)0 != (struct list_node *)0)))))->next, (((__builtin_offsetof() + ((int *)0 != (struct list_node *)0)))))) {
 if (th != main_thread) {
     if(0)printf("terminate_all: begin (thid: %p, status: %s)\n",
    ((void *)(th)->thread_id), thread_status_name(th, TRUE));
     rb_threadptr_pending_interrupt_enque(th, (((VALUE)(1))<<1 | RUBY_FIXNUM_FLAG));
     rb_threadptr_interrupt(th);
     if(0)printf("terminate_all: end (thid: %p, status: %s)\n",
    ((void *)(th)->thread_id), thread_status_name(th, TRUE));
 }
 else {
     if(0)printf("terminate_all: main thread (%p)\n", (void *)th);
 }
    }
}
...
{
    rb_thread_t *volatile th = rb_current_thread();
 ...
 rb_bug("rb_thread_terminate_all: called by child thread (%p, %p)",
    ...
    if ((__builtin_setjmp((_tag.buf)) ? rb_ec_tag_state((*(int volatile *)&(_ec))) : ((void)(_ec->tag = &_tag), 0)) == RUBY_TAG_NONE) {
      retry:
 if(0)printf("rb_thread_terminate_all (main thread: %p)\n", (void *)th);
 terminate_all(vm, th);
...
thread_cleanup_func_before_exec(void *th_ptr)
{
    ...
    th->ec->machine.stack_start = th->ec->machine.stack_end = ((void*)0);
...
thread_do_start(rb_thread_t *th, VALUE args)
{
    ...
    if (!th->first_func) {
 rb_proc_t *proc;
 (((proc)) = (rb_proc_t*)(((struct RData*)(((th->first_proc))))->data));
 th->ec->errinfo = ((VALUE)RUBY_Qnil);
 th->ec->root_lep = rb_vm_proc_local_ep(th->first_proc);
 th->ec->root_svar = ((VALUE)RUBY_Qfalse);
 do { const rb_event_flag_t flag_arg_ = (0x0400); if ((__builtin_expect(!!(ruby_vm_event_flags & (flag_arg_)), 0))) { rb_exec_event_hook_orig(th->ec, flag_arg_, th->self, 0, 0, 0, ((VALUE)RUBY_Qundef), 0); } } while (0);
 th->value = rb_vm_invoke_proc(th->ec, proc,
          (int)rb_array_len(args), rb_array_const_ptr(args),
          0);
 do { const rb_event_flag_t flag_arg_ = (0x0800); if ((__builtin_expect(!!(ruby_vm_event_flags & (flag_arg_)), 0))) { rb_exec_event_hook_orig(th->ec, flag_arg_, th->self, 0, 0, 0, ((VALUE)RUBY_Qundef), 0); } } while (0);
    }
    else {
...
thread_start_func_2(rb_thread_t *th, VALUE *stack_start)
{
 ...
 rb_bug("thread_start_func_2 must not be used for main thread");
    ...
    if(0)printf("thread start: %p\n", (void *)th);

    gvl_acquire(th->vm, th);
    {
 if(0)printf("thread start (get lock): %p\n", (void *)th);
 do { if ((th)->vm->running_thread != (th)) { (th)->running_time_us = 0; } (void)(ruby_current_execution_context_ptr = (th)->ec); (th)->vm->running_thread = (th); } while (0);

 do { rb_execution_context_t * const _ec = (th->ec); struct rb_vm_tag _tag; _tag.state = RUBY_TAG_NONE; _tag.tag = ((VALUE)RUBY_Qundef); _tag.prev = _ec->tag;;
 if ((state = (__builtin_setjmp((_tag.buf)) ? rb_ec_tag_state((*(int volatile *)&(_ec))) : ((void)(_ec->tag = &_tag), 0))) == RUBY_TAG_NONE) {
     do if (__builtin_setjmp(((th)->root_jmpbuf)) == 0) { thread_do_start(th, args); } else { rb_fiber_start(); } while (0);
 }
 else {
     errinfo = th->ec->errinfo;
     if (state == RUBY_TAG_FATAL) {
  ...
  if (th->report_on_exception) {
      VALUE mesg = rb_thread_to_s(th->self);
       ({ (__builtin_constant_p(" terminated with exception (report_on_exception is true):\n")) ? rb_str_cat((mesg), (" terminated with exception (report_on_exception is true):\n"), (long)strlen(" terminated with exception (report_on_exception is true):\n")) : rb_str_cat_cstr((mesg), (" terminated with exception (report_on_exception is true):\n")); });
      rb_write_error_str(mesg);
      rb_ec_error_print(th->ec, errinfo);
  }
  if (th->vm->thread_abort_on_exception ||
      th->abort_on_exception || !(((VALUE)((*rb_ruby_debug_ptr())) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) {

  }
  else {
      errinfo = ((VALUE)RUBY_Qnil);
  }
     }
     th->value = ((VALUE)RUBY_Qnil);
 }

 th->status = THREAD_KILLED;
 if(0)printf("thread end: %p\n", (void *)th);

 main_th = th->vm->main_thread;
 if (main_th == th) {
     ruby_stop(0);
 }
 if (( ((RUBY_T_OBJECT) == RUBY_T_FIXNUM) ? (((int)(long)(errinfo))&RUBY_FIXNUM_FLAG) : ((RUBY_T_OBJECT) == RUBY_T_TRUE) ? ((errinfo) == ((VALUE)RUBY_Qtrue)) : ((RUBY_T_OBJECT) == RUBY_T_FALSE) ? ((errinfo) == ((VALUE)RUBY_Qfalse)) : ((RUBY_T_OBJECT) == RUBY_T_NIL) ? ((errinfo) == ((VALUE)RUBY_Qnil)) : ((RUBY_T_OBJECT) == RUBY_T_UNDEF) ? ((errinfo) == ((VALUE)RUBY_Qundef)) : ((RUBY_T_OBJECT) == RUBY_T_SYMBOL) ? ((((VALUE)(errinfo)&~((~(VALUE)0)<<RUBY_SPECIAL_SHIFT)) == RUBY_SYMBOL_FLAG)||(!(((VALUE)(errinfo) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(errinfo) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(errinfo))->flags & RUBY_T_MASK) == (RUBY_T_SYMBOL))) : ((RUBY_T_OBJECT) == RUBY_T_FLOAT) ? ( ((((int)(long)(errinfo))&RUBY_FLONUM_MASK) == RUBY_FLONUM_FLAG) || (!(((VALUE)(errinfo) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(errinfo) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(errinfo))->flags & RUBY_T_MASK) == RUBY_T_FLOAT)) : (!(((VALUE)(errinfo) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(errinfo) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(errinfo))->flags & RUBY_T_MASK) == (RUBY_T_OBJECT)))) {
 ...
 if (th->locking_mutex != ((VALUE)RUBY_Qfalse)) {
     rb_bug("thread_start_func_2: locking_mutex must not be set (%p:%lx)",
...
static VALUE
thread_create_core(VALUE thval, VALUE args, VALUE (*fn)())
{
    rb_thread_t *th = rb_thread_ptr(thval), *current_th = rb_current_thread();
    int err;

    if ((!(!(((VALUE)(current_th->thgroup) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(current_th->thgroup) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(current_th->thgroup))->flags & RUBY_T_MASK) != RUBY_T_NODE) || (((struct RBasic*)(current_th->thgroup))->flags&RUBY_FL_FREEZE))) {
 rb_raise(rb_eThreadError,
   "can't start a new thread (frozen ThreadGroup)");
    ...
    th->first_func = fn;
    th->first_proc = fn ? ((VALUE)RUBY_Qfalse) : rb_block_proc();
    th->first_args = args;
    ...
    memset(&(((struct RBasicRaw *)((VALUE)(th->pending_interrupt_mask_stack)))->klass), 0, sizeof(VALUE));
...
thread_s_new(int argc, VALUE *argv, VALUE klass)
{
    ...
    if (rb_current_vm()->main_thread->status == THREAD_KILLED)
 rb_raise(rb_eThreadError, "can't alloc thread");

    rb_obj_call_init(thread, argc, argv);
    th = rb_thread_ptr(thread);
    if (!((th)->first_args != 0)) {
 rb_raise(rb_eThreadError, "uninitialized thread - check `%li\v#initialize'",
   klass);
    }
    return thread;
}
...
thread_initialize(VALUE thread, VALUE args)
{
 ...
 if (!proc || !!(((VALUE)(loc = rb_proc_location(proc)) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) {
     rb_raise(rb_eThreadError, "already initialized thread");
 }
 rb_raise(rb_eThreadError,
   "already initialized thread - %li\v:%li\v",
   (rb_array_const_ptr(loc)[0]), (rb_array_const_ptr(loc)[1]));
    }
    else {
...
thread_join_sleep(VALUE arg)
{
    struct join_arg *p = (struct join_arg *)arg;
    rb_thread_t *target_th = p->target, *th = p->waiting;
    const int forever = p->delay == 1E30;
 ...
 if (forever) {
     ...
     if (now > limit) {
  if(0)printf("thread_join: timeout (thid: %p)\n",
 ...
 if(0)printf("thread_join: interrupted (thid: %p, status: %s)\n",
       ((void *)(target_th)->thread_id), thread_status_name(target_th, TRUE));
    }
    return ((VALUE)RUBY_Qtrue);
}
...
thread_join(rb_thread_t *target_th, double delay)
{
 ...
 rb_raise(rb_eThreadError, "Target thread must not be current thread");
    ...
    if (rb_current_vm()->main_thread == target_th) {
 rb_raise(rb_eThreadError, "Target thread must not be main thread");
    ...
    if(0)printf("thread_join (thid: %p, status: %s)\n",
   ((void *)(target_th)->thread_id), thread_status_name(target_th, TRUE));
 ...
 if (!rb_ensure(thread_join_sleep, (VALUE)&arg,
    ...
    if(0)printf("thread_join: success (thid: %p, status: %s)\n",
   ((void *)(target_th)->thread_id), thread_status_name(target_th, TRUE));

    if (target_th->ec->errinfo != ((VALUE)RUBY_Qnil)) {
 VALUE err = target_th->ec->errinfo;

 if ((((int)(long)(err))&RUBY_FIXNUM_FLAG)) {
     switch (err) {
       case (((VALUE)(RUBY_TAG_FATAL))<<1 | RUBY_FIXNUM_FLAG):
  if(0)printf("thread_join: terminated (thid: %p, status: %s)\n",
        ((void *)(target_th)->thread_id), thread_status_name(target_th, TRUE));


  break;
       default:
  rb_bug("thread_join: Fixnum (%d) should not reach here.", ((int)rb_fix2int((VALUE)(err))));
     }
 }
 else if (( ((RUBY_T_IMEMO) == RUBY_T_FIXNUM) ? (((int)(long)((VALUE)(target_th->ec->errinfo)))&RUBY_FIXNUM_FLAG) : ((RUBY_T_IMEMO) == RUBY_T_TRUE) ? (((VALUE)(target_th->ec->errinfo)) == ((VALUE)RUBY_Qtrue)) : ((RUBY_T_IMEMO) == RUBY_T_FALSE) ? (((VALUE)(target_th->ec->errinfo)) == ((VALUE)RUBY_Qfalse)) : ((RUBY_T_IMEMO) == RUBY_T_NIL) ? (((VALUE)(target_th->ec->errinfo)) == ((VALUE)RUBY_Qnil)) : ((RUBY_T_IMEMO) == RUBY_T_UNDEF) ? (((VALUE)(target_th->ec->errinfo)) == ((VALUE)RUBY_Qundef)) : ((RUBY_T_IMEMO) == RUBY_T_SYMBOL) ? ((((VALUE)((VALUE)(target_th->ec->errinfo))&~((~(VALUE)0)<<RUBY_SPECIAL_SHIFT)) == RUBY_SYMBOL_FLAG)||(!(((VALUE)((VALUE)(target_th->ec->errinfo)) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)((VALUE)(target_th->ec->errinfo)) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)((VALUE)(target_th->ec->errinfo)))->flags & RUBY_T_MASK) == (RUBY_T_SYMBOL))) : ((RUBY_T_IMEMO) == RUBY_T_FLOAT) ? ( ((((int)(long)((VALUE)(target_th->ec->errinfo)))&RUBY_FLONUM_MASK) == RUBY_FLONUM_FLAG) || (!(((VALUE)((VALUE)(target_th->ec->errinfo)) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)((VALUE)(target_th->ec->errinfo)) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)((VALUE)(target_th->ec->errinfo)))->flags & RUBY_T_MASK) == RUBY_T_FLOAT)) : (!(((VALUE)((VALUE)(target_th->ec->errinfo)) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)((VALUE)(target_th->ec->errinfo)) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)((VALUE)(target_th->ec->errinfo)))->flags & RUBY_T_MASK) == (RUBY_T_IMEMO)))) {
...
thread_join_m(int argc, VALUE *argv, VALUE self)
{
    ...
    if (!!((VALUE)(limit) != ((VALUE)RUBY_Qnil))) {
...
double2timeval(double d)
{
    ...
    const double TIMEVAL_SEC_MAX_PLUS_ONE = (2*(double)((time_t) ((((time_t)1) << (sizeof(time_t) * 8 - 2)) | ((((time_t)1) << (sizeof(time_t) * 8 - 2)) - 1))/2+1));

    struct timeval time;

    if (TIMEVAL_SEC_MAX_PLUS_ONE <= d) {
        time.tv_sec = (time_t) ((((time_t)1) << (sizeof(time_t) * 8 - 2)) | ((((time_t)1) << (sizeof(time_t) * 8 - 2)) - 1));
        time.tv_usec = 999999;
    }
    else if (d <= (-(time_t) ((((time_t)1) << (sizeof(time_t) * 8 - 2)) | ((((time_t)1) << (sizeof(time_t) * 8 - 2)) - 1))-1)) {
        time.tv_sec = (-(time_t) ((((time_t)1) << (sizeof(time_t) * 8 - 2)) | ((((time_t)1) << (sizeof(time_t) * 8 - 2)) - 1))-1);
        time.tv_usec = 0;
    }
    else {
...
{
    if ((time_t) ((((time_t)1) << (sizeof(time_t) * 8 - 2)) | ((((time_t)1) << (sizeof(time_t) * 8 - 2)) - 1)) - tv->tv_sec < dst->tv_sec)
        dst->tv_sec = (time_t) ((((time_t)1) << (sizeof(time_t) * 8 - 2)) | ((((time_t)1) << (sizeof(time_t) * 8 - 2)) - 1));
    else
        dst->tv_sec += tv->tv_sec;
    if ((dst->tv_usec += tv->tv_usec) >= 1000000) {
 if (dst->tv_sec == (time_t) ((((time_t)1) << (sizeof(time_t) * 8 - 2)) | ((((time_t)1) << (sizeof(time_t) * 8 - 2)) - 1))) {
            dst->tv_usec = 999999;
 }
 else {
            dst->tv_sec++;
...
timeval_update_expire(struct timeval *tv, const struct timeval *to)
{
    ...
    if(0)printf("timeval_update_expire: %ld.%.6ld > %ld.%.6ld\n",
...
{
    if(0)printf("rb_thread_sleep_forever\n");
    sleep_forever(rb_current_thread(), FALSE, TRUE);
}

void
rb_thread_sleep_deadly(void)
{
    if(0)printf("rb_thread_sleep_deadly\n");
    sleep_forever(rb_current_thread(), TRUE, TRUE);
}

static void
rb_thread_sleep_deadly_allow_spurious_wakeup(void)
{
    if(0)printf("rb_thread_sleep_deadly_allow_spurious_wakeup\n");
    sleep_forever(rb_current_thread(), TRUE, FALSE);
}
...
{
    vm_check_ints_blocking(rb_current_execution_context());
}
...
{
    return (int)((rb_thread_ptr(thval)->ec)->interrupt_flag & ~(rb_thread_ptr(thval)->ec)->interrupt_mask & (PENDING_INTERRUPT_MASK|TRAP_INTERRUPT_MASK));
}

void
rb_thread_sleep(int sec)
{
    rb_thread_wait_for(rb_time_timeval((((VALUE)(sec))<<1 | RUBY_FIXNUM_FLAG)));
}

static void
rb_thread_schedule_limits(uint32_t limits_us)
{
    if(0)printf("rb_thread_schedule\n");
    if (!rb_thread_alone()) {
 rb_thread_t *th = rb_current_thread();

 if (th->running_time_us >= limits_us) {
     if(0)printf("rb_thread_schedule/switch start\n");
     do { ((void)0); ; _setjmp ((th)->ec->machine.regs); ; } while (0);
     gvl_yield(th->vm, th);
     do { if ((th)->vm->running_thread != (th)) { (th)->running_time_us = 0; } (void)(ruby_current_execution_context_ptr = (th)->ec); (th)->vm->running_thread = (th); } while (0);
     if(0)printf("rb_thread_schedule/switch done\n");
 }
    }
}
...
rb_thread_schedule(void)
{
    ...
    rb_vm_check_ints(rb_current_execution_context());
}
        ...
        rb_unblock_function_t *ubf, void *arg, int fail_if_interrupted)
{
 ...
 th->status = THREAD_STOPPED;
 if(0)printf("enter blocking region (%p)\n", (void *)th);
 do { ((void)0); ; _setjmp ((th)->ec->machine.regs); ; } while (0);
 gvl_release(th->vm);
 return TRUE;
    }
...
blocking_region_end(rb_thread_t *th, struct rb_blocking_region_buffer *region)
{
    gvl_acquire(th->vm, th);
    do { if ((th)->vm->running_thread != (th)) { (th)->running_time_us = 0; } (void)(ruby_current_execution_context_ptr = (th)->ec); (th)->vm->running_thread = (th); } while (0);
    if(0)printf("leave blocking region (%p)\n", (void *)th);
   ...
   rb_unblock_function_t *ubf, void *data2, int fail_if_interrupted)
{
    ...
    if (ubf == ((rb_unblock_function_t *)-1) || ubf == ((rb_unblock_function_t *)-1)) {
 ubf = ubf_select;
 data2 = th;
    }

    do { rb_thread_t *__th = rb_current_thread(); struct rb_blocking_region_buffer __region; if (blocking_region_begin(__th, &__region, (ubf), (data2), fail_if_interrupted) || !__builtin_choose_expr(__builtin_constant_p(fail_if_interrupted), (fail_if_interrupted), (TRUE))) { { val = func(data1); saved_errno = (*__errno_location ()); }; blocking_region_end(__th, &__region); }; } while(0);
...
{
    volatile VALUE val = ((VALUE)RUBY_Qundef);
    ...
    list_add_(&rb_ec_vm_ptr(ec)->waiting_fds, &wfd.wfd_node, "thread.c:1477");

    do { rb_execution_context_t * const _ec = (ec); struct rb_vm_tag _tag; _tag.state = RUBY_TAG_NONE; _tag.tag = ((VALUE)RUBY_Qundef); _tag.prev = _ec->tag;;
    if ((state = (__builtin_setjmp((_tag.buf)) ? rb_ec_tag_state((*(int volatile *)&(_ec))) : ((void)(_ec->tag = &_tag), 0))) == RUBY_TAG_NONE) {
 do { rb_thread_t *__th = rb_current_thread(); struct rb_blocking_region_buffer __region; if (blocking_region_begin(__th, &__region, (ubf_select), (rb_ec_thread_ptr(ec)), FALSE) || !__builtin_choose_expr(__builtin_constant_p(FALSE), (FALSE), (TRUE))) { { val = func(data1); saved_errno = (*__errno_location ()); }; blocking_region_end(__th, &__region); }; } while(0);
    ...
    list_del_(&wfd.wfd_node, "thread.c:1489");
...
{
    VALUE mask;
    long mask_stack_len = rb_array_len(th->pending_interrupt_mask_stack);
    const VALUE *mask_stack = rb_array_const_ptr(th->pending_interrupt_mask_stack);
    VALUE mod;
    long i;
 ...
 for (mod = err; mod; mod = RCLASS_SUPER(mod)) {
     ...
     if ((int)(((struct RBasic*)(mod))->flags & RUBY_T_MASK) == RUBY_T_ICLASS) {
  klass = ((struct RBasic*)(mod))->klass;
     }
     else if (mod != ((((struct RClass*)(mod))->ptr)->origin_)) {
  continue;
     }

     if ((sym = rb_hash_aref(mask, klass)) != ((VALUE)RUBY_Qnil)) {
...
{
    return rb_array_len(th->pending_interrupt_queue) == 0;
}
...
{
    int i;
    for (i=0; i<rb_array_len(th->pending_interrupt_queue); i++) {
 VALUE e = (rb_array_const_ptr(th->pending_interrupt_queue)[i]);
...
rb_threadptr_pending_interrupt_deque(rb_thread_t *th, enum handle_interrupt_timing timing)
{
    ...
    for (i=0; i<rb_array_len(th->pending_interrupt_queue); i++) {
 VALUE err = (rb_array_const_ptr(th->pending_interrupt_queue)[i]);

 enum handle_interrupt_timing mask_timing = rb_threadptr_pending_interrupt_check_mask(th, rb_class_of((VALUE)(err)));
...
rb_thread_s_handle_interrupt(VALUE self, VALUE mask_arg)
{
    ...
    volatile VALUE r = ((VALUE)RUBY_Qnil);
    enum ruby_tag_type state;
 ...
 rb_raise(rb_eArgError, "block is needed.");
    ...
    if (!mask) {
 return rb_yield(((VALUE)RUBY_Qnil));
    }
    (void)(((struct RBasic*)(mask))->flags |= RUBY_FL_FREEZE);
...
{
    return rb_thread_pending_interrupt_p(argc, argv, rb_current_thread()->self);
}
...
rb_threadptr_to_kill(rb_thread_t *th)
{
    ...
    th->status = THREAD_RUNNABLE;
    th->to_kill = 1;
    th->ec->errinfo = (((VALUE)(RUBY_TAG_FATAL))<<1 | RUBY_FIXNUM_FLAG);
    rb_ec_tag_jump(th->ec, RUBY_TAG_FATAL);
}
...
threadptr_get_interrupts(rb_thread_t *th)
{
 ...
 old = ({ int oldvaldup = (interrupt); __atomic_compare_exchange_n(&(ec->interrupt_flag), &oldvaldup, (interrupt & ec->interrupt_mask), 0, 5, 5); oldvaldup; });
    } while (old != interrupt);
...
rb_threadptr_execute_interrupts(rb_thread_t *th, int blocking_timing)
{
 ...
 if (pending_interrupt && threadptr_pending_interrupt_active_p(th)) {
     ...
     if(0)printf("rb_thread_execute_interrupts: %ld\n", err);

     if (err == ((VALUE)RUBY_Qundef)) {

     }
     else if (err == (((VALUE)(0))<<1 | RUBY_FIXNUM_FLAG) ||
       err == (((VALUE)(1))<<1 | RUBY_FIXNUM_FLAG) ||
       err == (((VALUE)(RUBY_TAG_FATAL))<<1 | RUBY_FIXNUM_FLAG) ) {
 ...
 if (timer_interrupt) {
  ...
  th->running_time_us += (100 * 1000);

     do { const rb_event_flag_t flag_arg_ = (0x040000); if ((__builtin_expect(!!(ruby_vm_event_flags & (flag_arg_)), 0))) { rb_exec_event_hook_orig(th->ec, flag_arg_, th->ec->cfp->self, 0, 0, 0, ((VALUE)RUBY_Qundef), 0); } } while (0);
...
rb_threadptr_raise(rb_thread_t *target_th, int argc, VALUE *argv)
{
 ...
 return ((VALUE)RUBY_Qnil);
    ...
    rb_ec_setup_exception(rb_current_execution_context(), exc, ((VALUE)RUBY_Qundef));
...
rb_threadptr_signal_raise(rb_thread_t *th, int sig)
{
    ...
    argv[0] = rb_eSignal;
    argv[1] = (((VALUE)(sig))<<1 | RUBY_FIXNUM_FLAG);
...
rb_threadptr_signal_exit(rb_thread_t *th)
{
    ...
    argv[1] =  ({ (__builtin_constant_p("exit")) ? rb_str_new_static(("exit"), (long)strlen("exit")) : rb_str_new_cstr("exit"); });
    rb_threadptr_raise(th->vm->main_thread, 2, argv);
}
...
rb_notify_fd_close(int fd)
{
    ...
    busy = 0;
    for ((wfd) = list_node_to_off_(((void)"thread.c:2230", (&vm->waiting_fds))->n.next, (((__builtin_offsetof() + ((int *)0 != (struct list_node *)0))))); list_node_from_off_((void *)(wfd), (((__builtin_offsetof() + ((int *)0 != (struct list_node *)0))))) != &((&vm->waiting_fds))->n; (wfd) = list_node_to_off_(list_node_from_off_((void *)(wfd), (((__builtin_offsetof() + ((int *)0 != (struct list_node *)0)))))->next, (((__builtin_offsetof() + ((int *)0 != (struct list_node *)0)))))) {
 if (wfd->fd == fd) {
...
thread_raise_m(int argc, VALUE *argv, VALUE self)
{
    ...
    const rb_thread_t *current_th = rb_current_thread();
...
rb_thread_kill(VALUE thread)
{
 ...
 rb_exit(0);
    ...
    if(0)printf("rb_thread_kill: %p (%p)\n", (void *)th, ((void *)(th)->thread_id));

    if (th == rb_current_thread()) {

 rb_threadptr_to_kill(th);
    }
    else {
 threadptr_check_pending_interrupt_queue(th);
 rb_threadptr_pending_interrupt_enque(th, (((VALUE)(0))<<1 | RUBY_FIXNUM_FLAG));
 rb_threadptr_interrupt(th);
    }
    return thread;
}
...
{
    if (!!(((VALUE)(rb_thread_wakeup_alive(thread)) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) {
...
rb_thread_wakeup_alive(VALUE thread)
{
    ...
    if (target_th->status == THREAD_KILLED) return ((VALUE)RUBY_Qnil);
...
rb_thread_list(void)
{
    ...
    for ((th) = list_node_to_off_(((void)"thread.c:2510", (&vm->living_threads))->n.next, (((__builtin_offsetof() + ((int *)0 != (struct list_node *)0))))); list_node_from_off_((void *)(th), (((__builtin_offsetof() + ((int *)0 != (struct list_node *)0))))) != &((&vm->living_threads))->n; (th) = list_node_to_off_(list_node_from_off_((void *)(th), (((__builtin_offsetof() + ((int *)0 != (struct list_node *)0)))))->next, (((__builtin_offsetof() + ((int *)0 != (struct list_node *)0)))))) {
 switch (th->status) {
   case THREAD_RUNNABLE:
   case THREAD_STOPPED:
   case THREAD_STOPPED_FOREVER:
...
rb_thread_main(void)
{
    return rb_current_thread()->vm->main_thread->self;
}
static VALUE
...
{
    return rb_current_thread()->vm->thread_abort_on_exception ? ((VALUE)RUBY_Qtrue) : ((VALUE)RUBY_Qfalse);
}
static VALUE
rb_thread_s_abort_exc_set(VALUE self, VALUE val)
{
    rb_current_thread()->vm->thread_abort_on_exception = !(((VALUE)(val) & (VALUE)~((VALUE)RUBY_Qnil)) == 0);
    return val;
}
static VALUE
rb_thread_abort_exc(VALUE thread)
{
    return rb_thread_ptr(thread)->abort_on_exception ? ((VALUE)RUBY_Qtrue) : ((VALUE)RUBY_Qfalse);
}
static VALUE
rb_thread_abort_exc_set(VALUE thread, VALUE val)
{
    rb_thread_ptr(thread)->abort_on_exception = !(((VALUE)(val) & (VALUE)~((VALUE)RUBY_Qnil)) == 0);
    return val;
}
static VALUE
rb_thread_s_report_exc(void)
{
    return rb_current_thread()->vm->thread_report_on_exception ? ((VALUE)RUBY_Qtrue) : ((VALUE)RUBY_Qfalse);
}
static VALUE
rb_thread_s_report_exc_set(VALUE self, VALUE val)
{
    rb_current_thread()->vm->thread_report_on_exception = !(((VALUE)(val) & (VALUE)~((VALUE)RUBY_Qnil)) == 0);
    return val;
}
static VALUE
rb_thread_report_exc(VALUE thread)
{
    return rb_thread_ptr(thread)->report_on_exception ? ((VALUE)RUBY_Qtrue) : ((VALUE)RUBY_Qfalse);
}
static VALUE
rb_thread_report_exc_set(VALUE thread, VALUE val)
{
    rb_thread_ptr(thread)->report_on_exception = !(((VALUE)(val) & (VALUE)~((VALUE)RUBY_Qnil)) == 0);
    return val;
}
VALUE
rb_thread_group(VALUE thread)
{
    ...
    return group == 0 ? ((VALUE)RUBY_Qnil) : group;
}
...
rb_thread_status(VALUE thread)
{
    ...
    if (rb_threadptr_dead(target_th)) {
 if (!!((VALUE)(target_th->ec->errinfo) != ((VALUE)RUBY_Qnil)) &&
     !(((int)(long)(target_th->ec->errinfo))&RUBY_FIXNUM_FLAG)) {
     return ((VALUE)RUBY_Qnil);
 }
 else {
     return ((VALUE)RUBY_Qfalse);
 ...
 return  ({ (__builtin_constant_p(thread_status_name(target_th, FALSE))) ? rb_str_new_static((thread_status_name(target_th, FALSE)), (long)strlen(thread_status_name(target_th, FALSE))) : rb_str_new_cstr(thread_status_name(target_th, FALSE)); });
    }
}
static VALUE
...
{
    return (((VALUE)((int)(rb_thread_ptr(thread)->ec->safe_level)))<<1 | RUBY_FIXNUM_FLAG);
}
static VALUE
...
rb_thread_to_s(VALUE thread)
{
    ...
    str = rb_sprintf("#<%li\v:%p", cname, (void *)thread);
    if (!!((VALUE)(target_th->name) != ((VALUE)RUBY_Qnil))) {
 rb_str_catf(str, "@%li\v", target_th->name);
    }
    if (!target_th->first_func && target_th->first_proc) {
 VALUE loc = rb_proc_location(target_th->first_proc);
 if (!!((VALUE)(loc) != ((VALUE)RUBY_Qnil))) {
     const VALUE *ptr = rb_array_const_ptr(loc);
     rb_str_catf(str, "@%li\v:%li\v", ptr[0], ptr[1]);
     rb_gc_force_recycle(loc);
 }
    }
    rb_str_catf(str, " %s>", status);
    ( (((!(((VALUE)(str) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(str) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(str))->flags & RUBY_T_MASK) != RUBY_T_NODE) && (int)(((struct RBasic*)(str))->flags & RUBY_T_MASK) != RUBY_T_BIGNUM && (int)(((struct RBasic*)(str))->flags & RUBY_T_MASK) != RUBY_T_FLOAT) && (!(((VALUE)(thread) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(thread) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(thread))->flags & RUBY_T_MASK) != RUBY_T_NODE)) ? (void)(((struct RBasic*)(str))->flags |= ((((struct RBasic*)(thread))->flags&(RUBY_FL_TAINT)))) : (void)0);
...
threadptr_local_aref(rb_thread_t *th, ID id)
{
 ...
 if (local_storage != ((void*)0) && st_lookup(local_storage, id, &val)) {
     return (VALUE)val;
 }
 else {
     return ((VALUE)RUBY_Qnil);
...
rb_thread_fetch(int argc, VALUE *argv, VALUE self)
{
 ...
 rb_exc_raise(rb_key_err_new(rb_sprintf("key not found: %+li\v", key), self, key));
    }
    else {
 return argv[1];
    }
...
threadptr_local_aset(rb_thread_t *th, ID id, VALUE val)
{
 ...
 if (!((VALUE)(val) != ((VALUE)RUBY_Qnil))) {
     if (!local_storage) return ((VALUE)RUBY_Qnil);
     st_delete_wrap(local_storage, id);
     return ((VALUE)RUBY_Qnil);
 }
 else {
     if (local_storage == ((void*)0)) {
...
{
    if ((!(!(((VALUE)(thread) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(thread) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(thread))->flags & RUBY_T_MASK) != RUBY_T_NODE) || (((struct RBasic*)(thread))->flags&RUBY_FL_FREEZE))) {
...
{
    return vm_living_thread_num(rb_current_vm()) == 1;
}
static VALUE
...
rb_thread_variable_p(VALUE thread, VALUE key)
{
    ...
    if (!id) return ((VALUE)RUBY_Qfalse);

    locals = rb_ivar_get(thread, id_locals);

    if (!((struct RHash*)(locals))->ntbl)
        return ((VALUE)RUBY_Qfalse);

    if (st_lookup(((struct RHash*)(locals))->ntbl, (rb_id2sym(id)), 0)) {
 return ((VALUE)RUBY_Qtrue);
    }
...
{
    return (((VALUE)((int)(rb_thread_ptr(thread)->priority)))<<1 | RUBY_FIXNUM_FLAG);
}
static VALUE
rb_thread_priority_set(VALUE thread, VALUE prio)
{
    ...
    priority = rb_num2int_inline(prio);
    if (priority > 3) {
 priority = 3;
    }
    else if (priority < -3) {
 priority = -3;
    ...
    return (((VALUE)((int)(target_th->priority)))<<1 | RUBY_FIXNUM_FLAG);
}
void
rb_fd_init(rb_fdset_t *fds)
{
    fds->maxfd = 0;
    fds->fdset = ((fd_set*)ruby_xmalloc(sizeof(fd_set)));
    do { int __d0, __d1; ; } while (0);
}

void
rb_fd_init_copy(rb_fdset_t *dst, rb_fdset_t *src)
{
    size_t size = (((((src)->maxfd)) + (((8 * (int) sizeof (__fd_mask))) - 1)) / ((8 * (int) sizeof (__fd_mask)))) * sizeof(fd_mask);

    if (size < sizeof(fd_set))
 size = sizeof(fd_set);
    dst->maxfd = src->maxfd;
    dst->fdset = ruby_xmalloc(size);
...
{
    if (fds->fdset) ruby_xfree(fds->fdset);
    fds->maxfd = 0;
    fds->fdset = 0;
}
...
{
    if (fds->fdset)
 memset((fds->fdset), 0, sizeof(fd_mask)*(size_t)((((fds->maxfd) + (((8 * (int) sizeof (__fd_mask))) - 1)) / ((8 * (int) sizeof (__fd_mask))))));
}

static void
rb_fd_resize(int n, rb_fdset_t *fds)
{
    size_t m = (((n + 1) + (((8 * (int) sizeof (__fd_mask))) - 1)) / ((8 * (int) sizeof (__fd_mask)))) * sizeof(fd_mask);
    size_t o = (((fds->maxfd) + (((8 * (int) sizeof (__fd_mask))) - 1)) / ((8 * (int) sizeof (__fd_mask)))) * sizeof(fd_mask);

    if (m < sizeof(fd_set)) m = sizeof(fd_set);
    if (o < sizeof(fd_set)) o = sizeof(fd_set);

    if (m > o) {
 fds->fdset = ruby_xrealloc(fds->fdset, m);
...
{
    rb_fd_resize(n, fds);
    ((void) (((fds->fdset)->fds_bits)[((n) / (8 * (int) sizeof (__fd_mask)))] |= ((__fd_mask) (1UL << ((n) % (8 * (int) sizeof (__fd_mask)))))));
}
...
rb_fd_isset(int n, const rb_fdset_t *fds)
{
    ...
    return ((((fds->fdset)->fds_bits)[((n) / (8 * (int) sizeof (__fd_mask)))] & ((__fd_mask) (1UL << ((n) % (8 * (int) sizeof (__fd_mask)))))) != 0) != 0;
}

void
rb_fd_copy(rb_fdset_t *dst, const fd_set *src, int max)
{
    size_t size = (((max) + (((8 * (int) sizeof (__fd_mask))) - 1)) / ((8 * (int) sizeof (__fd_mask)))) * sizeof(fd_mask);

    if (size < sizeof(fd_set)) size = sizeof(fd_set);
    dst->maxfd = max;
    dst->fdset = ruby_xrealloc(dst->fdset, size);
...
{
    fd_set *r = ((void*)0), *w = ((void*)0), *e = ((void*)0);
    if (readfds) {
   ...
   rb_fdset_t *const exceptfds, struct timeval *timeout)
{
    int  result;
    int lerrno;
    rb_fdset_t  orig_read;
    rb_fdset_t  orig_write;
    rb_fdset_t  orig_except;
    double limit = 0;
 ...
 timeout = &wait_rest;
    ...
    (readfds) ? rb_fd_init_copy(&orig_read, readfds) : ((void)((&orig_read)->fdset = 0), (void)((&orig_read)->maxfd = 0));
    (writefds) ? rb_fd_init_copy(&orig_write, writefds) : ((void)((&orig_write)->fdset = 0), (void)((&orig_write)->maxfd = 0));
    (exceptfds) ? rb_fd_init_copy(&orig_except, exceptfds) : ((void)((&orig_except)->fdset = 0), (void)((&orig_except)->maxfd = 0));


    do {
 lerrno = 0;

 do { rb_thread_t *__th = rb_current_thread(); struct rb_blocking_region_buffer __region; if (blocking_region_begin(__th, &__region, (ubf_select), (th), FALSE) || !__builtin_choose_expr(__builtin_constant_p(FALSE), (FALSE), (TRUE))) { { result = native_fd_select(n, readfds, writefds, exceptfds, timeout, th); if (result < 0) lerrno = (*__errno_location ()); }; blocking_region_end(__th, &__region); }; } while(0);
 ...
 vm_check_ints_blocking(th->ec);
    } while (result < 0 && retryable((*__errno_location ()) = lerrno) && (((readfds) ? rb_fd_dup(readfds, &orig_read) : (void)0), ((writefds) ? rb_fd_dup(writefds, &orig_write) : (void)0), ((exceptfds) ? rb_fd_dup(exceptfds, &orig_except) : (void)0), update_timeval(timeout, limit), TRUE));


    if (readfds) rb_fd_term(&orig_read);
    if (writefds) rb_fd_term(&orig_write);
    if (exceptfds) rb_fd_term(&orig_except);
...
{
    int result = 0;
    int events = read ? 0x001 : 0x004;

    if(0)printf("rb_thread_wait_fd_rw(%d, %s)\n", fd, read ? "read": "write");

    if (fd < 0) {
 rb_raise(rb_eIOError, "closed stream");
    }

    result = rb_wait_for_single_fd(fd, events, ((void*)0));
    if (result < 0) {
 rb_sys_fail(0);
    }

    if(0)printf("rb_thread_wait_fd_rw(%d, %s): done\n", fd, read ? "read": "write");
}
...
rb_wait_for_single_fd(int fd, int events, struct timeval *tv)
{
    ...
    double limit = 0;
    struct timespec ts;
    struct timespec *timeout = ((void*)0);
    rb_thread_t *th = rb_current_thread();
 ...
 fds.revents = 0;
 lerrno = 0;
 do { rb_thread_t *__th = rb_current_thread(); struct rb_blocking_region_buffer __region; if (blocking_region_begin(__th, &__region, (ubf_select), (th), FALSE) || !__builtin_choose_expr(__builtin_constant_p(FALSE), (FALSE), (TRUE))) { { result = ppoll(&fds, 1, timeout, ((void*)0)); if (result < 0) lerrno = (*__errno_location ()); }; blocking_region_end(__th, &__region); }; } while(0);




 vm_check_ints_blocking(th->ec);
    } while (result < 0 && retryable((*__errno_location ()) = lerrno) && (update_timespec(timeout, limit), TRUE));
    if (result < 0) return -1;

    if (fds.revents & 0x020) {
 (*__errno_location ()) = 9;
 return -1;
    }
    ...
    result = 0;
    if (fds.revents & (0x040 | 0x080 | 0x001 | 0x010 | 0x008))
 result |= 0x001;
    if (fds.revents & (0x200 | 0x100 | 0x004 | 0x008))
 result |= 0x004;
    if (fds.revents & (0x002))
 result |= 0x002;
...
timer_thread_function(void *arg)
{
 ...
 __atomic_fetch_or(&((vm->running_thread->ec)->interrupt_flag), (TIMER_INTERRUPT_MASK), 5);
    }
...
{
    if ((timer_thread.created != 0) && native_stop_timer_thread()) {
...
clear_coverage_i(st_data_t key, st_data_t val, st_data_t dummy)
{
    ...
    VALUE branches = (rb_array_const_ptr(coverage)[1]);

    if (lines) {
 for (i = 0; i < rb_array_len(lines); i++) {
     if ((rb_array_const_ptr(lines)[i]) != ((VALUE)RUBY_Qnil)) {
  do { const VALUE _ary = (lines); VALUE *ptr = (VALUE *)((VALUE *)rb_array_const_ptr(_ary)); ({
;
; int unaligned_member_access_result = (rb_obj_write((VALUE)(_ary), (VALUE *)(&ptr[i]), (VALUE)(((((VALUE)(0))<<1 | RUBY_FIXNUM_FLAG))), "thread.c", 4154));
; unaligned_member_access_result; }); ; } while (0);
     }
 }
    }
    if (branches) {
 VALUE counters = (rb_array_const_ptr(branches)[1]);
 for (i = 0; i < rb_array_len(counters); i++) {
     do { const VALUE _ary = (counters); VALUE *ptr = (VALUE *)((VALUE *)rb_array_const_ptr(_ary)); ({
;
; int unaligned_member_access_result = (rb_obj_write((VALUE)(_ary), (VALUE *)(&ptr[i]), (VALUE)(((((VALUE)(0))<<1 | RUBY_FIXNUM_FLAG))), "thread.c", 4161));
; unaligned_member_access_result; }); ; } while (0);
 }
    }
...
rb_thread_atfork_internal(rb_thread_t *th, void (*atfork)(rb_thread_t *, const rb_thread_t *))
{
    ...
    for ((i) = list_node_to_off_(((void)"thread.c:4186", (&vm->living_threads))->n.next, (((__builtin_offsetof() + ((int *)0 != (struct list_node *)0))))); list_node_from_off_((void *)(i), (((__builtin_offsetof() + ((int *)0 != (struct list_node *)0))))) != &((&vm->living_threads))->n; (i) = list_node_to_off_(list_node_from_off_((void *)(i), (((__builtin_offsetof() + ((int *)0 != (struct list_node *)0)))))->next, (((__builtin_offsetof() + ((int *)0 != (struct list_node *)0)))))) {
 atfork(i, th);
    }
...
static const rb_data_type_t thgroup_data_type = {
    "thgroup",
    {((void*)0), ((RUBY_DATA_FUNC)-1), thgroup_memsize,},
    0, 0, 1
};
static VALUE
thgroup_s_alloc(VALUE klass)
{
    ...
    group = ({ VALUE data_struct_obj = rb_data_typed_object_zalloc(klass, sizeof(struct thgroup), &thgroup_data_type); (void)((data) = (struct thgroup *)(((struct RData*)(data_struct_obj))->data));; data_struct_obj; });
    data->enclosed = 0;
    data->group = group;
...
thgroup_list(VALUE group)
{
    ...
    for ((th) = list_node_to_off_(((void)"thread.c:4308", (&vm->living_threads))->n.next, (((__builtin_offsetof() + ((int *)0 != (struct list_node *)0))))); list_node_from_off_((void *)(th), (((__builtin_offsetof() + ((int *)0 != (struct list_node *)0))))) != &((&vm->living_threads))->n; (th) = list_node_to_off_(list_node_from_off_((void *)(th), (((__builtin_offsetof() + ((int *)0 != (struct list_node *)0)))))->next, (((__builtin_offsetof() + ((int *)0 != (struct list_node *)0)))))) {
 if (th->thgroup == group) {
     rb_ary_push(ary, th->self);
 }
    }
...
thgroup_enclose(VALUE group)
{
    ...
    ((data) = (struct thgroup*)rb_check_typeddata((group), (&thgroup_data_type)));
    data->enclosed = 1;
...
thgroup_add(VALUE group, VALUE thread)
{
    ...
    if ((!(!(((VALUE)(group) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(group) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(group))->flags & RUBY_T_MASK) != RUBY_T_NODE) || (((struct RBasic*)(group))->flags&RUBY_FL_FREEZE))) {
 ...
 return ((VALUE)RUBY_Qnil);
    ...
    if ((!(!(((VALUE)(target_th->thgroup) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(target_th->thgroup) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(target_th->thgroup))->flags & RUBY_T_MASK) != RUBY_T_NODE) || (((struct RBasic*)(target_th->thgroup))->flags&RUBY_FL_FREEZE))) {
 rb_raise(rb_eThreadError, "can't move from the frozen thread group");
    }
    ((data) = (struct thgroup*)rb_check_typeddata((target_th->thgroup), (&thgroup_data_type)));
    if (data->enclosed) {
 rb_raise(rb_eThreadError,
...
{
    return rb_data_typed_object_wrap((klass),((void *)mutex_alloc(0)),(&thread_shield_data_type));
}
...
{
    unsigned int w = (int)((((struct RBasic*)(b))->flags&(((VALUE)RUBY_FL_USER0)|((VALUE)RUBY_FL_USER1)|((VALUE)RUBY_FL_USER2)|((VALUE)RUBY_FL_USER3)|((VALUE)RUBY_FL_USER4)|((VALUE)RUBY_FL_USER5)|((VALUE)RUBY_FL_USER6)|((VALUE)RUBY_FL_USER7)|((VALUE)RUBY_FL_USER8)|((VALUE)RUBY_FL_USER9)|((VALUE)RUBY_FL_USER10)|((VALUE)RUBY_FL_USER11)|((VALUE)RUBY_FL_USER12)|((VALUE)RUBY_FL_USER13)|((VALUE)RUBY_FL_USER14)|((VALUE)RUBY_FL_USER15)|((VALUE)RUBY_FL_USER16)|((VALUE)RUBY_FL_USER17)|((VALUE)RUBY_FL_USER18)|((VALUE)RUBY_FL_USER19)))>>(((VALUE)RUBY_FL_USHIFT)));
    w++;
    if (w > (unsigned int)((((VALUE)RUBY_FL_USER0)|((VALUE)RUBY_FL_USER1)|((VALUE)RUBY_FL_USER2)|((VALUE)RUBY_FL_USER3)|((VALUE)RUBY_FL_USER4)|((VALUE)RUBY_FL_USER5)|((VALUE)RUBY_FL_USER6)|((VALUE)RUBY_FL_USER7)|((VALUE)RUBY_FL_USER8)|((VALUE)RUBY_FL_USER9)|((VALUE)RUBY_FL_USER10)|((VALUE)RUBY_FL_USER11)|((VALUE)RUBY_FL_USER12)|((VALUE)RUBY_FL_USER13)|((VALUE)RUBY_FL_USER14)|((VALUE)RUBY_FL_USER15)|((VALUE)RUBY_FL_USER16)|((VALUE)RUBY_FL_USER17)|((VALUE)RUBY_FL_USER18)|((VALUE)RUBY_FL_USER19))>>(((VALUE)RUBY_FL_USHIFT))))
 rb_raise(rb_eRuntimeError, "waiting count overflow");
    ((struct RBasic*)(b))->flags &= ~(((VALUE)RUBY_FL_USER0)|((VALUE)RUBY_FL_USER1)|((VALUE)RUBY_FL_USER2)|((VALUE)RUBY_FL_USER3)|((VALUE)RUBY_FL_USER4)|((VALUE)RUBY_FL_USER5)|((VALUE)RUBY_FL_USER6)|((VALUE)RUBY_FL_USER7)|((VALUE)RUBY_FL_USER8)|((VALUE)RUBY_FL_USER9)|((VALUE)RUBY_FL_USER10)|((VALUE)RUBY_FL_USER11)|((VALUE)RUBY_FL_USER12)|((VALUE)RUBY_FL_USER13)|((VALUE)RUBY_FL_USER14)|((VALUE)RUBY_FL_USER15)|((VALUE)RUBY_FL_USER16)|((VALUE)RUBY_FL_USER17)|((VALUE)RUBY_FL_USER18)|((VALUE)RUBY_FL_USER19));
    ((struct RBasic*)(b))->flags |= ((VALUE)w << (((VALUE)RUBY_FL_USHIFT)));
}
...
rb_thread_shield_new(void)
{
    ...
    rb_mutex_lock((VALUE)(((struct RData*)(thread_shield))->data));
    return thread_shield;
}
VALUE
rb_thread_shield_wait(VALUE self)
{
    VALUE mutex = ((VALUE)rb_check_typeddata((self), &thread_shield_data_type));
    rb_mutex_t *m;

    if (!mutex) return ((VALUE)RUBY_Qfalse);
    (((m)) = (rb_mutex_t*)rb_check_typeddata(((mutex)), (&mutex_data_type)));
    if (m->th == rb_current_thread()) return ((VALUE)RUBY_Qnil);
    rb_thread_shield_waiting_inc(self);
    rb_mutex_lock(mutex);
    rb_thread_shield_waiting_dec(self);
    if ((((struct RData*)(self))->data)) return ((VALUE)RUBY_Qtrue);
    rb_mutex_unlock(mutex);
    return (int)((((struct RBasic*)(self))->flags&(((VALUE)RUBY_FL_USER0)|((VALUE)RUBY_FL_USER1)|((VALUE)RUBY_FL_USER2)|((VALUE)RUBY_FL_USER3)|((VALUE)RUBY_FL_USER4)|((VALUE)RUBY_FL_USER5)|((VALUE)RUBY_FL_USER6)|((VALUE)RUBY_FL_USER7)|((VALUE)RUBY_FL_USER8)|((VALUE)RUBY_FL_USER9)|((VALUE)RUBY_FL_USER10)|((VALUE)RUBY_FL_USER11)|((VALUE)RUBY_FL_USER12)|((VALUE)RUBY_FL_USER13)|((VALUE)RUBY_FL_USER14)|((VALUE)RUBY_FL_USER15)|((VALUE)RUBY_FL_USER16)|((VALUE)RUBY_FL_USER17)|((VALUE)RUBY_FL_USER18)|((VALUE)RUBY_FL_USER19)))>>(((VALUE)RUBY_FL_USHIFT))) > 0 ? ((VALUE)RUBY_Qnil) : ((VALUE)RUBY_Qfalse);
}
...
rb_thread_shield_release(VALUE self)
{
    ...
    rb_mutex_unlock(mutex);
    return (int)((((struct RBasic*)(self))->flags&(((VALUE)RUBY_FL_USER0)|((VALUE)RUBY_FL_USER1)|((VALUE)RUBY_FL_USER2)|((VALUE)RUBY_FL_USER3)|((VALUE)RUBY_FL_USER4)|((VALUE)RUBY_FL_USER5)|((VALUE)RUBY_FL_USER6)|((VALUE)RUBY_FL_USER7)|((VALUE)RUBY_FL_USER8)|((VALUE)RUBY_FL_USER9)|((VALUE)RUBY_FL_USER10)|((VALUE)RUBY_FL_USER11)|((VALUE)RUBY_FL_USER12)|((VALUE)RUBY_FL_USER13)|((VALUE)RUBY_FL_USER14)|((VALUE)RUBY_FL_USER15)|((VALUE)RUBY_FL_USER16)|((VALUE)RUBY_FL_USER17)|((VALUE)RUBY_FL_USER18)|((VALUE)RUBY_FL_USER19)))>>(((VALUE)RUBY_FL_USHIFT))) > 0 ? ((VALUE)RUBY_Qtrue) : ((VALUE)RUBY_Qfalse);
}
...
recursive_list_access(VALUE sym)
{
    ...
    VALUE list;
    if (!((VALUE)(hash) != ((VALUE)RUBY_Qnil)) || !( ((RUBY_T_HASH) == RUBY_T_FIXNUM) ? (((int)(long)(hash))&RUBY_FIXNUM_FLAG) : ((RUBY_T_HASH) == RUBY_T_TRUE) ? ((hash) == ((VALUE)RUBY_Qtrue)) : ((RUBY_T_HASH) == RUBY_T_FALSE) ? ((hash) == ((VALUE)RUBY_Qfalse)) : ((RUBY_T_HASH) == RUBY_T_NIL) ? ((hash) == ((VALUE)RUBY_Qnil)) : ((RUBY_T_HASH) == RUBY_T_UNDEF) ? ((hash) == ((VALUE)RUBY_Qundef)) : ((RUBY_T_HASH) == RUBY_T_SYMBOL) ? ((((VALUE)(hash)&~((~(VALUE)0)<<RUBY_SPECIAL_SHIFT)) == RUBY_SYMBOL_FLAG)||(!(((VALUE)(hash) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(hash) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(hash))->flags & RUBY_T_MASK) == (RUBY_T_SYMBOL))) : ((RUBY_T_HASH) == RUBY_T_FLOAT) ? ( ((((int)(long)(hash))&RUBY_FLONUM_MASK) == RUBY_FLONUM_FLAG) || (!(((VALUE)(hash) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(hash) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(hash))->flags & RUBY_T_MASK) == RUBY_T_FLOAT)) : (!(((VALUE)(hash) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(hash) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(hash))->flags & RUBY_T_MASK) == (RUBY_T_HASH)))) {
 hash = rb_ident_hash_new();
 threadptr_recursive_hash_set(th, hash);
 list = ((VALUE)RUBY_Qnil);
    }
    else {
 list = rb_hash_aref(hash, sym);
    }
    if (!((VALUE)(list) != ((VALUE)RUBY_Qnil)) || !( ((RUBY_T_HASH) == RUBY_T_FIXNUM) ? (((int)(long)(list))&RUBY_FIXNUM_FLAG) : ((RUBY_T_HASH) == RUBY_T_TRUE) ? ((list) == ((VALUE)RUBY_Qtrue)) : ((RUBY_T_HASH) == RUBY_T_FALSE) ? ((list) == ((VALUE)RUBY_Qfalse)) : ((RUBY_T_HASH) == RUBY_T_NIL) ? ((list) == ((VALUE)RUBY_Qnil)) : ((RUBY_T_HASH) == RUBY_T_UNDEF) ? ((list) == ((VALUE)RUBY_Qundef)) : ((RUBY_T_HASH) == RUBY_T_SYMBOL) ? ((((VALUE)(list)&~((~(VALUE)0)<<RUBY_SPECIAL_SHIFT)) == RUBY_SYMBOL_FLAG)||(!(((VALUE)(list) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(list) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(list))->flags & RUBY_T_MASK) == (RUBY_T_SYMBOL))) : ((RUBY_T_HASH) == RUBY_T_FLOAT) ? ( ((((int)(long)(list))&RUBY_FLONUM_MASK) == RUBY_FLONUM_FLAG) || (!(((VALUE)(list) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(list) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(list))->flags & RUBY_T_MASK) == RUBY_T_FLOAT)) : (!(((VALUE)(list) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(list) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(list))->flags & RUBY_T_MASK) == (RUBY_T_HASH)))) {
 list = rb_hash_new();
...
recursive_check(VALUE list, VALUE obj_id, VALUE paired_obj_id)
{
    ...
    VALUE pair_list = rb_hash_lookup2(list, obj_id, ((VALUE)RUBY_Qundef));
    if (pair_list == ((VALUE)RUBY_Qundef))
 return ((VALUE)RUBY_Qfalse);
    if (paired_obj_id) {
 if (!( ((RUBY_T_HASH) == RUBY_T_FIXNUM) ? (((int)(long)(pair_list))&RUBY_FIXNUM_FLAG) : ((RUBY_T_HASH) == RUBY_T_TRUE) ? ((pair_list) == ((VALUE)RUBY_Qtrue)) : ((RUBY_T_HASH) == RUBY_T_FALSE) ? ((pair_list) == ((VALUE)RUBY_Qfalse)) : ((RUBY_T_HASH) == RUBY_T_NIL) ? ((pair_list) == ((VALUE)RUBY_Qnil)) : ((RUBY_T_HASH) == RUBY_T_UNDEF) ? ((pair_list) == ((VALUE)RUBY_Qundef)) : ((RUBY_T_HASH) == RUBY_T_SYMBOL) ? ((((VALUE)(pair_list)&~((~(VALUE)0)<<RUBY_SPECIAL_SHIFT)) == RUBY_SYMBOL_FLAG)||(!(((VALUE)(pair_list) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(pair_list) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(pair_list))->flags & RUBY_T_MASK) == (RUBY_T_SYMBOL))) : ((RUBY_T_HASH) == RUBY_T_FLOAT) ? ( ((((int)(long)(pair_list))&RUBY_FLONUM_MASK) == RUBY_FLONUM_FLAG) || (!(((VALUE)(pair_list) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(pair_list) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(pair_list))->flags & RUBY_T_MASK) == RUBY_T_FLOAT)) : (!(((VALUE)(pair_list) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(pair_list) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(pair_list))->flags & RUBY_T_MASK) == (RUBY_T_HASH)))) {
     if (!((paired_obj_id) == (pair_list)))
  return ((VALUE)RUBY_Qfalse);
 }
 else {
     if (!((VALUE)(rb_hash_lookup(pair_list, paired_obj_id)) != ((VALUE)RUBY_Qnil)))
  return ((VALUE)RUBY_Qfalse);
 }
    }
    return ((VALUE)RUBY_Qtrue);
...
recursive_push(VALUE list, VALUE obj, VALUE paired_obj)
{
    ...
    if (!paired_obj) {
 rb_hash_aset(list, obj, ((VALUE)RUBY_Qtrue));
    }
    else if ((pair_list = rb_hash_lookup2(list, obj, ((VALUE)RUBY_Qundef))) == ((VALUE)RUBY_Qundef)) {
 ...
 if (!( ((RUBY_T_HASH) == RUBY_T_FIXNUM) ? (((int)(long)(pair_list))&RUBY_FIXNUM_FLAG) : ((RUBY_T_HASH) == RUBY_T_TRUE) ? ((pair_list) == ((VALUE)RUBY_Qtrue)) : ((RUBY_T_HASH) == RUBY_T_FALSE) ? ((pair_list) == ((VALUE)RUBY_Qfalse)) : ((RUBY_T_HASH) == RUBY_T_NIL) ? ((pair_list) == ((VALUE)RUBY_Qnil)) : ((RUBY_T_HASH) == RUBY_T_UNDEF) ? ((pair_list) == ((VALUE)RUBY_Qundef)) : ((RUBY_T_HASH) == RUBY_T_SYMBOL) ? ((((VALUE)(pair_list)&~((~(VALUE)0)<<RUBY_SPECIAL_SHIFT)) == RUBY_SYMBOL_FLAG)||(!(((VALUE)(pair_list) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(pair_list) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(pair_list))->flags & RUBY_T_MASK) == (RUBY_T_SYMBOL))) : ((RUBY_T_HASH) == RUBY_T_FLOAT) ? ( ((((int)(long)(pair_list))&RUBY_FLONUM_MASK) == RUBY_FLONUM_FLAG) || (!(((VALUE)(pair_list) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(pair_list) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(pair_list))->flags & RUBY_T_MASK) == RUBY_T_FLOAT)) : (!(((VALUE)(pair_list) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(pair_list) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(pair_list))->flags & RUBY_T_MASK) == (RUBY_T_HASH)))){
     ...
     rb_hash_aset(pair_list, other_paired_obj, ((VALUE)RUBY_Qtrue));
     rb_hash_aset(list, obj, pair_list);
 }
 rb_hash_aset(pair_list, paired_obj, ((VALUE)RUBY_Qtrue));
    }
}
static int
...
{
    if (paired_obj) {
 VALUE pair_list = rb_hash_lookup2(list, obj, ((VALUE)RUBY_Qundef));
 if (pair_list == ((VALUE)RUBY_Qundef)) {
     return 0;
 }
 if (( ((RUBY_T_HASH) == RUBY_T_FIXNUM) ? (((int)(long)(pair_list))&RUBY_FIXNUM_FLAG) : ((RUBY_T_HASH) == RUBY_T_TRUE) ? ((pair_list) == ((VALUE)RUBY_Qtrue)) : ((RUBY_T_HASH) == RUBY_T_FALSE) ? ((pair_list) == ((VALUE)RUBY_Qfalse)) : ((RUBY_T_HASH) == RUBY_T_NIL) ? ((pair_list) == ((VALUE)RUBY_Qnil)) : ((RUBY_T_HASH) == RUBY_T_UNDEF) ? ((pair_list) == ((VALUE)RUBY_Qundef)) : ((RUBY_T_HASH) == RUBY_T_SYMBOL) ? ((((VALUE)(pair_list)&~((~(VALUE)0)<<RUBY_SPECIAL_SHIFT)) == RUBY_SYMBOL_FLAG)||(!(((VALUE)(pair_list) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(pair_list) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(pair_list))->flags & RUBY_T_MASK) == (RUBY_T_SYMBOL))) : ((RUBY_T_HASH) == RUBY_T_FLOAT) ? ( ((((int)(long)(pair_list))&RUBY_FLONUM_MASK) == RUBY_FLONUM_FLAG) || (!(((VALUE)(pair_list) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(pair_list) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(pair_list))->flags & RUBY_T_MASK) == RUBY_T_FLOAT)) : (!(((VALUE)(pair_list) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(pair_list) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(pair_list))->flags & RUBY_T_MASK) == (RUBY_T_HASH)))) {
     rb_hash_delete_entry(pair_list, paired_obj);
     if (!((((struct RHash*)(pair_list))->ntbl ? ((struct RHash*)(pair_list))->ntbl->num_entries : (st_index_t)0) == 0)) {
  return 1;
     }
 }
    }
...
static VALUE
exec_recursive_i(VALUE tag, VALUE data, int argc, const VALUE *argv, VALUE blockarg)
{
...
exec_recursive(VALUE (*func) (VALUE, VALUE, int), VALUE obj, VALUE pairid, VALUE arg, int outer)
{
    ...
    const VALUE sym = mid ? (rb_id2sym(mid)) : (rb_id2sym(idNULL));
    ...
    p.pairid = pairid;
    p.arg = arg;
    outermost = outer && !recursive_check(p.list, (rb_id2sym(recursive_key)), 0);
 ...
 if (outermost) {
     recursive_push(p.list, (rb_id2sym(recursive_key)), 0);
     ...
     if (!recursive_pop(p.list, (rb_id2sym(recursive_key)), 0)) goto invalid;
     if (state != RUBY_TAG_NONE) rb_ec_tag_jump(rb_current_execution_context(), state);
     if (result == p.list) {
  result = (*func)(obj, arg, TRUE);
     }
 }
 else {
     volatile VALUE ret = ((VALUE)RUBY_Qundef);
     recursive_push(p.list, p.objid, p.pairid);
     do { rb_execution_context_t * const _ec = (rb_current_execution_context()); struct rb_vm_tag _tag; _tag.state = RUBY_TAG_NONE; _tag.tag = ((VALUE)RUBY_Qundef); _tag.prev = _ec->tag;;
     if ((state = (__builtin_setjmp((_tag.buf)) ? rb_ec_tag_state((*(int volatile *)&(_ec))) : ((void)(_ec->tag = &_tag), 0))) == RUBY_TAG_NONE) {
  ret = (*func)(obj, arg, FALSE);
     ...
     if (!recursive_pop(p.list, p.objid, p.pairid)) {
       invalid:
  rb_raise(rb_eTypeError, "invalid inspect_tbl pair_list for %+li\v in %+li\v",
...
Init_Thread(void)
{
    ...
    sym_never = (rb_id2sym((__builtin_constant_p("never") ?  (rb_intern2(("never"), (long)strlen("never"))) : (rb_intern)("never"))));
    sym_immediate = (rb_id2sym((__builtin_constant_p("immediate") ?  (rb_intern2(("immediate"), (long)strlen("immediate"))) : (rb_intern)("immediate"))));
    sym_on_blocking = (rb_id2sym((__builtin_constant_p("on_blocking") ?  (rb_intern2(("on_blocking"), (long)strlen("on_blocking"))) : (rb_intern)("on_blocking"))));
    id_locals = (__builtin_constant_p("locals") ?  (rb_intern2(("locals"), (long)strlen("locals"))) : (rb_intern)("locals"));
    ...
    rb_vm_register_special_exception_str(ruby_error_stream_closed, rb_eIOError, rb_usascii_str_new_static(("stream closed in another thread"), (long)(sizeof("stream closed in another thread") - 1)));
 ...
 rb_define_const(cThGroup, "Default", th->thgroup);
    ...
    recursive_key = (__builtin_constant_p("__recursive_key__") ?  (rb_intern2(("__recursive_key__"), (long)strlen("__recursive_key__"))) : (rb_intern)("__recursive_key__"));
...
{
    rb_thread_t *th = 0;
    VALUE sep =  ({ (__builtin_constant_p("\n   ")) ? rb_str_new_static(("\n   "), (long)strlen("\n   ")) : rb_str_new_cstr("\n   "); });

    rb_str_catf(msg, "\n%d threads, %d sleeps current:%p main thread:%p\n",
     vm_living_thread_num(vm), vm->sleeper, rb_current_thread(), vm->main_thread);
    for ((th) = list_node_to_off_(((void)"thread.c:4973", (&vm->living_threads))->n.next, (((__builtin_offsetof() + ((int *)0 != (struct list_node *)0))))); list_node_from_off_((void *)(th), (((__builtin_offsetof() + ((int *)0 != (struct list_node *)0))))) != &((&vm->living_threads))->n; (th) = list_node_to_off_(list_node_from_off_((void *)(th), (((__builtin_offsetof() + ((int *)0 != (struct list_node *)0)))))->next, (((__builtin_offsetof() + ((int *)0 != (struct list_node *)0)))))) {
 rb_str_catf(msg, "* %+li\v\n   rb_thread_t:%p native:%p int:%u",
      th->self, th, ((void *)(th)->thread_id), th->ec->interrupt_flag);
 if (th->locking_mutex) {
     rb_mutex_t *mutex;
     (((mutex)) = (rb_mutex_t*)rb_check_typeddata(((th->locking_mutex)), (&mutex_data_type)));
     rb_str_catf(msg, " mutex:%p cond:%zu",
...
rb_check_deadlock(rb_vm_t *vm)
{
    ...
    if (patrol_thread && patrol_thread != rb_current_thread()) return;

    for ((th) = list_node_to_off_(((void)"thread.c:5006", (&vm->living_threads))->n.next, (((__builtin_offsetof() + ((int *)0 != (struct list_node *)0))))); list_node_from_off_((void *)(th), (((__builtin_offsetof() + ((int *)0 != (struct list_node *)0))))) != &((&vm->living_threads))->n; (th) = list_node_to_off_(list_node_from_off_((void *)(th), (((__builtin_offsetof() + ((int *)0 != (struct list_node *)0)))))->next, (((__builtin_offsetof() + ((int *)0 != (struct list_node *)0)))))) {
 if (th->status != THREAD_STOPPED_FOREVER || ((th->ec)->interrupt_flag & ~(th->ec)->interrupt_mask & (PENDING_INTERRUPT_MASK|TRAP_INTERRUPT_MASK))) {
     found = 1;
 }
     ...
     if (mutex->th == th || (!mutex->th && !list_empty_(&mutex->waitq, "thread.c:5014"))) {
  found = 1;
     }
 }
 if (found)
 ...
 VALUE argv[2];
 argv[0] = rb_eFatal;
 argv[1] =  ({ (__builtin_constant_p("No live threads left. Deadlock?")) ? rb_str_new_static(("No live threads left. Deadlock?"), (long)strlen("No live threads left. Deadlock?")) : rb_str_new_cstr("No live threads left. Deadlock?"); });
...
{
    VALUE coverage = rb_iseq_coverage(rb_current_execution_context()->cfp->iseq);
    if (( ((RUBY_T_ARRAY) == RUBY_T_FIXNUM) ? (((int)(long)(coverage))&RUBY_FIXNUM_FLAG) : ((RUBY_T_ARRAY) == RUBY_T_TRUE) ? ((coverage) == ((VALUE)RUBY_Qtrue)) : ((RUBY_T_ARRAY) == RUBY_T_FALSE) ? ((coverage) == ((VALUE)RUBY_Qfalse)) : ((RUBY_T_ARRAY) == RUBY_T_NIL) ? ((coverage) == ((VALUE)RUBY_Qnil)) : ((RUBY_T_ARRAY) == RUBY_T_UNDEF) ? ((coverage) == ((VALUE)RUBY_Qundef)) : ((RUBY_T_ARRAY) == RUBY_T_SYMBOL) ? ((((VALUE)(coverage)&~((~(VALUE)0)<<RUBY_SPECIAL_SHIFT)) == RUBY_SYMBOL_FLAG)||(!(((VALUE)(coverage) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(coverage) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(coverage))->flags & RUBY_T_MASK) == (RUBY_T_SYMBOL))) : ((RUBY_T_ARRAY) == RUBY_T_FLOAT) ? ( ((((int)(long)(coverage))&RUBY_FLONUM_MASK) == RUBY_FLONUM_FLAG) || (!(((VALUE)(coverage) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(coverage) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(coverage))->flags & RUBY_T_MASK) == RUBY_T_FLOAT)) : (!(((VALUE)(coverage) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(coverage) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(coverage))->flags & RUBY_T_MASK) == (RUBY_T_ARRAY))) && !(((struct RBasic*)(coverage))->klass)) {
 VALUE lines = (rb_array_const_ptr(coverage)[0]);
 if (lines) {
     long line = ((int)rb_fix2int((VALUE)(trace_arg->data))) - 1;
     long count;
     VALUE num;
     if (line >= rb_array_len(lines)) {
  return;
     }
     num = (rb_array_const_ptr(lines)[line]);
     if (!(((int)(long)(num))&RUBY_FIXNUM_FLAG)) return;
     count = ((long)(((long)(num))>>(int)(1))) + 1;
     if (((count) < (9223372036854775807L>>1)+1)) {
  do { const VALUE _ary = (lines); VALUE *ptr = (VALUE *)((VALUE *)rb_array_const_ptr(_ary)); ({
;
; int unaligned_member_access_result = (rb_obj_write((VALUE)(_ary), (VALUE *)(&ptr[line]), (VALUE)(((((VALUE)(count))<<1 | RUBY_FIXNUM_FLAG))), "thread.c", 5049));
; unaligned_member_access_result; }); ; } while (0);
     }
 }
    }
...
update_branch_coverage(VALUE data, const rb_trace_arg_t *trace_arg)
{
 ...
 VALUE branches = (rb_array_const_ptr(coverage)[1]);
 if (branches) {
     long idx = ((int)rb_fix2int((VALUE)(trace_arg->data))), count;
     VALUE counters = (rb_array_const_ptr(branches)[1]);
     VALUE num = (rb_array_const_ptr(counters)[idx]);
     count = ((long)(((long)(num))>>(int)(1))) + 1;
     if (((count) < (9223372036854775807L>>1)+1)) {
  do { const VALUE _ary = (counters); VALUE *ptr = (VALUE *)((VALUE *)rb_array_const_ptr(_ary)); ({
;
; int unaligned_member_access_result = (rb_obj_write((VALUE)(_ary), (VALUE *)(&ptr[idx]), (VALUE)(((((VALUE)(count))<<1 | RUBY_FIXNUM_FLAG))), "thread.c", 5067));
; unaligned_member_access_result; }); ; } while (0);
     }
 }
    }
...
rb_resolve_me_location(const rb_method_entry_t *me, VALUE resolved_location[5])
{
 ...
 first_lineno = (((VALUE)(loc->code_range.first_loc.lineno))<<1 | RUBY_FIXNUM_FLAG);
 first_column = (((VALUE)(loc->code_range.first_loc.column))<<1 | RUBY_FIXNUM_FLAG);
 last_lineno = (((VALUE)(loc->code_range.last_loc.lineno))<<1 | RUBY_FIXNUM_FLAG);
 last_column = (((VALUE)(loc->code_range.last_loc.column))<<1 | RUBY_FIXNUM_FLAG);
 break;
      }
...
{
    const rb_control_frame_t *cfp = rb_current_execution_context()->cfp;
    ...
    count = (((int)(long)(rcount))&RUBY_FIXNUM_FLAG) ? ((long)(((long)(rcount))>>(int)(1))) + 1 : 1;
    if (((count) < (9223372036854775807L>>1)+1)) {
 rb_hash_aset(me2counter, (VALUE) me, (((VALUE)(count))<<1 | RUBY_FIXNUM_FLAG));
    }
}
...
{
    return rb_current_vm()->coverages;
}

void
rb_set_coverages(VALUE coverages, int mode, VALUE me2counter)
{
    rb_current_vm()->coverages = coverages;
    rb_current_vm()->coverage_mode = mode;
    rb_add_event_hook2((rb_event_hook_func_t) update_line_coverage, 0x010000, ((VALUE)RUBY_Qnil), RUBY_EVENT_HOOK_FLAG_SAFE | RUBY_EVENT_HOOK_FLAG_RAW_ARG);
    if (mode & 2) {
 rb_add_event_hook2((rb_event_hook_func_t) update_branch_coverage, 0x020000, ((VALUE)RUBY_Qnil), RUBY_EVENT_HOOK_FLAG_SAFE | RUBY_EVENT_HOOK_FLAG_RAW_ARG);
    }
    if (mode & 4) {
 rb_add_event_hook2((rb_event_hook_func_t) update_method_coverage, 0x0008, me2counter, RUBY_EVENT_HOOK_FLAG_SAFE | RUBY_EVENT_HOOK_FLAG_RAW_ARG);
    }
}
...
rb_default_coverage(int n)
{
    ...
    VALUE lines = ((VALUE)RUBY_Qfalse), branches = ((VALUE)RUBY_Qfalse);
    int mode = rb_current_vm()->coverage_mode;

    if (mode & 1) {
 lines = n > 0 ? rb_ary_tmp_new_fill(n) : rb_ary_tmp_new(0);
    }
    do { const VALUE _ary = (coverage); VALUE *ptr = (VALUE *)((VALUE *)rb_array_const_ptr(_ary)); ({
;
; int unaligned_member_access_result = (rb_obj_write((VALUE)(_ary), (VALUE *)(&ptr[0]), (VALUE)((lines)), "thread.c", 5207));
; unaligned_member_access_result; }); ; } while (0);

    if (mode & 2) {
 branches = rb_ary_tmp_new_fill(2);
 do { const VALUE _ary = (branches); VALUE *ptr = (VALUE *)((VALUE *)rb_array_const_ptr(_ary)); ({
;
; int unaligned_member_access_result = (rb_obj_write((VALUE)(_ary), (VALUE *)(&ptr[0]), (VALUE)((rb_ary_tmp_new(0))), "thread.c", 5223));
; unaligned_member_access_result; }); ; } while (0);

 do { const VALUE _ary = (branches); VALUE *ptr = (VALUE *)((VALUE *)rb_array_const_ptr(_ary)); ({
;
; int unaligned_member_access_result = (rb_obj_write((VALUE)(_ary), (VALUE *)(&ptr[1]), (VALUE)((rb_ary_tmp_new(0))), "thread.c", 5225));
; unaligned_member_access_result; }); ; } while (0);
    }
    do { const VALUE _ary = (coverage); VALUE *ptr = (VALUE *)((VALUE *)rb_array_const_ptr(_ary)); ({
;
; int unaligned_member_access_result = (rb_obj_write((VALUE)(_ary), (VALUE *)(&ptr[1]), (VALUE)((branches)), "thread.c", 5227));
; unaligned_member_access_result; }); ; } while (0);
...
VALUE
rb_uninterruptible(VALUE (*b_proc)(), VALUE data)
{
    VALUE interrupt_mask = rb_ident_hash_new();
    rb_thread_t *cur_th = rb_current_thread();

    rb_hash_aset(interrupt_mask, rb_cObject, sym_never);
    (void)(((struct RBasic*)(interrupt_mask))->flags |= RUBY_FL_FREEZE);
