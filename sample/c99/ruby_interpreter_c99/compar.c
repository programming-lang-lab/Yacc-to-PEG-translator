...
rb_cmperr(VALUE x, VALUE y)
{
 ...
 classname = rb_obj_class(y);
    ...
    rb_raise(rb_eArgError, "comparison of %li\v with %li\v failed",
...
{
    if (recursive) return ((VALUE)RUBY_Qnil);
    return rb_cmp(y, x);
}
...
rb_invcmp(VALUE x, VALUE y)
{
    ...
    if (invcmp == ((VALUE)RUBY_Qundef) || !((VALUE)(invcmp) != ((VALUE)RUBY_Qnil))) {
...
cmp_equal(VALUE x, VALUE y)
{
    ...
    if (!((VALUE)(c) != ((VALUE)RUBY_Qnil))) return ((VALUE)RUBY_Qfalse);
    if (rb_cmpint(c, x, y) == 0) return ((VALUE)RUBY_Qtrue);
...
{
    if (cmpint(x, y) > 0) return ((VALUE)RUBY_Qtrue);
    return ((VALUE)RUBY_Qfalse);
}
static VALUE
...
{
    if (cmpint(x, min) < 0) return ((VALUE)RUBY_Qfalse);
    if (cmpint(x, max) > 0) return ((VALUE)RUBY_Qfalse);
