...
finite(double n)
{
    return !(sizeof ((n)) == sizeof (float) ? __isnanf (n) : sizeof ((n)) == sizeof (double) ? __isnan (n) : __isnanl (n)) && !(sizeof ((n)) == sizeof (float) ? __isinff (n) : sizeof ((n)) == sizeof (double) ? __isinf (n) : __isinfl (n));
}
