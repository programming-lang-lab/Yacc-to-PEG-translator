...
lgamma_r(double x, int *signp)
{
    if (x <= 0) {
        ...
        f = modf(-x, &i);
        if (f == 0.0) {
            *signp = (sizeof ((x)) == sizeof (float) ? __builtin_signbitf (x) : sizeof ((x)) == sizeof (double) ? __builtin_signbit (x) : __builtin_signbitl (x)) ? -1 : 1;
            (*__errno_location ()) = 34;
            return (__builtin_huge_val ());
        }
        *signp = (fmod(i, 2.0) != 0.0) ? 1 : -1;
        s = sin(3.14159265358979324 * f);
        if (s < 0) s = -s;
        return 1.14472988584940017 - log(s) - loggamma(1 - x);
    }
    *signp = 1;
    return loggamma(x);
}
