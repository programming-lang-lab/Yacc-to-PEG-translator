...
extern int swapcontext (ucontext_t *restrict __oucp,
   const ucontext_t *restrict __ucp) ;
...
{
    const char *signame = signo2signm(rb_num2int_inline(signo));
    if (!signame) return ((VALUE)RUBY_Qnil);
    return  ({ (__builtin_constant_p(signame)) ? rb_str_new_static((signame), (long)strlen(signame)) : rb_str_new_cstr(signame); });
}
...
{
    int argnum = 1;
    VALUE sig = ((VALUE)RUBY_Qnil);
    int signo;
    const char *signm;

    if (argc > 0) {
 sig = rb_check_to_integer(argv[0], "to_int");
 if (!!((VALUE)(sig) != ((VALUE)RUBY_Qnil))) argnum = 2;
 else sig = argv[0];
    }
    rb_check_arity(argc, 1, argnum);
    if (argnum == 2) {
 signo = rb_num2int_inline(sig);
 if (signo < 0 || signo > (64 + 1)) {
 ...
 if (((((VALUE)(sig)&~((~(VALUE)0)<<RUBY_SPECIAL_SHIFT)) == RUBY_SYMBOL_FLAG)||(!(((VALUE)(sig) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(sig) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(sig))->flags & RUBY_T_MASK) == (RUBY_T_SYMBOL)))) sig = rb_sym2str(sig); else rb_string_value(&(sig));
 signm = (!(((struct RBasic*)(sig))->flags & RSTRING_NOEMBED) ? ((struct RString*)(sig))->as.ary : ((struct RString*)(sig))->as.heap.ptr);
 ...
 if (!signo) {
     rb_raise(rb_eArgError, "unsupported name `%.*s%li\v'",
       len, signame_prefix, sig);
 }
 sig = rb_sprintf("SIG%s", signm);
    }
    rb_call_super(1, &sig);
    rb_ivar_set(self, ruby_static_id_signo, (((VALUE)((int)(signo)))<<1 | RUBY_FIXNUM_FLAG));
...
{
    return rb_ivar_get(self, ruby_static_id_signo);
}
...
interrupt_init(int argc, VALUE *argv, VALUE self)
{
    ...
    args[0] = (((VALUE)(2))<<1 | RUBY_FIXNUM_FLAG);
...
rb_f_kill(int argc, const VALUE *argv)
{
    ...
    switch (rb_type((VALUE)(argv[0]))) {
      case RUBY_T_FIXNUM:
 sig = ((int)rb_fix2int((VALUE)(argv[0])));
 break;
 ...
 if ((sig = signm2signo(s)) == 0) {
     long ofs = s - (!(((struct RBasic*)(str))->flags & RSTRING_NOEMBED) ? ((struct RString*)(str))->as.ary : ((struct RString*)(str))->as.heap.ptr);
     if (ofs) str = rb_str_subseq(str, ofs, (!(((struct RBasic*)(str))->flags & RSTRING_NOEMBED) ? (long)((((struct RBasic*)(str))->flags >> RSTRING_EMBED_LEN_SHIFT) & (RSTRING_EMBED_LEN_MASK >> RSTRING_EMBED_LEN_SHIFT)) : ((struct RString*)(str))->as.heap.len)-ofs);
     rb_raise(rb_eArgError, "unsupported name `SIG%li\v'", str);
 }
 ...
 break;
    ...
    if (argc <= 1) return (((VALUE)(0))<<1 | RUBY_FIXNUM_FLAG);

    if (sig < 0) {
 sig = -sig;
 for (i=1; i<argc; i++) {
     if (killpg(rb_num2int_inline(argv[i]), sig) < 0)
  rb_sys_fail(0);
 }
    }
    else {
 const pid_t self = (rb_current_thread() == rb_current_vm()->main_thread) ? getpid() : -1;
 int wakeup = 0;

 for (i=1; i<argc; i++) {
     pid_t pid = rb_num2int_inline(argv[i]);
 ...
 if (wakeup) {
     rb_threadptr_check_signal(rb_current_vm()->main_thread);
 }
    }
    rb_thread_execute_interrupts(rb_thread_current());

    return (((VALUE)(i-1))<<1 | RUBY_FIXNUM_FLAG);
}

static struct {
    rb_atomic_t cnt[(64 + 1)];
    rb_atomic_t size;
} signal_buff;
...
static ruby_sighandler_t
ruby_signal(int signum, ruby_sighandler_t handler)
{
    ...
    if (handler == ((__sighandler_t) 1) || handler == ((__sighandler_t) 0)) {
        ...
        sigact.__sigaction_handler.sa_sigaction = (ruby_sigaction_t*)handler;
        sigact.sa_flags = 4;
    }
 ...
 return ((__sighandler_t) -1);
    ...
    if (old.sa_flags & 4)
 handler = (ruby_sighandler_t)old.__sigaction_handler.sa_sigaction;
    else
 handler = old.__sigaction_handler.sa_handler;
    ((__builtin_expect(!!(!!(handler != ((__sighandler_t) -1))), 1)) ? (void)0 : __builtin_unreachable());
    return handler;
}

ruby_sighandler_t
ruby_posix_signal(int signum, ruby_sighandler_t handler)
{
...
{
    ruby_sighandler_t func;

    struct sigaction old;
    (void)0;
    if (sigaction(sig, ((void*)0), &old) < 0) return FALSE;
    func = old.__sigaction_handler.sa_handler;





    if (func == ((__sighandler_t) 1)) return 1;
...
{
    __atomic_fetch_add(&(signal_buff.cnt[sig]), 1, 5);
    __atomic_fetch_add(&(signal_buff.size), 1, 5);
}

static void
sighandler(int sig)
{
    int old_errnum = (*__errno_location ());
...
rb_get_next_signal(void)
{
 ...
 for (i=1; i<(64 + 1); i++) {
  ...
  __atomic_fetch_sub(&(signal_buff.cnt[i]), 1, 5);
  __atomic_fetch_sub(&(signal_buff.size), 1, 5);
  sig = i;
  break;
     }
 }
...
reset_sigmask(int sig)
{
    ...
    (void)(ruby_disable_gc = 0, received_signal = 0);

    sigemptyset(&mask);
    sigaddset(&mask, sig);
    if (pthread_sigmask(1, &mask, ((void*)0))) {
 rb_bug_errno("pthread_sigmask:unblock", (*__errno_location ()));
    }
...
static void
sigbus(int sig , siginfo_t *info, void *ctx)
{
    check_reserved_signal_("BUS", sizeof("BUS")-1);
    ...
    check_stack_overflow(sig, (uintptr_t)info->_sifields._sigfault.si_addr, ctx);

    rb_bug_context(ctx, "Bus Error at %p", info->_sifields._sigfault.si_addr);
}
...
static void
sigsegv(int sig , siginfo_t *info, void *ctx)
{
    check_reserved_signal_("SEGV", sizeof("SEGV")-1);
    check_stack_overflow(sig, (uintptr_t)info->_sifields._sigfault.si_addr, ctx);
    rb_bug_context(ctx, "Segmentation fault at %p", info->_sifields._sigfault.si_addr);
}



static void
sigill(int sig , siginfo_t *info, void *ctx)
{
    check_reserved_signal_("ILL", sizeof("ILL")-1);



    rb_bug_context(ctx, "Illegal instruction at %p", info->_sifields._sigfault.si_addr);
}
...
{
    const char *prev = __atomic_exchange_n(&(received_signal), (name), 5);

    if (prev) {
 ssize_t err ;

 static const char msg1[sizeof(" received in ")-1] = " received in ";
 static const char msg2[sizeof(" handler\n")-1] = " handler\n";
...
signal_exec(VALUE cmd, int safe, int sig)
{
 ...
 VALUE signum = (((VALUE)((int)(sig)))<<1 | RUBY_FIXNUM_FLAG);
 rb_eval_cmd(cmd,  ({ const VALUE args_to_new_ary[] = {signum}; if (__builtin_constant_p(1)) { extern int (*__Static_assert_function (void)) [!!sizeof (struct { int __error_if_negative: (((int)(sizeof(args_to_new_ary) / sizeof((args_to_new_ary)[0]))) == (1)) ? 2 : -1; })]; } rb_ary_new_from_values(((int)(sizeof(args_to_new_ary) / sizeof((args_to_new_ary)[0]))), args_to_new_ary); }), safe);
    }
...
default_handler(int sig)
{
    ...
    switch (sig) {
      ...
      case 7:
        func = (ruby_sighandler_t)sigbus;
        break;


      case 11:
        func = (ruby_sighandler_t)sigsegv;
        break;
...
{
    ruby_sighandler_t func = sighandler;
    VALUE command;

    if (!((VALUE)(*cmd) != ((VALUE)RUBY_Qnil))) {
 func = ((__sighandler_t) 1);
    }
    else {
 command = rb_check_string_type(*cmd);
 if (!((VALUE)(command) != ((VALUE)RUBY_Qnil)) && ((((VALUE)(*cmd)&~((~(VALUE)0)<<RUBY_SPECIAL_SHIFT)) == RUBY_SYMBOL_FLAG)||(!(((VALUE)(*cmd) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(*cmd) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(*cmd))->flags & RUBY_T_MASK) == (RUBY_T_SYMBOL)))) {
     command = rb_sym2str(*cmd);
     if (!command) rb_raise(rb_eArgError, "bad handler");
 }
 if (!!((VALUE)(command) != ((VALUE)RUBY_Qnil))) {
     const char *cptr;
     long len;
     do { rb_string_value(&(command)); rb_check_safe_obj(command);} while (0);
     *cmd = command;
     (!(((struct RBasic*)(command))->flags & RSTRING_NOEMBED) ? ((cptr) = ((struct RString*)(command))->as.ary, (len) = (long)((((struct RBasic*)(command))->flags >> RSTRING_EMBED_LEN_SHIFT) & (RSTRING_EMBED_LEN_MASK >> RSTRING_EMBED_LEN_SHIFT))) : ((cptr) = ((struct RString*)(command))->as.heap.ptr, (len) = ((struct RString*)(command))->as.heap.len));
     switch (len) {
       case 0:
                goto sig_ign;
  break;
...
sig_dfl:
  ...
  break;
     ...
     rb_proc_t *proc;
     (((proc)) = (rb_proc_t*)(((struct RData*)(((*cmd))))->data));
     (void)proc;
 }
    }
...
trap_signm(VALUE vsig)
{
    ...
    switch (rb_type((VALUE)(vsig))) {
      case RUBY_T_FIXNUM:
 sig = ((int)rb_fix2int((VALUE)(vsig)));
 if (sig < 0 || sig >= (64 + 1)) {
 ...
 s = (!(((struct RBasic*)(vsig))->flags & RSTRING_NOEMBED) ? ((struct RString*)(vsig))->as.ary : ((struct RString*)(vsig))->as.heap.ptr);
 goto str_signal;

      default:
 s = rb_string_value_ptr(&(vsig));
 ...
 if (sig == 0 && strcmp(s, "EXIT") != 0) {
     long ofs = s - (!(((struct RBasic*)(vsig))->flags & RSTRING_NOEMBED) ? ((struct RString*)(vsig))->as.ary : ((struct RString*)(vsig))->as.heap.ptr);
     if (ofs) vsig = rb_str_subseq(vsig, ofs, (!(((struct RBasic*)(vsig))->flags & RSTRING_NOEMBED) ? (long)((((struct RBasic*)(vsig))->flags >> RSTRING_EMBED_LEN_SHIFT) & (RSTRING_EMBED_LEN_MASK >> RSTRING_EMBED_LEN_SHIFT)) : ((struct RString*)(vsig))->as.heap.len)-ofs);
     rb_raise(rb_eArgError, "unsupported signal SIG%li\v", vsig);
 }
    }
    return sig;
}
...
static VALUE
trap(int sig, ruby_sighandler_t func, VALUE command)
{
    ruby_sighandler_t oldfunc;
    VALUE oldcmd;
    rb_vm_t *vm = rb_current_vm();
    ...
    if (sig == 0) {
 oldfunc = ((__sighandler_t) -1);
    }
    else {
 oldfunc = ruby_signal(sig, func);
 if (oldfunc == ((__sighandler_t) -1)) rb_sys_fail_str(rb_signo2signm(sig));
    }
    oldcmd = vm->trap_list.cmd[sig];
    switch (oldcmd) {
      case 0:
      case ((VALUE)RUBY_Qtrue):
 if (oldfunc == ((__sighandler_t) 1)) oldcmd =  ({ (__builtin_constant_p("IGNORE")) ? rb_str_new_static(("IGNORE"), (long)strlen("IGNORE")) : rb_str_new_cstr("IGNORE"); });
        else if (oldfunc == ((__sighandler_t) 0)) oldcmd =  ({ (__builtin_constant_p("SYSTEM_DEFAULT")) ? rb_str_new_static(("SYSTEM_DEFAULT"), (long)strlen("SYSTEM_DEFAULT")) : rb_str_new_cstr("SYSTEM_DEFAULT"); });
 else if (oldfunc == sighandler) oldcmd =  ({ (__builtin_constant_p("DEFAULT")) ? rb_str_new_static(("DEFAULT"), (long)strlen("DEFAULT")) : rb_str_new_cstr("DEFAULT"); });
 else oldcmd = ((VALUE)RUBY_Qnil);
 break;
      case ((VALUE)RUBY_Qnil):
 break;
      case ((VALUE)RUBY_Qundef):
 oldcmd =  ({ (__builtin_constant_p("EXIT")) ? rb_str_new_static(("EXIT"), (long)strlen("EXIT")) : rb_str_new_cstr("EXIT"); });
 break;
    }
...
sig_list(void)
{
    ...
    for (sigs = siglist; sigs->signm; sigs++) {
 rb_hash_aset(h,  ({ (__builtin_constant_p(sigs->signm)) ? rb_fstring_new((sigs->signm), (long)strlen(sigs->signm)) : rb_fstring_cstr(sigs->signm); }), (((VALUE)(sigs->signo))<<1 | RUBY_FIXNUM_FLAG));
    }
    return h;
}
...
static int
install_sighandler(int signum, ruby_sighandler_t handler)
{
    ruby_sighandler_t old;

    old = ruby_signal(signum, handler);
    if (old == ((__sighandler_t) -1)) return -1;

    if (old != ((__sighandler_t) 0)) {
...
init_sigchld(int sig)
{
    ...
    oldfunc = ruby_signal(sig, ((__sighandler_t) 0));
    if (oldfunc == ((__sighandler_t) -1)) return -1;
    if (oldfunc != ((__sighandler_t) 0) && oldfunc != ((__sighandler_t) 1)) {
 ruby_signal(sig, oldfunc);
    }
    else {
 rb_current_vm()->trap_list.cmd[sig] = 0;
    }
    return 0;
}
...
ruby_sig_finalize(void)
{
    ...
    oldfunc = ruby_signal(2, ((__sighandler_t) 1));
    if (oldfunc == sighandler) {
 ruby_signal(2, ((__sighandler_t) 0));
    }
}
...
Init_signal(void)
{
    ...
    rb_alias(rb_eSignal, (__builtin_constant_p("signm") ?  (rb_intern2(("signm"), (long)strlen("signm"))) : (rb_intern)("signm")), (__builtin_constant_p("message") ?  (rb_intern2(("message"), (long)strlen("message"))) : (rb_intern)("message")));
    rb_define_method(rb_eInterrupt, "initialize", interrupt_init, -1);


    rb_disable_interrupt();

    do { static const char failed[] = "failed to install SIGINT handler"; if (!(install_sighandler(2, sighandler))) break; if (reserved_signal_p(2)) rb_bug(failed); perror(failed); } while (0);

    do { static const char failed[] = "failed to install SIGHUP handler"; if (!(install_sighandler(1, sighandler))) break; if (reserved_signal_p(1)) rb_bug(failed); perror(failed); } while (0);


    do { static const char failed[] = "failed to install SIGQUIT handler"; if (!(install_sighandler(3, sighandler))) break; if (reserved_signal_p(3)) rb_bug(failed); perror(failed); } while (0);


    do { static const char failed[] = "failed to install SIGTERM handler"; if (!(install_sighandler(15, sighandler))) break; if (reserved_signal_p(15)) rb_bug(failed); perror(failed); } while (0);


    do { static const char failed[] = "failed to install SIGALRM handler"; if (!(install_sighandler(14, sighandler))) break; if (reserved_signal_p(14)) rb_bug(failed); perror(failed); } while (0);


    do { static const char failed[] = "failed to install SIGUSR1 handler"; if (!(install_sighandler(10, sighandler))) break; if (reserved_signal_p(10)) rb_bug(failed); perror(failed); } while (0);


    do { static const char failed[] = "failed to install SIGUSR2 handler"; if (!(install_sighandler(12, sighandler))) break; if (reserved_signal_p(12)) rb_bug(failed); perror(failed); } while (0);


    if (!ruby_enable_coredump) {

 do { static const char failed[] = "failed to install SIGBUS handler"; if (!(install_sighandler(7, (ruby_sighandler_t)sigbus))) break; if (reserved_signal_p(7)) rb_bug(failed); perror(failed); } while (0);


 do { static const char failed[] = "failed to install SIGILL handler"; if (!(install_sighandler(4, (ruby_sighandler_t)sigill))) break; if (reserved_signal_p(4)) rb_bug(failed); perror(failed); } while (0);



 rb_register_sigaltstack(rb_current_thread());

 do { static const char failed[] = "failed to install SIGSEGV handler"; if (!(install_sighandler(11, (ruby_sighandler_t)sigsegv))) break; if (reserved_signal_p(11)) rb_bug(failed); perror(failed); } while (0);

    }

    do { static const char failed[] = "failed to install SIGPIPE handler"; if (!(install_sighandler(13, sig_do_nothing))) break; if (reserved_signal_p(13)) rb_bug(failed); perror(failed); } while (0);


    do { static const char failed[] = "failed to install SIGSYS handler"; if (!(install_sighandler(31, sig_do_nothing))) break; if (reserved_signal_p(31)) rb_bug(failed); perror(failed); } while (0);



    do { static const char failed[] = "failed to install SIGCLD handler"; if (!(init_sigchld(17))) break; if (reserved_signal_p(17)) rb_bug(failed); perror(failed); } while (0);
