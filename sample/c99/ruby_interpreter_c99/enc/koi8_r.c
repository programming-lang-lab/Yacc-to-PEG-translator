...
static const OnigUChar EncKOI8_R_ToLowerCaseTable[256] = {
       ...
       OnigUChar* lower, OnigEncoding enc )
{
  ...
  *lower = EncKOI8_R_ToLowerCaseTable[*p];
  (*pp)++;
  return 1;
}
static int
...
{
  if (code < 256)
    return ((EncKOI8_R_CtypeTable[code] & (1<<(ctype))) != 0);
  else
    return 0;
}
...
const OnigEncodingType OnigEncodingKOI8_R = {
