...
static OnigCodePoint utf32be_mbc_to_code(const OnigUChar* p, const OnigUChar* end, OnigEncoding enc);
static int
utf32be_mbc_enc_len(const OnigUChar* p , const OnigUChar* e,
      OnigEncoding enc)
{
  if (e < p) {
    return (-1);
    ...
    if (!( ((c) <= 0x10ffff) && !((c) < 0x10000 && ((((c) >> 8) & 0xf8) == 0xd8))))
      return (-1);
    return (4);
  }
}
...
static int
utf32be_is_mbc_newline(const OnigUChar* p, const OnigUChar* end,
         OnigEncoding enc )
{
  if (p + 3 < end) {
...
utf32be_mbc_to_code(const OnigUChar* p, const OnigUChar* end ,
      OnigEncoding enc )
{
...
static int
utf32be_code_to_mbc(OnigCodePoint code, OnigUChar *buf,
      OnigEncoding enc )
{
  OnigUChar* p = buf;

  *p++ = (OnigUChar )((code & 0xff000000) >>24);
  *p++ = (OnigUChar )((code & 0xff0000) >>16);
  *p++ = (OnigUChar )((code & 0xff00) >> 8);
...
utf32be_mbc_case_fold(OnigCaseFoldType flag,
        const OnigUChar** pp, const OnigUChar* end, OnigUChar* fold,
        OnigEncoding enc)
{
  const OnigUChar* p = *pp;

  if (((*(p+3)) < 0x80) && *(p+2) == 0 && *(p+1) == 0 && *p == 0) {
    *fold++ = 0;
    *fold++ = 0;
    *fold++ = 0;
    *fold = OnigEncAsciiToLowerCaseTable[*(p+3)];
    *pp += 4;
    return 4;
  }
  else
...
static OnigUChar*
utf32be_left_adjust_char_head(const OnigUChar* start, const OnigUChar* s, const OnigUChar* end,
         OnigEncoding enc )
{
  ptrdiff_t rem;
...
const OnigEncodingType OnigEncodingUTF_32BE = {
  utf32be_mbc_enc_len,
  "UTF-32BE",
  4,
  4,
