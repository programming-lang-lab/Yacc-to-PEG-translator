...
static const OnigUChar EncCP1251_ToLowerCaseTable[256] = {
             ...
             const OnigUChar** pp, const OnigUChar* end , OnigUChar* lower, OnigEncoding enc )
{
  ...
  *lower = EncCP1251_ToLowerCaseTable[*p];
  (*pp)++;
  return 1;
}
...
{
  if (code < 256)
    return ((EncCP1251_CtypeTable[code] & (1<<(ctype))) != 0);
  else
    return 0;
}
  ...
  const struct OnigEncodingTypeST* enc)
{
  ...
  while (*pp < end && to < to_end) {
    code = *(*pp)++;
    if ((EncCP1251_CtypeTable[code] & (1<< 10))
      && (flags & ((1<<14) | (1<<19)))) {
      flags |= (1<<18);
      code = EncCP1251_ToLowerCaseTable[code];
    }
    else if (code == 0xB5)
      ;
    else if ((EncCP1251_CtypeTable[code]&(1<< 6))
      && (flags & (1<<13))) {
      flags |= (1<<18);
...
const OnigEncodingType OnigEncodingWindows_1251 = {
