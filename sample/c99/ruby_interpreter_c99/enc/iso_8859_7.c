...
static const OnigUChar EncISO_8859_7_ToLowerCaseTable[256] = {
       ...
       OnigEncoding enc )
{
  ...
  *lower = EncISO_8859_7_ToLowerCaseTable[*p];
  (*pp)++;
  return 1;
}
static int
...
{
  if (code < 256)
    return ((EncISO_8859_7_CtypeTable[code] & (1<<(ctype))) != 0);
  else
    return 0;
}
  ...
  const struct OnigEncodingTypeST* enc)
{
 ...
 code = 0xF3;
    ...
    else if ((EncISO_8859_7_CtypeTable[code] & (1<< 10))
      && (flags & ((1<<14) | (1<<19)))) {
      flags |= (1<<18);
      code = EncISO_8859_7_ToLowerCaseTable[code];
    }
    else if (code == 0xC0 || code == 0xE0)
      ;
    else if ((EncISO_8859_7_CtypeTable[code]&(1<< 6))
      && (flags & (1<<13))) {
      flags |= (1<<18);
      if (code == 0xDC) {
 code -= 0x26;
...
const OnigEncodingType OnigEncodingISO_8859_7 = {
