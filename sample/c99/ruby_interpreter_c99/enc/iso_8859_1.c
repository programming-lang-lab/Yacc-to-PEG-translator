...
static int
mbc_case_fold(OnigCaseFoldType flag, const OnigUChar** pp, const OnigUChar* end ,
       OnigUChar* lower, OnigEncoding enc )
{
  ...
  *lower = OnigEncISO_8859_1_ToLowerCaseTable[*p];
  (*pp)++;
  return 1;
}
static int
...
{
  if (code < 256)
    return ((EncISO_8859_1_CtypeTable[code] & (1<<(ctype))) != 0);
  else
    return 0;
}
 ...
 *to++ = 's';
 code = 's';
    ...
    else if ((EncISO_8859_1_CtypeTable[code] & (1<< 10))
      && (flags & ((1<<14) | (1<<19)))) {
      flags |= (1<<18);
      code += 0x20;
    }
      ...
      ;
    else if ((EncISO_8859_1_CtypeTable[code] & (1<< 6))
      && (flags & (1<<13))) {
      flags |= (1<<18);
      code -= 0x20;
    }
...
const OnigEncodingType OnigEncodingISO_8859_1 = {
