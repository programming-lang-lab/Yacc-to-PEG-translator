...
static int
euckr_mbc_enc_len(const OnigUChar* p, const OnigUChar* e, OnigEncoding enc )
{
  int firstbyte = *p++;
  ...
  if (p == e) return (-1-(EncLen_EUCKR[firstbyte]-1));
  s = trans[s][*p++];
  return s == ACCEPT ? (2) : (-1);
...
euckr_mbc_to_code(const OnigUChar* p, const OnigUChar* end, OnigEncoding enc)
{
  return onigenc_mbn_mbc_to_code(enc, p, end);
}

static int
euckr_code_to_mbc(OnigCodePoint code, OnigUChar *buf, OnigEncoding enc)
{
  return onigenc_mb2_code_to_mbc(enc, code, buf);
}

static int
euckr_mbc_case_fold(OnigCaseFoldType flag, const OnigUChar** pp, const OnigUChar* end,
...
static OnigUChar*
euckr_left_adjust_char_head(const OnigUChar* start, const OnigUChar* s, const OnigUChar* end, OnigEncoding enc)
{
...
static int
euckr_is_allowed_reverse_match(const OnigUChar* s, const OnigUChar* end , OnigEncoding enc )
{
  const OnigUChar c = *s;
  if (c <= 0x7e) return 1;
  else return 0;
...
const OnigEncodingType OnigEncodingEUC_KR = {
  euckr_mbc_enc_len,
  "EUC-KR",
  2,
  1,
