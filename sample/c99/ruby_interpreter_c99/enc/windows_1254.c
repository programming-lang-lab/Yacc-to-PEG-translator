...
static const OnigUChar EncCP1254_ToLowerCaseTable[256] = {
       ...
       OnigEncoding enc )
{
  ...
  *lower = EncCP1254_ToLowerCaseTable[*p];
  (*pp)++;
  return 1;
}
static int
...
{
  if (code < 256)
    return ((EncCP1254_CtypeTable[code] & (1<<(ctype))) != 0);
  else
    return 0;
}
 ...
 *to++ = 's';
 code = 's';
    ...
    else if ((EncCP1254_CtypeTable[code] & (1<< 10))
      && (flags & ((1<<14) | (1<<19)))) {
      flags |= (1<<18);
      if (code == 'I')
 code = flags & (1<<20) ? (0xFD) : 'i';
      else
        code = EncCP1254_ToLowerCaseTable[code];
    }
    else if (code == 0x83 || code == 0xAA || code == 0xBA || code == 0xB5)
      ;
    else if ((EncCP1254_CtypeTable[code] & (1<< 6))
      && (flags & (1<<13))) {
      flags |= (1<<18);
      if (code == 'i')
...
const OnigEncodingType OnigEncodingWindows_1254 = {
