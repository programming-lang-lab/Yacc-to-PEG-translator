...
static const OnigUChar EncISO_8859_9_ToLowerCaseTable[256] = {
       ...
       OnigEncoding enc )
{
  ...
  *lower = EncISO_8859_9_ToLowerCaseTable[*p];
  (*pp)++;
  return 1;
}
static int
...
{
  if (code < 256)
    return ((EncISO_8859_9_CtypeTable[code] & (1<<(ctype))) != 0);
  else
    return 0;
}
 ...
 *to++ = 's';
 code = 's';
      ...
      ;
    else if ((EncISO_8859_9_CtypeTable[code] & (1<< 10))
      && (flags & ((1<<14) | (1<<19)))) {
      flags |= (1<<18);
      if (code == 'I')
 code = flags & (1<<20) ? (0xFD) : 'i';
      else
 code = EncISO_8859_9_ToLowerCaseTable[code];
    }
    else if ((EncISO_8859_9_CtypeTable[code]&(1<< 6))
      && (flags & (1<<13))) {
      flags |= (1<<18);
      if (code == 'i')
 code = flags & (1<<20) ? (0xDD) : 'I';
      else if (code == (0xFD))
 code = 'I';
      else
 code -= 0x20;
...
const OnigEncodingType OnigEncodingISO_8859_9 = {
