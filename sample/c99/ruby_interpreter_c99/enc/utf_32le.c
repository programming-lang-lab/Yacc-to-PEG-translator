...
static OnigCodePoint utf32le_mbc_to_code(const OnigUChar* p, const OnigUChar* end, OnigEncoding enc);
static int
utf32le_mbc_enc_len(const OnigUChar* p , const OnigUChar* e,
      OnigEncoding enc)
{
  if (e < p) {
    return (-1);
...
static int
utf32le_is_mbc_newline(const OnigUChar* p, const OnigUChar* end,
         OnigEncoding enc )
{
  if (p + 3 < end) {
...
utf32le_mbc_to_code(const OnigUChar* p, const OnigUChar* end ,
      OnigEncoding enc )
{
...
static int
utf32le_code_to_mbc(OnigCodePoint code, OnigUChar *buf,
      OnigEncoding enc )
{
  OnigUChar* p = buf;
...
static OnigUChar*
utf32le_left_adjust_char_head(const OnigUChar* start, const OnigUChar* s, const OnigUChar* end,
         OnigEncoding enc )
{
  ptrdiff_t rem;
...
const OnigEncodingType OnigEncodingUTF_32LE = {
  utf32le_mbc_enc_len,
  "UTF-32LE",
  4,
  4,
