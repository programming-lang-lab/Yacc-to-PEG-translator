...
static int
utf16be_mbc_enc_len(const OnigUChar* p, const OnigUChar* e ,
      OnigEncoding enc )
{
  int byte = p[0];
  if (!(((byte) & 0xf8) == 0xd8)) {
...
static int
utf16be_is_mbc_newline(const OnigUChar* p, const OnigUChar* end,
         OnigEncoding enc)
{
  if (p + 1 < end) {
    if (*(p+1) == 0x0a && *p == 0x00)
...
utf16be_mbc_to_code(const OnigUChar* p, const OnigUChar* end ,
      OnigEncoding enc)
{
  OnigCodePoint code;
...
static int
utf16be_code_to_mbc(OnigCodePoint code, OnigUChar *buf,
      OnigEncoding enc)
{
  OnigUChar* p = buf;
...
static OnigUChar*
utf16be_left_adjust_char_head(const OnigUChar* start, const OnigUChar* s, const OnigUChar* end,
         OnigEncoding enc )
{
...
const OnigEncodingType OnigEncodingUTF_16BE = {
  utf16be_mbc_enc_len,
  "UTF-16BE",
  4,
  2,
