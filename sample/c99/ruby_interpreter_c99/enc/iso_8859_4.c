...
static const OnigUChar EncISO_8859_4_ToLowerCaseTable[256] = {
       ...
       OnigEncoding enc )
{
  ...
  *lower = EncISO_8859_4_ToLowerCaseTable[*p];
  (*pp)++;
  return 1;
}
static int
...
{
  if (code < 256)
    return ((EncISO_8859_4_CtypeTable[code] & (1<<(ctype))) != 0);
  else
    return 0;
}
 ...
 *to++ = 's';
 code = 's';
    ...
    else if ((EncISO_8859_4_CtypeTable[code] & (1<< 10))
      && (flags & ((1<<14) | (1<<19)))) {
      flags |= (1<<18);
      code = EncISO_8859_4_ToLowerCaseTable[code];
    }
    else if (code == 0xA2)
      ;
    else if ((EncISO_8859_4_CtypeTable[code]&(1<< 6))
      && (flags & (1<<13))) {
      flags |= (1<<18);
...
const OnigEncodingType OnigEncodingISO_8859_4 = {
