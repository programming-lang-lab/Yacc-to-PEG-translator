...
static int
utf16le_mbc_enc_len(const OnigUChar* p, const OnigUChar* e,
      OnigEncoding enc )
{
  int len = (int)(e - p);
  OnigUChar byte;
...
static int
utf16le_is_mbc_newline(const OnigUChar* p, const OnigUChar* end,
         OnigEncoding enc )
{
  if (p + 1 < end) {
    if (*p == 0x0a && *(p+1) == 0x00)
...
utf16le_mbc_to_code(const OnigUChar* p, const OnigUChar* end ,
      OnigEncoding enc )
{
  OnigCodePoint code;
  OnigUChar c0 = *p;
...
static int
utf16le_code_to_mbc(OnigCodePoint code, OnigUChar *buf,
      OnigEncoding enc )
{
  OnigUChar* p = buf;
...
static OnigUChar*
utf16le_left_adjust_char_head(const OnigUChar* start, const OnigUChar* s, const OnigUChar* end,
         OnigEncoding enc )
{
...
const OnigEncodingType OnigEncodingUTF_16LE = {
  utf16le_mbc_enc_len,
  "UTF-16LE",
  4,
  2,
