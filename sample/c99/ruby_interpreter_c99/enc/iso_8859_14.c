...
static const OnigUChar EncISO_8859_14_ToLowerCaseTable[256] = {
     ...
     OnigUChar* lower, OnigEncoding enc )
{
  ...
  *lower = EncISO_8859_14_ToLowerCaseTable[*p];
  (*pp)++;
  return 1;
}
static int
...
{
  if (code < 256)
    return ((EncISO_8859_14_CtypeTable[code] & (1<<(ctype))) != 0);
  else
    return 0;
}
 ...
 *to++ = 's';
 code = 's';
    ...
    else if ((EncISO_8859_14_CtypeTable[code] & (1<< 10))
      && (flags & ((1<<14) | (1<<19)))) {
      flags |= (1<<18);
      code = EncISO_8859_14_ToLowerCaseTable[code];
    }
    else if ((EncISO_8859_14_CtypeTable[code] & (1<< 6))
      && (flags & (1<<13))) {
      flags |= (1<<18);
...
const OnigEncodingType OnigEncodingISO_8859_14 = {
