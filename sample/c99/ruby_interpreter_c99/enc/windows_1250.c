...
static const OnigUChar EncCP1250_ToLowerCaseTable[256] = {
...
static int
cp1250_mbc_case_fold(OnigCaseFoldType flag ,
             const OnigUChar** pp, const OnigUChar* end , OnigUChar* lower, OnigEncoding enc )
{
  const OnigUChar* p = *pp;

  *lower = EncCP1250_ToLowerCaseTable[*p];
  (*pp)++;
  return 1;
}

static int
cp1250_is_code_ctype(OnigCodePoint code, unsigned int ctype, OnigEncoding enc )
{
  if (code < 256)
    return ((EncCP1250_CtypeTable[code] & (1<<(ctype))) != 0);
  else
    return 0;
}
 ...
 *to++ = 's';
 code = 's';
    ...
    else if ((EncCP1250_CtypeTable[code] & (1<< 10))
      && (flags & ((1<<14) | (1<<19)))) {
      flags |= (1<<18);
      code = EncCP1250_ToLowerCaseTable[code];
    }
    else if (code == 0xB5)
      ;
    else if ((EncCP1250_CtypeTable[code]&(1<< 6))
      && (flags & (1<<13))) {
      flags |= (1<<18);
      if (code == 0xB9)
 code = 0xA5;
...
const OnigEncodingType OnigEncodingWindows_1250 = {
