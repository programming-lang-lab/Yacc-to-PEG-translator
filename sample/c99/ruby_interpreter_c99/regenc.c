...
OnigEncoding OnigEncDefaultCharEncoding = (&OnigEncodingASCII);
...
{
  int ret = (enc)->precise_mbc_enc_len(p,e,enc);
  if ((0 < (ret)))
    return (ret);
  else if (((ret) < -1))
    return (int )(e - p) + (-1-(ret));
  return 1;
}

extern OnigUChar*
onigenc_get_right_adjust_char_head(OnigEncoding enc, const OnigUChar* start, const OnigUChar* s, const OnigUChar* end)
{
  OnigUChar* p = (enc)->left_adjust_char_head(start, s, end, enc);
  if (p < s) {
    p += ((enc->max_enc_len == enc->min_enc_len) ? enc->min_enc_len : onigenc_mbclen_approximate(p,end,enc));
  }
  return p;
}
...
onigenc_get_right_adjust_char_head_with_prev(OnigEncoding enc,
       const OnigUChar* start, const OnigUChar* s, const OnigUChar* end, const OnigUChar** prev)
{
  OnigUChar* p = (enc)->left_adjust_char_head(start, s, end, enc);

  if (p < s) {
    if (prev) *prev = (const OnigUChar* )p;
    p += ((enc->max_enc_len == enc->min_enc_len) ? enc->min_enc_len : onigenc_mbclen_approximate(p,end,enc));
  }
  else {
    if (prev) *prev = (const OnigUChar* )((void*)0);
  }
  return p;
}

extern OnigUChar*
onigenc_get_prev_char_head(OnigEncoding enc, const OnigUChar* start, const OnigUChar* s, const OnigUChar* end)
{
  if (s <= start)
    return (OnigUChar* )((void*)0);

  return (enc)->left_adjust_char_head(start, s - 1, end, enc);
}

extern OnigUChar*
onigenc_step_back(OnigEncoding enc, const OnigUChar* start, const OnigUChar* s, const OnigUChar* end, int n)
{
  while ((((void*)(s)) != (void*)0) && n-- > 0) {
    if (s <= start)
      return (OnigUChar* )((void*)0);

    s = (enc)->left_adjust_char_head(start, s - 1, end, enc);
  }
  return (OnigUChar* )s;
}
...
onigenc_step(OnigEncoding enc, const OnigUChar* p, const OnigUChar* end, int n)
{
  OnigUChar* q = (OnigUChar* )p;
  while (n-- > 0) {
    q += onigenc_mbclen_approximate(q,end,enc);
  }
  return (q <= end ? q : ((void*)0));
}

extern int
onigenc_strlen(OnigEncoding enc, const OnigUChar* p, const OnigUChar* end)
{
  int n = 0;
  OnigUChar* q = (OnigUChar* )p;
...
extern int
onigenc_strlen_null(OnigEncoding enc, const OnigUChar* s)
{
  int n = 0;
  OnigUChar* p = (OnigUChar* )s;
  OnigUChar* e;

  while (1) {
    if (*p == '\0') {
      OnigUChar* q;
      int len = ((enc)->min_enc_len);
    ...
    e = p + ((enc)->max_enc_len);
    p += onigenc_mbclen_approximate(p,e,enc);
    n++;
  }
}

extern int
onigenc_str_bytelen_null(OnigEncoding enc, const OnigUChar* s)
{
  OnigUChar* start = (OnigUChar* )s;
  OnigUChar* p = (OnigUChar* )s;
  OnigUChar* e;
...
const OnigUChar OnigEncAsciiToLowerCaseTable[] = {
...
const OnigUChar OnigEncISO_8859_1_ToLowerCaseTable[256] = {
  '\000', '\001', '\002', '\003', '\004', '\005', '\006', '\007',
  '\010', '\011', '\012', '\013', '\014', '\015', '\016', '\017',
  '\020', '\021', '\022', '\023', '\024', '\025', '\026', '\027',
  '\030', '\031', '\032', '\033', '\034', '\035', '\036', '\037',
...
extern OnigUChar*
onigenc_get_left_adjust_char_head(OnigEncoding enc, const OnigUChar* start, const OnigUChar* s, const OnigUChar* end)
{
  return (enc)->left_adjust_char_head(start, s, end, enc);
}
      ...
      OnigEncoding enc )
{
  ...
  for (i = 0; i < (int )(sizeof(OnigAsciiLowerMap) / sizeof((OnigAsciiLowerMap)[0])); i++) {
...
extern int
onigenc_is_mbc_newline_0x0a(const OnigUChar* p, const OnigUChar* end, OnigEncoding enc )
{
  if (p < end) {
    if (*p == 0x0a) return 1;
  }
...
extern int
onigenc_ascii_mbc_case_fold(OnigCaseFoldType flag , const OnigUChar** p,
       const OnigUChar* end, OnigUChar* lower, OnigEncoding enc )
{
...
extern int
onigenc_single_byte_mbc_enc_len(const OnigUChar* p , const OnigUChar* e ,
    OnigEncoding enc )
{
  return 1;
}
...
onigenc_single_byte_mbc_to_code(const OnigUChar* p, const OnigUChar* end ,
    OnigEncoding enc )
{
...
extern int
onigenc_single_byte_code_to_mbc(OnigCodePoint code, OnigUChar *buf, OnigEncoding enc )
{

  if (code > 0xff)
    rb_raise(rb_eRangeError, "%u out of char range", code);

  *buf = (OnigUChar )(code & 0xff);
  return 1;
}

extern OnigUChar*
onigenc_single_byte_left_adjust_char_head(const OnigUChar* start ,
       const OnigUChar* s,
       const OnigUChar* end ,
       OnigEncoding enc )
{
...
extern int
onigenc_always_true_is_allowed_reverse_match(const OnigUChar* s ,
          const OnigUChar* end ,
          OnigEncoding enc )
{
  return 1;
...
extern int
onigenc_always_false_is_allowed_reverse_match(const OnigUChar* s ,
           const OnigUChar* end ,
           OnigEncoding enc )
{
  return 0;
...
extern OnigCodePoint
onigenc_mbn_mbc_to_code(OnigEncoding enc, const OnigUChar* p, const OnigUChar* end)
{
  int c, i, len;
  OnigCodePoint n;
...
extern int
onigenc_mb2_code_to_mbc(OnigEncoding enc, OnigCodePoint code, OnigUChar *buf)
{
  OnigUChar *p = buf;

  if ((code & 0xff00) != 0) {
    *p++ = (OnigUChar )((code >> 8) & 0xff);
  }
  *p++ = (OnigUChar )(code & 0xff);


  if (((enc->max_enc_len == enc->min_enc_len) ? enc->min_enc_len : onigenc_mbclen_approximate(buf,p,enc)) != (p - buf))
    return -400;
...
extern int
onigenc_mb4_code_to_mbc(OnigEncoding enc, OnigCodePoint code, OnigUChar *buf)
{
  OnigUChar *p = buf;

  if ((code & 0xff000000) != 0) {
    *p++ = (OnigUChar )((code >> 24) & 0xff);
  }
  if ((code & 0xff0000) != 0 || p != buf) {
    *p++ = (OnigUChar )((code >> 16) & 0xff);
  }
...
extern int
onigenc_minimum_property_name_to_ctype(OnigEncoding enc, const OnigUChar* p, const OnigUChar* end)
{
     ...
     unsigned int ctype)
{
  if (code < 128)
    ...
    if (((ctype) == 12 || (ctype) == 5 || (ctype) == 7)) {
      return ((enc)->code_to_mbclen(code,enc) > 1 ? 1 : 0);
    }
  }
...
extern int
onigenc_with_ascii_strncmp(OnigEncoding enc, const OnigUChar* p, const OnigUChar* end,
                           const OnigUChar* sascii , int n)
{
  int x, c;
  ...
  while (n-- > 0) {
    ...
    c = (int )(enc)->mbc_to_code((p),(end),enc);
    x = *sascii - c;
    if (x) return x;
...
extern int
onigenc_with_ascii_strnicmp(OnigEncoding enc, const OnigUChar* p, const OnigUChar* end,
                            const OnigUChar* sascii , int n)
{
  int x, c;
  ...
  while (n-- > 0) {
    ...
    c = (int )(enc)->mbc_to_code((p),(end),enc);
    if (((c) < 0x80))
      c = OnigEncAsciiToLowerCaseTable[c];
    x = OnigEncAsciiToLowerCaseTable[*sascii] - c;
    if (x) return x;
       ...
       OnigUChar* to, OnigUChar* to_end, const struct OnigEncodingTypeST* enc)
{
  ...
  while (*pp < end && to < to_end) {
    codepoint_length = (enc)->precise_mbc_enc_len(*pp,end,enc);
    if (codepoint_length < 0)
      return codepoint_length;
    code = (enc)->mbc_to_code((*pp),(end),enc);
    *pp += codepoint_length;

    if (code >= 'a' && code <= 'z' && (flags & (1<<13))) {
      flags |= (1<<18);
      code += 'A' - 'a';
 ...
 (flags & ((1<<14) | (1<<19)))) {
    ...
    to += (enc)->code_to_mbc(code,to,enc);
    if (flags & (1<<15))
      flags ^= ((1<<13) | (1<<14) | (1<<15));
  }
  *flagP = flags;
