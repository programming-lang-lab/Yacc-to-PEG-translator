...
main(int argc, char **argv)
{
 ...
 ruby_init();
 return goruby_run_node(goruby_options(argc, argv));
    }
}
extern void *ruby_options(int argc, char **argv);
extern int ruby_run_node(void*);
...
goruby_options(int argc, char **argv)
{
 ...
 close(rw[1]);
 ret = n > 0 ? ruby_options(argc, argv) : ((void*)0);
 dup2(infd, 0);
 close(infd);
 return ret;
    }
...
{
    int state;
    if (!((VALUE)(rb_protect(init_golf, ((VALUE)RUBY_Qtrue), &state)) != ((VALUE)RUBY_Qnil))) {
