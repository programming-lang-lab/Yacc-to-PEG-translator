...
static char binary_filename[4096];
  ...
  obj_info_t *obj, line_info_t *lines, int offset)
{
 ...
 switch (op) {
 case 0x01:
     do { fill_line(num_traces, traces, addr, file, line, include_directories, filenames, obj, lines, offset); } while (0);
     break;
 case 0x02:
     a = uleb128(&p);
     addr += a;
   ...
   obj_info_t **objp, line_info_t *lines, int offset)
{
 ...
 return;
    ...
    if (len >= 4096 - global_debug_dir_len)
 len = 4096 - global_debug_dir_len - 1;
    ...
    strlcpy(binary_filename + len, debuglink, 4096 - len);
    ...
    obj_info_t **objp, line_info_t *lines, int offset)
{
    ...
    char *section_name;
    Elf64_Ehdr *ehdr;
    Elf64_Shdr *shdr, *shstr_shdr;
    Elf64_Shdr *debug_line_shdr = ((void*)0), *gnu_debuglink_shdr = ((void*)0);
    int fd;
    off_t filesize;
    char *file;
    Elf64_Shdr *symtab_shdr = ((void*)0), *strtab_shdr = ((void*)0);
    Elf64_Shdr *dynsym_shdr = ((void*)0), *dynstr_shdr = ((void*)0);
    obj_info_t *obj = *objp;
    uintptr_t dladdr_fbase = 0;
    _Bool compressed_p = 0;

    fd = open(binary_filename, 00);
    if (fd < 0) {
 goto fail;
    }
 ...
 goto fail;
    ...
    file = (char *)mmap(((void*)0), (size_t)filesize, 0x1, 0x01, fd, 0);
    if (file == ((void *) -1)) {
 int e = (*__errno_location ());
 close(fd);
 kprintf("mmap: %s\n", strerror(e));
 ...
 if (dynsym_shdr && dynstr_shdr) {
     ...
     Elf64_Sym *symtab = (Elf64_Sym *)(file + dynsym_shdr->sh_offset);
     int symtab_count = (int)(dynsym_shdr->sh_size / sizeof(Elf64_Sym));
     for (j = 0; j < symtab_count; j++) {
  Elf64_Sym *sym = &symtab[j];
  Dl_info info;
  void *h, *s;
  if (((sym->st_info) & 0xf) != 2 || sym->st_size <= 0) continue;
  h = dlopen(((void*)0), 0x00002|0);
  if (!h) continue;
 ...
 Elf64_Sym *symtab = (Elf64_Sym *)(file + symtab_shdr->sh_offset);
 int symtab_count = (int)(symtab_shdr->sh_size / sizeof(Elf64_Sym));
 for (j = 0; j < symtab_count; j++) {
     Elf64_Sym *sym = &symtab[j];
...
main_exe_path(void)
{
    ...
    ssize_t len = readlink("/proc/self/exe", binary_filename, 4096);
...
rb_dump_backtrace_with_lines(int num_traces, void **traces)
{
    ...
    obj_info_t *obj = ((void*)0);

    void **dladdr_fbases = (void **)calloc(num_traces+2, sizeof(void *));

    char *main_path = ((void*)0);
    ssize_t len;
    if ((len = main_exe_path()) > 0) {
 main_path = (char *)__builtin_alloca (len + 1);
 if (main_path) {
     uintptr_t addr;
 ...
 if (dladdr(traces[i], &info)) {
  ...
  if (*p == info.dli_fbase) {
     ...
     *p = info.dli_fbase;
     ...
     obj->path = path;
     lines[i].path = path;
     strlcpy(binary_filename, path, 4096);
...
kprintf(const char *fmt, ...)
{
 ...
 retval = kvprintf(fmt, putce, ((void*)0), 10, ap);
 __builtin_va_end(ap);
 return retval;
}
...
kvprintf(char const *fmt, void (*func)(int), void *arg, int radix, va_list ap)
{
 ...
 for (;;) {
  ...
  while ((ch = (unsigned char)*fmt++) != '%' || stop) {
   if (ch == '\0')
    return (retval);
   {int cc=(ch); if (func) (*func)(cc); else *d++ = cc; retval++; };
  }
  percent = fmt - 1;
...
reswitch: switch (ch = (unsigned char)*fmt++) {
  ...
  case '5': case '6': case '7': case '8': case '9':
   ...
   goto reswitch;
  case 'b':
   num = (unsigned int)__builtin_va_arg();
   p = __builtin_va_arg();
   for (q = ksprintn(nbuf, num, *p++, ((void*)0), 0); *q;)
    {int cc=(*q--); if (func) (*func)(cc); else *d++ = cc; retval++; };
   ...
   for (tmp = 0; *p;) {
    n = *p++;
    if (num & (1 << (n - 1))) {
     {int cc=(tmp ? ',' : '<'); if (func) (*func)(cc); else *d++ = cc; retval++; };
     for (; (n = *p) > ' '; ++p)
      {int cc=(n); if (func) (*func)(cc); else *d++ = cc; retval++; };
     tmp = 1;
    } else
     for (; *p > ' '; ++p)
      continue;
   ...
   if (tmp)
    {int cc=('>'); if (func) (*func)(cc); else *d++ = cc; retval++; };
   break;
  case 'c':
   {int cc=(__builtin_va_arg()); if (func) (*func)(cc); else *d++ = cc; retval++; };
   break;
  case 'D':
   up = __builtin_va_arg();
   p = __builtin_va_arg();
   if (!width)
    width = 16;
   while(width--) {
    {int cc=((hex2ascii_data[*up >> 4])); if (func) (*func)(cc); else *d++ = cc; retval++; };
    {int cc=((hex2ascii_data[*up & 0x0f])); if (func) (*func)(cc); else *d++ = cc; retval++; };
    up++;
    if (width)
     for (q=p;*q;q++)
      {int cc=(*q); if (func) (*func)(cc); else *d++ = cc; retval++; };
  ...
  case 'p':
   ...
   sharpflag = (width == 0);
   sign = 0;
   num = (uintptr_t)__builtin_va_arg();
   goto number;
  case 'q':
   qflag = 1;
   goto reswitch;
  ...
  case 's':
   ...
   if (!ladjust && width > 0)
    while (width--)
     {int cc=(padc); if (func) (*func)(cc); else *d++ = cc; retval++; };
   while (n--)
    {int cc=(*p++); if (func) (*func)(cc); else *d++ = cc; retval++; };
   if (ladjust && width > 0)
    while (width--)
     {int cc=(padc); if (func) (*func)(cc); else *d++ = cc; retval++; };
   break;
...
handle_nosign:
   ...
   if (jflag)
    ...
    num = (unsigned short)__builtin_va_arg();
   else if (cflag)
    num = (unsigned char)__builtin_va_arg();
   else
    num = __builtin_va_arg();
   goto number;
handle_sign:
   if (jflag)
    ...
    num = (short)__builtin_va_arg();
   else if (cflag)
    num = (char)__builtin_va_arg();
   else
    num = __builtin_va_arg();
number:
   ...
   if (sharpflag && num != 0) {
    if (base == 8) {
     {int cc=('0'); if (func) (*func)(cc); else *d++ = cc; retval++; };
    } else if (base == 16) {
     {int cc=('0'); if (func) (*func)(cc); else *d++ = cc; retval++; };
     {int cc=('x'); if (func) (*func)(cc); else *d++ = cc; retval++; };
    }
   }
   while (dwidth-- > 0)
    {int cc=('0'); if (func) (*func)(cc); else *d++ = cc; retval++; };
  ...
  default:
   while (percent < fmt)
    {int cc=(*percent++); if (func) (*func)(cc); else *d++ = cc; retval++; };
