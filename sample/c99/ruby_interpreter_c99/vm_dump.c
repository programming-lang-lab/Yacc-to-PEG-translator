...
control_frame_dump(const rb_execution_context_t *ec, const rb_control_frame_t *cfp)
{
 ...
 selfstr = rb_string_value_cstr(&(tmp));
    }
    else {
 selfstr = "";
    }
 ...
 if (imemo_type_p((VALUE)cfp->iseq, imemo_ifunc)) {
     iseq_name = "<ifunc>";
 }
 else if (((((VALUE)(cfp->iseq)&~((~(VALUE)0)<<RUBY_SPECIAL_SHIFT)) == RUBY_SYMBOL_FLAG)||(!(((VALUE)(cfp->iseq) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(cfp->iseq) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(cfp->iseq))->flags & RUBY_T_MASK) == (RUBY_T_SYMBOL)))) {
     tmp = rb_sym2str((VALUE)cfp->iseq);
     iseq_name = (!(((struct RBasic*)(tmp))->flags & RSTRING_NOEMBED) ? ((struct RString*)(tmp))->as.ary : ((struct RString*)(tmp))->as.heap.ptr);
     ruby_snprintf(posbuf, 128, ":%s", iseq_name);
     line = -1;
 }
 else {
     pc = cfp->pc - cfp->iseq->body->iseq_encoded;
     iseq_name = (!(((struct RBasic*)(cfp->iseq->body->location.label))->flags & RSTRING_NOEMBED) ? ((struct RString*)(cfp->iseq->body->location.label))->as.ary : ((struct RString*)(cfp->iseq->body->location.label))->as.heap.ptr);
     line = rb_vm_get_sourceline(cfp);
     if (line) {
  ruby_snprintf(posbuf, 128, "%s:%d", (!(((struct RBasic*)(rb_iseq_path(cfp->iseq)))->flags & RSTRING_NOEMBED) ? ((struct RString*)(rb_iseq_path(cfp->iseq)))->as.ary : ((struct RString*)(rb_iseq_path(cfp->iseq)))->as.heap.ptr), line);
     }
 }
    }
    else if ((me = rb_vm_frame_method_entry(cfp)) != ((void*)0)) {
 ...
 fprintf(stderr, "p:%04td ", pc);
    }
    fprintf(stderr, "s:%04td ", cfp->sp - ec->vm_stack);
    fprintf(stderr, ep_in_heap == ' ' ? "e:%06td ": "E:%06tx ", ep % 10000);
...
rb_vmdebug_env_dump_raw(const rb_env_t *env, const VALUE *ep)
{
 ...
 for (i = 0; i < env->env_size; i++) {
     fprintf(stderr, "%04d: %08lx (%p)", i, env->env[i], (void *)&env->env[i]);
...
rb_vmdebug_proc_dump_raw(rb_proc_t *proc)
{
    ...
    selfstr = rb_string_value_cstr(&(val));
...
rb_vmdebug_debug_print_register(const rb_execution_context_t *ec)
{
 ...
 ep = -1;
    ...
    fprintf(stderr, "  [PC] %04td, [SP] %04td, [EP] %04td, [CFP] %04td\n",
...
rb_vmdebug_debug_print_pre(const rb_execution_context_t *ec, const rb_control_frame_t *cfp, const VALUE *_pc)
{
 ...
 for (i=0; i<(int)((rb_control_frame_t *)((ec)->vm_stack + (ec)->vm_stack_size) - (rb_control_frame_t *)(cfp)); i++) {
     printf(" ");
 }
 printf("| ");
...
print_machine_register(size_t reg, const char *reg_name, int col_count, int max_col)
{
    ...
    ret = ruby_snprintf(buf, sizeof(buf), " %3.3s: 0x%016zx", reg_name, reg);
...
rb_dump_machine_register(const ucontext_t *ctx)
{
 ...
 (col_count = print_machine_register(mctx->gregs[REG_RIP], "RIP", col_count, 80));
 (col_count = print_machine_register(mctx->gregs[REG_RBP], "RBP", col_count, 80));
 (col_count = print_machine_register(mctx->gregs[REG_RSP], "RSP", col_count, 80));
 (col_count = print_machine_register(mctx->gregs[REG_RAX], "RAX", col_count, 80));
 (col_count = print_machine_register(mctx->gregs[REG_RBX], "RBX", col_count, 80));
 (col_count = print_machine_register(mctx->gregs[REG_RCX], "RCX", col_count, 80));
 (col_count = print_machine_register(mctx->gregs[REG_RDX], "RDX", col_count, 80));
 (col_count = print_machine_register(mctx->gregs[REG_RDI], "RDI", col_count, 80));
 (col_count = print_machine_register(mctx->gregs[REG_RSI], "RSI", col_count, 80));
 (col_count = print_machine_register(mctx->gregs[REG_R8], "R8", col_count, 80));
 (col_count = print_machine_register(mctx->gregs[REG_R9], "R9", col_count, 80));
 (col_count = print_machine_register(mctx->gregs[REG_R10], "R10", col_count, 80));
 (col_count = print_machine_register(mctx->gregs[REG_R11], "R11", col_count, 80));
 (col_count = print_machine_register(mctx->gregs[REG_R12], "R12", col_count, 80));
 (col_count = print_machine_register(mctx->gregs[REG_R13], "R13", col_count, 80));
 (col_count = print_machine_register(mctx->gregs[REG_R14], "R14", col_count, 80));
 (col_count = print_machine_register(mctx->gregs[REG_R15], "R15", col_count, 80));
 (col_count = print_machine_register(mctx->gregs[REG_EFL], "EFL", col_count, 80));
    }
    fprintf(stderr, "\n\n");
}
...
rb_vm_bugreport(const void *ctx)
{
    ...
    const rb_vm_t *const vm = rb_current_vm();
 ...
 fprintf(stderr, "* Loaded script: %.*s\n",
  (((len = (!(((struct RBasic*)(name))->flags & RSTRING_NOEMBED) ? (long)((((struct RBasic*)(name))->flags >> RSTRING_EMBED_LEN_SHIFT) & (RSTRING_EMBED_LEN_MASK >> RSTRING_EMBED_LEN_SHIFT)) : ((struct RString*)(name))->as.heap.len)) > max_name_length) ? max_name_length : (int)len), (!(((struct RBasic*)(name))->flags & RSTRING_NOEMBED) ? ((struct RString*)(name))->as.ary : ((struct RString*)(name))->as.heap.ptr));
 fprintf(stderr, "\n");
 fprintf(stderr, "* Loaded features:\n\n");
 for (i=0; i<rb_array_len(vm->loaded_features); i++) {
     name = (rb_array_const_ptr(vm->loaded_features)[i]);
  ...
  fprintf(stderr, " %4d %.*s\n", i,
   (((len = (!(((struct RBasic*)(name))->flags & RSTRING_NOEMBED) ? (long)((((struct RBasic*)(name))->flags >> RSTRING_EMBED_LEN_SHIFT) & (RSTRING_EMBED_LEN_MASK >> RSTRING_EMBED_LEN_SHIFT)) : ((struct RString*)(name))->as.heap.len)) > max_name_length) ? max_name_length : (int)len), (!(((struct RBasic*)(name))->flags & RSTRING_NOEMBED) ? ((struct RString*)(name))->as.ary : ((struct RString*)(name))->as.heap.ptr));
     ...
     else if (( ((RUBY_T_CLASS) == RUBY_T_FIXNUM) ? (((int)(long)(name))&RUBY_FIXNUM_FLAG) : ((RUBY_T_CLASS) == RUBY_T_TRUE) ? ((name) == ((VALUE)RUBY_Qtrue)) : ((RUBY_T_CLASS) == RUBY_T_FALSE) ? ((name) == ((VALUE)RUBY_Qfalse)) : ((RUBY_T_CLASS) == RUBY_T_NIL) ? ((name) == ((VALUE)RUBY_Qnil)) : ((RUBY_T_CLASS) == RUBY_T_UNDEF) ? ((name) == ((VALUE)RUBY_Qundef)) : ((RUBY_T_CLASS) == RUBY_T_SYMBOL) ? ((((VALUE)(name)&~((~(VALUE)0)<<RUBY_SPECIAL_SHIFT)) == RUBY_SYMBOL_FLAG)||(!(((VALUE)(name) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(name) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(name))->flags & RUBY_T_MASK) == (RUBY_T_SYMBOL))) : ((RUBY_T_CLASS) == RUBY_T_FLOAT) ? ( ((((int)(long)(name))&RUBY_FLONUM_MASK) == RUBY_FLONUM_FLAG) || (!(((VALUE)(name) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(name) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(name))->flags & RUBY_T_MASK) == RUBY_T_FLOAT)) : (!(((VALUE)(name) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(name) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(name))->flags & RUBY_T_MASK) == (RUBY_T_CLASS))) || ( ((RUBY_T_MODULE) == RUBY_T_FIXNUM) ? (((int)(long)(name))&RUBY_FIXNUM_FLAG) : ((RUBY_T_MODULE) == RUBY_T_TRUE) ? ((name) == ((VALUE)RUBY_Qtrue)) : ((RUBY_T_MODULE) == RUBY_T_FALSE) ? ((name) == ((VALUE)RUBY_Qfalse)) : ((RUBY_T_MODULE) == RUBY_T_NIL) ? ((name) == ((VALUE)RUBY_Qnil)) : ((RUBY_T_MODULE) == RUBY_T_UNDEF) ? ((name) == ((VALUE)RUBY_Qundef)) : ((RUBY_T_MODULE) == RUBY_T_SYMBOL) ? ((((VALUE)(name)&~((~(VALUE)0)<<RUBY_SPECIAL_SHIFT)) == RUBY_SYMBOL_FLAG)||(!(((VALUE)(name) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(name) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(name))->flags & RUBY_T_MASK) == (RUBY_T_SYMBOL))) : ((RUBY_T_MODULE) == RUBY_T_FLOAT) ? ( ((((int)(long)(name))&RUBY_FLONUM_MASK) == RUBY_FLONUM_FLAG) || (!(((VALUE)(name) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(name) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(name))->flags & RUBY_T_MASK) == RUBY_T_FLOAT)) : (!(((VALUE)(name) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(name) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(name))->flags & RUBY_T_MASK) == (RUBY_T_MODULE)))) {
  const char *const type = ( ((RUBY_T_CLASS) == RUBY_T_FIXNUM) ? (((int)(long)(name))&RUBY_FIXNUM_FLAG) : ((RUBY_T_CLASS) == RUBY_T_TRUE) ? ((name) == ((VALUE)RUBY_Qtrue)) : ((RUBY_T_CLASS) == RUBY_T_FALSE) ? ((name) == ((VALUE)RUBY_Qfalse)) : ((RUBY_T_CLASS) == RUBY_T_NIL) ? ((name) == ((VALUE)RUBY_Qnil)) : ((RUBY_T_CLASS) == RUBY_T_UNDEF) ? ((name) == ((VALUE)RUBY_Qundef)) : ((RUBY_T_CLASS) == RUBY_T_SYMBOL) ? ((((VALUE)(name)&~((~(VALUE)0)<<RUBY_SPECIAL_SHIFT)) == RUBY_SYMBOL_FLAG)||(!(((VALUE)(name) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(name) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(name))->flags & RUBY_T_MASK) == (RUBY_T_SYMBOL))) : ((RUBY_T_CLASS) == RUBY_T_FLOAT) ? ( ((((int)(long)(name))&RUBY_FLONUM_MASK) == RUBY_FLONUM_FLAG) || (!(((VALUE)(name) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(name) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(name))->flags & RUBY_T_MASK) == RUBY_T_FLOAT)) : (!(((VALUE)(name) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(name) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(name))->flags & RUBY_T_MASK) == (RUBY_T_CLASS))) ?
      "class": "module";
  ...
  if (!( ((RUBY_T_STRING) == RUBY_T_FIXNUM) ? (((int)(long)(klass))&RUBY_FIXNUM_FLAG) : ((RUBY_T_STRING) == RUBY_T_TRUE) ? ((klass) == ((VALUE)RUBY_Qtrue)) : ((RUBY_T_STRING) == RUBY_T_FALSE) ? ((klass) == ((VALUE)RUBY_Qfalse)) : ((RUBY_T_STRING) == RUBY_T_NIL) ? ((klass) == ((VALUE)RUBY_Qnil)) : ((RUBY_T_STRING) == RUBY_T_UNDEF) ? ((klass) == ((VALUE)RUBY_Qundef)) : ((RUBY_T_STRING) == RUBY_T_SYMBOL) ? ((((VALUE)(klass)&~((~(VALUE)0)<<RUBY_SPECIAL_SHIFT)) == RUBY_SYMBOL_FLAG)||(!(((VALUE)(klass) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(klass) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(klass))->flags & RUBY_T_MASK) == (RUBY_T_SYMBOL))) : ((RUBY_T_STRING) == RUBY_T_FLOAT) ? ( ((((int)(long)(klass))&RUBY_FLONUM_MASK) == RUBY_FLONUM_FLAG) || (!(((VALUE)(klass) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(klass) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(klass))->flags & RUBY_T_MASK) == RUBY_T_FLOAT)) : (!(((VALUE)(klass) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(klass) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(klass))->flags & RUBY_T_MASK) == (RUBY_T_STRING)))) {
      fprintf(stderr, " %4d #<%p:%p>\n", i,
       (void *)rb_class_of((VALUE)(name)), (void *)name);
      continue;
  }
  fprintf(stderr, " %4d #<%.*s:%p>\n", i,
   (((len = (!(((struct RBasic*)(klass))->flags & RSTRING_NOEMBED) ? (long)((((struct RBasic*)(klass))->flags >> RSTRING_EMBED_LEN_SHIFT) & (RSTRING_EMBED_LEN_MASK >> RSTRING_EMBED_LEN_SHIFT)) : ((struct RString*)(klass))->as.heap.len)) > max_name_length) ? max_name_length : (int)len), (!(((struct RBasic*)(klass))->flags & RSTRING_NOEMBED) ? ((struct RString*)(klass))->as.ary : ((struct RString*)(klass))->as.heap.ptr),
   (void *)name);
     }
 }
 fprintf(stderr, "\n");
...
rb_vmdebug_stack_dump_all_threads(void)
{
    ...
    rb_thread_t *th = ((void*)0);

    for ((th) = list_node_to_off_(((void)"vm_dump.c:1080", (&vm->living_threads))->n.next, (((__builtin_offsetof() + ((int *)0 != (struct list_node *)0))))); list_node_from_off_((void *)(th), (((__builtin_offsetof() + ((int *)0 != (struct list_node *)0))))) != &((&vm->living_threads))->n; (th) = list_node_to_off_(list_node_from_off_((void *)(th), (((__builtin_offsetof() + ((int *)0 != (struct list_node *)0)))))->next, (((__builtin_offsetof() + ((int *)0 != (struct list_node *)0)))))) {
