...
typedef int int_must_be_32bit_at_least[sizeof(int) * 8 < 32 ? -1 : 1];
enum {MT_MAX_STATE = 624};
...
init_by_array(struct MT *mt, const uint32_t init_key[], int key_length)
{
    ...
    for (; k; k--) {
        ...
        i++; j++;
        if (i>=624) { mt->state[0] = mt->state[624 -1]; i=1; }
        if (j>=key_length) j=0;
    }
    for (k=624 -1; k; k--) {
...
next_state(struct MT *mt)
{
    ...
    for (j=624 -397 +1; --j; p++)
        *p = p[397] ^ ((( (((p[0])) & 0x80000000U) | (((p[1])) & 0x7fffffffU) ) >> 1) ^ ((p[1])&1U ? 0x9908b0dfU : 0U));

    for (j=397; --j; p++)
        *p = p[397 -624] ^ ((( (((p[0])) & 0x80000000U) | (((p[1])) & 0x7fffffffU) ) >> 1) ^ ((p[1])&1U ? 0x9908b0dfU : 0U));

    *p = p[397 -624] ^ ((( (((p[0])) & 0x80000000U) | (((mt->state[0])) & 0x7fffffffU) ) >> 1) ^ ((mt->state[0])&1U ? 0x9908b0dfU : 0U));
}
...
int_pair_to_real_inclusive(uint32_t a, uint32_t b)
{
    ...
    const unsigned int m = ((unsigned int)1 << dig) | 1;
    unsigned int x = ((unsigned int)a << 32) | b;
...
{
    rb_random_t *ptr;
    ((ptr) = (rb_random_t*)rb_check_typeddata((obj), (&random_data_type)));
...
try_get_rnd(VALUE obj)
{
 ...
 return rand_start(&default_rand);
    ...
    if (!rb_typeddata_is_kind_of(obj, &random_data_type)) return ((void*)0);
    return rand_start((((struct RData*)(obj))->data));
}
...
{
    rb_random_t *rnd;
    VALUE obj = ({ VALUE data_struct_obj = rb_data_typed_object_zalloc(klass, sizeof(rb_random_t), &random_data_type); (void)((rnd) = (rb_random_t *)(((struct RData*)(data_struct_obj))->data));; data_struct_obj; });
    rnd->seed = (((VALUE)(0))<<1 | RUBY_FIXNUM_FLAG);
    return obj;
}
...
{
    uint32_t buf0[8 / (31/8 + 1) * 4], *buf = buf0;
    size_t len;
    int sign;

    len = rb_absint_numwords(seed, 32, ((void*)0));
    if (len > ((int)(sizeof(buf0) / sizeof((buf0)[0]))))
        buf = ((uint32_t*)ruby_xmalloc2((size_t)(len),sizeof(uint32_t)));
...
fill_random_bytes_urandom(void *seed, size_t size)
{
    ...
    if (fd < 0) return -1;
    rb_update_max_fd(fd);
    if (fstat(fd, &statbuf) == 0 && ((((statbuf.st_mode)) & 0170000) == (0020000))) {
 do {
...
fill_random_bytes_syscall(void *seed, size_t size, int need_secure)
{
 ...
 do {
     (*__errno_location ()) = 0;
     ret = syscall(318, ((char*)seed) + offset, size - offset, flags);
     if (ret == -1) {
  (void)__atomic_exchange_n(&(try_syscall), (0), 5);
  return -1;
     }
     offset += (size_t)ret;
 } while(offset < size);
...
fill_random_seed(uint32_t *seed, size_t cnt)
{
    ...
    seed[0] ^= (uint32_t)((time_t)tv.tv_sec >> 4 * 8);

    seed[2] ^= getpid() ^ (n++ << 16);
    seed[3] ^= (uint32_t)(VALUE)&seed;

    seed[2] ^= (uint32_t)((VALUE)&seed >> 4 * 8);
...
random_seed(void)
{
    VALUE v;
    uint32_t buf[4 +1];
    fill_random_seed(buf, 4);
    v = make_seed_value(buf, 4);
    explicit_bzero(buf, (4 * (int)sizeof(int32_t)));
    return v;
}
static VALUE
random_raw_seed(VALUE self, VALUE size)
{
    long n = rb_num2ulong_inline(size);
    VALUE buf =  ({ (__builtin_constant_p(0) && __builtin_constant_p(n)) ? rb_str_new_static((0), (n)) : rb_str_new((0), (n)); });
    if (n == 0) return buf;
    if (fill_random_bytes((!(((struct RBasic*)(buf))->flags & RSTRING_NOEMBED) ? ((struct RString*)(buf))->as.ary : ((struct RString*)(buf))->as.heap.ptr), n, FALSE))
...
random_copy(VALUE obj, VALUE orig)
{
    ...
    *rnd1 = *rnd2;
    mt->next = mt->state + ((int)(sizeof(mt->state) / sizeof((mt->state)[0]))) - mt->left + 1;
    return obj;
}
...
{
    return rb_integer_unpack(mt->state, ((int)(sizeof(mt->state) / sizeof((mt->state)[0]))),
        sizeof(*mt->state), 0,
        0x02|0x40);
}
...
{
    rb_random_t *rnd = get_rnd(obj);
    return (((VALUE)(rnd->mt.left))<<1 | RUBY_FIXNUM_FLAG);
}
...
{
    return (((VALUE)(default_rand.mt.left))<<1 | RUBY_FIXNUM_FLAG);
}
...
{
    rb_random_t *rnd = get_rnd(obj);
    VALUE dump = rb_ary_new_capa(3);

    rb_ary_push(dump, mt_state(&rnd->mt));
    rb_ary_push(dump, (((VALUE)(rnd->mt.left))<<1 | RUBY_FIXNUM_FLAG));
...
random_load(VALUE obj, VALUE dump)
{
    rb_random_t *rnd = get_rnd(obj);
    struct MT *mt = &rnd->mt;
    VALUE state, left = (((VALUE)(1))<<1 | RUBY_FIXNUM_FLAG), seed = (((VALUE)(0))<<1 | RUBY_FIXNUM_FLAG);
    const VALUE *ary;
    unsigned long x;

    rb_check_copyable(obj, dump);
    (!( (((RUBY_T_ARRAY)) == RUBY_T_FIXNUM) ? (((int)(long)((VALUE)(dump)))&RUBY_FIXNUM_FLAG) : (((RUBY_T_ARRAY)) == RUBY_T_TRUE) ? (((VALUE)(dump)) == ((VALUE)RUBY_Qtrue)) : (((RUBY_T_ARRAY)) == RUBY_T_FALSE) ? (((VALUE)(dump)) == ((VALUE)RUBY_Qfalse)) : (((RUBY_T_ARRAY)) == RUBY_T_NIL) ? (((VALUE)(dump)) == ((VALUE)RUBY_Qnil)) : (((RUBY_T_ARRAY)) == RUBY_T_UNDEF) ? (((VALUE)(dump)) == ((VALUE)RUBY_Qundef)) : (((RUBY_T_ARRAY)) == RUBY_T_SYMBOL) ? ((((VALUE)((VALUE)(dump))&~((~(VALUE)0)<<RUBY_SPECIAL_SHIFT)) == RUBY_SYMBOL_FLAG)||(!(((VALUE)((VALUE)(dump)) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)((VALUE)(dump)) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)((VALUE)(dump)))->flags & RUBY_T_MASK) == (RUBY_T_SYMBOL))) : (((RUBY_T_ARRAY)) == RUBY_T_FLOAT) ? ( ((((int)(long)((VALUE)(dump)))&RUBY_FLONUM_MASK) == RUBY_FLONUM_FLAG) || (!(((VALUE)((VALUE)(dump)) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)((VALUE)(dump)) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)((VALUE)(dump)))->flags & RUBY_T_MASK) == RUBY_T_FLOAT)) : (!(((VALUE)((VALUE)(dump)) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)((VALUE)(dump)) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)((VALUE)(dump)))->flags & RUBY_T_MASK) == ((RUBY_T_ARRAY)))) || ((RUBY_T_ARRAY) == RUBY_T_DATA && (((struct RTypedData*)(dump))->typed_flag == 1)) ? rb_unexpected_type((VALUE)(dump), (RUBY_T_ARRAY)) : (void)0);
    ary = rb_array_const_ptr(dump);
    switch (rb_array_len(dump)) {
      case 3:
 seed = ary[2];
      case 2:
 left = ary[1];
 ...
 rb_raise(rb_eArgError, "wrong dump data");
    ...
    rb_integer_pack(state, mt->state, ((int)(sizeof(mt->state) / sizeof((mt->state)[0]))),
        sizeof(*mt->state), 0,
        0x02|0x40);
    x = rb_num2ulong_inline(left);
    if (x > ((int)(sizeof(mt->state) / sizeof((mt->state)[0])))) {
 rb_raise(rb_eArgError, "wrong value");
    }
    mt->left = (unsigned int)x;
    mt->next = mt->state + ((int)(sizeof(mt->state) / sizeof((mt->state)[0]))) - x + 1;
...
limited_big_rand(struct MT *mt, VALUE limit)
{
    ...
    len = rb_absint_numwords(limit, 32, ((void*)0));
    tmp = ((uint32_t*)(((size_t)(len*2) < 1024 / sizeof(uint32_t)) ? ((* ({ volatile VALUE *rb_gc_guarded_ptr = &(vtmp); ; rb_gc_guarded_ptr; })) = 0, __builtin_alloca ((size_t)(len*2) * sizeof(uint32_t))) : rb_alloc_tmp_buffer2(&(vtmp), (long)(len*2), sizeof(uint32_t))));
    lim_array = tmp;
    rnd_array = tmp + len;
...
{
    VALUE len = rb_long2num_inline(n);
    VALUE v = rb_funcallv_public(obj, id_bytes, 1, &len);
    long l;
    (!( (((RUBY_T_STRING)) == RUBY_T_FIXNUM) ? (((int)(long)((VALUE)(v)))&RUBY_FIXNUM_FLAG) : (((RUBY_T_STRING)) == RUBY_T_TRUE) ? (((VALUE)(v)) == ((VALUE)RUBY_Qtrue)) : (((RUBY_T_STRING)) == RUBY_T_FALSE) ? (((VALUE)(v)) == ((VALUE)RUBY_Qfalse)) : (((RUBY_T_STRING)) == RUBY_T_NIL) ? (((VALUE)(v)) == ((VALUE)RUBY_Qnil)) : (((RUBY_T_STRING)) == RUBY_T_UNDEF) ? (((VALUE)(v)) == ((VALUE)RUBY_Qundef)) : (((RUBY_T_STRING)) == RUBY_T_SYMBOL) ? ((((VALUE)((VALUE)(v))&~((~(VALUE)0)<<RUBY_SPECIAL_SHIFT)) == RUBY_SYMBOL_FLAG)||(!(((VALUE)((VALUE)(v)) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)((VALUE)(v)) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)((VALUE)(v)))->flags & RUBY_T_MASK) == (RUBY_T_SYMBOL))) : (((RUBY_T_STRING)) == RUBY_T_FLOAT) ? ( ((((int)(long)((VALUE)(v)))&RUBY_FLONUM_MASK) == RUBY_FLONUM_FLAG) || (!(((VALUE)((VALUE)(v)) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)((VALUE)(v)) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)((VALUE)(v)))->flags & RUBY_T_MASK) == RUBY_T_FLOAT)) : (!(((VALUE)((VALUE)(v)) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)((VALUE)(v)) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)((VALUE)(v)))->flags & RUBY_T_MASK) == ((RUBY_T_STRING)))) || ((RUBY_T_STRING) == RUBY_T_DATA && (((struct RTypedData*)(v))->typed_flag == 1)) ? rb_unexpected_type((VALUE)(v), (RUBY_T_STRING)) : (void)0);
    l = (!(((struct RBasic*)(v))->flags & RSTRING_NOEMBED) ? (long)((((struct RBasic*)(v))->flags >> RSTRING_EMBED_LEN_SHIFT) & (RSTRING_EMBED_LEN_MASK >> RSTRING_EMBED_LEN_SHIFT)) : ((struct RString*)(v))->as.heap.len);
    if (l < n)
 rb_raise(rb_eRangeError, "random data too short %ld", l);
    else if (l > n)
 rb_raise(rb_eRangeError, "random data too long %ld", l);
    if (p) memcpy(p, (!(((struct RBasic*)(v))->flags & RSTRING_NOEMBED) ? ((struct RString*)(v))->as.ary : ((struct RString*)(v))->as.heap.ptr), n);
    return v;
}
...
ulong_to_num_plus_1(unsigned long n)
{
    ...
    return rb_ull2inum((long long)n+1);
...
random_ulong_limited(VALUE obj, rb_random_t *rnd, unsigned long limit)
{
    if (!limit) return 0;
    if (!rnd) {
 const int w = sizeof(limit) * 8 - nlz_long(limit);
...
rb_random_ulong_limited(VALUE obj, unsigned long limit)
{
 ...
 unsigned long r = rb_num2ulong_inline(v);
...
random_ulong_limited_big(VALUE obj, rb_random_t *rnd, VALUE vmax)
{
 ...
 uint32_t *tmp = ((uint32_t*)(((size_t)(len * 2) < 1024 / sizeof(uint32_t)) ? ((* ({ volatile VALUE *rb_gc_guarded_ptr = &(vtmp); ; rb_gc_guarded_ptr; })) = 0, __builtin_alloca ((size_t)(len * 2) * sizeof(uint32_t))) : rb_alloc_tmp_buffer2(&(vtmp), (long)(len * 2), sizeof(uint32_t))));
...
{
    return genrand_bytes(get_rnd(obj), rb_num2long_inline(rb_to_int(len)));
}
...
genrand_bytes(rb_random_t *rnd, long n)
{
    ...
    bytes =  ({ (__builtin_constant_p(0) && __builtin_constant_p(n)) ? rb_str_new_static((0), (n)) : rb_str_new((0), (n)); });
    ptr = (!(((struct RBasic*)(bytes))->flags & RSTRING_NOEMBED) ? ((struct RString*)(bytes))->as.ary : ((struct RString*)(bytes))->as.heap.ptr);
    for (; n >= (31/8 + 1); n -= (31/8 + 1)) {
...
rb_random_bytes(VALUE obj, long n)
{
    rb_random_t *rnd = try_get_rnd(obj);
    if (!rnd) {
 return obj_random_bytes(obj, ((void*)0), n);
    }
...
range_values(VALUE vmax, VALUE *begp, VALUE *endp, int *exclp)
{
    ...
    if (!rb_range_values(vmax, begp, &end, exclp)) return ((VALUE)RUBY_Qfalse);
    if (endp) *endp = end;
    if (!rb_respond_to(end, '-')) return ((VALUE)RUBY_Qfalse);
    r = rb_funcallv(end, '-', 1, begp);
    if (!((VALUE)(r) != ((VALUE)RUBY_Qnil))) return ((VALUE)RUBY_Qfalse);
    return r;
}
...
rand_int(VALUE obj, rb_random_t *rnd, VALUE vmax, int restrictive)
{
    ...
    if ((((int)(long)(vmax))&RUBY_FIXNUM_FLAG)) {
 long max = ((long)(((long)(vmax))>>(int)(1)));
 if (!max) return ((VALUE)RUBY_Qnil);
 if (max < 0) {
     if (restrictive) return ((VALUE)RUBY_Qnil);
     max = -max;
 }
 r = random_ulong_limited(obj, rnd, (unsigned long)max - 1);
 return rb_ulong2num_inline(r);
    }
    else {
 VALUE ret;
 if (rb_bigzero_p(vmax)) return ((VALUE)RUBY_Qnil);
 if (!((((struct RBasic*)(vmax))->flags & ((VALUE)((VALUE)RUBY_FL_USER1))) != 0)) {
     if (restrictive) return ((VALUE)RUBY_Qnil);
            vmax = rb_big_uminus(vmax);
 }
 vmax = rb_big_minus(vmax, (((VALUE)(1))<<1 | RUBY_FIXNUM_FLAG));
 if ((((int)(long)(vmax))&RUBY_FIXNUM_FLAG)) {
     long max = ((long)(((long)(vmax))>>(int)(1)));
     if (max == -1) return ((VALUE)RUBY_Qnil);
     r = random_ulong_limited(obj, rnd, max);
     return rb_long2num_inline(r);
 }
 ret = random_ulong_limited_big(obj, rnd, vmax);
 (* ({ volatile VALUE *rb_gc_guarded_ptr = &(vmax); ; rb_gc_guarded_ptr; }));
 return ret;
    }
}
...
domain_error(void)
{
    VALUE error = (((VALUE)(33))<<1 | RUBY_FIXNUM_FLAG);
...
{
    rb_raise(rb_eArgError, "invalid argument - %li\v", arg0);
}
...
check_random_number(VALUE v, const VALUE *argv)
{
 ...
 (void)rb_num2long_inline(argv[0]);
 break;
...
float_value(VALUE v)
{
    double x = rb_float_value_inline(v);
    if ((sizeof ((x)) == sizeof (float) ? __isinff (x) : sizeof ((x)) == sizeof (double) ? __isinf (x) : __isinfl (x)) || (sizeof ((x)) == sizeof (float) ? __isnanf (x) : sizeof ((x)) == sizeof (double) ? __isnan (x) : __isnanl (x))) {
 domain_error();
    }
    return x;
}
...
{
    VALUE beg = ((VALUE)RUBY_Qundef), end = ((VALUE)RUBY_Qundef), vmax, v;
    int excl = 0;

    if ((v = vmax = range_values(range, &beg, &end, &excl)) == ((VALUE)RUBY_Qfalse))
 return ((VALUE)RUBY_Qfalse);
    if (!( ((RUBY_T_FLOAT) == RUBY_T_FIXNUM) ? (((int)(long)(vmax))&RUBY_FIXNUM_FLAG) : ((RUBY_T_FLOAT) == RUBY_T_TRUE) ? ((vmax) == ((VALUE)RUBY_Qtrue)) : ((RUBY_T_FLOAT) == RUBY_T_FALSE) ? ((vmax) == ((VALUE)RUBY_Qfalse)) : ((RUBY_T_FLOAT) == RUBY_T_NIL) ? ((vmax) == ((VALUE)RUBY_Qnil)) : ((RUBY_T_FLOAT) == RUBY_T_UNDEF) ? ((vmax) == ((VALUE)RUBY_Qundef)) : ((RUBY_T_FLOAT) == RUBY_T_SYMBOL) ? ((((VALUE)(vmax)&~((~(VALUE)0)<<RUBY_SPECIAL_SHIFT)) == RUBY_SYMBOL_FLAG)||(!(((VALUE)(vmax) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(vmax) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(vmax))->flags & RUBY_T_MASK) == (RUBY_T_SYMBOL))) : ((RUBY_T_FLOAT) == RUBY_T_FLOAT) ? ( ((((int)(long)(vmax))&RUBY_FLONUM_MASK) == RUBY_FLONUM_FLAG) || (!(((VALUE)(vmax) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(vmax) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(vmax))->flags & RUBY_T_MASK) == RUBY_T_FLOAT)) : (!(((VALUE)(vmax) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(vmax) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(vmax))->flags & RUBY_T_MASK) == (RUBY_T_FLOAT))) && (v = rb_check_to_int(vmax), !!((VALUE)(v) != ((VALUE)RUBY_Qnil)))) {
 long max;
 vmax = v;
 v = ((VALUE)RUBY_Qnil);
 if ((((int)(long)(vmax))&RUBY_FIXNUM_FLAG)) {
   fixnum:
     if ((max = ((long)(((long)(vmax))>>(int)(1))) - excl) >= 0) {
  unsigned long r = random_ulong_limited(obj, rnd, (unsigned long)max);
  v = rb_ulong2num_inline(r);
     }
 }
 else if ((int)(((struct RBasic*)(vmax))->flags & RUBY_T_MASK) == RUBY_T_BIGNUM && ((((struct RBasic*)(vmax))->flags & ((VALUE)((VALUE)RUBY_FL_USER1))) != 0) && !rb_bigzero_p(vmax)) {
     vmax = excl ? rb_big_minus(vmax, (((VALUE)(1))<<1 | RUBY_FIXNUM_FLAG)) : rb_big_norm(vmax);
     if ((((int)(long)(vmax))&RUBY_FIXNUM_FLAG)) {
  excl = 0;
  goto fixnum;
     }
    ...
    else if (v = rb_check_to_float(vmax), !!((VALUE)(v) != ((VALUE)RUBY_Qnil))) {
 int scale = 1;
 double max = rb_float_value_inline(v), mid = 0.5, r;
 if ((sizeof ((max)) == sizeof (float) ? __isinff (max) : sizeof ((max)) == sizeof (double) ? __isinf (max) : __isinfl (max))) {
 ...
 else if ((sizeof ((max)) == sizeof (float) ? __isnanf (max) : sizeof ((max)) == sizeof (double) ? __isnan (max) : __isnanl (max))) {
     domain_error();
 }
 v = ((VALUE)RUBY_Qnil);
 if (max > 0.0) {
     ...
     if (scale > 1) {
  return rb_float_new_inline(+(+(+(r - 0.5) * max) * scale) + mid);
     }
     v = rb_float_new_inline(r * max);
 }
 else if (max == 0.0 && !excl) {
     v = rb_float_new_inline(0.0);
 }
    }

    if ((((int)(long)(beg))&RUBY_FIXNUM_FLAG) && (((int)(long)(v))&RUBY_FIXNUM_FLAG)) {
 long x = ((long)(((long)(beg))>>(int)(1))) + ((long)(((long)(v))>>(int)(1)));
 return rb_long2num_inline(x);
    }
    switch (rb_type((VALUE)(v))) {
      case RUBY_T_NIL:
 break;
      case RUBY_T_BIGNUM:
 ...
 if (!!((VALUE)(f) != ((VALUE)RUBY_Qnil))) {
     return rb_float_new_inline(rb_float_value_inline(v) + rb_float_value_inline(f));
 }
      }
      default:
 return rb_funcallv(beg, '+', 1, &v);
    }
...
rand_random(int argc, VALUE *argv, VALUE obj, rb_random_t *rnd)
{
    ...
    if (rb_check_arity(argc, 0, 1) == 0) {
 return rb_float_new_inline(random_real(obj, rnd, TRUE));
    }
    vmax = argv[0];
    if (!((VALUE)(vmax) != ((VALUE)RUBY_Qnil))) return ((VALUE)RUBY_Qnil);
    if (!( ((RUBY_T_FLOAT) == RUBY_T_FIXNUM) ? (((int)(long)(vmax))&RUBY_FIXNUM_FLAG) : ((RUBY_T_FLOAT) == RUBY_T_TRUE) ? ((vmax) == ((VALUE)RUBY_Qtrue)) : ((RUBY_T_FLOAT) == RUBY_T_FALSE) ? ((vmax) == ((VALUE)RUBY_Qfalse)) : ((RUBY_T_FLOAT) == RUBY_T_NIL) ? ((vmax) == ((VALUE)RUBY_Qnil)) : ((RUBY_T_FLOAT) == RUBY_T_UNDEF) ? ((vmax) == ((VALUE)RUBY_Qundef)) : ((RUBY_T_FLOAT) == RUBY_T_SYMBOL) ? ((((VALUE)(vmax)&~((~(VALUE)0)<<RUBY_SPECIAL_SHIFT)) == RUBY_SYMBOL_FLAG)||(!(((VALUE)(vmax) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(vmax) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(vmax))->flags & RUBY_T_MASK) == (RUBY_T_SYMBOL))) : ((RUBY_T_FLOAT) == RUBY_T_FLOAT) ? ( ((((int)(long)(vmax))&RUBY_FLONUM_MASK) == RUBY_FLONUM_FLAG) || (!(((VALUE)(vmax) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(vmax) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(vmax))->flags & RUBY_T_MASK) == RUBY_T_FLOAT)) : (!(((VALUE)(vmax) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(vmax) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(vmax))->flags & RUBY_T_MASK) == (RUBY_T_FLOAT)))) {
 v = rb_check_to_int(vmax);
 if (!!((VALUE)(v) != ((VALUE)RUBY_Qnil))) return rand_int(obj, rnd, v, 1);
    }
...
rand_random_number(int argc, VALUE *argv, VALUE obj)
{
    ...
    if (!((VALUE)(v) != ((VALUE)RUBY_Qnil))) v = rand_random(0, 0, obj, rnd);
...
{
    rb_random_t *r1, *r2;
    if (rb_obj_class(self) != rb_obj_class(other)) return ((VALUE)RUBY_Qfalse);
    r1 = get_rnd(self);
    r2 = get_rnd(other);
    if (memcmp(r1->mt.state, r2->mt.state, sizeof(r1->mt.state))) return ((VALUE)RUBY_Qfalse);
    if ((r1->mt.next - r1->mt.state) != (r2->mt.next - r2->mt.state)) return ((VALUE)RUBY_Qfalse);
    if (r1->mt.left != r2->mt.left) return ((VALUE)RUBY_Qfalse);
...
rb_f_rand(int argc, VALUE *argv, VALUE obj)
{
    ...
    if (rb_check_arity(argc, 0, 1) && !!((VALUE)(vmax = argv[0]) != ((VALUE)RUBY_Qnil))) {
 VALUE v = rand_range(((VALUE)RUBY_Qnil), rnd, vmax);
 if (v != ((VALUE)RUBY_Qfalse)) return v;
 vmax = rb_to_int(vmax);
 if (vmax != (((VALUE)(0))<<1 | RUBY_FIXNUM_FLAG)) {
     v = rand_int(((VALUE)RUBY_Qnil), rnd, vmax, 0);
     if (!!((VALUE)(v) != ((VALUE)RUBY_Qnil))) return v;
 }
    }
    return rb_float_new_inline(genrand_real(&rnd->mt));
}
static VALUE
random_s_rand(int argc, VALUE *argv, VALUE obj)
{
    VALUE v = rand_random(argc, argv, ((VALUE)RUBY_Qnil), rand_start(&default_rand));
    check_random_number(v, argv);
    return v;
}
...
uint64_t ruby_sip_hash13(const uint8_t key[16], const uint8_t *data, size_t len);
static const union {
    char bin[32];
    uint64_t u64[4];
} sip_init_state_bin = {"uespemosmodnarodarenegylsetybdet"};
uint64_t
ruby_sip_hash13(const uint8_t key[16], const uint8_t *data, size_t len)
{
    uint64_t k0, k1;
    ...
    k0 = ((uint64_t)(((uint32_t)((key)[0]) ) | ((uint32_t)((key)[1]) << 8) | ((uint32_t)((key)[2]) << 16) | ((uint32_t)((key)[3]) << 24)) | ((uint64_t)(((uint32_t)(((key) + 4)[0]) ) | ((uint32_t)(((key) + 4)[1]) << 8) | ((uint32_t)(((key) + 4)[2]) << 16) | ((uint32_t)(((key) + 4)[3]) << 24))) << 32 );
    k1 = ((uint64_t)(((uint32_t)((key + sizeof(uint64_t))[0]) ) | ((uint32_t)((key + sizeof(uint64_t))[1]) << 8) | ((uint32_t)((key + sizeof(uint64_t))[2]) << 16) | ((uint32_t)((key + sizeof(uint64_t))[3]) << 24)) | ((uint64_t)(((uint32_t)(((key + sizeof(uint64_t)) + 4)[0]) ) | ((uint32_t)(((key + sizeof(uint64_t)) + 4)[1]) << 8) | ((uint32_t)(((key + sizeof(uint64_t)) + 4)[2]) << 16) | ((uint32_t)(((key + sizeof(uint64_t)) + 4)[3]) << 24))) << 32 );

    v0 = k0; ((v0) ^= (sip_init_state_bin.u64[0]));
    v1 = k1; ((v1) ^= (sip_init_state_bin.u64[1]));
    v2 = k0; ((v2) ^= (sip_init_state_bin.u64[2]));
    v3 = k1; ((v3) ^= (sip_init_state_bin.u64[3]));
...

    {
     ...
     m = *data64++;
     do { (((v3)) ^= ((m))); do { (((v0)) += ((v1))); (((v2)) += ((v3))); (((v1)) = ((((v1))) << ((13))) | ((((v1))) >> (64 - ((13))))); (((v3)) = ((((v3))) << ((16))) | ((((v3))) >> (64 - ((16))))); (((v1)) ^= ((v0))); (((v3)) ^= ((v2))); (((v0)) = ((((v0))) << ((32))) | ((((v0))) >> (64 - ((32))))); (((v2)) += ((v1))); (((v0)) += ((v3))); (((v1)) = ((((v1))) << ((17))) | ((((v1))) >> (64 - ((17))))); (((v3)) = ((((v3))) << ((21))) | ((((v3))) >> (64 - ((21))))); (((v1)) ^= ((v2))); (((v3)) ^= ((v0))); (((v2)) = ((((v2))) << ((32))) | ((((v2))) >> (64 - ((32))))); } while(0); (((v0)) ^= ((m))); } while (0);
        }
    }
    last = (uint64_t)len << 56;
    switch (len % sizeof(uint64_t)) {
 case 7:
     (last |= ((uint64_t) end[6]) << ((6) * 8));
 case 6:
     (last |= ((uint64_t) end[5]) << ((5) * 8));
 case 5:
     (last |= ((uint64_t) end[4]) << ((4) * 8));
 case 4:
 ...
 case 3:
     (last |= ((uint64_t) end[2]) << ((2) * 8));
 case 2:
     (last |= ((uint64_t) end[1]) << ((1) * 8));
 case 1:
     (last |= ((uint64_t) end[0]) << ((0) * 8));
     break;
 case 0:
     break;
    }

    do { (((v3)) ^= ((last))); do { (((v0)) += ((v1))); (((v2)) += ((v3))); (((v1)) = ((((v1))) << ((13))) | ((((v1))) >> (64 - ((13))))); (((v3)) = ((((v3))) << ((16))) | ((((v3))) >> (64 - ((16))))); (((v1)) ^= ((v0))); (((v3)) ^= ((v2))); (((v0)) = ((((v0))) << ((32))) | ((((v0))) >> (64 - ((32))))); (((v2)) += ((v1))); (((v0)) += ((v3))); (((v1)) = ((((v1))) << ((17))) | ((((v1))) >> (64 - ((17))))); (((v3)) = ((((v3))) << ((21))) | ((((v3))) >> (64 - ((21))))); (((v1)) ^= ((v2))); (((v3)) ^= ((v0))); (((v2)) = ((((v2))) << ((32))) | ((((v2))) >> (64 - ((32))))); } while(0); (((v0)) ^= ((last))); } while (0);

    ((v2) ^= (0xff));

    do { (((v0)) += ((v1))); (((v2)) += ((v3))); (((v1)) = ((((v1))) << ((13))) | ((((v1))) >> (64 - ((13))))); (((v3)) = ((((v3))) << ((16))) | ((((v3))) >> (64 - ((16))))); (((v1)) ^= ((v0))); (((v3)) ^= ((v2))); (((v0)) = ((((v0))) << ((32))) | ((((v0))) >> (64 - ((32))))); (((v2)) += ((v1))); (((v0)) += ((v3))); (((v1)) = ((((v1))) << ((17))) | ((((v1))) >> (64 - ((17))))); (((v3)) = ((((v3))) << ((21))) | ((((v3))) >> (64 - ((21))))); (((v1)) ^= ((v2))); (((v3)) ^= ((v0))); (((v2)) = ((((v2))) << ((32))) | ((((v2))) >> (64 - ((32))))); } while(0);
    do { (((v0)) += ((v1))); (((v2)) += ((v3))); (((v1)) = ((((v1))) << ((13))) | ((((v1))) >> (64 - ((13))))); (((v3)) = ((((v3))) << ((16))) | ((((v3))) >> (64 - ((16))))); (((v1)) ^= ((v0))); (((v3)) ^= ((v2))); (((v0)) = ((((v0))) << ((32))) | ((((v0))) >> (64 - ((32))))); (((v2)) += ((v1))); (((v0)) += ((v3))); (((v1)) = ((((v1))) << ((17))) | ((((v1))) >> (64 - ((17))))); (((v3)) = ((((v3))) << ((21))) | ((((v3))) >> (64 - ((21))))); (((v1)) ^= ((v2))); (((v3)) ^= ((v0))); (((v2)) = ((((v2))) << ((32))) | ((((v2))) >> (64 - ((32))))); } while(0);
    do { (((v0)) += ((v1))); (((v2)) += ((v3))); (((v1)) = ((((v1))) << ((13))) | ((((v1))) >> (64 - ((13))))); (((v3)) = ((((v3))) << ((16))) | ((((v3))) >> (64 - ((16))))); (((v1)) ^= ((v0))); (((v3)) ^= ((v2))); (((v0)) = ((((v0))) << ((32))) | ((((v0))) >> (64 - ((32))))); (((v2)) += ((v1))); (((v0)) += ((v3))); (((v1)) = ((((v1))) << ((17))) | ((((v1))) >> (64 - ((17))))); (((v3)) = ((((v3))) << ((21))) | ((((v3))) >> (64 - ((21))))); (((v1)) ^= ((v2))); (((v3)) ^= ((v0))); (((v2)) = ((((v2))) << ((32))) | ((((v2))) >> (64 - ((32))))); } while(0);
...
static union {
    seed_keys_t key;
    uint32_t u32[(((sizeof(seed_keys_t)) + (sizeof(uint32_t)) - 1) / (sizeof(uint32_t)))];
} seed;
...
init_seed(struct MT *mt)
{
    ...
    for (i = 0; i < ((int)(sizeof(seed.u32) / sizeof((seed.u32)[0]))); ++i)
...
{
    return ((st_index_t)(seed.key.hash + h));
}

st_index_t
rb_memhash(const void *ptr, long len)
{
    sip_uint64_t h = ruby_sip_hash13(seed.key.sip, ptr, len);
...
Init_RandomSeedCore(void)
{
    ...
    fill_random_seed(initial_seed, 4);
    init_by_array(&mt, initial_seed, 4);

    init_seed(&mt);

    explicit_bzero(initial_seed, (4 * (int)sizeof(int32_t)));
}

static VALUE
init_randomseed(struct MT *mt)
{
    uint32_t initial[4 +1];
    VALUE seed;

    fill_random_seed(initial, 4);
    init_by_array(mt, initial, 4);
    seed = make_seed_value(initial, 4);
    explicit_bzero(initial, (4 * (int)sizeof(int32_t)));
    return seed;
}
...
Init_Random_default(void)
{
    rb_random_t *r = &default_rand;
    struct MT *mt = &r->mt;
    VALUE v = rb_data_typed_object_wrap((rb_cRandom),(r),(&random_data_type));
...
rb_reset_random_seed(void)
{
    rb_random_t *r = &default_rand;
    ((&r->mt)->next = 0);
    r->seed = (((VALUE)(0))<<1 | RUBY_FIXNUM_FLAG);
}
void
InitVM_Random(void)
{
 ...
 rb_define_const(rb_cRandom, "DEFAULT", rand_default);
    ...
    rb_define_private_method(rb_class_of((VALUE)(rb_cRandom)), "state", random_s_state, 0);
    rb_define_private_method(rb_class_of((VALUE)(rb_cRandom)), "left", random_s_left, 0);
...
Init_Random(void)
{
    ...
    {void InitVM_Random(void);InitVM_Random();};
}
