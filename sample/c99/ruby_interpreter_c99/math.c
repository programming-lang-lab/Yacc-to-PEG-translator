...
math_atan2(VALUE unused_obj, VALUE y, VALUE x)
{
    ...
    dy = rb_num_to_dbl(y);
    if (dx == 0.0 && dy == 0.0) {
 if (!(sizeof ((dx)) == sizeof (float) ? __builtin_signbitf (dx) : sizeof ((dx)) == sizeof (double) ? __builtin_signbit (dx) : __builtin_signbitl (dx)))
     return rb_float_new_inline(dy);
        if (!(sizeof ((dy)) == sizeof (float) ? __builtin_signbitf (dy) : sizeof ((dy)) == sizeof (double) ? __builtin_signbit (dy) : __builtin_signbitl (dy)))
     return rb_float_new_inline(3.14159265358979323846);
 return rb_float_new_inline(-3.14159265358979323846);
    }
    return rb_float_new_inline(atan2(dy, dx));
}
static VALUE
math_cos(VALUE unused_obj, VALUE x)
{
    return rb_float_new_inline(cos(rb_num_to_dbl(x)));
}
static VALUE
math_sin(VALUE unused_obj, VALUE x)
{
    return rb_float_new_inline(sin(rb_num_to_dbl(x)));
}
static VALUE
math_tan(VALUE unused_obj, VALUE x)
{
    return rb_float_new_inline(tan(rb_num_to_dbl(x)));
}
static VALUE
math_acos(VALUE unused_obj, VALUE x)
{
    ...
    if (d < -1.0 || 1.0 < d) rb_raise(rb_eMathDomainError, "Numerical argument is out of domain - \"acos\"");
    return rb_float_new_inline(acos(d));
}
static VALUE
math_asin(VALUE unused_obj, VALUE x)
{
    ...
    if (d < -1.0 || 1.0 < d) rb_raise(rb_eMathDomainError, "Numerical argument is out of domain - \"asin\"");
    return rb_float_new_inline(asin(d));
}
static VALUE
math_atan(VALUE unused_obj, VALUE x)
{
    return rb_float_new_inline(atan(rb_num_to_dbl(x)));
}
static VALUE
math_cosh(VALUE unused_obj, VALUE x)
{
    return rb_float_new_inline(cosh(rb_num_to_dbl(x)));
}
static VALUE
math_sinh(VALUE unused_obj, VALUE x)
{
    return rb_float_new_inline(sinh(rb_num_to_dbl(x)));
}
static VALUE
math_tanh(VALUE unused_obj, VALUE x)
{
    return rb_float_new_inline(tanh(rb_num_to_dbl(x)));
}
static VALUE
math_acosh(VALUE unused_obj, VALUE x)
{
    ...
    if (d < 1.0) rb_raise(rb_eMathDomainError, "Numerical argument is out of domain - \"acosh\"");
    return rb_float_new_inline(acosh(d));
}
static VALUE
math_asinh(VALUE unused_obj, VALUE x)
{
    return rb_float_new_inline(asinh(rb_num_to_dbl(x)));
}
static VALUE
math_atanh(VALUE unused_obj, VALUE x)
{
    ...
    if (d < -1.0 || +1.0 < d) rb_raise(rb_eMathDomainError, "Numerical argument is out of domain - \"atanh\"");

    if (d == -1.0) return rb_float_new_inline(-(__builtin_inff ()));
    if (d == +1.0) return rb_float_new_inline(+(__builtin_inff ()));
    return rb_float_new_inline(atanh(d));
}
static VALUE
math_exp(VALUE unused_obj, VALUE x)
{
    return rb_float_new_inline(exp(rb_num_to_dbl(x)));
}
...
math_log1(VALUE x)
{
    ...
    if (d < 0.0) rb_raise(rb_eMathDomainError, "Numerical argument is out of domain - \"log\"");

    if (d == 0.0) return -(__builtin_inff ());

    return log(d) + numbits * 0.69314718055994530942;
}
...
math_log2(VALUE unused_obj, VALUE x)
{
    ...
    if (d < 0.0) rb_raise(rb_eMathDomainError, "Numerical argument is out of domain - \"log2\"");

    if (d == 0.0) return rb_float_new_inline(-(__builtin_inff ()));

    return rb_float_new_inline(log2(d) + numbits);
}
static VALUE
math_log10(VALUE unused_obj, VALUE x)
{
    ...
    if (d < 0.0) rb_raise(rb_eMathDomainError, "Numerical argument is out of domain - \"log10\"");

    if (d == 0.0) return rb_float_new_inline(-(__builtin_inff ()));

    return rb_float_new_inline(log10(d) + numbits * log10(2));
}
static VALUE
...
f_signbit(VALUE x)
{
    if (( ((RUBY_T_FLOAT) == RUBY_T_FIXNUM) ? (((int)(long)(x))&RUBY_FIXNUM_FLAG) : ((RUBY_T_FLOAT) == RUBY_T_TRUE) ? ((x) == ((VALUE)RUBY_Qtrue)) : ((RUBY_T_FLOAT) == RUBY_T_FALSE) ? ((x) == ((VALUE)RUBY_Qfalse)) : ((RUBY_T_FLOAT) == RUBY_T_NIL) ? ((x) == ((VALUE)RUBY_Qnil)) : ((RUBY_T_FLOAT) == RUBY_T_UNDEF) ? ((x) == ((VALUE)RUBY_Qundef)) : ((RUBY_T_FLOAT) == RUBY_T_SYMBOL) ? ((((VALUE)(x)&~((~(VALUE)0)<<RUBY_SPECIAL_SHIFT)) == RUBY_SYMBOL_FLAG)||(!(((VALUE)(x) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(x) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(x))->flags & RUBY_T_MASK) == (RUBY_T_SYMBOL))) : ((RUBY_T_FLOAT) == RUBY_T_FLOAT) ? ( ((((int)(long)(x))&RUBY_FLONUM_MASK) == RUBY_FLONUM_FLAG) || (!(((VALUE)(x) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(x) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(x))->flags & RUBY_T_MASK) == RUBY_T_FLOAT)) : (!(((VALUE)(x) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(x) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(x))->flags & RUBY_T_MASK) == (RUBY_T_FLOAT)))) {
        double f = rb_float_value_inline(x);
        return ((!(sizeof ((f)) == sizeof (float) ? __isnanf (f) : sizeof ((f)) == sizeof (double) ? __isnan (f) : __isnanl (f)) && (sizeof ((f)) == sizeof (float) ? __builtin_signbitf (f) : sizeof ((f)) == sizeof (double) ? __builtin_signbit (f) : __builtin_signbitl (f))) ? ((VALUE)RUBY_Qtrue) : ((VALUE)RUBY_Qfalse));
    }
...
math_frexp(VALUE unused_obj, VALUE x)
{
    ...
    d = frexp(rb_num_to_dbl(x), &exp);
    return rb_assoc_new(rb_float_new_inline(d), (((VALUE)((int)(exp)))<<1 | RUBY_FIXNUM_FLAG));
}
static VALUE
math_ldexp(VALUE unused_obj, VALUE x, VALUE n)
{
    return rb_float_new_inline(ldexp(rb_num_to_dbl(x), rb_num2int_inline(n)));
}
static VALUE
math_hypot(VALUE unused_obj, VALUE x, VALUE y)
{
    return rb_float_new_inline(hypot(rb_num_to_dbl(x), rb_num_to_dbl(y)));
}
static VALUE
math_erf(VALUE unused_obj, VALUE x)
{
    return rb_float_new_inline(erf(rb_num_to_dbl(x)));
}
static VALUE
math_erfc(VALUE unused_obj, VALUE x)
{
    return rb_float_new_inline(erfc(rb_num_to_dbl(x)));
}
static VALUE
math_gamma(VALUE unused_obj, VALUE x)
{
    ...
    enum {NFACT_TABLE = ((int)(sizeof(fact_table) / sizeof((fact_table)[0])))};
    double d;
    d = rb_num_to_dbl(x);

    if ((sizeof ((d)) == sizeof (float) ? __isinff (d) : sizeof ((d)) == sizeof (double) ? __isinf (d) : __isinfl (d))) {
 if ((sizeof ((d)) == sizeof (float) ? __builtin_signbitf (d) : sizeof ((d)) == sizeof (double) ? __builtin_signbit (d) : __builtin_signbitl (d))) rb_raise(rb_eMathDomainError, "Numerical argument is out of domain - \"gamma\"");
 return rb_float_new_inline((__builtin_inff ()));
    }
    if (d == 0.0) {
 return (sizeof ((d)) == sizeof (float) ? __builtin_signbitf (d) : sizeof ((d)) == sizeof (double) ? __builtin_signbit (d) : __builtin_signbitl (d)) ? rb_float_new_inline(-(__builtin_inff ())) : rb_float_new_inline((__builtin_inff ()));
    }
    if (d == floor(d)) {
 if (d < 0.0) rb_raise(rb_eMathDomainError, "Numerical argument is out of domain - \"gamma\"");
 if (1.0 <= d && d <= (double)NFACT_TABLE) {
     return rb_float_new_inline(fact_table[(int)d - 1]);
 }
    }
    return rb_float_new_inline(tgamma(d));
}
static VALUE
math_lgamma(VALUE unused_obj, VALUE x)
{
    ...
    if ((sizeof ((d)) == sizeof (float) ? __isinff (d) : sizeof ((d)) == sizeof (double) ? __isinf (d) : __isinfl (d))) {
 if ((sizeof ((d)) == sizeof (float) ? __builtin_signbitf (d) : sizeof ((d)) == sizeof (double) ? __builtin_signbit (d) : __builtin_signbitl (d))) rb_raise(rb_eMathDomainError, "Numerical argument is out of domain - \"lgamma\"");
 return rb_assoc_new(rb_float_new_inline((__builtin_inff ())), (((VALUE)(1))<<1 | RUBY_FIXNUM_FLAG));
    }
    if (d == 0.0) {
 VALUE vsign = (sizeof ((d)) == sizeof (float) ? __builtin_signbitf (d) : sizeof ((d)) == sizeof (double) ? __builtin_signbit (d) : __builtin_signbitl (d)) ? (((VALUE)(-1))<<1 | RUBY_FIXNUM_FLAG) : (((VALUE)(+1))<<1 | RUBY_FIXNUM_FLAG);
 return rb_assoc_new(rb_float_new_inline((__builtin_inff ())), vsign);
    }
    v = rb_float_new_inline(lgamma_r(d, &sign));
    return rb_assoc_new(v, (((VALUE)(sign))<<1 | RUBY_FIXNUM_FLAG));
}
VALUE rb_math_atan2(VALUE x, VALUE y){ return math_atan2(0, x, y);}
VALUE rb_math_cos(VALUE x){ return math_cos(0, x);}
VALUE rb_math_cosh(VALUE x){ return math_cosh(0, x);}
VALUE rb_math_exp(VALUE x){ return math_exp(0, x);}
VALUE rb_math_hypot(VALUE x, VALUE y){ return math_hypot(0, x, y);}
...
VALUE rb_math_sin(VALUE x){ return math_sin(0, x);}
VALUE rb_math_sinh(VALUE x){ return math_sinh(0, x);}
void
InitVM_Math(void)
{
    ...
    rb_define_const(rb_mMath, "PI", rb_float_new_inline(3.14159265358979323846));



    rb_define_const(rb_mMath, "E", rb_float_new_inline(2.7182818284590452354));
...
Init_Math(void)
{
    {void InitVM_Math(void);InitVM_Math();};
}
