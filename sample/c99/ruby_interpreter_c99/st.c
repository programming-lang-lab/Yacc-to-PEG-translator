...
st_memsize(const st_table *tab)
{
    return(sizeof(st_table)
           + (tab->bins == ((void*)0) ? 0 : bins_size(tab))
...
rebuild_table(st_table *tab)
{
 ...
 if (__builtin_expect(bins != ((void*)0), 1)) {
     bin_ind = find_table_bin_ind_direct(new_tab, curr_entry_ptr->hash,
      curr_entry_ptr->key);
     ((void)(0 && (bin_ind != (~(st_index_t) 0))));
     ((void)(0 && (tab == new_tab || new_tab->rebuilds_num == 0)));
     ((void)(0 && ((((get_bin((new_tab)->bins, get_size_ind(new_tab), bin_ind)) == 0)))));
 ...
 ((void)(0 && (tab->num_entries == ni)));
 ((void)(0 && (new_tab->num_entries == ni)));
 if (tab->bins != ((void*)0))
     ruby_xfree(tab->bins);
...
find_entry(st_table *tab, st_hash_t hash_value, st_data_t key)
{
    ...
    entries = tab->entries;
    for (i = tab->entries_start; i < bound; i++) {
 do { unsigned int _old_rebuilds_num = (tab)->rebuilds_num; eq_p = ((&entries[i])->hash == (hash_value) && (((key)) == ((&entries[i])->key) || (*((tab))->type->compare)(((key)),((&entries[i])->key)) == 0)); rebuilt_p = _old_rebuilds_num != (tab)->rebuilds_num; } while (FALSE);
 if (__builtin_expect(rebuilt_p, 0))
     return (~(st_index_t) 1);
 if (eq_p)
     return i;
    }
...
find_table_entry_ind(st_table *tab, st_hash_t hash_value, st_data_t key)
{
    ...
    for (;;) {
        ...
        if (! ((bin) <= 1)) {
     do { unsigned int _old_rebuilds_num = (tab)->rebuilds_num; eq_p = ((&entries[bin - 2])->hash == (hash_value) && (((key)) == ((&entries[bin - 2])->key) || (*((tab))->type->compare)(((key)),((&entries[bin - 2])->key)) == 0)); rebuilt_p = _old_rebuilds_num != (tab)->rebuilds_num; } while (FALSE);
     if (__builtin_expect(rebuilt_p, 0))
  return (~(st_index_t) 1);
     if (eq_p)
  break;
...
find_table_bin_ind_direct(st_table *tab, st_hash_t hash_value, st_data_t key)
{
    ...
    for (;;) {
        ...
        if (((bin) <= 1))
     return ind;
 ((void)(0 && (entries[bin - 2].hash != hash_value)));
          ...
          st_data_t key, st_index_t *bin_ind)
{
    ...
    ((void)(0 && (tab != ((void*)0))));
    ((void)(0 && (tab->bins != ((void*)0))));
    ((void)(0 && (tab->entries_bound <= get_allocated_entries(tab))));
    ((void)(0 && (tab->entries_start <= tab->entries_bound)));
    ...
    for (;;) {
     ...
     do { unsigned int _old_rebuilds_num = (tab)->rebuilds_num; eq_p = ((&entries[entry_index - 2])->hash == (curr_hash_value) && (((key)) == ((&entries[entry_index - 2])->key) || (*((tab))->type->compare)(((key)),((&entries[entry_index - 2])->key)) == 0)); rebuilt_p = _old_rebuilds_num != (tab)->rebuilds_num; } while (FALSE);
     if (__builtin_expect(rebuilt_p, 0))
  return (~(st_index_t) 1);
            if (eq_p)
                break;
...
st_lookup(st_table *tab, st_data_t key, st_data_t *value)
{
 ...
 if (__builtin_expect(bin == (~(st_index_t) 1), 0))
     goto retry;
 if (bin == (~(st_index_t) 0))
     return 0;
    }
           ...
           st_data_t (*func)(st_data_t))
{
 ...
 if (bin_ind != (~(st_index_t) 0))
     set_bin(tab->bins, get_size_ind(tab), bin_ind, ind + 2);
 ((void)(0 && (do_hash(key, tab) == hash_value)));
...
st_copy(st_table *old_tab)
{
    ...
    *new_tab = *old_tab;
    ...
    new_tab->entries = (st_table_entry *) ruby_xmalloc(get_allocated_entries(old_tab)
       * sizeof(st_table_entry));
    memcpy((new_tab->entries), (old_tab->entries), sizeof(st_table_entry)*(size_t)(get_allocated_entries(old_tab)));

    if (old_tab->bins != ((void*)0))
        memcpy((new_tab->bins), (old_tab->bins), sizeof(char)*(size_t)(bins_size(old_tab)));
...
st_general_delete(st_table *tab, st_data_t *key, st_data_t *value)
{
 ...
 if (__builtin_expect(bin_ind == (~(st_index_t) 1), 0))
     goto retry;
 if (bin_ind == (~(st_index_t) 0)) {
     if (value != 0) *value = 0;
     return 0;
 ...
 do { ((void)(0 && (bin_ind != (~(st_index_t) 0)))); ((void)(0 && (! (((get_bin((tab)->bins, get_size_ind(tab), bin_ind)) <= 1))))); set_bin((tab)->bins, get_size_ind(tab), bin_ind, 1); } while (0);
    }
...
st_shift(st_table *tab, st_data_t *key, st_data_t *value)
{
 ...
 retry:
  ...
  do { ((void)(0 && (bin_ind != (~(st_index_t) 0)))); ((void)(0 && (! (((get_bin((tab)->bins, get_size_ind(tab), bin_ind)) <= 1))))); set_bin((tab)->bins, get_size_ind(tab), bin_ind, 1); } while (0);
     ...
     ((void)(0 && (entry_hash != curr_entry_ptr->hash && entry_key == curr_entry_ptr->key)));
     ...
     int check_p)
{
    ...
    st_hash_t hash;
    st_data_t key;
    int error_p, packed_p = tab->bins == ((void*)0);
 ...
 retry:
     entries = tab->entries;
     packed_p = tab->bins == ((void*)0);
     if (packed_p) {
         i = find_entry(tab, hash, key);
  if (__builtin_expect(i == (~(st_index_t) 1), 0))
      goto retry;
...
murmur_step(st_index_t h, st_index_t k)
{
    ...
    k *= ((st_index_t)(0x87c37b91)<<32|(st_index_t)(0x114253d5));;
    h ^= ((k)<<((33))|(k)>>(8*8 -((33))));
    h *= ((st_index_t)(0x4cf5ad43)<<32|(st_index_t)(0x2745937f));;
    h = ((h)<<((24))|(h)>>(8*8 -((24))));
    return h;
}
...
{
    const st_index_t c1 = ((st_index_t)(0xbf58476d)<<32|(st_index_t)(0x1ce4e5b9));
    const st_index_t c2 = ((st_index_t)(0x94d049bb)<<32|(st_index_t)(0x133111eb));
    ...
    if (len >= sizeof(st_index_t)) {
 {
  ...
  len -= sizeof(st_index_t);
    ...
    switch (len) {
      ...
      case 7: t |= (st_index_t)((unsigned char)data[(6)]) << 48;
      case 6: t |= (st_index_t)((unsigned char)data[(5)]) << 40;
      case 5: t |= (st_index_t)((unsigned char)data[(4)]) << 32;
      case 4:
 t |= (st_index_t)*(uint32_t*)data;
 goto skip_tail;


      case 3: t |= (st_index_t)((unsigned char)data[(2)]) << 16;
      case 2: t |= (st_index_t)((unsigned char)data[(1)]) << 8;
      case 1: t |= (st_index_t)((unsigned char)data[(0)]);
      skip_tail:
...
strhash(st_data_t arg)
{
    ...
    return st_hash(string, strlen(string), 0x811c9dc5);
}
...
{
    register const char *string = (const char *)arg;
    register st_index_t hval = 0x811c9dc5;
...
st_expand_table(st_table *tab, st_index_t siz)
{
    ...
    memcpy((tmp->entries), (tab->entries), sizeof(st_table_entry)*(size_t)(n));
    ruby_xfree(tab->entries);
    if (tab->bins != ((void*)0))
        ruby_xfree(tab->bins);
    if (tmp->bins != ((void*)0))
...
st_rehash_linear(st_table *tab)
{
    ...
    for (i = tab->entries_start; i < tab->entries_bound; i++) {
        p = &tab->entries[i];
        if (((p)->hash == (~(st_hash_t) 0)))
            continue;
        for (j = i + 1; j < tab->entries_bound; j++) {
            q = &tab->entries[j];
            if (((q)->hash == (~(st_hash_t) 0)))
                continue;
     do { unsigned int _old_rebuilds_num = (tab)->rebuilds_num; eq_p = ((p)->hash == (q->hash) && (((q->key)) == ((p)->key) || (*((tab))->type->compare)(((q->key)),((p)->key)) == 0)); rebuilt_p = _old_rebuilds_num != (tab)->rebuilds_num; } while (FALSE);
     if (__builtin_expect(rebuilt_p, 0))
  return TRUE;
     if (eq_p) {
                ((void)(0 && (p < q)));
...
st_rehash_indexed(st_table *tab)
{
    ...
    st_index_t *bins = ruby_xrealloc(tab->bins, n);
    ((void)(0 && (bins != ((void*)0))));
    tab->bins = bins;
    initialize_bins(tab);
    for (i = tab->entries_start; i < tab->entries_bound; i++) {
        ...
        for(;;) {
            ...
            st_table_entry *q = &tab->entries[bin - 2];
            if (((bin) <= 1)) {

                set_bin(bins, size_ind, ind, i + 2);
                break;
            }
            else {
  do { unsigned int _old_rebuilds_num = (tab)->rebuilds_num; eq_p = ((q)->hash == (p->hash) && (((p->key)) == ((q)->key) || (*((tab))->type->compare)(((p->key)),((q)->key)) == 0)); rebuilt_p = _old_rebuilds_num != (tab)->rebuilds_num; } while (FALSE);
  if (__builtin_expect(rebuilt_p, 0))
      return TRUE;
  if (eq_p) {
...
st_insert_single(st_table *tab, VALUE hash, VALUE key, VALUE val)
{
    ...
    tab->num_entries++;
    rb_obj_written((VALUE)(hash), (VALUE)(((VALUE)RUBY_Qundef)), (VALUE)(k), "st.c", 2244);
    rb_obj_written((VALUE)(hash), (VALUE)(((VALUE)RUBY_Qundef)), (VALUE)(val), "st.c", 2245);
}
...
st_insert_linear(st_table *tab, long argc, const VALUE *argv, VALUE hash)
{
    ...
    for (i = 0; i < argc; ) {
        ...
        st_data_t v = argv[i++];
        st_insert(tab, k, v);
        rb_obj_written((VALUE)(hash), (VALUE)(((VALUE)RUBY_Qundef)), (VALUE)(k), "st.c", 2257);
        rb_obj_written((VALUE)(hash), (VALUE)(((VALUE)RUBY_Qundef)), (VALUE)(v), "st.c", 2258);
    }
}
...
{
    st_index_t n;
    st_table *tab = ((struct RHash*)(hash))->ntbl;
    ...
    if (! tab) {
        ...
        memset(&(((struct RBasicRaw *)((VALUE)(tmp)))->klass), 0, sizeof(VALUE));
        ((struct RHash*)(hash))->ntbl = tab = ((struct RHash*)(tmp))->ntbl;
        ((struct RHash*)(tmp))->ntbl = ((void*)0);
    }
    n = tab->num_entries + argc / 2;
    st_expand_table(tab, n);
    if ((__builtin_expect(!!(tab->num_entries), 0)))
        st_insert_generic(tab, argc, argv, hash);
    else if (argc <= 2)
        st_insert_single(tab, hash, argv[0], argv[1]);
    else if (tab->bin_power <= 4)
