...
ruby_scan_hex(const char *start, size_t len, size_t *retlen)
{
    ...
    while (len-- && *s && (tmp = strchr(ruby_hexdigits, *s))) {
 retval <<= 4;
 retval |= (tmp - ruby_hexdigits) & 15;
 s++;
    }
    *retlen = (int)(s - start);
    return retval;
...
ruby_getcwd(void)
{
    VALUE guard = rb_data_object_wrap(((VALUE)0),(((void*)0)),(RUBY_DATA_FUNC)(((void*)0)),(RUBY_DATA_FUNC)(free));
    char *buf, *cwd = getcwd(((void*)0), 0);
    (((struct RData*)(guard))->data) = cwd;
    if (!cwd) rb_sys_fail("getcwd");
    buf = ruby_strdup(cwd);
    free(cwd);

    (((struct RData*)((* ({ volatile VALUE *rb_gc_guarded_ptr = &(guard); ; rb_gc_guarded_ptr; }))))->data) = ((void*)0);
    return buf;
}
...
Balloc(int k)
{
    ...
    if (k <= 15 && (rv = freelist[k]) != 0) {
        ...
        len = (sizeof(Bigint) + (x-1)*sizeof(unsigned int) + sizeof(double) - 1)
                /sizeof(double);
        if (k <= 15 && pmem_next - private_mem + len <= ((2304 +sizeof(double)-1)/sizeof(double))) {
            rv = (Bigint*)pmem_next;
            pmem_next += len;
        }
        else
            rv = (Bigint*)ruby_xmalloc(len*sizeof(double));
    ...
    for (; xb < xbe; xc0++) {
        if ((y = *xb++) != 0) {
            ...
            do {
                ...
                carry = z >> 32;
                *xc++ = (unsigned int)(z & 0xffffffffUL);
            } while (x < xae);
...
diff(Bigint *a, Bigint *b)
{
    ...
    do {
        ...
        *xc++ = (unsigned int)(y & 0xffffffffUL);
    } while (xb < xbe);
    while (xa < xae) {
        y = *xa++ - borrow;
        borrow = y >> 32 & (unsigned int)1;
...
break2:
 ...
 if (s[1] == 'x' || s[1] == 'X') {
     ...
     if (!*++s || !(s1 = strchr(ruby_hexdigits, *s))) goto ret0;
     if (*s == '0') {
  while (*++s == '0');
  s1 = strchr(ruby_hexdigits, *s);
     }
     if (s1 != ((void*)0)) {
  do {
      adj += aadj * ((s1 - ruby_hexdigits) & 15);
      nd0 += 4;
      aadj /= 16;
  } while (*++s && (s1 = strchr(ruby_hexdigits, *s)));
     }
  ...
  for (; *s && (s1 = strchr(ruby_hexdigits, *s)); ++s) {
      adj += aadj * ((s1 - ruby_hexdigits) & 15);
      if ((aadj /= 16) == 0.0) {
   while (strchr(ruby_hexdigits, *++s));
   break;
      }
  }
     }
...
quorem(Bigint *b, Bigint *S)
{
    ...
    if (q) {
        ...
        do {
            ...
            borrow = y >> 32 & (unsigned int)1;
            *bx++ = (unsigned int)(y & 0xffffffffUL);
        } while (sx <= sxe);
        if (!*bxe) {
            bx = b->x;
            while (--bxe > bx && !*bxe)
...
ruby_dtoa(double d_, int mode, int ndigits, int *decpt, int *sign, char **rve)
{
...

    {
        ...
        *decpt = 9999;
        ...
        if (!((d).L[0]) && !(((d).L[1]) & 0xfffff))
            return nrv_alloc((INFSTR), (rve), strlen(INFSTR)+1);

        return nrv_alloc((NANSTR), (rve), strlen(NANSTR)+1);
    }
    ...
    if (!((d).d)) {
        *decpt = 1;
        return nrv_alloc((ZEROSTR), (rve), strlen(ZEROSTR)+1);
    }
    b = d2b(((d).d), &be, &bbits);
...
ruby_each_words(const char *str, void (*func)(const char*, int, void*), void *arg)
{
    ...
    if (!str) return;
    for (; *str; str = end) {
 while (rb_isspace(*str) || *str == ',') str++;
 if (!*str) break;
 end = str;
 while (*end && !rb_isspace(*end) && *end != ',') end++;
 len = (int)(end - str);
 (*func)(str, len, arg);
    }
}
    ...
    char **rve)
{
 ...
 else if (((int)(((u).L[1]) >> 20) & ~0x100000)) {
     *decpt = ((int)(((u).L[1]) >> 20) & ~0x100000) - (1024 - 2);
 }
 else {
     u.d *= 5.363123171977039e+154 ;
     *decpt = ((int)(((u).L[1]) >> 20) & ~0x100000) - (514 + (1024 - 2));
 }
 ...
 bufsize = (ndigits > 0) ? ndigits : ((53 + 3) / 4 + 1);
 s0 = ruby_xmalloc(bufsize+1);


 if (((53 + 3) / 4 + 1) > ndigits && ndigits > 0) {
  float redux = 1.0f;
  int offset = 4 * ndigits + 1024 - 4 - 53;
  (((u).L[1]) = (((int)(((u).L[1])) & ~0x7ff00000) | ((offset) << 20)));
  u.d += redux;
  u.d -= redux;
  *decpt += ((int)(((u).L[1]) >> 20) & ~0x100000) - offset;
 }

 manh = ((uint32_t)(((u).L[1]) & 0xfffff));
 manl = ((uint32_t)((u).L[0]));
 *s0 = '1';
 ...
 if (ndigits < 0) {
  for (ndigits = ((53 + 3) / 4 + 1); s0[ndigits - 1] == '0'; ndigits--)
   ;
 }
