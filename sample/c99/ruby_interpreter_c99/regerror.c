...
{
  snprintf(s, 3, "%02x", (v & 0377));
}

static void sprint_byte_with_x(char* s, unsigned int v)
{
  snprintf(s, 5, "\\x%02x", (v & 0377));
}

static int to_ascii(OnigEncoding enc, OnigUChar *s, OnigUChar *end,
      OnigUChar buf[], int buf_size, int *is_over)
{
  int len;
  OnigUChar *p;
  OnigCodePoint code;
  ...
  if (((enc)->min_enc_len) > 1) {
    ...
    len = 0;
    while (p < end) {
      code = (enc)->mbc_to_code((p),(end),enc);
      if (code >= 0x80) {
 ...
 buf[len++] = (OnigUChar )code;
    ...
    *is_over = ((p < end) ? 1 : 0);
    ...
    len = (int )((((end - s))>(buf_size))?(buf_size):((end - s)));
...
extern int
onig_error_code_to_str(OnigUChar* s, OnigPosition code, ...)
{
  OnigUChar *p, *q;
  OnigErrorInfo* einfo;
  size_t len;
 ...
 *p++ = *q++;
  ...
  default:
    q = onig_error_code_to_format(code);
    if (q) {
      len = onigenc_str_bytelen_null((&OnigEncodingASCII), q);
      memcpy(s, q, len);
    } else {
      len = 0;
    }
...
void
onig_vsnprintf_with_pattern(OnigUChar buf[], int bufsize, OnigEncoding enc,
                           OnigUChar* pat, OnigUChar* pat_end, const OnigUChar *fmt, va_list args)
{
  size_t need;
  int n, len;
  OnigUChar *p, *s, *bp;
  ...
  if (n + need < (size_t )bufsize) {
    ...
    s = buf + onigenc_str_bytelen_null((&OnigEncodingASCII), buf);

    p = pat;
    while (p < pat_end) {
      if ((onigenc_mbclen_approximate(p,pat_end,enc) != 1)) {
        len = ((enc->max_enc_len == enc->min_enc_len) ? enc->min_enc_len : onigenc_mbclen_approximate(p,pat_end,enc));
        if (((enc)->min_enc_len) == 1) {
          while (len-- > 0) *s++ = *p++;
        }
        else {
          ...
          while (len-- > 0) {
            ...
            blen = onigenc_str_bytelen_null((&OnigEncodingASCII), bs);
            bp = bs;
            while (blen-- > 0) *s++ = *bp++;
          }
        }
 ...
 *s++ = *p++;
      ...
      else if (!(enc)->is_code_ctype(*p,7,enc) &&
        !(enc)->is_code_ctype(*p,9,enc)) {
 sprint_byte_with_x((char* )bs, (unsigned int )(*p++));
 len = onigenc_str_bytelen_null((&OnigEncodingASCII), bs);
        bp = bs;
 while (len-- > 0) *s++ = *bp++;
      }
      else {
