...
const union {
    ...
    enum {
 RUBY_FMODE_READABLE = 0x00000001,
 RUBY_FMODE_WRITABLE = 0x00000002,
 RUBY_FMODE_READWRITE = (0x00000001|0x00000002),
 RUBY_FMODE_BINMODE = 0x00000004,
 RUBY_FMODE_SYNC = 0x00000008,
 RUBY_FMODE_TTY = 0x00000010,
 RUBY_FMODE_DUPLEX = 0x00000020,
 RUBY_FMODE_APPEND = 0x00000040,
 RUBY_FMODE_CREATE = 0x00000080,
 RUBY_FMODE_NOREVLOOKUP = 0x00000100,
 RUBY_FMODE_TRUNC = 0x00000800,
 RUBY_FMODE_TEXTMODE = 0x00001000,
 RUBY_FMODE_PREP = 0x00010000,
 RUBY_FMODE_SETENC_BY_BOM = 0x00100000,
 RUBY_FMODE_UNIX = 0x00200000,
 RUBY_FMODE_INET = 0x00400000,
 RUBY_FMODE_INET6 = 0x00800000,

        RUBY_NODE_TYPESHIFT = 8,
        RUBY_NODE_TYPEMASK = (((VALUE)0x7f)<<8),
        RUBY_NODE_LSHIFT = (8 +7),
        RUBY_NODE_FL_NEWLINE = (((VALUE)1)<<7)
    } various;
    union {
 enum imemo_type types;
 enum {RUBY_IMEMO_MASK = 0x0f} mask;
 struct RIMemo *ptr;
    } imemo;
...
const long RUBY_NODE_LMASK = (((long)1<<(sizeof(VALUE)*8 -(8 +7)))-1);
...
ruby_debug_print_node(int level, int debug_level, const char *header, const NODE *node)
{
 ...
 fprintf(stderr, "DBG> %s: %s (%u)\n", header,
  ruby_node_name(((int) (((node)->flags & (((VALUE)0x7f)<<8))>>8))), (int)(((long)(node)->flags)>>(8 +7)));
    }
    return (NODE *)node;
}
...
ruby_env_debug_option(const char *str, int len, void *arg)
{
    ...
    size_t retlen;
    unsigned long n;
    do { if (len == sizeof("gc_stress") - 1 && strncmp(str, ("gc_stress"), len) == 0) { (*ruby_initial_gc_stress_ptr) = (((VALUE)RUBY_Qtrue)); return 1; } } while (0);
    do { if (len == sizeof("core") - 1 && strncmp(str, ("core"), len) == 0) { (ruby_enable_coredump) = (1); return 1; } } while (0);
    if (((size_t)len >= sizeof("rgengc")-1 && strncmp(str, ("rgengc"), sizeof("rgengc")-1) == 0 && ((len == sizeof("rgengc")-1 && !(len = 0)) || (str[sizeof("rgengc")-1] == '=' && (str += sizeof("rgengc"), len -= sizeof("rgengc"), 1))))) {
 if (!len) ruby_rgengc_debug = 1;
 else do { int i; for (i = 0; i < (1); ++i) { do { n = ruby_scan_digits(str, len, 10, &retlen, &ov); if (!ov && retlen) { (&ruby_rgengc_debug)[i] = (unsigned int)n; } str += retlen; len -= retlen; } while (0); if (!len || *str != ':') break; ++str; --len; } if (len > 0) { fprintf(stderr, "ignored rgengc option: `%.*s'\n", len, str); } } while (0);
 return 1;
    }
    return 0;
}
