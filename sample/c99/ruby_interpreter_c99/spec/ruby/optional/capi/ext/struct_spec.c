...
static VALUE struct_spec_rb_struct_getmember(VALUE self, VALUE st, VALUE key) {
  return rb_struct_getmember(st, (rb_sym2id(key)));
}
...
static VALUE struct_spec_struct_define(VALUE self, VALUE name,
  ...
  const char *a1 = rb_string_value_ptr(&(attr1));
  const char *a2 = rb_string_value_ptr(&(attr2));
  const char *a3 = rb_string_value_ptr(&(attr3));
  char *nm = ((void*)0);

  if (name != ((VALUE)RUBY_Qnil)) nm = rb_string_value_ptr(&(name));

  return rb_struct_define(nm, a1, a2, a3, ((void*)0));
}
...
static VALUE struct_spec_struct_define_under(VALUE self, VALUE outer,
  ...
  const char *nm = rb_string_value_ptr(&(name));
  const char *a1 = rb_string_value_ptr(&(attr1));
  const char *a2 = rb_string_value_ptr(&(attr2));
  const char *a3 = rb_string_value_ptr(&(attr3));

  return rb_struct_define_under(outer, nm, a1, a2, a3, ((void*)0));
}
