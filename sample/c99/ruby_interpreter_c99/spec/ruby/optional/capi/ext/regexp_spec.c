...
VALUE regexp_spec_rb_reg_options(VALUE self, VALUE regexp) {
  return (((VALUE)(rb_reg_options(regexp)))<<1 | RUBY_FIXNUM_FLAG);
}
...
VALUE regexp_spec_match(VALUE self, VALUE regexp, VALUE str) {
  return rb_funcall(regexp, (__builtin_constant_p("match") ?  ({ static ID rb_intern_id_cache; if (!rb_intern_id_cache) rb_intern_id_cache = rb_intern2((("match")), (long)strlen(("match"))); (ID) rb_intern_id_cache; }) : rb_intern("match")), 1, str);
}
