...
static int io_spec_get_fd(VALUE io) {
  rb_io_t* fp;
  rb_io_check_closed((fp) = ((struct RFile*)(rb_io_taint_check(io)))->fptr);
  return fp->fd;
}

VALUE io_spec_GetOpenFile_fd(VALUE self, VALUE io) {
  return (((VALUE)((int)(io_spec_get_fd(io))))<<1 | RUBY_FIXNUM_FLAG);
}
...
VALUE io_spec_rb_io_printf(VALUE self, VALUE io, VALUE ary) {
  long argc = rb_array_len(ary);
  VALUE *argv = __builtin_alloca (sizeof(VALUE) * argc);
  int i;
...
VALUE io_spec_rb_io_wait_readable(VALUE self, VALUE io, VALUE read_p) {
  ...
  if(!(((VALUE)(read_p) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) {
    if (read(fd, buf, 13) != -1) {
      return ((VALUE)RUBY_Qnil);
    }
    saved_errno = (*__errno_location ());
    rb_ivar_set(self, (__builtin_constant_p("@write_data") ?  ({ static ID rb_intern_id_cache; if (!rb_intern_id_cache) rb_intern_id_cache = rb_intern2((("@write_data")), (long)strlen(("@write_data"))); (ID) rb_intern_id_cache; }) : rb_intern("@write_data")), ((VALUE)RUBY_Qtrue));
  ...
  if(!(((VALUE)(read_p) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) {
    ssize_t r = read(fd, buf, 13);
    if (r != 13) {
      perror("read");
      return rb_long2num_inline(r);
    }
    rb_ivar_set(self, (__builtin_constant_p("@read_data") ?  ({ static ID rb_intern_id_cache; if (!rb_intern_id_cache) rb_intern_id_cache = rb_intern2((("@read_data")), (long)strlen(("@read_data"))); (ID) rb_intern_id_cache; }) : rb_intern("@read_data")),
         ({ (__builtin_constant_p(buf) && __builtin_constant_p(13)) ? rb_str_new_static((buf), (13)) : rb_str_new((buf), (13)); }));
  }

  return ret ? ((VALUE)RUBY_Qtrue) : ((VALUE)RUBY_Qfalse);
}
...
VALUE io_spec_rb_cloexec_open(VALUE self, VALUE path, VALUE flags, VALUE mode) {
  const char *pathname = rb_string_value_ptr(&(path));
  int fd = rb_cloexec_open(pathname, ((int)rb_fix2int((VALUE)(flags))), ((int)rb_fix2int((VALUE)(mode))));
  return rb_funcall(rb_cIO, (__builtin_constant_p("for_fd") ?  ({ static ID rb_intern_id_cache; if (!rb_intern_id_cache) rb_intern_id_cache = rb_intern2((("for_fd")), (long)strlen(("for_fd"))); (ID) rb_intern_id_cache; }) : rb_intern("for_fd")), 1, (((VALUE)(fd))<<1 | RUBY_FIXNUM_FLAG));
}
