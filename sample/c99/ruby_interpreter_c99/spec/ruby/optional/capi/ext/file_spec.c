...
VALUE file_spec_rb_file_open(VALUE self, VALUE name, VALUE mode) {
  return rb_file_open((!(((struct RBasic*)(name))->flags & RSTRING_NOEMBED) ? ((struct RString*)(name))->as.ary : ((struct RString*)(name))->as.heap.ptr), (!(((struct RBasic*)(mode))->flags & RSTRING_NOEMBED) ? ((struct RString*)(mode))->as.ary : ((struct RString*)(mode))->as.heap.ptr));
}



VALUE file_spec_rb_file_open_str(VALUE self, VALUE name, VALUE mode) {
  return rb_file_open_str(name, (!(((struct RBasic*)(mode))->flags & RSTRING_NOEMBED) ? ((struct RString*)(mode))->as.ary : ((struct RString*)(mode))->as.heap.ptr));
}



VALUE file_spec_FilePathValue(VALUE self, VALUE obj) {
  return ((* ({ volatile VALUE *rb_gc_guarded_ptr = &(obj); ; rb_gc_guarded_ptr; })) = rb_get_path(obj));
}
