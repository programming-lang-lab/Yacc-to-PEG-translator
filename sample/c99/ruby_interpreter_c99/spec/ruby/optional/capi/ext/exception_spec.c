...
VALUE exception_spec_rb_exc_new(VALUE self, VALUE str) {
  char *cstr = rb_string_value_ptr(&(str));
...
VALUE exception_spec_rb_exc_new2(VALUE self, VALUE str) {
  char *cstr = rb_string_value_ptr(&(str));
  return  ({ (__builtin_constant_p(cstr)) ? rb_exc_new((rb_eException), (cstr), (long)strlen(cstr)) : rb_exc_new_cstr((rb_eException), (cstr)); });
}



VALUE exception_spec_rb_exc_new3(VALUE self, VALUE str) {
  return rb_exc_new_str(rb_eException, str);
}



VALUE exception_spec_rb_exc_raise(VALUE self, VALUE exc) {
    if (self != ((VALUE)RUBY_Qundef)) rb_exc_raise(exc);
