...
VALUE util_spec_rb_scan_args(VALUE self, VALUE argv, VALUE fmt, VALUE expected, VALUE acc) {
  int i, result, argc = (int)rb_array_len(argv);
  VALUE args[6], failed, a1, a2, a3, a4, a5, a6;

  failed = (__builtin_constant_p("failed") ?  ({ static ID rb_intern_id_cache; if (!rb_intern_id_cache) rb_intern_id_cache = rb_intern2((("failed")), (long)strlen(("failed"))); (ID) rb_intern_id_cache; }) : rb_intern("failed"));
  ...
  result = rb_scan_args(argc, args, (!(((struct RBasic*)(fmt))->flags & RSTRING_NOEMBED) ? ((struct RString*)(fmt))->as.ary : ((struct RString*)(fmt))->as.heap.ptr), &a1, &a2, &a3, &a4, &a5, &a6);

  switch(rb_num2int_inline(expected)) {
  case 6:
  ...
  return (((VALUE)((int)(result)))<<1 | RUBY_FIXNUM_FLAG);
}



static VALUE util_spec_rb_long2int(VALUE self, VALUE n) {
  return (((VALUE)((int)(rb_long2int_inline(rb_num2long_inline(n)))))<<1 | RUBY_FIXNUM_FLAG);
}
...
static VALUE util_spec_rb_sourcefile(VALUE self) {
  return  ({ (__builtin_constant_p(rb_sourcefile())) ? rb_str_new_static((rb_sourcefile()), (long)strlen(rb_sourcefile())) : rb_str_new_cstr(rb_sourcefile()); });
}



static VALUE util_spec_rb_sourceline(VALUE self) {
  return (((VALUE)((int)(rb_sourceline())))<<1 | RUBY_FIXNUM_FLAG);
}
