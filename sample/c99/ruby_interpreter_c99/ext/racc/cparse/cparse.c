...
get_stack_tail(VALUE stack, long len)
{
    ...
    if (len > rb_array_len(stack)) len = rb_array_len(stack);
    return rb_ary_subseq(stack, rb_array_len(stack) - len, len);
}
...
static VALUE lexer_i (VALUE block_args, VALUE data, int argc, const VALUE *argv, VALUE blockarg);
...
racc_cparse(VALUE parser, VALUE arg, VALUE sysdebug)
{
    ...
    vparams = ({ VALUE data_struct_obj = rb_data_typed_object_zalloc(CparseParams, sizeof(struct cparse_params), &cparse_params_type); (void)((v) = (struct cparse_params *)(((struct RData*)(data_struct_obj))->data));; data_struct_obj; });

                             ;
    v->sys_debug = !(((VALUE)(sysdebug) & (VALUE)~((VALUE)RUBY_Qnil)) == 0);
    vparams = initialize_params(vparams, parser, arg, ((VALUE)RUBY_Qnil), ((VALUE)RUBY_Qnil));
    v->lex_is_iterator = 0;
    parse_main(v, ((VALUE)RUBY_Qnil), ((VALUE)RUBY_Qnil), 0);

    (* ({ volatile VALUE *rb_gc_guarded_ptr = &(vparams); ; rb_gc_guarded_ptr; }));
    return v->retval;
}
...
static VALUE
lexer_i(VALUE block_args, VALUE data, int argc, const VALUE *argv, VALUE blockarg)
{
    struct cparse_params *v = rb_check_typeddata(data, &cparse_params_type);
    VALUE tok, val;
...
{
    rb_check_type((VALUE)(a),(RUBY_T_ARRAY));
    return a;
}
...
assert_hash(VALUE h)
{
    rb_check_type((VALUE)(h),(RUBY_T_HASH));
    return h;
}
...
initialize_params(VALUE vparams, VALUE parser, VALUE arg, VALUE lexer, VALUE lexmid)
{
    ...
    v->parser = parser;
    v->lexer = lexer;
    if (! !((VALUE)(lexmid) != ((VALUE)RUBY_Qnil)))
        v->lexmid = value_to_id(lexmid);

    v->debug = !(((VALUE)(rb_ivar_get(parser, id_yydebug)) & (VALUE)~((VALUE)RUBY_Qnil)) == 0);

    rb_check_type((VALUE)(arg),(RUBY_T_ARRAY));
    if (!(13 <= rb_array_len(arg) && rb_array_len(arg) <= 14))
        rb_raise(RaccBug, "[Racc Bug] wrong arg.size %ld", rb_array_len(arg));
    ...
    if (rb_array_len(arg) > 13) {
        v->use_result_var = !(((VALUE)(rb_ary_entry(arg, 13)) & (VALUE)~((VALUE)RUBY_Qnil)) == 0);
    }
    else {
        v->use_result_var = 1;
    }

    v->tstack = v->debug ? rb_ary_new_capa(64) : ((VALUE)RUBY_Qnil);
    v->vstack = rb_ary_new_capa(64);
    v->state = rb_ary_new_capa(64);
    v->curstate = 0;
    rb_ary_store(v->state, rb_array_len(v->state), (((VALUE)(0))<<1 | RUBY_FIXNUM_FLAG));
    v->t = (((VALUE)(0 + 1))<<1 | RUBY_FIXNUM_FLAG);
    v->nerr = 0;
    v->errstatus = 0;
    rb_ivar_set(parser, id_errstatus, rb_long2num_inline(v->errstatus));

    v->retval = ((VALUE)RUBY_Qnil);
    v->fin = 0;
    ...
    if (v->debug) {
        ...
        rb_iv_set(parser, "@tstack", ((VALUE)RUBY_Qnil));
    }
...
extract_user_token(struct cparse_params *v, VALUE block_args,
                   VALUE *tok, VALUE *val)
{
    if (!((VALUE)(block_args) != ((VALUE)RUBY_Qnil))) {

        *tok = ((VALUE)RUBY_Qfalse);
        *val =  ({ (__builtin_constant_p("$") && __builtin_constant_p(1)) ? rb_str_new_static(("$"), (1)) : rb_str_new(("$"), (1)); });
        return;
    }

    if (!( ((RUBY_T_ARRAY) == RUBY_T_FIXNUM) ? (((int)(long)(block_args))&RUBY_FIXNUM_FLAG) : ((RUBY_T_ARRAY) == RUBY_T_TRUE) ? ((block_args) == ((VALUE)RUBY_Qtrue)) : ((RUBY_T_ARRAY) == RUBY_T_FALSE) ? ((block_args) == ((VALUE)RUBY_Qfalse)) : ((RUBY_T_ARRAY) == RUBY_T_NIL) ? ((block_args) == ((VALUE)RUBY_Qnil)) : ((RUBY_T_ARRAY) == RUBY_T_UNDEF) ? ((block_args) == ((VALUE)RUBY_Qundef)) : ((RUBY_T_ARRAY) == RUBY_T_SYMBOL) ? ((((VALUE)(block_args)&~((~(VALUE)0)<<RUBY_SPECIAL_SHIFT)) == RUBY_SYMBOL_FLAG)||(!(((VALUE)(block_args) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(block_args) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(block_args))->flags & RUBY_T_MASK) == (RUBY_T_SYMBOL))) : ((RUBY_T_ARRAY) == RUBY_T_FLOAT) ? ( ((((int)(long)(block_args))&RUBY_FLONUM_MASK) == RUBY_FLONUM_FLAG) || (!(((VALUE)(block_args) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(block_args) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(block_args))->flags & RUBY_T_MASK) == RUBY_T_FLOAT)) : (!(((VALUE)(block_args) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(block_args) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(block_args))->flags & RUBY_T_MASK) == (RUBY_T_ARRAY)))) {
        rb_raise(rb_eTypeError,
                 "%s() %s %li\v (must be Array[2])",
                 v->lex_is_iterator ? rb_id2name(v->lexmid) : "next_token",
                 v->lex_is_iterator ? "yielded": "returned",
                 rb_obj_class(block_args));
    }
    if (rb_array_len(block_args) != 2) {
        rb_raise(rb_eArgError,
                 "%s() %s wrong size of array (%ld for 2)",
                 v->lex_is_iterator ? rb_id2name(v->lexmid) : "next_token",
                 v->lex_is_iterator ? "yielded": "returned",
                 rb_array_len(block_args));
    }
    *tok = ((0 <= 0 && 0 < rb_array_len(block_args)) ? rb_ary_entry(block_args, 0) : ((VALUE)RUBY_Qnil));
    *val = ((0 <= 1 && 1 < rb_array_len(block_args)) ? rb_ary_entry(block_args, 1) : ((VALUE)RUBY_Qnil));
}
static void
parse_main(struct cparse_params *v, VALUE tok, VALUE val, int resume)
{
    ...
    while (1) {
                  ...
                  ;
                                               ...
                                               ;
        tmp = ((0 <= v->curstate && v->curstate < rb_array_len(v->action_pointer)) ? rb_ary_entry(v->action_pointer, v->curstate) : ((VALUE)RUBY_Qnil));
        if (!((VALUE)(tmp) != ((VALUE)RUBY_Qnil))) goto notfound;
                                      ;
        i = rb_num2long_inline(tmp);

                                             ;
        if (read_next && (v->t != (((VALUE)(0))<<1 | RUBY_FIXNUM_FLAG))) {
            ...
            if (!((VALUE)(v->t) != ((VALUE)RUBY_Qnil))) {
                v->t = (((VALUE)(1))<<1 | RUBY_FIXNUM_FLAG);
            }
                                                         ;
            if (v->debug) {
        ...
        i += rb_num2long_inline(v->t);
                                    ;
        if (i < 0) goto notfound;

        act_value = ((0 <= i && i < rb_array_len(v->action_table)) ? rb_ary_entry(v->action_table, i) : ((VALUE)RUBY_Qnil));
        if (!((VALUE)(act_value) != ((VALUE)RUBY_Qnil))) goto notfound;
        act = rb_num2long_inline(act_value);
                                             ;

        tmp = ((0 <= i && i < rb_array_len(v->action_check)) ? rb_ary_entry(v->action_check, i) : ((VALUE)RUBY_Qnil));
        if (!((VALUE)(tmp) != ((VALUE)RUBY_Qnil))) goto notfound;
        if (rb_num2long_inline(tmp) != v->curstate) goto notfound;
                                                       ;
      ...
      notfound:
                                              ;
        act_value = ((0 <= v->curstate && v->curstate < rb_array_len(v->action_default)) ? rb_ary_entry(v->action_default, v->curstate) : ((VALUE)RUBY_Qnil));
        act = rb_num2long_inline(act_value);
        goto act_fixed;
      ...
      handle_act:
        if (act > 0 && act < v->shift_n) {
            ...
            if (v->errstatus > 0) {
                v->errstatus--;
                rb_ivar_set(v->parser, id_errstatus, rb_long2num_inline(v->errstatus));
            }
            shift(v,act,v->t,val);
            read_next = 1;
        }
                            ...
                            ;
            do { switch (reduce(v,act)) { case 0: break; case 1: goto user_yyerror; case 2: ; goto accept; default: break; } } while (0);
        }
        ...
        if (v->debug) {
            rb_funcall(v->parser, id_d_next_state,
                       2, rb_long2num_inline(v->curstate), v->state);
        }
    }
  ...
  user_yyerror:
    if (v->errstatus == 3) {
        if (v->t == (((VALUE)(0))<<1 | RUBY_FIXNUM_FLAG)) {
            v->retval = ((VALUE)RUBY_Qfalse);
            v->fin = 2;
            return;
        }
        read_next = 1;
    ...
    while (1) {
        tmp = ((0 <= v->curstate && v->curstate < rb_array_len(v->action_pointer)) ? rb_ary_entry(v->action_pointer, v->curstate) : ((VALUE)RUBY_Qnil));
        if (!((VALUE)(tmp) != ((VALUE)RUBY_Qnil))) goto error_pop;
                                      ;

        i = rb_num2long_inline(tmp) + 1;
                                    ;
        if (i < 0) goto error_pop;

        act_value = ((0 <= i && i < rb_array_len(v->action_table)) ? rb_ary_entry(v->action_table, i) : ((VALUE)RUBY_Qnil));
        if (!((VALUE)(act_value) != ((VALUE)RUBY_Qnil))) {
                                           ;
            goto error_pop;
        }
        act = rb_num2long_inline(act_value);
        ...
        if (rb_num2long_inline(tmp) != v->curstate) {
                                          ;
            goto error_pop;
        }
      ...
      error_pop:
        ...
        rb_ary_pop(v->state);
        rb_ary_pop(v->vstack);
        v->curstate = num_to_long(((rb_array_len(v->state) > 0) ? rb_ary_entry(v->state, rb_array_len(v->state) - 1) : ((VALUE)RUBY_Qnil)));
        if (v->debug) {
            rb_ary_pop(v->tstack);
...
{
    rb_ary_store(v->vstack, rb_array_len(v->vstack), val);
    if (v->debug) {
        rb_ary_store(v->tstack, rb_array_len(v->tstack), tok);
        rb_funcall(v->parser, id_d_shift,
                   3, tok, v->tstack, v->vstack);
    }
    v->curstate = act;
    rb_ary_store(v->state, rb_array_len(v->state), rb_long2num_inline(v->curstate));
}
...
reduce0(VALUE val, VALUE data, VALUE self)
{
    ...
    long len;
    ID mid;
    VALUE tmp, tmp_t = ((VALUE)RUBY_Qundef), tmp_v = ((VALUE)RUBY_Qundef);
    long i, k1, k2;
    VALUE goto_state;
    ...
    len = rb_num2long_inline(reduce_len);
    ...
    if (len == 0) {
        ...
        if (mid != id_noreduce) {
            tmp_v = get_stack_tail(v->vstack, len);
            tmp = rb_ary_entry(tmp_v, 0);
        }
        else {
            tmp = rb_ary_entry(v->vstack, rb_array_len(v->vstack) - len);
        }
        cut_stack_tail(v->vstack, len);
        if (v->debug) {
            tmp_t = get_stack_tail(v->tstack, len);
            cut_stack_tail(v->tstack, len);
        }
        cut_stack_tail(v->state, len);
    }
    if (mid != id_noreduce) {
        if (v->use_result_var) {
    ...
    rb_ary_store(v->vstack, rb_array_len(v->vstack), tmp);
    if (v->debug) {
        rb_ary_store(v->tstack, rb_array_len(v->tstack), reduce_to);
        rb_funcall(v->parser, id_d_reduce,
                   4, tmp_t, reduce_to, v->tstack, v->vstack);
    }


    if (rb_array_len(v->state) == 0)
        rb_raise(RaccBug, "state stack unexpectedly empty");
    k2 = num_to_long(((rb_array_len(v->state) > 0) ? rb_ary_entry(v->state, rb_array_len(v->state) - 1) : ((VALUE)RUBY_Qnil)));
    k1 = num_to_long(reduce_to) - v->nt_base;
                                   ;
                                   ;

    tmp = ((0 <= k1 && k1 < rb_array_len(v->goto_pointer)) ? rb_ary_entry(v->goto_pointer, k1) : ((VALUE)RUBY_Qnil));
    if (!((VALUE)(tmp) != ((VALUE)RUBY_Qnil))) goto notfound;

    i = rb_num2long_inline(tmp) + k2;
                                 ;
    if (i < 0) goto notfound;

    goto_state = ((0 <= i && i < rb_array_len(v->goto_table)) ? rb_ary_entry(v->goto_table, i) : ((VALUE)RUBY_Qnil));
    if (!((VALUE)(goto_state) != ((VALUE)RUBY_Qnil))) {
                                        ;
        goto notfound;
    }
                                                                        ;

    tmp = ((0 <= i && i < rb_array_len(v->goto_check)) ? rb_ary_entry(v->goto_check, i) : ((VALUE)RUBY_Qnil));
    if (!((VALUE)(tmp) != ((VALUE)RUBY_Qnil))) {
                                        ;
        goto notfound;
    }
    if (tmp != rb_long2num_inline(k1)) {
                                             ;
        goto notfound;
    }
                                                    ;
                          ...
                          ;
  transit:
    rb_ary_store(v->state, rb_array_len(v->state), goto_state);
    v->curstate = rb_num2long_inline(goto_state);
    return (((VALUE)(0))<<1 | RUBY_FIXNUM_FLAG);

  notfound:
                                           ;

    goto_state = ((0 <= k1 && k1 < rb_array_len(v->goto_default)) ? rb_ary_entry(v->goto_default, k1) : ((VALUE)RUBY_Qnil));
    goto transit;
}
...
Init_cparse(void)
{
    ...
    VALUE Racc, Parser;
    ID id_racc = (__builtin_constant_p("Racc") ?  (rb_intern2(("Racc"), (long)strlen("Racc"))) : (rb_intern)("Racc"));

    if (rb_const_defined(rb_cObject, id_racc)) {
        Racc = rb_const_get(rb_cObject, id_racc);
        Parser = rb_const_get_at(Racc, (__builtin_constant_p("Parser") ?  (rb_intern2(("Parser"), (long)strlen("Parser"))) : (rb_intern)("Parser")));
    }
    else {
    ...
    rb_define_const(Parser, "Racc_Runtime_Core_Version_C",
                     ({ (__builtin_constant_p("1.4.5")) ? rb_str_new_static(("1.4.5"), (long)strlen("1.4.5")) : rb_str_new_cstr("1.4.5"); }));
    rb_define_const(Parser, "Racc_Runtime_Core_Id_C",
         ({ (__builtin_constant_p("$originalId: cparse.c,v 1.8 2006/07/06 11:39:46 aamine Exp $")) ? rb_str_new_static(("$originalId: cparse.c,v 1.8 2006/07/06 11:39:46 aamine Exp $"), (long)strlen("$originalId: cparse.c,v 1.8 2006/07/06 11:39:46 aamine Exp $")) : rb_str_new_cstr("$originalId: cparse.c,v 1.8 2006/07/06 11:39:46 aamine Exp $"); }));
    ...
    id_yydebug = (__builtin_constant_p("@yydebug") ?  (rb_intern2(("@yydebug"), (long)strlen("@yydebug"))) : (rb_intern)("@yydebug"));
    id_nexttoken = (__builtin_constant_p("next_token") ?  (rb_intern2(("next_token"), (long)strlen("next_token"))) : (rb_intern)("next_token"));
    id_onerror = (__builtin_constant_p("on_error") ?  (rb_intern2(("on_error"), (long)strlen("on_error"))) : (rb_intern)("on_error"));
    id_noreduce = (__builtin_constant_p("_reduce_none") ?  (rb_intern2(("_reduce_none"), (long)strlen("_reduce_none"))) : (rb_intern)("_reduce_none"));
    id_errstatus = (__builtin_constant_p("@racc_error_status") ?  (rb_intern2(("@racc_error_status"), (long)strlen("@racc_error_status"))) : (rb_intern)("@racc_error_status"));

    id_d_shift = (__builtin_constant_p("racc_shift") ?  (rb_intern2(("racc_shift"), (long)strlen("racc_shift"))) : (rb_intern)("racc_shift"));
    id_d_reduce = (__builtin_constant_p("racc_reduce") ?  (rb_intern2(("racc_reduce"), (long)strlen("racc_reduce"))) : (rb_intern)("racc_reduce"));
    id_d_accept = (__builtin_constant_p("racc_accept") ?  (rb_intern2(("racc_accept"), (long)strlen("racc_accept"))) : (rb_intern)("racc_accept"));
    id_d_read_token = (__builtin_constant_p("racc_read_token") ?  (rb_intern2(("racc_read_token"), (long)strlen("racc_read_token"))) : (rb_intern)("racc_read_token"));
    id_d_next_state = (__builtin_constant_p("racc_next_state") ?  (rb_intern2(("racc_next_state"), (long)strlen("racc_next_state"))) : (rb_intern)("racc_next_state"));
    id_d_e_pop = (__builtin_constant_p("racc_e_pop") ?  (rb_intern2(("racc_e_pop"), (long)strlen("racc_e_pop"))) : (rb_intern)("racc_e_pop"));
}
