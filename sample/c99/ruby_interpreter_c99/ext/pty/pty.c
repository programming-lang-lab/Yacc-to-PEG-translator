...
{
    return rb_ivar_get(self, (__builtin_constant_p("status") ?  ({ static ID rb_intern_id_cache; if (!rb_intern_id_cache) rb_intern_id_cache = rb_intern2((("status")), (long)strlen(("status"))); (ID) rb_intern_id_cache; }) : rb_intern("status")));
}
...
static void getDevice(int*, int*, char [16], int);
...
chfunc(void *data, char *errbuf, size_t errbuf_len)
{
    ...
    (void) setsid();
    close(master);
    (void) ioctl(slave, 0x540E, (char *)0);
    dup2(slave,0);
    dup2(slave,1);
    dup2(slave,2);
    close(slave);

    if (seteuid(getuid())) do { strlcpy(errbuf, ("seteuid()"), errbuf_len); return -1; } while (0);
        ...
        char SlaveName[16])
{
 ...
 v =  ({ (__builtin_constant_p(shellname)) ? rb_str_new_static((shellname), (long)strlen(shellname)) : rb_str_new_cstr(shellname); });
 argc = 1;
 argv = &v;
    }
    ...
    errbuf[0] = '\0';
    pid = rb_fork_async_signal_safe(&status, chfunc, &carg, ((VALUE)RUBY_Qnil), errbuf, sizeof(errbuf));
 ...
 rb_sys_fail(errbuf[0] ? errbuf : "fork failed");
    ...
    (* ({ volatile VALUE *rb_gc_guarded_ptr = &(carg.execarg_obj); ; rb_gc_guarded_ptr; }));
}
static int
get_device_once(int *master, int *slave, char SlaveName[16], int nomesg, int fail)
{
 ...
 "p0","p1","p2","p3","p4","p5","p6","p7", "p8","p9","pa","pb","pc","pd","pe","pf", "q0","q1","q2","q3","q4","q5","q6","q7", "q8","q9","qa","qb","qc","qd","qe","qf", "r0","r1","r2","r3","r4","r5","r6","r7", "r8","r9","ra","rb","rc","rd","re","rf", "s0","s1","s2","s3","s4","s5","s6","s7", "s8","s9","sa","sb","sc","sd","se","sf",
    };


    for (i = 0; i < ((int)(sizeof(deviceNo) / sizeof((deviceNo)[0]))); i++) {
 const char *const devno = deviceNo[i];
 ruby_snprintf(MasterName, sizeof MasterName, MasterDevice, devno);
 if ((masterfd = rb_cloexec_open(MasterName,02,0)) >= 0) {
            rb_update_max_fd(masterfd);
     *master = masterfd;
     ruby_snprintf(SlaveName, 16, SlaveDevice, devno);
     if ((slavefd = rb_cloexec_open(SlaveName,02,0)) >= 0) {
...
static void
getDevice(int *master, int *slave, char SlaveName[16], int nomesg)
{
...
pty_open(VALUE klass)
{
    ...
    do { (master_fptr) = rb_io_make_open_file(master_io);} while (0);
    master_fptr->mode = (0x00000001|0x00000002) | 0x00000008 | 0x00000020;
    master_fptr->fd = master_fd;
    master_fptr->pathv = rb_obj_freeze(rb_sprintf("masterpty:%s", slavename));

    slave_file = rb_obj_alloc(rb_cFile);
    do { (slave_fptr) = rb_io_make_open_file(slave_file);} while (0);
    slave_fptr->mode = (0x00000001|0x00000002) | 0x00000008 | 0x00000020 | 0x00000010;
    slave_fptr->fd = slave_fd;
    slave_fptr->pathv = rb_obj_freeze( ({ (__builtin_constant_p(slavename)) ? rb_str_new_static((slavename), (long)strlen(slavename)) : rb_str_new_cstr(slavename); }));
...
pty_detach_process(struct pty_info *info)
{
    ...
    int st;
    if (rb_waitpid(info->child_pid, &st, 1) <= 0)
 return ((VALUE)RUBY_Qnil);
...
pty_getpty(int argc, VALUE *argv, VALUE self)
{
    ...
    do { (rfptr) = rb_io_make_open_file(rport);} while (0);
    do { (wfptr) = rb_io_make_open_file(wport);} while (0);

    establishShell(argc, argv, &info, SlaveName);

    rfptr->mode = rb_io_modestr_fmode("r");
    rfptr->fd = info.fd;
    rfptr->pathv = rb_obj_freeze( ({ (__builtin_constant_p(SlaveName)) ? rb_str_new_static((SlaveName), (long)strlen(SlaveName)) : rb_str_new_cstr(SlaveName); }));

    wfptr->mode = rb_io_modestr_fmode("w") | 0x00000008;
    ...
    rb_ary_store(res,2,(((VALUE)((int)(info.child_pid)))<<1 | RUBY_FIXNUM_FLAG));
...
pty_check(int argc, VALUE *argv, VALUE self)
{
 ...
 0;
    ...
    cpid = rb_waitpid(rb_num2int_inline(pid), &status, flag);
    if (cpid == -1 || cpid == 0) return ((VALUE)RUBY_Qnil);

    if (!!(((VALUE)(exc) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) return rb_last_status_get();
