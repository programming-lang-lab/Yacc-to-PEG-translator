...
sdbm_open(register char *file, register int flags, register int mode)
{
 ...
 if (file == ((void*)0) || !*file)
  return (*__errno_location ()) = 22, (DBM *) ((void*)0);



 n = strlen(file) * 2 + strlen(".dir") + strlen(".pag") + 2;

 if ((dirname = malloc(n)) == ((void*)0))
  return (*__errno_location ()) = 12, (DBM *) ((void*)0);



 dirname = strcat(strcpy(dirname, file), ".dir");
 pagname = strcpy(dirname + strlen(dirname) + 1, file);
 pagname = strcat(pagname, ".pag");
...
sdbm_prep(char *dirname, char *pagname, int flags, int mode)
{
 ...
 if ((db = (DBM *) malloc(sizeof(DBM))) == ((void*)0))
  return (*__errno_location ()) = 12, (DBM *) ((void*)0);
 ...
 if ((db->pagf = open(pagname, flags, mode)) == -1) goto err;
        ...
        db->pagbno = -1;
        db->maxbno = dstat.st_size * (long) 8;

        (void) memset(db->pagbuf, 0, 1024);
        (void) memset(db->dirbuf, 0, 4096);
...
{
 if (db == ((void*)0) || ((key).dptr == ((void*)0) || (key).dsize < 0))
  return (*__errno_location ()) = 22, nullitem;

 if (getpage(db, sdbm_hash((key).dptr, (key).dsize)))
  return getpair(db->pagbuf, key);

 return ((db)->flags |= 0x2), nullitem;
}
...
sdbm_delete(register DBM *db, datum key)
{
 ...
 if (getpage(db, sdbm_hash((key).dptr, (key).dsize))) {
  ...
  if (lseek(db->pagf, (long) (db->pagbno) * 1024, 0) < 0
      || write(db->pagf, db->pagbuf, 1024) < 0)
   return ((db)->flags |= 0x2), -1;
...
sdbm_store(register DBM *db, datum key, datum val, int flags)
{
 ...
 if (getpage(db, (hash = sdbm_hash((key).dptr, (key).dsize)))) {
 ...
 do {
  ...
  if (hash & (db->hmask + 1)) {
   if (lseek(db->pagf, (long) (db->pagbno) * 1024, 0) < 0
    ...
    return 0;
   db->pagbno = newp;
   (void) memcpy(pag, new, 1024);
  }
  else if (lseek(db->pagf, (long) (newp) * 1024, 0) < 0
    || write(db->pagf, new, 1024) < 0)
   return 0;
...
sdbm_firstkey(register DBM *db)
{
 ...
 (void) memset(db->pagbuf, 0, 1024);
 if (lseek(db->pagf, (long) (0) * 1024, 0) < 0
     || read(db->pagf, db->pagbuf, 1024) < 0)
  return ((db)->flags |= 0x2), nullitem;
 db->pagbno = 0;
 db->blkptr = 0;
 db->keyptr = 0;
...
getpage(register DBM *db, register long int hash)
{
 ...
 if (pagb != db->pagbno) {
  ...
  if (lseek(db->pagf, (long) (pagb) * 1024, 0) < 0
      || read(db->pagf, db->pagbuf, 1024) < 0)
   return 0;
...
getdbit(register DBM *db, register long int dbit)
{
 ...
 if (dirb != db->dirbno) {
  if (lseek(db->dirf, (long) (dirb) * 4096, 0) < 0
      || read(db->dirf, db->dirbuf, 4096) < 0)
   return 0;
  db->dirbno = dirb;
 ...
 return db->dirbuf[c % 4096] & (1 << (dbit % 8));
}
...
setdbit(register DBM *db, register long int dbit)
{
 ...
 db->dirbuf[c % 4096] |= (1 << (dbit % 8));

 if (dbit >= db->maxbno)
  db->maxbno += (long) 4096 * 8;

 if (lseek(db->dirf, (long) (dirb) * 4096, 0) < 0
     || write(db->dirf, db->dirbuf, 4096) < 0)
  return 0;
...
getnext(register DBM *db)
{
 ...
 for (;;) {
  ...
  if (db->pagbno != db->blkptr++)
   if (lseek(db->pagf, (long) (db->blkptr) * 1024, 0) < 0)
    break;
  db->pagbno = db->blkptr;
  if (read(db->pagf, db->pagbuf, 1024) <= 0)
   break;
  if (!chkpage(db->pagbuf)) {
   break;
  }
...
duppair(char *pag, datum key)
{
 register short *ino = (short *) pag;
 return ((ino)[(0)]) > 0 &&
     seepair(pag, ((ino)[(0)]), key.dptr, key.dsize) > 0;
}
...
delpair(char *pag, datum key)
{
 ...
 if (i < n - 1) {
  register int m;
  register char *dst = pag + (i == 1 ? 1024 : ((ino)[(i - 1)]));
...
splpage(char *pag, char *new, long int sbit)
{
 ...
 (void) memset(pag, 0, 1024);
 (void) memset(new, 0, 1024);
 ...
 for (ino++; n > 0; ino += 2) {
  ...
  (void) putpair((sdbm_hash((key).dptr, (key).dsize) & sbit) ? new : pag, key, val);
...
chkpage(char *pag)
{
 ...
 if ((n = ((ino)[(0)])) < 0 || n > 1024 / (int)sizeof(short))
  return 0;
