...
preserve_original_state(VALUE orig, VALUE dest)
{
    ...
    (void)(((struct RBasic*)(dest))->flags |= ((((struct RBasic*)(orig))->flags&(RUBY_FL_TAINT))));
}
...
optimized_escape_html(VALUE str)
{
    ...
    cstr = (!(((struct RBasic*)(str))->flags & RSTRING_NOEMBED) ? ((struct RString*)(str))->as.ary : ((struct RString*)(str))->as.heap.ptr);
...
optimized_unescape_html(VALUE str)
{
    ...
    unsigned long charlimit = (strcasecmp((enc)->name, "UTF-8") == 0 ? UNICODE_MAX :
          strcasecmp((enc)->name, "ISO-8859-1") == 0 ? 256 :
          128);
    long i, len, beg = 0;
    size_t clen, plen;
    int overflow;
 ...
 switch (c) {
   case 'a':
     ++i;
     if ((len - i >= (int)(sizeof("pos;") - 1) && memcmp(&cstr[i], "pos;", (sizeof("pos;") - 1)) == 0 && (i += (sizeof("pos;") - 1) - 1, 1))) {
  c = '\'';
     }
     else if ((len - i >= (int)(sizeof("mp;") - 1) && memcmp(&cstr[i], "mp;", (sizeof("mp;") - 1)) == 0 && (i += (sizeof("mp;") - 1) - 1, 1))) {
  c = '&';
     }
     else continue;
     break;
   case 'q':
     ++i;
     if ((len - i >= (int)(sizeof("uot;") - 1) && memcmp(&cstr[i], "uot;", (sizeof("uot;") - 1)) == 0 && (i += (sizeof("uot;") - 1) - 1, 1))) {
  c = '"';
     }
     else continue;
     break;
   case 'g':
     ++i;
     if ((len - i >= (int)(sizeof("t;") - 1) && memcmp(&cstr[i], "t;", (sizeof("t;") - 1)) == 0 && (i += (sizeof("t;") - 1) - 1, 1))) {
  c = '>';
     }
     else continue;
     break;
  ...
  c = '<';
     ...
     break;
   case '#':
     if (len - ++i >= 2 && rb_isdigit(cstr[i])) {
  cc = ruby_scan_digits(&cstr[i], len-i, 10, &clen, &overflow);
     }
     else if ((cstr[i] == 'x' || cstr[i] == 'X') && len - ++i >= 2 && rb_isxdigit(cstr[i])) {
  ...
  dest = rb_str_buf_new(len);
     ...
     if (charlimit > 256) {
  rb_str_cat(dest, buf, ((enc))->code_to_mbc(((OnigCodePoint)cc),(OnigUChar*)(buf),(enc)));
     }
     else {
  c = (unsigned char)cc;
  rb_str_cat(dest, &c, 1);
...
optimized_escape(VALUE str)
{
 ...
 if (!url_unreserved_char(c)) {
  ...
  dest = rb_str_buf_new(len);
     ...
     if (c == ' ') {
   ({ (__builtin_constant_p("+")) ? rb_str_cat((dest), ("+"), (long)strlen("+")) : rb_str_cat_cstr((dest), ("+")); });
     }
     else {
  buf[1] = (ruby_hexdigits+16)[(c >> 4) & 0xf];
  buf[2] = (ruby_hexdigits+16)[c & 0xf];
...
optimized_unescape(VALUE str, VALUE encoding)
{
 ...
 if (c == '%') {
     if (i + 3 > len) break;
     if (!rb_isxdigit(cstr[i+1])) continue;
     if (!rb_isxdigit(cstr[i+2])) continue;
     buf[0] = ((ruby_digit36_to_number_table[(unsigned char)(cstr[i+1])] << 4)
        | ruby_digit36_to_number_table[(unsigned char)(cstr[i+2])]);
     clen = 2;
 }
 else if (c == '+') {
     buf[0] = ' ';
 ...
 if (!rb_enc_coderange_clean_p(rb_enc_str_coderange(dest))) {
     rb_enc_associate_index(dest, origenc);
     if (cr != RUBY_ENC_CODERANGE_UNKNOWN)
  ( ((struct RBasic*)(dest))->flags = (((struct RBasic*)(dest))->flags & ~RUBY_ENC_CODERANGE_MASK) | (cr));
 }
    }
    return dest;
}
...
cgiesc_escape_html(VALUE self, VALUE str)
{
    ...
    if (rb_enc_asciicompat_inline(rb_enc_get(str))) {
...
accept_charset(int argc, VALUE *argv, VALUE self)
{
    if (argc > 0)
 return argv[0];
    return rb_cvar_get(rb_class_of((VALUE)(self)), id_accept_charset);
}
static VALUE
...
Init_escape(void)
{
    id_accept_charset = (__builtin_constant_p("@@accept_charset") ?  (rb_intern2(("@@accept_charset"), (long)strlen("@@accept_charset"))) : (rb_intern)("@@accept_charset"));
    {void InitVM_escape(void);InitVM_escape();};
}
