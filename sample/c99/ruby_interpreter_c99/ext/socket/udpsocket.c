...
udp_connect(VALUE sock, VALUE host, VALUE port)
{
    ...
    rb_io_check_closed((arg.fptr) = ((struct RFile*)(rb_io_taint_check(sock)))->fptr);
...
udp_send_internal(struct udp_send_arg *arg)
{
 ...
 n = (int)(long)rb_thread_io_blocking_region((rsock_sendto_blocking), (&arg->sarg), (&arg->sarg)->fd);
 if (n >= 0) {
...
udp_send(int argc, VALUE *argv, VALUE sock)
{
 ...
 return rsock_bsock_send(argc, argv, sock);
    ...
    rb_string_value(&(arg.sarg.mesg));
    rb_io_check_closed((arg.fptr) = ((struct RFile*)(rb_io_taint_check(sock)))->fptr);
    arg.sarg.fd = arg.fptr->fd;
    arg.sarg.flags = rb_num2int_inline(flags);
    arg.res = rsock_addrinfo(host, port, rsock_fd_family(arg.fptr->fd), SOCK_DGRAM, 0);
