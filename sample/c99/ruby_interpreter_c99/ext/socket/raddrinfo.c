...
nogvl_getaddrinfo(void *arg)
{
    ...
    ret = getaddrinfo__compat(ptr->node, ptr->service, ptr->hints, ptr->res);




    if (ret == 11 && (*__errno_location ()) == 2)
 ret = 8;
               ...
               struct rb_addrinfo **res)
{
    ...
    if (ret == 0)
        ...
        memset((&arg), 0, sizeof(struct getaddrinfo_arg)*(size_t)(1));
        arg.node = node;
        arg.service = service;
        arg.hints = hints;
        arg.res = &ai;
        ret = (int)(VALUE)rb_thread_call_without_gvl(nogvl_getaddrinfo, &arg, ((rb_unblock_function_t *)-1), 0);

    }

    if (ret == 0) {
        *res = (struct rb_addrinfo *)ruby_xmalloc(sizeof(struct rb_addrinfo));
...
rb_freeaddrinfo(struct rb_addrinfo *ai)
{
    if (!ai->allocated_by_malloc)
        freeaddrinfo__compat(ai->ai);
    else {
        struct addrinfo *ai1, *ai2;
        ai1 = ai->ai;
        while (ai1) {
            ai2 = ai1->ai_next;
            ruby_xfree(ai1->ai_addr);
            ruby_xfree(ai1);
            ai1 = ai2;
        }
    }
...
nogvl_getnameinfo(void *arg)
{
    ...
    return (void *)(VALUE)getnameinfo__compat(ptr->sa, ptr->salen,
           ...
           char *serv, size_t servlen, int flags)
{
    ...
    arg.servlen = servlen;
    arg.flags = flags;
    ret = (int)(VALUE)rb_thread_call_without_gvl(nogvl_getnameinfo, &arg, ((rb_unblock_function_t *)-1), 0);
    return ret;
...
make_ipaddr0(struct sockaddr *addr, socklen_t addrlen, char *buf, size_t buflen)
{
    ...
    error = rb_getnameinfo(addr, addrlen, buf, buflen, ((void*)0), 0, 0x00000002);
    if (error) {
...
rsock_make_ipaddr(struct sockaddr *addr, socklen_t addrlen)
{
    ...
    return  ({ (__builtin_constant_p(hbuf)) ? rb_str_new_static((hbuf), (long)strlen(hbuf)) : rb_str_new_cstr(hbuf); });
}
...
make_inetaddr(unsigned int host, char *buf, size_t buflen)
{
    ...
    do { struct sockaddr_in *init_sockaddr_ptr = (&sin); socklen_t init_sockaddr_len = (sizeof(sin)); memset(init_sockaddr_ptr, 0, init_sockaddr_len); init_sockaddr_ptr->sin_family = 2; (void)((init_sockaddr_len)); } while (0);
...
{
    if (!((VALUE)(host) != ((VALUE)RUBY_Qnil))) {
        return ((void*)0);
    }
    else if (rb_obj_is_kind_of(host, rb_cInteger)) {
        unsigned int i = ((unsigned int)rb_num2uint(host));

        make_inetaddr(htonl(i), hbuf, hbuflen);
        if (flags_ptr) *flags_ptr |= 0x00000004;
        return hbuf;
    }
    else {
        const char *name;
        ...
        do { rb_string_value_cstr(&(host)); rb_check_safe_obj(host);} while(0);
        (!(((struct RBasic*)(host))->flags & RSTRING_NOEMBED) ? ((name) = ((struct RString*)(host))->as.ary, (len) = (long)((((struct RBasic*)(host))->flags >> RSTRING_EMBED_LEN_SHIFT) & (RSTRING_EMBED_LEN_MASK >> RSTRING_EMBED_LEN_SHIFT))) : ((name) = ((struct RString*)(host))->as.heap.ptr, (len) = ((struct RString*)(host))->as.heap.len));
        if (!len || ((name)[0] == "<any>"[0] && (sizeof("<any>") - 1) == (len) && memcmp(name, "<any>", len) == 0)) {
            make_inetaddr(((in_addr_t) 0x00000000), hbuf, hbuflen);
            if (flags_ptr) *flags_ptr |= 0x00000004;
        }
        else if (((name)[0] == "<broadcast>"[0] && (sizeof("<broadcast>") - 1) == (len) && memcmp(name, "<broadcast>", len) == 0)) {
            make_inetaddr(((in_addr_t) 0xffffffff), hbuf, hbuflen);
            if (flags_ptr) *flags_ptr |= 0x00000004;
        }
        else if (len >= hbuflen) {
            rb_raise(rb_eArgError, "hostname too long (%zu)",
                     len);
        }
        else {
...
port_str(VALUE port, char *pbuf, size_t pbuflen, int *flags_ptr)
{
    if (!((VALUE)(port) != ((VALUE)RUBY_Qnil))) {
    ...
    else if ((((int)(long)(port))&RUBY_FIXNUM_FLAG)) {
        ruby_snprintf(pbuf, pbuflen, "%ld", ((long)(((long)(port))>>(int)(1))));

        if (flags_ptr) *flags_ptr |= 0x00000008;
        ...
        do { rb_string_value_cstr(&(port)); rb_check_safe_obj(port);} while(0);
        (!(((struct RBasic*)(port))->flags & RSTRING_NOEMBED) ? ((serv) = ((struct RString*)(port))->as.ary, (len) = (long)((((struct RBasic*)(port))->flags >> RSTRING_EMBED_LEN_SHIFT) & (RSTRING_EMBED_LEN_MASK >> RSTRING_EMBED_LEN_SHIFT))) : ((serv) = ((struct RString*)(port))->as.heap.ptr, (len) = ((struct RString*)(port))->as.heap.len));
        if (len >= pbuflen) {
            rb_raise(rb_eArgError, "service name too long (%zu)",
                     len);
        }
...
rsock_fd_family(int fd)
{
    ...
    if (fd < 0 || getsockname(fd, &sa, &sa_len) != 0 ||
        (size_t)sa_len < __builtin_offsetof() + sizeof(sa.sa_family)) {
 return 0;
    }
    return sa.sa_family;
}
...
rsock_addrinfo(VALUE host, VALUE port, int family, int socktype, int flags)
{
    ...
    memset((&hints), 0, sizeof(struct addrinfo)*(size_t)(1));
...
rsock_ipaddr(struct sockaddr *sockaddr, socklen_t sockaddrlen, int norevlookup)
{
    ...
    if (id) {
        ...
        family =  ({ (__builtin_constant_p(pbuf)) ? rb_str_new_static((pbuf), (long)strlen(pbuf)) : rb_str_new_cstr(pbuf); });
    }

    addr1 = ((VALUE)RUBY_Qnil);
    if (!norevlookup) {
        error = rb_getnameinfo(sockaddr, sockaddrlen, hbuf, sizeof(hbuf),
                               ((void*)0), 0, 0);
        if (! error) {
            addr1 =  ({ (__builtin_constant_p(hbuf)) ? rb_str_new_static((hbuf), (long)strlen(hbuf)) : rb_str_new_cstr(hbuf); });
        }
    }
    error = rb_getnameinfo(sockaddr, sockaddrlen, hbuf, sizeof(hbuf),
                           pbuf, sizeof(pbuf), 0x00000002 | 0x00000008);
    if (error) {
        rsock_raise_socket_error("getnameinfo", error);
    }
    addr2 =  ({ (__builtin_constant_p(hbuf)) ? rb_str_new_static((hbuf), (long)strlen(hbuf)) : rb_str_new_cstr(hbuf); });
    if (addr1 == ((VALUE)RUBY_Qnil)) {
        addr1 = addr2;
    }
    port = (((VALUE)(atoi(pbuf)))<<1 | RUBY_FIXNUM_FLAG);
    ary =  ({ const VALUE args_to_new_ary[] = {family, port, addr1, addr2}; if (__builtin_constant_p(4)) { extern int (*__Static_assert_function (void)) [!!sizeof (struct { int __error_if_negative: (((int)(sizeof(args_to_new_ary) / sizeof((args_to_new_ary)[0]))) == (4)) ? 2 : -1; })]; } rb_ary_new_from_values(((int)(sizeof(args_to_new_ary) / sizeof((args_to_new_ary)[0]))), args_to_new_ary); });
...
make_hostent_internal(struct hostent_arg *arg)
{
    ...
    if (addr->ai_canonname) {
        ...
        hostp = host_str(host, hbuf, sizeof(hbuf), ((void*)0));
    }
    rb_ary_push(ary,  ({ (__builtin_constant_p(hostp)) ? rb_str_new_static((hostp), (long)strlen(hostp)) : rb_str_new_cstr(hostp); }));

    if (addr->ai_canonname && strlen(addr->ai_canonname) < 1025 &&
 (h = gethostbyname(addr->ai_canonname))) {
        names = rb_ary_new();
        if (h->h_aliases != ((void*)0)) {
            for (pch = h->h_aliases; *pch; pch++) {
                rb_ary_push(names,  ({ (__builtin_constant_p(*pch)) ? rb_str_new_static((*pch), (long)strlen(*pch)) : rb_str_new_cstr(*pch); }));
            }
        }
    }
    else {
        names = rb_ary_new_capa(0);
    }
    rb_ary_push(ary, names);
    rb_ary_push(ary, (((VALUE)((int)(addr->ai_family)))<<1 | RUBY_FIXNUM_FLAG));
...
static const rb_data_type_t addrinfo_type = {
    "socket/addrinfo",
    {addrinfo_mark, ((RUBY_DATA_FUNC)-1), addrinfo_memsize,},
};

static VALUE
addrinfo_s_allocate(VALUE klass)
{
    return rb_data_typed_object_wrap((klass),(0),(&addrinfo_type));
}
...
alloc_addrinfo(void)
{
    rb_addrinfo_t *rai = (((rb_addrinfo_t*)ruby_xcalloc((size_t)(1),sizeof(rb_addrinfo_t))));
    rai->inspectname = ((VALUE)RUBY_Qnil);
    rai->canonname = ((VALUE)RUBY_Qnil);
    return rai;
}
                   ...
                   VALUE canonname, VALUE inspectname)
{
    ...
    (((struct RData*)(a))->data) = rai = alloc_addrinfo();
                 ...
                 int socktype_hack)
{
    ...
    memset((&hints), 0, sizeof(struct addrinfo)*(size_t)(1));
    hints.ai_family = !((VALUE)(family) != ((VALUE)RUBY_Qnil)) ? 0 : rsock_family_arg(family);

    if (!!((VALUE)(socktype) != ((VALUE)RUBY_Qnil))) {
 hints.ai_socktype = rsock_socktype_arg(socktype);
    }
    if (!!((VALUE)(protocol) != ((VALUE)RUBY_Qnil))) {
 hints.ai_protocol = rb_num2int_inline(protocol);
    }
    if (!!((VALUE)(flags) != ((VALUE)RUBY_Qnil))) {
 hints.ai_flags = rb_num2int_inline(flags);
    }
                          ...
                          VALUE inspectnode, VALUE inspectservice)
{
    ...
    VALUE canonname;
    VALUE inspectname = rb_str_equal(node, inspectnode) ? ((VALUE)RUBY_Qnil) : make_inspectname(inspectnode, inspectservice, res->ai);

    canonname = ((VALUE)RUBY_Qnil);
    if (res->ai->ai_canonname) {
        canonname =  ({ (__builtin_constant_p(res->ai->ai_canonname)) ? rb_tainted_str_new((res->ai->ai_canonname), (long)strlen(res->ai->ai_canonname)) : rb_tainted_str_new_cstr(res->ai->ai_canonname); });
        rb_obj_freeze_inline((VALUE)canonname);
    }

    init_addrinfo(rai, res->ai->ai_addr, res->ai->ai_addrlen,
                  rb_num2int_inline(family), rb_num2int_inline(socktype), rb_num2int_inline(protocol),
...
{
    VALUE inspectname = ((VALUE)RUBY_Qnil);
    ...
    if (res) {
        ...
        if (ret == 0) {
            if (( ((RUBY_T_STRING) == RUBY_T_FIXNUM) ? (((int)(long)(node))&RUBY_FIXNUM_FLAG) : ((RUBY_T_STRING) == RUBY_T_TRUE) ? ((node) == ((VALUE)RUBY_Qtrue)) : ((RUBY_T_STRING) == RUBY_T_FALSE) ? ((node) == ((VALUE)RUBY_Qfalse)) : ((RUBY_T_STRING) == RUBY_T_NIL) ? ((node) == ((VALUE)RUBY_Qnil)) : ((RUBY_T_STRING) == RUBY_T_UNDEF) ? ((node) == ((VALUE)RUBY_Qundef)) : ((RUBY_T_STRING) == RUBY_T_SYMBOL) ? ((((VALUE)(node)&~((~(VALUE)0)<<RUBY_SPECIAL_SHIFT)) == RUBY_SYMBOL_FLAG)||(!(((VALUE)(node) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(node) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(node))->flags & RUBY_T_MASK) == (RUBY_T_SYMBOL))) : ((RUBY_T_STRING) == RUBY_T_FLOAT) ? ( ((((int)(long)(node))&RUBY_FLONUM_MASK) == RUBY_FLONUM_FLAG) || (!(((VALUE)(node) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(node) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(node))->flags & RUBY_T_MASK) == RUBY_T_FLOAT)) : (!(((VALUE)(node) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(node) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(node))->flags & RUBY_T_MASK) == (RUBY_T_STRING))) && strcmp(hbuf, (!(((struct RBasic*)(node))->flags & RSTRING_NOEMBED) ? ((struct RString*)(node))->as.ary : ((struct RString*)(node))->as.heap.ptr)) == 0)
                node = ((VALUE)RUBY_Qnil);
            if (( ((RUBY_T_STRING) == RUBY_T_FIXNUM) ? (((int)(long)(service))&RUBY_FIXNUM_FLAG) : ((RUBY_T_STRING) == RUBY_T_TRUE) ? ((service) == ((VALUE)RUBY_Qtrue)) : ((RUBY_T_STRING) == RUBY_T_FALSE) ? ((service) == ((VALUE)RUBY_Qfalse)) : ((RUBY_T_STRING) == RUBY_T_NIL) ? ((service) == ((VALUE)RUBY_Qnil)) : ((RUBY_T_STRING) == RUBY_T_UNDEF) ? ((service) == ((VALUE)RUBY_Qundef)) : ((RUBY_T_STRING) == RUBY_T_SYMBOL) ? ((((VALUE)(service)&~((~(VALUE)0)<<RUBY_SPECIAL_SHIFT)) == RUBY_SYMBOL_FLAG)||(!(((VALUE)(service) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(service) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(service))->flags & RUBY_T_MASK) == (RUBY_T_SYMBOL))) : ((RUBY_T_STRING) == RUBY_T_FLOAT) ? ( ((((int)(long)(service))&RUBY_FLONUM_MASK) == RUBY_FLONUM_FLAG) || (!(((VALUE)(service) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(service) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(service))->flags & RUBY_T_MASK) == RUBY_T_FLOAT)) : (!(((VALUE)(service) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(service) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(service))->flags & RUBY_T_MASK) == (RUBY_T_STRING))) && strcmp(pbuf, (!(((struct RBasic*)(service))->flags & RSTRING_NOEMBED) ? ((struct RString*)(service))->as.ary : ((struct RString*)(service))->as.heap.ptr)) == 0)
                service = ((VALUE)RUBY_Qnil);
            else if (( ((RUBY_T_FIXNUM) == RUBY_T_FIXNUM) ? (((int)(long)(service))&RUBY_FIXNUM_FLAG) : ((RUBY_T_FIXNUM) == RUBY_T_TRUE) ? ((service) == ((VALUE)RUBY_Qtrue)) : ((RUBY_T_FIXNUM) == RUBY_T_FALSE) ? ((service) == ((VALUE)RUBY_Qfalse)) : ((RUBY_T_FIXNUM) == RUBY_T_NIL) ? ((service) == ((VALUE)RUBY_Qnil)) : ((RUBY_T_FIXNUM) == RUBY_T_UNDEF) ? ((service) == ((VALUE)RUBY_Qundef)) : ((RUBY_T_FIXNUM) == RUBY_T_SYMBOL) ? ((((VALUE)(service)&~((~(VALUE)0)<<RUBY_SPECIAL_SHIFT)) == RUBY_SYMBOL_FLAG)||(!(((VALUE)(service) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(service) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(service))->flags & RUBY_T_MASK) == (RUBY_T_SYMBOL))) : ((RUBY_T_FIXNUM) == RUBY_T_FLOAT) ? ( ((((int)(long)(service))&RUBY_FLONUM_MASK) == RUBY_FLONUM_FLAG) || (!(((VALUE)(service) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(service) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(service))->flags & RUBY_T_MASK) == RUBY_T_FLOAT)) : (!(((VALUE)(service) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(service) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(service))->flags & RUBY_T_MASK) == (RUBY_T_FIXNUM))) && atoi(pbuf) == ((int)rb_fix2int((VALUE)(service))))
                service = ((VALUE)RUBY_Qnil);
        }
    }

    if (( ((RUBY_T_STRING) == RUBY_T_FIXNUM) ? (((int)(long)(node))&RUBY_FIXNUM_FLAG) : ((RUBY_T_STRING) == RUBY_T_TRUE) ? ((node) == ((VALUE)RUBY_Qtrue)) : ((RUBY_T_STRING) == RUBY_T_FALSE) ? ((node) == ((VALUE)RUBY_Qfalse)) : ((RUBY_T_STRING) == RUBY_T_NIL) ? ((node) == ((VALUE)RUBY_Qnil)) : ((RUBY_T_STRING) == RUBY_T_UNDEF) ? ((node) == ((VALUE)RUBY_Qundef)) : ((RUBY_T_STRING) == RUBY_T_SYMBOL) ? ((((VALUE)(node)&~((~(VALUE)0)<<RUBY_SPECIAL_SHIFT)) == RUBY_SYMBOL_FLAG)||(!(((VALUE)(node) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(node) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(node))->flags & RUBY_T_MASK) == (RUBY_T_SYMBOL))) : ((RUBY_T_STRING) == RUBY_T_FLOAT) ? ( ((((int)(long)(node))&RUBY_FLONUM_MASK) == RUBY_FLONUM_FLAG) || (!(((VALUE)(node) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(node) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(node))->flags & RUBY_T_MASK) == RUBY_T_FLOAT)) : (!(((VALUE)(node) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(node) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(node))->flags & RUBY_T_MASK) == (RUBY_T_STRING)))) {
        inspectname = rb_str_dup(node);
    }
    if (( ((RUBY_T_STRING) == RUBY_T_FIXNUM) ? (((int)(long)(service))&RUBY_FIXNUM_FLAG) : ((RUBY_T_STRING) == RUBY_T_TRUE) ? ((service) == ((VALUE)RUBY_Qtrue)) : ((RUBY_T_STRING) == RUBY_T_FALSE) ? ((service) == ((VALUE)RUBY_Qfalse)) : ((RUBY_T_STRING) == RUBY_T_NIL) ? ((service) == ((VALUE)RUBY_Qnil)) : ((RUBY_T_STRING) == RUBY_T_UNDEF) ? ((service) == ((VALUE)RUBY_Qundef)) : ((RUBY_T_STRING) == RUBY_T_SYMBOL) ? ((((VALUE)(service)&~((~(VALUE)0)<<RUBY_SPECIAL_SHIFT)) == RUBY_SYMBOL_FLAG)||(!(((VALUE)(service) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(service) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(service))->flags & RUBY_T_MASK) == (RUBY_T_SYMBOL))) : ((RUBY_T_STRING) == RUBY_T_FLOAT) ? ( ((((int)(long)(service))&RUBY_FLONUM_MASK) == RUBY_FLONUM_FLAG) || (!(((VALUE)(service) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(service) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(service))->flags & RUBY_T_MASK) == RUBY_T_FLOAT)) : (!(((VALUE)(service) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(service) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(service))->flags & RUBY_T_MASK) == (RUBY_T_STRING)))) {
        if (!((VALUE)(inspectname) != ((VALUE)RUBY_Qnil)))
            inspectname = rb_sprintf(":%s", rb_string_value_cstr(&(service)));
        else
            rb_str_catf(inspectname, ":%s", rb_string_value_cstr(&(service)));
    }
    else if (( ((RUBY_T_FIXNUM) == RUBY_T_FIXNUM) ? (((int)(long)(service))&RUBY_FIXNUM_FLAG) : ((RUBY_T_FIXNUM) == RUBY_T_TRUE) ? ((service) == ((VALUE)RUBY_Qtrue)) : ((RUBY_T_FIXNUM) == RUBY_T_FALSE) ? ((service) == ((VALUE)RUBY_Qfalse)) : ((RUBY_T_FIXNUM) == RUBY_T_NIL) ? ((service) == ((VALUE)RUBY_Qnil)) : ((RUBY_T_FIXNUM) == RUBY_T_UNDEF) ? ((service) == ((VALUE)RUBY_Qundef)) : ((RUBY_T_FIXNUM) == RUBY_T_SYMBOL) ? ((((VALUE)(service)&~((~(VALUE)0)<<RUBY_SPECIAL_SHIFT)) == RUBY_SYMBOL_FLAG)||(!(((VALUE)(service) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(service) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(service))->flags & RUBY_T_MASK) == (RUBY_T_SYMBOL))) : ((RUBY_T_FIXNUM) == RUBY_T_FLOAT) ? ( ((((int)(long)(service))&RUBY_FLONUM_MASK) == RUBY_FLONUM_FLAG) || (!(((VALUE)(service) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(service) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(service))->flags & RUBY_T_MASK) == RUBY_T_FLOAT)) : (!(((VALUE)(service) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(service) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(service))->flags & RUBY_T_MASK) == (RUBY_T_FIXNUM))) && ((int)rb_fix2int((VALUE)(service))) != 0)
    {
        if (!((VALUE)(inspectname) != ((VALUE)RUBY_Qnil)))
            inspectname = rb_sprintf(":%d", ((int)rb_fix2int((VALUE)(service))));
        else
            rb_str_catf(inspectname, ":%d", ((int)rb_fix2int((VALUE)(service))));
    }
    if (!!((VALUE)(inspectname) != ((VALUE)RUBY_Qnil))) {
        ( (((!(((VALUE)(inspectname) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(inspectname) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(inspectname))->flags & RUBY_T_MASK) != RUBY_T_NODE) && (int)(((struct RBasic*)(inspectname))->flags & RUBY_T_MASK) != RUBY_T_BIGNUM && (int)(((struct RBasic*)(inspectname))->flags & RUBY_T_MASK) != RUBY_T_FLOAT) && (!(((VALUE)(node) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(node) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(node))->flags & RUBY_T_MASK) != RUBY_T_NODE)) ? (void)(((struct RBasic*)(inspectname))->flags |= ((((struct RBasic*)(node))->flags&(RUBY_FL_TAINT)))) : (void)0);
        ( (((!(((VALUE)(inspectname) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(inspectname) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(inspectname))->flags & RUBY_T_MASK) != RUBY_T_NODE) && (int)(((struct RBasic*)(inspectname))->flags & RUBY_T_MASK) != RUBY_T_BIGNUM && (int)(((struct RBasic*)(inspectname))->flags & RUBY_T_MASK) != RUBY_T_FLOAT) && (!(((VALUE)(service) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(service) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(service))->flags & RUBY_T_MASK) != RUBY_T_NODE)) ? (void)(((struct RBasic*)(inspectname))->flags |= ((((struct RBasic*)(service))->flags&(RUBY_FL_TAINT)))) : (void)0);
        rb_obj_freeze_inline((VALUE)inspectname);
    }
    return inspectname;
}
...
addrinfo_list_new(VALUE node, VALUE service, VALUE family, VALUE socktype, VALUE protocol, VALUE flags)
{
    ...
    for (r = res->ai; r; r = r->ai_next) {
        VALUE addr;
        VALUE canonname = ((VALUE)RUBY_Qnil);

        if (r->ai_canonname) {
            canonname =  ({ (__builtin_constant_p(r->ai_canonname)) ? rb_tainted_str_new((r->ai_canonname), (long)strlen(r->ai_canonname)) : rb_tainted_str_new_cstr(r->ai_canonname); });
            rb_obj_freeze_inline((VALUE)canonname);
        }
...
addrinfo_initialize(int argc, VALUE *argv, VALUE self)
{
    ...
    socklen_t sockaddr_len;
    VALUE canonname = ((VALUE)RUBY_Qnil), inspectname = ((VALUE)RUBY_Qnil);

    if (check_addrinfo(self))
        rb_raise(rb_eTypeError, "already initialized socket address");
    (((struct RData*)(self))->data) = rai = alloc_addrinfo();

    rb_scan_args(argc, argv, "13", &sockaddr_arg, &pfamily, &socktype, &protocol);

    i_pfamily = !((VALUE)(pfamily) != ((VALUE)RUBY_Qnil)) ? 0 : rsock_family_arg(pfamily);
    i_socktype = !((VALUE)(socktype) != ((VALUE)RUBY_Qnil)) ? 0 : rsock_socktype_arg(socktype);
    i_protocol = !((VALUE)(protocol) != ((VALUE)RUBY_Qnil)) ? 0 : rb_num2int_inline(protocol);

    sockaddr_ary = rb_check_array_type(sockaddr_arg);
    if (!!((VALUE)(sockaddr_ary) != ((VALUE)RUBY_Qnil))) {
        VALUE afamily = rb_ary_entry(sockaddr_ary, 0);
        int af;
        rb_string_value(&(afamily));
        if (rsock_family_to_int((!(((struct RBasic*)(afamily))->flags & RSTRING_NOEMBED) ? ((struct RString*)(afamily))->as.ary : ((struct RString*)(afamily))->as.heap.ptr), (!(((struct RBasic*)(afamily))->flags & RSTRING_NOEMBED) ? (long)((((struct RBasic*)(afamily))->flags >> RSTRING_EMBED_LEN_SHIFT) & (RSTRING_EMBED_LEN_MASK >> RSTRING_EMBED_LEN_SHIFT)) : ((struct RString*)(afamily))->as.heap.len), &af) == -1)
     rb_raise(rb_eSocket, "unknown address family: %s", rb_string_value_cstr(&(afamily)));
        switch (af) {
          case 2:
...

          {
            ...
            service = (((VALUE)((int)(rb_num2int_inline(service))))<<1 | RUBY_FIXNUM_FLAG);
            if (!!((VALUE)(nodename) != ((VALUE)RUBY_Qnil)))
                rb_string_value(&(nodename));
            rb_string_value(&(numericnode));
            flags = 0x00000004;
            ...
            init_addrinfo_getaddrinfo(rai, numericnode, service,
                    (((VALUE)((int)(i_pfamily ? i_pfamily : af)))<<1 | RUBY_FIXNUM_FLAG), (((VALUE)((int)(i_socktype)))<<1 | RUBY_FIXNUM_FLAG), (((VALUE)((int)(i_protocol)))<<1 | RUBY_FIXNUM_FLAG),
                    (((VALUE)((int)(flags)))<<1 | RUBY_FIXNUM_FLAG),
                    nodename, service);
            break;
          }
          default:
        ...
        rb_string_value(&(sockaddr_arg));
        sockaddr_ptr = (struct sockaddr *)(!(((struct RBasic*)(sockaddr_arg))->flags & RSTRING_NOEMBED) ? ((struct RString*)(sockaddr_arg))->as.ary : ((struct RString*)(sockaddr_arg))->as.heap.ptr);
        sockaddr_len = (socklen_t)rb_long2int_inline((!(((struct RBasic*)(sockaddr_arg))->flags & RSTRING_NOEMBED) ? (long)((((struct RBasic*)(sockaddr_arg))->flags >> RSTRING_EMBED_LEN_SHIFT) & (RSTRING_EMBED_LEN_MASK >> RSTRING_EMBED_LEN_SHIFT)) : ((struct RString*)(sockaddr_arg))->as.heap.len));
...
rsock_inspect_sockaddr(struct sockaddr *sockaddr_arg, socklen_t socklen, VALUE ret)
{
    ...
    if (socklen == 0) {
         ({ (__builtin_constant_p("empty-sockaddr")) ? rb_str_cat((ret), ("empty-sockaddr"), (long)strlen("empty-sockaddr")) : rb_str_cat_cstr((ret), ("empty-sockaddr")); });
    }
    else if ((long)socklen < ((char*)&sockaddr->addr.sa_family + sizeof(sockaddr->addr.sa_family)) - (char*)sockaddr)
         ({ (__builtin_constant_p("too-short-sockaddr")) ? rb_str_cat((ret), ("too-short-sockaddr"), (long)strlen("too-short-sockaddr")) : rb_str_cat_cstr((ret), ("too-short-sockaddr")); });
    else {
        switch (sockaddr->addr.sa_family) {
          case 0:
   {
      ({ (__builtin_constant_p("UNSPEC")) ? rb_str_cat((ret), ("UNSPEC"), (long)strlen("UNSPEC")) : rb_str_cat_cstr((ret), ("UNSPEC")); });
            break;
   }
    ...
    (int)sizeof(struct sockaddr_in));
          ...
          case 10:
          {
            ...
            if (socklen < (socklen_t)sizeof(struct sockaddr_in6)) {
                ...
                error = getnameinfo__compat(&sockaddr->addr, socklen,
                                    hbuf, (socklen_t)sizeof(hbuf), ((void*)0), 0,
                                    0x00000002|0x00000008);
                if (error) {
                    rsock_raise_socket_error("getnameinfo", error);
                }
                if (addr->sin6_port == 0) {
                     ({ (__builtin_constant_p(hbuf)) ? rb_str_cat((ret), (hbuf), (long)strlen(hbuf)) : rb_str_cat_cstr((ret), (hbuf)); });
                }
                else {
          ...
          case 17:
          {
            ...
            addr = (struct sockaddr_ll *)sockaddr;
             ...
             ({ (__builtin_constant_p("PACKET")) ? rb_str_cat((ret), ("PACKET"), (long)strlen("PACKET")) : rb_str_cat_cstr((ret), ("PACKET")); });

            if (__builtin_offsetof() + sizeof(addr->sll_protocol) <= (size_t)socklen) {
                do {  ({ (__builtin_constant_p(sep)) ? rb_str_cat((ret), (sep), (long)strlen(sep)) : rb_str_cat_cstr((ret), (sep)); }); sep = " "; } while (0);;
                rb_str_catf(ret, "protocol=%d", ntohs(addr->sll_protocol));
            }
            if (__builtin_offsetof() + sizeof(addr->sll_ifindex) <= (size_t)socklen) {
                char buf[IFNAMSIZ];
                do {  ({ (__builtin_constant_p(sep)) ? rb_str_cat((ret), (sep), (long)strlen(sep)) : rb_str_cat_cstr((ret), (sep)); }); sep = " "; } while (0);;
                if (if_indextoname(addr->sll_ifindex, buf) == ((void*)0))
                    rb_str_catf(ret, "ifindex=%d", addr->sll_ifindex);
                else
                    rb_str_catf(ret, "%s", buf);
            }
            if (__builtin_offsetof() + sizeof(addr->sll_hatype) <= (size_t)socklen) {
                do {  ({ (__builtin_constant_p(sep)) ? rb_str_cat((ret), (sep), (long)strlen(sep)) : rb_str_cat_cstr((ret), (sep)); }); sep = " "; } while (0);;
                rb_str_catf(ret, "hatype=%d", addr->sll_hatype);
            }
            if (__builtin_offsetof() + sizeof(addr->sll_pkttype) <= (size_t)socklen) {
                do {  ({ (__builtin_constant_p(sep)) ? rb_str_cat((ret), (sep), (long)strlen(sep)) : rb_str_cat_cstr((ret), (sep)); }); sep = " "; } while (0);;
                if (addr->sll_pkttype == PACKET_HOST)
                     ({ (__builtin_constant_p("HOST")) ? rb_str_cat((ret), ("HOST"), (long)strlen("HOST")) : rb_str_cat_cstr((ret), ("HOST")); });
                else if (addr->sll_pkttype == PACKET_BROADCAST)
                     ({ (__builtin_constant_p("BROADCAST")) ? rb_str_cat((ret), ("BROADCAST"), (long)strlen("BROADCAST")) : rb_str_cat_cstr((ret), ("BROADCAST")); });
                else if (addr->sll_pkttype == PACKET_MULTICAST)
                     ({ (__builtin_constant_p("MULTICAST")) ? rb_str_cat((ret), ("MULTICAST"), (long)strlen("MULTICAST")) : rb_str_cat_cstr((ret), ("MULTICAST")); });
                else if (addr->sll_pkttype == PACKET_OTHERHOST)
                     ({ (__builtin_constant_p("OTHERHOST")) ? rb_str_cat((ret), ("OTHERHOST"), (long)strlen("OTHERHOST")) : rb_str_cat_cstr((ret), ("OTHERHOST")); });
                else if (addr->sll_pkttype == PACKET_OUTGOING)
                     ({ (__builtin_constant_p("OUTGOING")) ? rb_str_cat((ret), ("OUTGOING"), (long)strlen("OUTGOING")) : rb_str_cat_cstr((ret), ("OUTGOING")); });
                else
                    rb_str_catf(ret, "pkttype=%d", addr->sll_pkttype);
            }
            if (socklen != (socklen_t)(__builtin_offsetof() + addr->sll_halen)) {
                do {  ({ (__builtin_constant_p(sep)) ? rb_str_cat((ret), (sep), (long)strlen(sep)) : rb_str_cat_cstr((ret), (sep)); }); sep = " "; } while (0);;
                if (__builtin_offsetof() + sizeof(addr->sll_halen) <= (size_t)socklen) {
                    rb_str_catf(ret, "halen=%d", addr->sll_halen);
                }
            }
            if (__builtin_offsetof() < (size_t)socklen) {
                socklen_t len, i;
                do {  ({ (__builtin_constant_p(sep)) ? rb_str_cat((ret), (sep), (long)strlen(sep)) : rb_str_cat_cstr((ret), (sep)); }); sep = " "; } while (0);;
                 ({ (__builtin_constant_p("hwaddr")) ? rb_str_cat((ret), ("hwaddr"), (long)strlen("hwaddr")) : rb_str_cat_cstr((ret), ("hwaddr")); });
                len = addr->sll_halen;
                if ((size_t)socklen < __builtin_offsetof() + len)
                    len = socklen - __builtin_offsetof();
                for (i = 0; i < len; i++) {
                     ({ (__builtin_constant_p(i == 0 ? "=": ":")) ? rb_str_cat((ret), (i == 0 ? "=": ":"), (long)strlen(i == 0 ? "=": ":")) : rb_str_cat_cstr((ret), (i == 0 ? "=": ":")); });
                    rb_str_catf(ret, "%02x", addr->sll_addr[i]);
                }
            }

            if (socklen < (socklen_t)(__builtin_offsetof() + sizeof(addr->sll_halen)) ||
                (socklen_t)(__builtin_offsetof() + addr->sll_halen) != socklen) {
                do {  ({ (__builtin_constant_p(sep)) ? rb_str_cat((ret), (sep), (long)strlen(sep)) : rb_str_cat_cstr((ret), (sep)); }); sep = " "; } while (0);;
...
addrinfo_inspect(VALUE self)
{
    ...
    internet_p = rai->pfamily == 2;



    if (internet_p && rai->socktype == SOCK_STREAM &&
        (rai->protocol == 0 || rai->protocol == IPPROTO_TCP)) {
         ({ (__builtin_constant_p(" TCP")) ? rb_str_cat((ret), (" TCP"), (long)strlen(" TCP")) : rb_str_cat_cstr((ret), (" TCP")); });
    }
    else if (internet_p && rai->socktype == SOCK_DGRAM &&
        (rai->protocol == 0 || rai->protocol == IPPROTO_UDP)) {
         ({ (__builtin_constant_p(" UDP")) ? rb_str_cat((ret), (" UDP"), (long)strlen(" UDP")) : rb_str_cat_cstr((ret), (" UDP")); });
    }
    else {
        if (rai->socktype) {
    ...
    if (!!((VALUE)(rai->canonname) != ((VALUE)RUBY_Qnil))) {
        VALUE name = rai->canonname;
        rb_str_catf(ret, " %s", rb_string_value_cstr(&(name)));
    }

    if (!!((VALUE)(rai->inspectname) != ((VALUE)RUBY_Qnil))) {
...
{
    return inspect_sockaddr(self,  ({ (__builtin_constant_p("") && __builtin_constant_p(0)) ? rb_str_new_static((""), (0)) : rb_str_new((""), (0)); }));
}
...
addrinfo_mdump(VALUE self)
{
    ...
    if (rai->socktype == 0)
        socktype = (((VALUE)(0))<<1 | RUBY_FIXNUM_FLAG);
    else {
    ...
    if (rai->protocol == 0)
        protocol = (((VALUE)(0))<<1 | RUBY_FIXNUM_FLAG);
    else if (((afamily_int) == 2 || (afamily_int) == 10)) {
    ...
    switch(afamily_int) {
      default:
      {
        char hbuf[1025], pbuf[32];
        int error;
        error = getnameinfo__compat(&rai->addr.addr, rai->sockaddr_len,
                            hbuf, (socklen_t)sizeof(hbuf), pbuf, (socklen_t)sizeof(pbuf),
                            0x00000002|0x00000008);
        if (error) {
            rsock_raise_socket_error("getnameinfo", error);
        }
        sockaddr = rb_assoc_new( ({ (__builtin_constant_p(hbuf)) ? rb_str_new_static((hbuf), (long)strlen(hbuf)) : rb_str_new_cstr(hbuf); }),  ({ (__builtin_constant_p(pbuf)) ? rb_str_new_static((pbuf), (long)strlen(pbuf)) : rb_str_new_cstr(pbuf); }));
        break;
      }
    }

    return  ({ const VALUE args_to_new_ary[] = {afamily, sockaddr, pfamily, socktype, protocol, canonname, inspectname}; if (__builtin_constant_p(7)) { extern int (*__Static_assert_function (void)) [!!sizeof (struct { int __error_if_negative: (((int)(sizeof(args_to_new_ary) / sizeof((args_to_new_ary)[0]))) == (7)) ? 2 : -1; })]; } rb_ary_new_from_values(((int)(sizeof(args_to_new_ary) / sizeof((args_to_new_ary)[0]))), args_to_new_ary); });
}
...
addrinfo_mload(VALUE self, VALUE ary)
{
    ...
    ary = rb_convert_type(ary, RUBY_T_ARRAY, "Array", "to_ary");

    v = rb_ary_entry(ary, 0);
    rb_string_value(&(v));
    if (rsock_family_to_int((!(((struct RBasic*)(v))->flags & RSTRING_NOEMBED) ? ((struct RString*)(v))->as.ary : ((struct RString*)(v))->as.heap.ptr), (!(((struct RBasic*)(v))->flags & RSTRING_NOEMBED) ? (long)((((struct RBasic*)(v))->flags >> RSTRING_EMBED_LEN_SHIFT) & (RSTRING_EMBED_LEN_MASK >> RSTRING_EMBED_LEN_SHIFT)) : ((struct RString*)(v))->as.heap.len), &afamily) == -1)
        rb_raise(rb_eTypeError, "unexpected address family");

    v = rb_ary_entry(ary, 2);
    rb_string_value(&(v));
    if (rsock_family_to_int((!(((struct RBasic*)(v))->flags & RSTRING_NOEMBED) ? ((struct RString*)(v))->as.ary : ((struct RString*)(v))->as.heap.ptr), (!(((struct RBasic*)(v))->flags & RSTRING_NOEMBED) ? (long)((((struct RBasic*)(v))->flags >> RSTRING_EMBED_LEN_SHIFT) & (RSTRING_EMBED_LEN_MASK >> RSTRING_EMBED_LEN_SHIFT)) : ((struct RString*)(v))->as.heap.len), &pfamily) == -1)
        rb_raise(rb_eTypeError, "unexpected protocol family");

    v = rb_ary_entry(ary, 3);
    if (v == (((VALUE)(0))<<1 | RUBY_FIXNUM_FLAG))
        socktype = 0;
    else {
        rb_string_value(&(v));
        if (rsock_socktype_to_int((!(((struct RBasic*)(v))->flags & RSTRING_NOEMBED) ? ((struct RString*)(v))->as.ary : ((struct RString*)(v))->as.heap.ptr), (!(((struct RBasic*)(v))->flags & RSTRING_NOEMBED) ? (long)((((struct RBasic*)(v))->flags >> RSTRING_EMBED_LEN_SHIFT) & (RSTRING_EMBED_LEN_MASK >> RSTRING_EMBED_LEN_SHIFT)) : ((struct RString*)(v))->as.heap.len), &socktype) == -1)
    ...
    if (v == (((VALUE)(0))<<1 | RUBY_FIXNUM_FLAG))
        ...
        rb_string_value(&(v));
        if (((afamily) == 2 || (afamily) == 10)) {
            if (rsock_ipproto_to_int((!(((struct RBasic*)(v))->flags & RSTRING_NOEMBED) ? ((struct RString*)(v))->as.ary : ((struct RString*)(v))->as.heap.ptr), (!(((struct RBasic*)(v))->flags & RSTRING_NOEMBED) ? (long)((((struct RBasic*)(v))->flags >> RSTRING_EMBED_LEN_SHIFT) & (RSTRING_EMBED_LEN_MASK >> RSTRING_EMBED_LEN_SHIFT)) : ((struct RString*)(v))->as.heap.len), &protocol) == -1)
    ...
    switch(afamily) {
      default:
      {
        VALUE pair = rb_convert_type(v, RUBY_T_ARRAY, "Array", "to_ary");
        struct rb_addrinfo *res;
        ...
        res = call_getaddrinfo(rb_ary_entry(pair, 0), rb_ary_entry(pair, 1),
                               (((VALUE)((int)(pfamily)))<<1 | RUBY_FIXNUM_FLAG), (((VALUE)((int)(socktype)))<<1 | RUBY_FIXNUM_FLAG), (((VALUE)((int)(protocol)))<<1 | RUBY_FIXNUM_FLAG),
                               (((VALUE)((int)(flags)))<<1 | RUBY_FIXNUM_FLAG), 1);
...
{
    rb_addrinfo_t *rai = get_addrinfo(self);
    return (((VALUE)((int)(ai_get_afamily(rai))))<<1 | RUBY_FIXNUM_FLAG);
}
static VALUE
addrinfo_pfamily(VALUE self)
{
    rb_addrinfo_t *rai = get_addrinfo(self);
    return (((VALUE)((int)(rai->pfamily)))<<1 | RUBY_FIXNUM_FLAG);
}
static VALUE
addrinfo_socktype(VALUE self)
{
    rb_addrinfo_t *rai = get_addrinfo(self);
    return (((VALUE)((int)(rai->socktype)))<<1 | RUBY_FIXNUM_FLAG);
}
static VALUE
addrinfo_protocol(VALUE self)
{
    rb_addrinfo_t *rai = get_addrinfo(self);
    return (((VALUE)((int)(rai->protocol)))<<1 | RUBY_FIXNUM_FLAG);
}
static VALUE
addrinfo_to_sockaddr(VALUE self)
{
    rb_addrinfo_t *rai = get_addrinfo(self);
    VALUE ret;
    ret =  ({ (__builtin_constant_p((char*)&rai->addr) && __builtin_constant_p(rai->sockaddr_len)) ? rb_str_new_static(((char*)&rai->addr), (rai->sockaddr_len)) : rb_str_new(((char*)&rai->addr), (rai->sockaddr_len)); });
    ( (((!(((VALUE)(ret) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(ret) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(ret))->flags & RUBY_T_MASK) != RUBY_T_NODE) && (int)(((struct RBasic*)(ret))->flags & RUBY_T_MASK) != RUBY_T_BIGNUM && (int)(((struct RBasic*)(ret))->flags & RUBY_T_MASK) != RUBY_T_FLOAT) && (!(((VALUE)(self) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(self) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(self))->flags & RUBY_T_MASK) != RUBY_T_NODE)) ? (void)(((struct RBasic*)(ret))->flags |= ((((struct RBasic*)(self))->flags&(RUBY_FL_TAINT)))) : (void)0);
    return ret;
}
static VALUE
...
addrinfo_ip_p(VALUE self)
{
    ...
    return ((family) == 2 || (family) == 10) ? ((VALUE)RUBY_Qtrue) : ((VALUE)RUBY_Qfalse);
}
static VALUE
addrinfo_ipv4_p(VALUE self)
{
    rb_addrinfo_t *rai = get_addrinfo(self);
    return ai_get_afamily(rai) == 2 ? ((VALUE)RUBY_Qtrue) : ((VALUE)RUBY_Qfalse);
}
static VALUE
addrinfo_ipv6_p(VALUE self)
{
    ...
    rb_addrinfo_t *rai = get_addrinfo(self);
    return ai_get_afamily(rai) == 10 ? ((VALUE)RUBY_Qtrue) : ((VALUE)RUBY_Qfalse);
...
addrinfo_unix_p(VALUE self)
{
    ...
    return ai_get_afamily(rai) == 1 ? ((VALUE)RUBY_Qtrue) : ((VALUE)RUBY_Qfalse);
...
addrinfo_getnameinfo(int argc, VALUE *argv, VALUE self)
{
    ...
    flags = !((VALUE)(vflags) != ((VALUE)RUBY_Qnil)) ? 0 : rb_num2int_inline(vflags);
    ...
    return rb_assoc_new( ({ (__builtin_constant_p(hbuf)) ? rb_str_new_static((hbuf), (long)strlen(hbuf)) : rb_str_new_cstr(hbuf); }),  ({ (__builtin_constant_p(pbuf)) ? rb_str_new_static((pbuf), (long)strlen(pbuf)) : rb_str_new_cstr(pbuf); }));
}
static VALUE
addrinfo_ip_unpack(VALUE self)
{
 ...
 rb_raise(rb_eSocket, "need IPv4 or IPv6 address");
    ...
    vflags = (((VALUE)((int)(0x00000002|0x00000008)))<<1 | RUBY_FIXNUM_FLAG);
    ret = addrinfo_getnameinfo(1, &vflags, self);
    portstr = rb_ary_entry(ret, 1);
    rb_ary_store(ret, 1, (((VALUE)((int)(atoi(rb_string_value_cstr(&(portstr))))))<<1 | RUBY_FIXNUM_FLAG));
    return ret;
}
static VALUE
...
addrinfo_ip_port(VALUE self)
{
 ...
 goto bad_family;
    ...
    return (((VALUE)((int)(port)))<<1 | RUBY_FIXNUM_FLAG);
}
...
{
    uint32_t a;
    if (!extract_in_addr(self, &a)) return ((VALUE)RUBY_Qfalse);
...
addrinfo_s_ip(VALUE self, VALUE host)
{
    VALUE ret;
    rb_addrinfo_t *rai;
    ret = addrinfo_firstonly_new(host, ((VALUE)RUBY_Qnil),
            (((VALUE)((int)(0)))<<1 | RUBY_FIXNUM_FLAG), (((VALUE)(0))<<1 | RUBY_FIXNUM_FLAG), (((VALUE)(0))<<1 | RUBY_FIXNUM_FLAG), (((VALUE)(0))<<1 | RUBY_FIXNUM_FLAG));
...
addrinfo_s_tcp(VALUE self, VALUE host, VALUE port)
{
    return addrinfo_firstonly_new(host, port,
            (((VALUE)((int)(0)))<<1 | RUBY_FIXNUM_FLAG), (((VALUE)((int)(SOCK_STREAM)))<<1 | RUBY_FIXNUM_FLAG), (((VALUE)((int)(IPPROTO_TCP)))<<1 | RUBY_FIXNUM_FLAG), (((VALUE)(0))<<1 | RUBY_FIXNUM_FLAG));
}
static VALUE
addrinfo_s_udp(VALUE self, VALUE host, VALUE port)
{
    return addrinfo_firstonly_new(host, port,
            (((VALUE)((int)(0)))<<1 | RUBY_FIXNUM_FLAG), (((VALUE)((int)(SOCK_DGRAM)))<<1 | RUBY_FIXNUM_FLAG), (((VALUE)((int)(IPPROTO_UDP)))<<1 | RUBY_FIXNUM_FLAG), (((VALUE)(0))<<1 | RUBY_FIXNUM_FLAG));
}
VALUE
rsock_sockaddr_string_value(volatile VALUE *v)
{
    VALUE val = *v;
    if (rb_typeddata_is_kind_of((val), &addrinfo_type)) {
...
rsock_sockaddr_string_value_ptr(volatile VALUE *v)
{
    ...
    return (!(((struct RBasic*)(*v))->flags & RSTRING_NOEMBED) ? ((struct RString*)(*v))->as.ary : ((struct RString*)(*v))->as.heap.ptr);
}
...
rsock_fd_socket_addrinfo(int fd, struct sockaddr *addr, socklen_t len)
{
    ...
    ret = getsockopt(fd, 1, 3, (void*)&socktype, &optlen);
    if (ret == -1) {
        rb_sys_fail("getsockopt(SO_TYPE)");
    }

    return rsock_addrinfo_new(addr, len, family, socktype, 0, ((VALUE)RUBY_Qnil), ((VALUE)RUBY_Qnil));
}
...
rsock_io_socket_addrinfo(VALUE io, struct sockaddr *addr, socklen_t len)
{
    ...
    switch (rb_type((VALUE)(io))) {
      case RUBY_T_FIXNUM:
        return rsock_fd_socket_addrinfo(((int)rb_fix2int((VALUE)(io))), addr, len);

      case RUBY_T_BIGNUM:
        return rsock_fd_socket_addrinfo(rb_num2int_inline(io), addr, len);
