...
static int get_name (const char *, const struct afd *, struct addrinfo **, char *, struct addrinfo *, int);
...
void
freeaddrinfo__compat(struct addrinfo *ai)
{
...
int
getaddrinfo__compat(const char *hostname, const char *servname, const struct addrinfo *hints, struct addrinfo **res)
{
 struct addrinfo sentinel;
 struct addrinfo *top = ((void*)0);
 struct addrinfo *cur;
 int i, error = 0;
 char pton[4];
 struct addrinfo ai;
 ...
 pai->ai_protocol = 0;
 pai->ai_addrlen = 0;
 pai->ai_canonname = ((void*)0);
 pai->ai_addr = ((void*)0);
 pai->ai_next = ((void*)0);
 port = 0;

 if (hostname == ((void*)0) && servname == ((void*)0))
  return 8;
 if (hints) {
  ...
  if (hints->ai_addrlen || hints->ai_canonname ||
   ...
   { error = (12); goto bad; };
  if (hints->ai_flags & ~(0x00000001 | 0x00000002 | 0x00000004 | 0x00000008))
   { error = (3); goto bad; };
 ...
 if (servname) {
  if (str_isnumber(servname)) {
   ...
   port = htons((unsigned short)atoi(servname));
                } else if (pai->ai_flags & 0x00000008) {
                        { error = (8); goto bad; };
  } else {
   struct servent *sp;
   const char *proto;
   ...
   if ((sp = getservbyname((char*)servname, proto)) == ((void*)0))
    { error = (9); goto bad; };
   port = sp->s_port;
   if (pai->ai_socktype == 0)
 ...
 if (hostname == ((void*)0)) {
  ...
  for (afd = &afdl[0]; afd->a_af; afd++) {
   ...
   if (pai->ai_flags & 0x00000001) {
    { char *p; if (((cur->ai_next) = (struct addrinfo *)malloc(sizeof(struct addrinfo) + ((afd)->a_socklen))) == ((void*)0)) { error = 6; goto free; } memcpy((cur->ai_next), pai, sizeof(struct addrinfo)); (cur->ai_next)->ai_addr = (struct sockaddr *)((cur->ai_next) + 1); (cur->ai_next)->ai_family = (afd)->a_af; (cur->ai_next)->ai_addrlen = (afd)->a_socklen; do { struct sockaddr *init_sockaddr_ptr = ((cur->ai_next)->ai_addr); socklen_t init_sockaddr_len = ((afd)->a_socklen); memset(init_sockaddr_ptr, 0, init_sockaddr_len); init_sockaddr_ptr->sa_family = ((afd)->a_af); (void)(init_sockaddr_len); } while (0); ((struct sockinet *)(cur->ai_next)->ai_addr)->si_port = (port); p = (char *)((cur->ai_next)->ai_addr); memcpy(p + (afd)->a_off, (afd->a_addrany), (afd)->a_addrlen);};



   } else {
    { char *p; if (((cur->ai_next) = (struct addrinfo *)malloc(sizeof(struct addrinfo) + ((afd)->a_socklen))) == ((void*)0)) { error = 6; goto free; } memcpy((cur->ai_next), pai, sizeof(struct addrinfo)); (cur->ai_next)->ai_addr = (struct sockaddr *)((cur->ai_next) + 1); (cur->ai_next)->ai_family = (afd)->a_af; (cur->ai_next)->ai_addrlen = (afd)->a_socklen; do { struct sockaddr *init_sockaddr_ptr = ((cur->ai_next)->ai_addr); socklen_t init_sockaddr_len = ((afd)->a_socklen); memset(init_sockaddr_ptr, 0, init_sockaddr_len); init_sockaddr_ptr->sa_family = ((afd)->a_af); (void)(init_sockaddr_len); } while (0); ((struct sockinet *)(cur->ai_next)->ai_addr)->si_port = (port); p = (char *)((cur->ai_next)->ai_addr); memcpy(p + (afd)->a_off, (afd->a_loopback), (afd)->a_addrlen);};
 ...
 for (i = 0; afdl[i].a_af; i++) {
  if (inet_pton(afdl[i].a_af, hostname, pton)) {
   ...
   if (pai->ai_family == afdl[i].a_af ||
       pai->ai_family == 0) {
    if (! (pai->ai_flags & 0x00000002)) {
     { char *p; if (((top) = (struct addrinfo *)malloc(sizeof(struct addrinfo) + ((&afdl[i])->a_socklen))) == ((void*)0)) { error = 6; goto free; } memcpy((top), pai, sizeof(struct addrinfo)); (top)->ai_addr = (struct sockaddr *)((top) + 1); (top)->ai_family = (&afdl[i])->a_af; (top)->ai_addrlen = (&afdl[i])->a_socklen; do { struct sockaddr *init_sockaddr_ptr = ((top)->ai_addr); socklen_t init_sockaddr_len = ((&afdl[i])->a_socklen); memset(init_sockaddr_ptr, 0, init_sockaddr_len); init_sockaddr_ptr->sa_family = ((&afdl[i])->a_af); (void)(init_sockaddr_len); } while (0); ((struct sockinet *)(top)->ai_addr)->si_port = (port); p = (char *)((top)->ai_addr); memcpy(p + (&afdl[i])->a_off, (pton), (&afdl[i])->a_addrlen);};
     goto good;
    }
...
get_name(const char *addr, const struct afd *afd, struct addrinfo **res, char *numaddr, struct addrinfo *pai, int port0)
{
 ...
 hp = gethostbyaddr((char*)addr, afd->a_addrlen, 2);

 if (hp && hp->h_name && hp->h_name[0] && hp->h_addr_list[0]) {
  { char *p; if (((cur) = (struct addrinfo *)malloc(sizeof(struct addrinfo) + ((afd)->a_socklen))) == ((void*)0)) { error = 6; goto free; } memcpy((cur), pai, sizeof(struct addrinfo)); (cur)->ai_addr = (struct sockaddr *)((cur) + 1); (cur)->ai_family = (afd)->a_af; (cur)->ai_addrlen = (afd)->a_socklen; do { struct sockaddr *init_sockaddr_ptr = ((cur)->ai_addr); socklen_t init_sockaddr_len = ((afd)->a_socklen); memset(init_sockaddr_ptr, 0, init_sockaddr_len); init_sockaddr_ptr->sa_family = ((afd)->a_af); (void)(init_sockaddr_len); } while (0); ((struct sockinet *)(cur)->ai_addr)->si_port = (port); p = (char *)((cur)->ai_addr); memcpy(p + (afd)->a_off, (hp->h_addr_list[0]), (afd)->a_addrlen);};
  if (pai->ai_flags & 0x00000002) { if (((cur)->ai_canonname = (char *)malloc(strlen(hp->h_name) + 1)) != ((void*)0)) { strcpy((cur)->ai_canonname, (hp->h_name)); } else { error = 6; goto free; }};
 } else
  { char *p; if (((cur) = (struct addrinfo *)malloc(sizeof(struct addrinfo) + ((afd)->a_socklen))) == ((void*)0)) { error = 6; goto free; } memcpy((cur), pai, sizeof(struct addrinfo)); (cur)->ai_addr = (struct sockaddr *)((cur) + 1); (cur)->ai_family = (afd)->a_af; (cur)->ai_addrlen = (afd)->a_socklen; do { struct sockaddr *init_sockaddr_ptr = ((cur)->ai_addr); socklen_t init_sockaddr_len = ((afd)->a_socklen); memset(init_sockaddr_ptr, 0, init_sockaddr_len); init_sockaddr_ptr->sa_family = ((afd)->a_af); (void)(init_sockaddr_len); } while (0); ((struct sockinet *)(cur)->ai_addr)->si_port = (port); p = (char *)((cur)->ai_addr); memcpy(p + (afd)->a_off, (numaddr), (afd)->a_addrlen);};
...
get_addr(const char *hostname, int af, struct addrinfo **res, struct addrinfo *pai, int port0)
{
 ...
 if (strlen(hostname) >= 1025) { error = (7); goto bad; };
 ...
 if ((hp->h_name == ((void*)0)) || (hp->h_name[0] == 0) ||
     (hp->h_addr_list[0] == ((void*)0)))
  { error = (4); goto bad; };

 for (i = 0; (ap = hp->h_addr_list[i]) != ((void*)0); i++) {
  switch (af) {
  ...
  { char *p; if (((cur->ai_next) = (struct addrinfo *)malloc(sizeof(struct addrinfo) + ((afd)->a_socklen))) == ((void*)0)) { error = 6; goto free; } memcpy((cur->ai_next), pai, sizeof(struct addrinfo)); (cur->ai_next)->ai_addr = (struct sockaddr *)((cur->ai_next) + 1); (cur->ai_next)->ai_family = (afd)->a_af; (cur->ai_next)->ai_addrlen = (afd)->a_socklen; do { struct sockaddr *init_sockaddr_ptr = ((cur->ai_next)->ai_addr); socklen_t init_sockaddr_len = ((afd)->a_socklen); memset(init_sockaddr_ptr, 0, init_sockaddr_len); init_sockaddr_ptr->sa_family = ((afd)->a_af); (void)(init_sockaddr_len); } while (0); ((struct sockinet *)(cur->ai_next)->ai_addr)->si_port = (port); p = (char *)((cur->ai_next)->ai_addr); memcpy(p + (afd)->a_off, (ap), (afd)->a_addrlen);};
  if (cur == &sentinel) {
   top = cur->ai_next;
   if (pai->ai_flags & 0x00000002) { if (((top)->ai_canonname = (char *)malloc(strlen(hp->h_name) + 1)) != ((void*)0)) { strcpy((top)->ai_canonname, (hp->h_name)); } else { error = 6; goto free; }};
  }
  cur = cur->ai_next;
 }
