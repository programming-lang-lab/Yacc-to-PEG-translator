...
static int writer(void *ctx, unsigned char *buffer, size_t size)
{
    ...
    VALUE str =  ({ (__builtin_constant_p((const char *)buffer) && __builtin_constant_p((long)size)) ? rb_enc_str_new_static(((const char *)buffer), ((long)size), (rb_utf8_encoding())) : rb_enc_str_new(((const char *)buffer), ((long)size), (rb_utf8_encoding())); });
    VALUE wrote = rb_funcall(io, id_write, 1, str);
    return (int)rb_num2int_inline(wrote);
}
...
{
    yaml_emitter_t * emitter;
    VALUE obj = ({ VALUE data_struct_obj = rb_data_typed_object_zalloc(klass, sizeof(yaml_emitter_t), &psych_emitter_type); (void)((emitter) = (yaml_emitter_t *)(((struct RData*)(data_struct_obj))->data));; data_struct_obj; });
...
static VALUE initialize(int argc, VALUE *argv, VALUE self)
{
    ...
    ((emitter) = (yaml_emitter_t*)rb_check_typeddata((self), (&psych_emitter_type)));
 ...
 yaml_emitter_set_width(emitter, rb_num2int_inline(line_width));
 yaml_emitter_set_indent(emitter, rb_num2int_inline(indent));
 yaml_emitter_set_canonical(emitter, ((VALUE)RUBY_Qtrue) == canonical ? 1 : 0);
    }
...
static VALUE start_stream(VALUE self, VALUE encoding)
{
    ...
    yaml_event_t event;
    ((emitter) = (yaml_emitter_t*)rb_check_typeddata((self), (&psych_emitter_type)));
    rb_check_type((VALUE)(encoding),(RUBY_T_FIXNUM));

    yaml_stream_start_event_initialize(&event, (yaml_encoding_t)rb_num2int_inline(encoding));
...
{
    yaml_emitter_t * emitter;
    yaml_tag_directive_t * head = ((void*)0);
    yaml_tag_directive_t * tail = ((void*)0);
    yaml_event_t event;
    yaml_version_directive_t version_directive;
    ((emitter) = (yaml_emitter_t*)rb_check_typeddata((self), (&psych_emitter_type)));


    rb_check_type((VALUE)(version),(RUBY_T_ARRAY));

    if(rb_array_len(version) > 0) {
 VALUE major = rb_ary_entry(version, (long)0);
 VALUE minor = rb_ary_entry(version, (long)1);

 version_directive.major = rb_num2int_inline(major);
 version_directive.minor = rb_num2int_inline(minor);
    }

    if(!(((VALUE)(tags) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) {
 long i = 0;
 long len;
 rb_encoding * encoding = rb_utf8_encoding();

 rb_check_type((VALUE)(tags),(RUBY_T_ARRAY));

 len = rb_array_len(tags);
 head = ruby_xcalloc((size_t)len, sizeof(yaml_tag_directive_t));
 tail = head;

 for(i = 0; i < len && i < rb_array_len(tags); i++) {
     VALUE tuple = (rb_array_const_ptr(tags)[i]);
     VALUE name;
     VALUE value;

     rb_check_type((VALUE)(tuple),(RUBY_T_ARRAY));

     if(rb_array_len(tuple) < 2) {
  ruby_xfree(head);
  rb_raise(rb_eRuntimeError, "tag tuple must be of length 2");
     }
     name = (rb_array_const_ptr(tuple)[0]);
     value = (rb_array_const_ptr(tuple)[1]);
     rb_string_value(&(name));
     rb_string_value(&(value));
     name = rb_str_export_to_enc(name, encoding);
     value = rb_str_export_to_enc(value, encoding);

     tail->handle = (yaml_char_t *)rb_string_value_cstr(&(name));
     tail->prefix = (yaml_char_t *)rb_string_value_cstr(&(value));
    ...
    yaml_document_start_event_initialize(
     &event,
     (rb_array_len(version) > 0) ? &version_directive : ((void*)0),
     head,
     tail,
     imp ? 1 : 0
     );
...
static VALUE scalar(
 ...
 VALUE style
    ...
    rb_check_type((VALUE)(value),(RUBY_T_STRING));

    encoding = rb_utf8_encoding();

    value = rb_str_export_to_enc(value, encoding);

    if(!!((VALUE)(anchor) != ((VALUE)RUBY_Qnil))) {
 rb_check_type((VALUE)(anchor),(RUBY_T_STRING));
 anchor = rb_str_export_to_enc(anchor, encoding);
    }

    if(!!((VALUE)(tag) != ((VALUE)RUBY_Qnil))) {
 rb_check_type((VALUE)(tag),(RUBY_T_STRING));
 tag = rb_str_export_to_enc(tag, encoding);
    }

    yaml_scalar_event_initialize(
     &event,
     (yaml_char_t *)(!((VALUE)(anchor) != ((VALUE)RUBY_Qnil)) ? ((void*)0) : rb_string_value_cstr(&(anchor))),
     (yaml_char_t *)(!((VALUE)(tag) != ((VALUE)RUBY_Qnil)) ? ((void*)0) : rb_string_value_cstr(&(tag))),
     (yaml_char_t*)rb_string_value_ptr(&(value)),
     (int)(!(((struct RBasic*)(value))->flags & RSTRING_NOEMBED) ? (long)((((struct RBasic*)(value))->flags >> RSTRING_EMBED_LEN_SHIFT) & (RSTRING_EMBED_LEN_MASK >> RSTRING_EMBED_LEN_SHIFT)) : ((struct RString*)(value))->as.heap.len),
     plain ? 1 : 0,
     quoted ? 1 : 0,
     (yaml_scalar_style_t)rb_num2int_inline(style)
     );
...
static VALUE start_sequence(
 ...
 tag = rb_str_export_to_enc(tag, encoding);
    ...
    yaml_sequence_start_event_initialize(
     ...
     (yaml_char_t *)(!((VALUE)(tag) != ((VALUE)RUBY_Qnil)) ? ((void*)0) : rb_string_value_cstr(&(tag))),
     implicit ? 1 : 0,
     (yaml_sequence_style_t)rb_num2int_inline(style)
     );
...
static VALUE start_mapping(
 ...
 tag = rb_str_export_to_enc(tag, encoding);
    ...
    yaml_mapping_start_event_initialize(
     ...
     (yaml_char_t *)(!((VALUE)(tag) != ((VALUE)RUBY_Qnil)) ? ((void*)0) : rb_string_value_cstr(&(tag))),
     implicit ? 1 : 0,
     (yaml_mapping_style_t)rb_num2int_inline(style)
     );
...
static VALUE set_canonical(VALUE self, VALUE style)
{
    ...
    yaml_emitter_set_canonical(emitter, ((VALUE)RUBY_Qtrue) == style ? 1 : 0);
...
static VALUE canonical(VALUE self)
{
    ...
    return (emitter->canonical == 0) ? ((VALUE)RUBY_Qfalse) : ((VALUE)RUBY_Qtrue);
}
...
static VALUE set_indentation(VALUE self, VALUE level)
{
    ...
    yaml_emitter_set_indent(emitter, rb_num2int_inline(level));
...
static VALUE indentation(VALUE self)
{
    ...
    return (((VALUE)((int)(emitter->best_indent)))<<1 | RUBY_FIXNUM_FLAG);
}
...
static VALUE line_width(VALUE self)
{
    ...
    return (((VALUE)((int)(emitter->best_width)))<<1 | RUBY_FIXNUM_FLAG);
}
...
static VALUE set_line_width(VALUE self, VALUE width)
{
    ...
    yaml_emitter_set_width(emitter, rb_num2int_inline(width));
...
void Init_psych_emitter(void)
{
    ...
    id_io = (__builtin_constant_p("io") ?  ({ static ID rb_intern_id_cache; if (!rb_intern_id_cache) rb_intern_id_cache = rb_intern2((("io")), (long)strlen(("io"))); (ID) rb_intern_id_cache; }) : rb_intern("io"));
    ...
    id_line_width = (__builtin_constant_p("line_width") ?  ({ static ID rb_intern_id_cache; if (!rb_intern_id_cache) rb_intern_id_cache = rb_intern2((("line_width")), (long)strlen(("line_width"))); (ID) rb_intern_id_cache; }) : rb_intern("line_width"));
    id_indentation = (__builtin_constant_p("indentation") ?  ({ static ID rb_intern_id_cache; if (!rb_intern_id_cache) rb_intern_id_cache = rb_intern2((("indentation")), (long)strlen(("indentation"))); (ID) rb_intern_id_cache; }) : rb_intern("indentation"));
    id_canonical = (__builtin_constant_p("canonical") ?  ({ static ID rb_intern_id_cache; if (!rb_intern_id_cache) rb_intern_id_cache = rb_intern2((("canonical")), (long)strlen(("canonical"))); (ID) rb_intern_id_cache; }) : rb_intern("canonical"));
}
