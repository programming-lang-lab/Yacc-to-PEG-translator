...
{
    return rb_attr_get(target, (__builtin_constant_p(rb_string_value_cstr(&(prop))) ?  ({ static ID rb_intern_id_cache; if (!rb_intern_id_cache) rb_intern_id_cache = rb_intern2(((rb_string_value_cstr(&(prop)))), (long)strlen((rb_string_value_cstr(&(prop))))); (ID) rb_intern_id_cache; }) : rb_intern(rb_string_value_cstr(&(prop)))));
}
