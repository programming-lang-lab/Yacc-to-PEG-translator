...
newobj_i(VALUE tpval, void *data)
{
    ...
    const char *path_cstr = !(((VALUE)(path) & (VALUE)~((VALUE)RUBY_Qnil)) == 0) ? make_unique_str(arg->str_table, (!(((struct RBasic*)(path))->flags & RSTRING_NOEMBED) ? ((struct RString*)(path))->as.ary : ((struct RString*)(path))->as.heap.ptr), (!(((struct RBasic*)(path))->flags & RSTRING_NOEMBED) ? (long)((((struct RBasic*)(path))->flags >> RSTRING_EMBED_LEN_SHIFT) & (RSTRING_EMBED_LEN_MASK >> RSTRING_EMBED_LEN_SHIFT)) : ((struct RString*)(path))->as.heap.len)) : 0;
    VALUE class_path = (!(((VALUE)(klass) & (VALUE)~((VALUE)RUBY_Qnil)) == 0) && !(!(!(((VALUE)(klass) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(klass) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(klass))->flags & RUBY_T_MASK) != RUBY_T_NODE) || (((struct RBasic*)(klass))->flags&RUBY_FL_FREEZE))) ? rb_class_path_cached(klass) : ((VALUE)RUBY_Qnil);
    const char *class_path_cstr = !(((VALUE)(class_path) & (VALUE)~((VALUE)RUBY_Qnil)) == 0) ? make_unique_str(arg->str_table, (!(((struct RBasic*)(class_path))->flags & RSTRING_NOEMBED) ? ((struct RString*)(class_path))->as.ary : ((struct RString*)(class_path))->as.heap.ptr), (!(((struct RBasic*)(class_path))->flags & RSTRING_NOEMBED) ? (long)((((struct RBasic*)(class_path))->flags >> RSTRING_EMBED_LEN_SHIFT) & (RSTRING_EMBED_LEN_MASK >> RSTRING_EMBED_LEN_SHIFT)) : ((struct RString*)(class_path))->as.heap.len)) : 0;
 ...
 info = (struct allocation_info *)ruby_xmalloc(sizeof(struct allocation_info));
    ...
    info->living = 1;
    info->flags = ((struct RBasic*)(obj))->flags;
    info->klass = (((struct RBasic*)(obj))->klass);

    info->path = path_cstr;
    info->line = rb_num2int_inline(line);
    info->mid = mid;
...
{
    if (tmp_trace_arg == 0) {
 tmp_trace_arg = ((struct traceobj_arg*)ruby_xmalloc2((size_t)(1),sizeof(struct traceobj_arg)));
...
trace_object_allocations_start(VALUE self)
{
 ...
 if (arg->newobj_trace == 0) {
     arg->newobj_trace = rb_tracepoint_new(0, 0x100000, newobj_i, arg);
     arg->freeobj_trace = rb_tracepoint_new(0, 0x200000, freeobj_i, arg);
 }
...
trace_object_allocations(VALUE self)
{
    ...
    return rb_ensure(rb_yield, ((VALUE)RUBY_Qnil), trace_object_allocations_stop, self);
}
...
object_allocations_reporter_i(st_data_t key, st_data_t val, st_data_t ptr)
{
    ...
    if (!!((VALUE)(info->mid) != ((VALUE)RUBY_Qnil))) {
 VALUE m = rb_sym2str(info->mid);
 fprintf(out, " (%s)", (!(((struct RBasic*)(m))->flags & RSTRING_NOEMBED) ? ((struct RString*)(m))->as.ary : ((struct RString*)(m))->as.heap.ptr));
    }
    fprintf(out, ")\n");
...
allocation_sourcefile(VALUE self, VALUE obj)
{
    ...
    if (info && info->path) {
 return  ({ (__builtin_constant_p(info->path)) ? rb_str_new_static((info->path), (long)strlen(info->path)) : rb_str_new_cstr(info->path); });
    }
    else {
 return ((VALUE)RUBY_Qnil);
    }
...
allocation_sourceline(VALUE self, VALUE obj)
{
    ...
    if (info) {
 return (((VALUE)(info->line))<<1 | RUBY_FIXNUM_FLAG);
    }
    else {
 return ((VALUE)RUBY_Qnil);
    }
...
allocation_class_path(VALUE self, VALUE obj)
{
 ...
 return  ({ (__builtin_constant_p(info->class_path)) ? rb_str_new_static((info->class_path), (long)strlen(info->class_path)) : rb_str_new_cstr(info->class_path); });
    }
    else {
 return ((VALUE)RUBY_Qnil);
    }
...
allocation_generation(VALUE self, VALUE obj)
{
    struct allocation_info *info = lookup_allocation_info(obj);
    if (info) {
 return rb_ulong2num_inline(info->generation);
    }
    else {
 return ((VALUE)RUBY_Qnil);
    }
