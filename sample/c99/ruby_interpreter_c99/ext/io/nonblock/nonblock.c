...
rb_io_nonblock_p(VALUE io)
{
    ...
    if (io_nonblock_mode(fptr->fd) & 04000)
...
rb_io_nonblock_set(VALUE io, VALUE nb)
{
    rb_io_t *fptr;
    rb_io_check_closed((fptr) = ((struct RFile*)(rb_io_taint_check(io)))->fptr);
    if (!(((VALUE)(nb) & (VALUE)~((VALUE)RUBY_Qnil)) == 0))
 rb_io_set_nonblock(fptr);
    else
 io_nonblock_set(fptr->fd, io_nonblock_mode(fptr->fd), !(((VALUE)(nb) & (VALUE)~((VALUE)RUBY_Qnil)) == 0));
    return io;
}
...
{
    int *restore = (int *)arg;
    if (fcntl(restore[0], 4, restore[1]) == -1)
 rb_sys_fail(0);
...
rb_io_nonblock_block(int argc, VALUE *argv, VALUE io)
{
 ...
 nb = !(((VALUE)(v) & (VALUE)~((VALUE)RUBY_Qnil)) == 0);
    }
