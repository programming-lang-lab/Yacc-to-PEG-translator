...
gc_start_end_i(VALUE tpval, void *data)
{
 ...
 fprintf(stderr, "trace: %s\n", rb_tracearg_event_flag(tparg) == 0x400000 ? "gc_start": "gc_end");
    }
...
{
    VALUE tpval;
    ID tp_key = (__builtin_constant_p(tp_str) ?  ({ static ID rb_intern_id_cache; if (!rb_intern_id_cache) rb_intern_id_cache = rb_intern2(((tp_str)), (long)strlen((tp_str))); (ID) rb_intern_id_cache; }) : rb_intern(tp_str));
    ID proc_key = (__builtin_constant_p(proc_str) ?  ({ static ID rb_intern_id_cache; if (!rb_intern_id_cache) rb_intern_id_cache = rb_intern2(((proc_str)), (long)strlen((proc_str))); (ID) rb_intern_id_cache; }) : rb_intern(proc_str));


    if (rb_ivar_defined(module, tp_key) != 0 &&
 !(((VALUE)(tpval = rb_ivar_get(module, tp_key)) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) {
 rb_tracepoint_disable(tpval);
 rb_ivar_set(module, tp_key, ((VALUE)RUBY_Qnil));
 rb_ivar_set(module, proc_key, ((VALUE)RUBY_Qnil));
    }

    if (!(((VALUE)(proc) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) {
...
{
    return set_gc_hook(module, proc, 0x400000,
