...
cmp_1(const void *ap, const void *bp, void *dummy)
{
    ...
    VALUE a =  ({ (__builtin_constant_p(ap) && __builtin_constant_p(d->elsize)) ? rb_enc_str_new_static((ap), (d->elsize), (d->enc)) : rb_enc_str_new((ap), (d->elsize), (d->enc)); });
    VALUE b =  ({ (__builtin_constant_p(bp) && __builtin_constant_p(d->elsize)) ? rb_enc_str_new_static((bp), (d->elsize), (d->enc)) : rb_enc_str_new((bp), (d->elsize), (d->enc)); });
...
bug_str_qsort_bang(int argc, VALUE *argv, VALUE str)
{
    ...
    if (!!((VALUE)(beg) != ((VALUE)RUBY_Qnil)) && (b = rb_num2int_inline(beg)) < 0 && (b += l) < 0) {
 rb_raise(rb_eArgError, "out of bounds");
    }
    if (!!((VALUE)(size) != ((VALUE)RUBY_Qnil)) && (s = rb_num2int_inline(size)) < 0) {
 rb_raise(rb_eArgError, "negative size");
    }
    if (!((VALUE)(len) != ((VALUE)RUBY_Qnil)) ||
 (((n = rb_num2int_inline(len)) < 0) ?
 ...
 n = (l - b) / s;
    ...
    d.elsize = s;
    qsort_r((!(((struct RBasic*)(str))->flags & RSTRING_NOEMBED) ? ((struct RString*)(str))->as.ary : ((struct RString*)(str))->as.heap.ptr) + b, n, s,
