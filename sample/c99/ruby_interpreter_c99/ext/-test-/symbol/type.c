...
static VALUE bug_sym_const_p(VALUE self, VALUE name) { int t; do { t = rb_is_const_id(rb_to_id(name)); } while (0); return (t ? ((VALUE)RUBY_Qtrue) : ((VALUE)RUBY_Qfalse)); } static VALUE bug_sym_class_p(VALUE self, VALUE name) { int t; do { t = rb_is_class_id(rb_to_id(name)); } while (0); return (t ? ((VALUE)RUBY_Qtrue) : ((VALUE)RUBY_Qfalse)); } static VALUE bug_sym_global_p(VALUE self, VALUE name) { int t; do { t = rb_is_global_id(rb_to_id(name)); } while (0); return (t ? ((VALUE)RUBY_Qtrue) : ((VALUE)RUBY_Qfalse)); } static VALUE bug_sym_instance_p(VALUE self, VALUE name) { int t; do { t = rb_is_instance_id(rb_to_id(name)); } while (0); return (t ? ((VALUE)RUBY_Qtrue) : ((VALUE)RUBY_Qfalse)); } static VALUE bug_sym_attrset_p(VALUE self, VALUE name) { int t; do { t = rb_is_attrset_id(rb_to_id(name)); } while (0); return (t ? ((VALUE)RUBY_Qtrue) : ((VALUE)RUBY_Qfalse)); } static VALUE bug_sym_local_p(VALUE self, VALUE name) { int t; do { t = rb_is_local_id(rb_to_id(name)); } while (0); return (t ? ((VALUE)RUBY_Qtrue) : ((VALUE)RUBY_Qfalse)); } static VALUE bug_sym_junk_p(VALUE self, VALUE name) { int t; do { t = rb_is_junk_id(rb_to_id(name)); } while (0); return (t ? ((VALUE)RUBY_Qtrue) : ((VALUE)RUBY_Qfalse)); }
...
{
    return (((VALUE)(sym)&~((~(VALUE)0)<<RUBY_SPECIAL_SHIFT)) == RUBY_SYMBOL_FLAG) ? ((VALUE)RUBY_Qtrue) : ((VALUE)RUBY_Qfalse);
}

static VALUE
bug_dynamic_p(VALUE self, VALUE sym)
{
    return (!(((VALUE)(sym) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(sym) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)(sym))->flags & RUBY_T_MASK) == (RUBY_T_SYMBOL)) ? ((VALUE)RUBY_Qtrue) : ((VALUE)RUBY_Qfalse);
}
void
Init_type(VALUE klass)
{
    rb_define_singleton_method(klass, "const?", bug_sym_const_p, 1); rb_define_singleton_method(klass, "class?", bug_sym_class_p, 1); rb_define_singleton_method(klass, "global?", bug_sym_global_p, 1); rb_define_singleton_method(klass, "instance?", bug_sym_instance_p, 1); rb_define_singleton_method(klass, "attrset?", bug_sym_attrset_p, 1); rb_define_singleton_method(klass, "local?", bug_sym_local_p, 1); rb_define_singleton_method(klass, "junk?", bug_sym_junk_p, 1);;
