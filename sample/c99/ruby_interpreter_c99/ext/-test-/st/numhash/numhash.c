...
{
    return rb_data_typed_object_wrap((klass),(0),(&numhash_type));
}

static VALUE
numhash_init(VALUE self)
{
    st_table *tbl = (st_table *)rb_check_typeddata((VALUE)(self),(&numhash_type));
    if (tbl) st_free_table(tbl);
    (((struct RData*)(self))->data) = st_init_numtable();
    return self;
}
...
numhash_aref(VALUE self, VALUE key)
{
    st_data_t data;
    st_table *tbl = (st_table *)rb_check_typeddata((VALUE)(self),(&numhash_type));
    if (!(((VALUE)(key) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(key) & (VALUE)~((VALUE)RUBY_Qnil)) == 0))) rb_raise(rb_eArgError, "not a special const");
...
numhash_aset(VALUE self, VALUE key, VALUE data)
{
    st_table *tbl = (st_table *)rb_check_typeddata((VALUE)(self),(&numhash_type));
    if (!(((VALUE)(key) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(key) & (VALUE)~((VALUE)RUBY_Qnil)) == 0))) rb_raise(rb_eArgError, "not a special const");
    if (!(((VALUE)(data) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)(data) & (VALUE)~((VALUE)RUBY_Qnil)) == 0))) rb_raise(rb_eArgError, "not a special const");
...
numhash_i(st_data_t key, st_data_t value, st_data_t arg)
{
    ...
    if (ret == ((VALUE)RUBY_Qtrue)) return ST_CHECK;
    return ST_CONTINUE;
}
...
{
    st_table *table = (st_table *)rb_check_typeddata((VALUE)(self),(&numhash_type));
    st_data_t data = (st_data_t)self;
    return st_foreach_check(table, numhash_i, data, data) ? ((VALUE)RUBY_Qtrue) : ((VALUE)RUBY_Qfalse);
}
...
{
    st_table *table = (st_table *)rb_check_typeddata((VALUE)(self),(&numhash_type));
    return rb_ulong2num_inline(table->num_entries);
}
