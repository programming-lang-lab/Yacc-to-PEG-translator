...
force_unpack_check(struct checker *c, st_data_t key, st_data_t val)
{
 ...
 if (c->tbl->bins != ((void*)0)) rb_bug("should be packed\n");


 for (i = 1; i < expect_size; i++)
     st_add_direct(c->tbl, i, i);

 if (c->tbl->bins == ((void*)0)) rb_bug("should be unpacked\n");
    }
...
unp_fec_i(st_data_t key, st_data_t val, st_data_t args, int error)
{
    ...
    if (error) {
 if (c->test == (rb_id2sym((__builtin_constant_p("delete2") ?  ({ static ID rb_intern_id_cache; if (!rb_intern_id_cache) rb_intern_id_cache = rb_intern2((("delete2")), (long)strlen(("delete2"))); (ID) rb_intern_id_cache; }) : rb_intern("delete2")))))
     return ST_STOP;
 ...
 rb_bug("unexpected error");
    ...
    if (c->test == (rb_id2sym((__builtin_constant_p("check") ?  ({ static ID rb_intern_id_cache; if (!rb_intern_id_cache) rb_intern_id_cache = rb_intern2((("check")), (long)strlen(("check"))); (ID) rb_intern_id_cache; }) : rb_intern("check"))))) {
 return ST_CHECK;
    }
    if (c->test == (rb_id2sym((__builtin_constant_p("delete1") ?  ({ static ID rb_intern_id_cache; if (!rb_intern_id_cache) rb_intern_id_cache = rb_intern2((("delete1")), (long)strlen(("delete1"))); (ID) rb_intern_id_cache; }) : rb_intern("delete1"))))) {
 ...
 if (c->nr == 1) {
  ...
  rb_bug("failed to delete\n");
     ...
     if (v != 0) {
  rb_bug("unexpected value deleted: %lu (expected 0)", (VALUE)v);
     }
 }
 return ST_CHECK;
    }

    rb_raise(rb_eArgError, "unexpected arg: %+li\v", c->test);
}
...
unp_fec(VALUE self, VALUE test)
{
    ...
    if (tbl->bins != ((void*)0)) rb_bug("should still be packed\n");
 ...
 rb_bug("mismatched iteration: %lu (expected %lu)\n",
  (VALUE)c.nr, (VALUE)expect_size);
    ...
    if (tbl->bins == ((void*)0)) rb_bug("should be unpacked\n");
...
unp_fe_i(st_data_t key, st_data_t val, st_data_t args, int error)
{
    ...
    force_unpack_check(c, key, val);
    if (c->test == (rb_id2sym((__builtin_constant_p("unpacked") ?  ({ static ID rb_intern_id_cache; if (!rb_intern_id_cache) rb_intern_id_cache = rb_intern2((("unpacked")), (long)strlen(("unpacked"))); (ID) rb_intern_id_cache; }) : rb_intern("unpacked"))))) {
 return ST_CONTINUE;
    }
    else if (c->test == (rb_id2sym((__builtin_constant_p("unpack_delete") ?  ({ static ID rb_intern_id_cache; if (!rb_intern_id_cache) rb_intern_id_cache = rb_intern2((("unpack_delete")), (long)strlen(("unpack_delete"))); (ID) rb_intern_id_cache; }) : rb_intern("unpack_delete"))))) {
 if (c->nr == 1) {
     st_data_t k = 0;
     st_data_t v;
...
unp_fe(VALUE self, VALUE test)
{
    ...
    if (c.test == (rb_id2sym((__builtin_constant_p("unpack_delete") ?  ({ static ID rb_intern_id_cache; if (!rb_intern_id_cache) rb_intern_id_cache = rb_intern2((("unpack_delete")), (long)strlen(("unpack_delete"))); (ID) rb_intern_id_cache; }) : rb_intern("unpack_delete"))))) {
 if (c.nr != 1) {
     rb_bug("mismatched iteration: %lu (expected 1)\n", (VALUE)c.nr);
 }
    }
    else if (c.nr != expect_size) {
 rb_bug("mismatched iteration: %lu (expected %luo)\n",
  (VALUE)c.nr, (VALUE)expect_size);
    }
