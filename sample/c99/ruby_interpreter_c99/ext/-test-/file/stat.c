...
{
    struct stat st;
    if (fstat(rb_num2int_inline(fileno), &st)) rb_sys_fail(0);
...
stat_for_path(VALUE self, VALUE path)
{
    ...
    if (stat((!(((struct RBasic*)(path))->flags & RSTRING_NOEMBED) ? ((struct RString*)(path))->as.ary : ((struct RString*)(path))->as.heap.ptr), &st)) rb_sys_fail(0);
    return rb_stat_new(&st);
}
