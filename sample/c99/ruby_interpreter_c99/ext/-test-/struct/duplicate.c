...
{
    const char *n = !((VALUE)(name) != ((VALUE)RUBY_Qnil)) ? 0 : rb_string_value_cstr(&(name));
    const char *m = rb_string_value_cstr(&(mem));
    return rb_struct_define(n, m, m, ((void*)0));
}

static VALUE
bug_struct_new_duplicate_under(VALUE obj, VALUE name, VALUE mem)
{
    const char *n = rb_string_value_cstr(&(name));
    const char *m = rb_string_value_cstr(&(mem));
    return rb_struct_define_under(obj, n, m, m, ((void*)0));
}
