...
{
    int *p;
    return ({ VALUE data_struct_obj = rb_data_typed_object_zalloc(klass, sizeof(int), &usrmarshal_type); (void)((p) = (int *)(((struct RData*)(data_struct_obj))->data));; data_struct_obj; });
}

static VALUE
usr_init(VALUE self, VALUE val)
{
    int *ptr = rb_check_typeddata((VALUE)(self),(&usrmarshal_type));
    *ptr = rb_num2int_inline(val);
    return self;
}
...
usr_value(VALUE self)
{
    int *ptr = rb_check_typeddata((VALUE)(self),(&usrmarshal_type));
    int val = *ptr;
    return (((VALUE)((int)(val)))<<1 | RUBY_FIXNUM_FLAG);
}
