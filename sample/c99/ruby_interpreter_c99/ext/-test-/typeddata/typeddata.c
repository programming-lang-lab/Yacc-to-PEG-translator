...
{
    char *p;
    return ({ VALUE data_struct_obj = rb_data_typed_object_zalloc(klass, sizeof(char), &test_data); (void)((p) = (char *)(((struct RData*)(data_struct_obj))->data));; data_struct_obj; });
}
...
{
    unsigned long i, n = ((unsigned int)rb_num2uint(num));
