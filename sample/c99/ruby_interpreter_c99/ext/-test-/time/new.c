...
bug_time_s_timespec_new(VALUE klass, VALUE sec, VALUE nsec, VALUE gmtoff)
{
    ...
    return rb_time_timespec_new(&ts, rb_num2int_inline(gmtoff));
}
...
bug_time_s_timespec_now(VALUE klass)
{
    ...
    VALUE v;
    rb_timespec_now(&ts);
    v = rb_Rational(rb_long2num_inline(ts.tv_nsec), rb_long2num_inline(1000000000L));
    return rb_num_coerce_bin(rb_long2num_inline(ts.tv_sec), v, '+');
}
