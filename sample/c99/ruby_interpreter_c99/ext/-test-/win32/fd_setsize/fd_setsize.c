...
test_select(VALUE self)
{
    ...
    ((void) (((&read)->fds_bits)[((sd) / (8 * (int) sizeof (__fd_mask)))] |= ((__fd_mask) (1UL << ((sd) % (8 * (int) sizeof (__fd_mask)))))));
    ((void) (((&write)->fds_bits)[((sd) / (8 * (int) sizeof (__fd_mask)))] |= ((__fd_mask) (1UL << ((sd) % (8 * (int) sizeof (__fd_mask)))))));
    ((void) (((&error)->fds_bits)[((sd) / (8 * (int) sizeof (__fd_mask)))] |= ((__fd_mask) (1UL << ((sd) % (8 * (int) sizeof (__fd_mask)))))));
...
test_fdset(VALUE self)
{
    ...
    for (i = 0; i < 1024 * 2; i++) {
 int sd = socket(AF_INET, SOCK_DGRAM, 0);
 ((void) (((&set)->fds_bits)[((sd) / (8 * (int) sizeof (__fd_mask)))] |= ((__fd_mask) (1UL << ((sd) % (8 * (int) sizeof (__fd_mask)))))));
 if (set.fd_count > 1024) {
