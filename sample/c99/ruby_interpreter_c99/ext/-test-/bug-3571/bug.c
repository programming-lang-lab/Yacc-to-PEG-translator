...
static VALUE
bug_i(VALUE i, VALUE arg, int argc, const VALUE *argv, VALUE blockarg)
{
    rb_notimplement();
    return (rb_id2sym(rb_frame_this_func()));
}

static VALUE
bug_start(VALUE self, VALUE hash)
{
    VALUE ary = rb_ary_new_from_args(1, ((VALUE)RUBY_Qnil));
    rb_block_call(ary, (__builtin_constant_p("map") ?  ({ static ID rb_intern_id_cache; if (!rb_intern_id_cache) rb_intern_id_cache = rb_intern2((("map")), (long)strlen(("map"))); (ID) rb_intern_id_cache; }) : rb_intern("map")), 0, 0, bug_i, self);
    return ary;
}
