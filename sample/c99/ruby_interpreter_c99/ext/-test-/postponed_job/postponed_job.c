...
pjob_callback(void *data)
{
    ...
    rb_check_type((VALUE)(ary),(RUBY_T_ARRAY));

    rb_ary_replace(ary, rb_funcall(((VALUE)RUBY_Qnil), (__builtin_constant_p("caller") ?  ({ static ID rb_intern_id_cache; if (!rb_intern_id_cache) rb_intern_id_cache = rb_intern2((("caller")), (long)strlen(("caller"))); (ID) rb_intern_id_cache; }) : rb_intern("caller")), 0));
}
...
pjob_one_callback(void *data)
{
    ...
    rb_ary_push(ary, (((VALUE)(1))<<1 | RUBY_FIXNUM_FLAG));
}
