...
bubblebabble_str_new(VALUE str_digest)
{
    ...
    if ((9223372036854775807L - 2) / 3 < (digest_len | 1)) {
 rb_raise(rb_eRuntimeError, "digest string too long");
    }

    str =  ({ (__builtin_constant_p(0) && __builtin_constant_p((digest_len | 1) * 3 + 2)) ? rb_str_new_static((0), ((digest_len | 1) * 3 + 2)) : rb_str_new((0), ((digest_len | 1) * 3 + 2)); });
