...
{
    ffi_cif *ptr = p;
    if (ptr->arg_types) ruby_xfree(ptr->arg_types);
    ruby_xfree(ptr);
}
...
allocate(VALUE klass)
{
    ...
    return ({ VALUE data_struct_obj = rb_data_typed_object_zalloc(klass, sizeof(ffi_cif), &function_data_type); (void)((cif) = (ffi_cif *)(((struct RData*)(data_struct_obj))->data));; data_struct_obj; });
}
...
{
    if (key == (rb_id2sym((__builtin_constant_p("name") ?  ({ static ID rb_intern_id_cache; if (!rb_intern_id_cache) rb_intern_id_cache = rb_intern2((("name")), (long)strlen(("name"))); (ID) rb_intern_id_cache; }) : rb_intern("name"))))) {
 rb_iv_set(self, "@name", value);
    } else {
 rb_raise(rb_eArgError, "unknown keyword: %li\v",
   (key));
    }
    return ST_CONTINUE;
}
...
initialize(int argc, VALUE argv[], VALUE self)
{
    ...
    ptr = rb_Integer(ptr);
    cfunc = ((void*)(rb_num2ulong_inline(ptr)));
    (rb_long2num_inline((long)(cfunc)));
    nabi = !((VALUE)(abi) != ((VALUE)RUBY_Qnil)) ? FFI_DEFAULT_ABI : rb_num2int_inline(abi);
    abi = (((VALUE)(nabi))<<1 | RUBY_FIXNUM_FLAG);
    i = rb_num2int_inline(ret_type);
    rtype = int_to_ffi_type(i);
    ret_type = (((VALUE)(i))<<1 | RUBY_FIXNUM_FLAG);

    rb_check_type((VALUE)(args),(RUBY_T_ARRAY));
    len = rb_long2int_inline(rb_array_len(args));
    if ((size_t)(len) < ((18446744073709551615UL) / (sizeof(void *) + sizeof(fiddle_generic)) - 1)) { } else { rb_raise(rb_eTypeError, "args is so large that it can cause integer overflow (%d)", (len)); };
    ary = rb_ary_subseq(args, 0, len);
    for (i = 0; i < rb_array_len(args); i++) {
 VALUE a = ((VALUE *)rb_array_const_ptr(( __builtin_choose_expr( 1, rb_obj_wb_unprotect((VALUE)(args), "ext/fiddle/function.c", 116), ((VALUE)(args))))))[i];
 int type = rb_num2int_inline(a);
 (void)int_to_ffi_type(type);
 if ((((VALUE)(type))<<1 | RUBY_FIXNUM_FLAG) != a) rb_ary_store(ary, i, (((VALUE)(type))<<1 | RUBY_FIXNUM_FLAG));
    }
    ...
    if (!!((VALUE)(kwds) != ((VALUE)RUBY_Qnil))) rb_hash_foreach(kwds, parse_keyword_arg_i, self);

    ((cif) = (ffi_cif*)rb_check_typeddata((self), (&function_data_type)));

    arg_types = ruby_xcalloc(len + 1, sizeof(ffi_type *));

    for (i = 0; i < rb_array_len(args); i++) {
 int type = rb_num2int_inline((rb_array_const_ptr(args)[i]));
 arg_types[i] = int_to_ffi_type(type);
    }
    arg_types[len] = ((void*)0);
...
function_call(int argc, VALUE argv[], VALUE self)
{
    ...
    cPointer = rb_const_get(mFiddle, (__builtin_constant_p("Pointer") ?  ({ static ID rb_intern_id_cache; if (!rb_intern_id_cache) rb_intern_id_cache = rb_intern2((("Pointer")), (long)strlen(("Pointer"))); (ID) rb_intern_id_cache; }) : rb_intern("Pointer")));

    if ((size_t)(argc) < ((18446744073709551615UL) / (sizeof(void *) + sizeof(fiddle_generic)) - 1)) { } else { rb_raise(rb_eTypeError, "number of arguments is so large that it can cause integer overflow (%d)", (argc)); };
    if (argc != (i = rb_long2int_inline(rb_array_len(types)))) {
 rb_error_arity(argc, i, i);
    }

    ((args.cif) = (ffi_cif*)rb_check_typeddata((self), (&function_data_type)));

    if (rb_safe_level() >= 1) {
 for (i = 0; i < argc; i++) {
     VALUE src = argv[i];
     if ((!!((!(((VALUE)((src)) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)((src)) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)((src)))->flags & RUBY_T_MASK) != RUBY_T_NODE)?(((struct RBasic*)(((src))))->flags&((RUBY_FL_TAINT))):0))) {
  rb_raise(rb_eSecurityError, "tainted parameter not allowed");
     }
 }
    }

    generic_args = (((size_t)(argc + 1) * sizeof(void *) + (size_t)argc * sizeof(fiddle_generic)) < 1024 ? ((* ({ volatile VALUE *rb_gc_guarded_ptr = &(alloc_buffer); ; rb_gc_guarded_ptr; })) = 0, __builtin_alloca ((size_t)(argc + 1) * sizeof(void *) + (size_t)argc * sizeof(fiddle_generic))) : rb_alloc_tmp_buffer(&(alloc_buffer), ((size_t)(argc + 1) * sizeof(void *) + (size_t)argc * sizeof(fiddle_generic))));

    args.values = (void **)((char *)generic_args +
       (size_t)argc * sizeof(fiddle_generic));

    for (i = 0; i < argc; i++) {
 VALUE type = (rb_array_const_ptr(types)[i]);
 VALUE src = argv[i];
 int argtype = ((int)rb_fix2int((VALUE)(type)));

 if (argtype == 1) {
     if(!((VALUE)(src) != ((VALUE)RUBY_Qnil))) {
  src = (((VALUE)(0))<<1 | RUBY_FIXNUM_FLAG);
     } else if(cPointer != rb_class_of((VALUE)(src))) {
  src = rb_funcall(cPointer, (__builtin_constant_p("[]") ?  ({ static ID rb_intern_id_cache; if (!rb_intern_id_cache) rb_intern_id_cache = rb_intern2((("[]")), (long)strlen(("[]"))); (ID) rb_intern_id_cache; }) : rb_intern("[]")), 1, src);
     }
     src = rb_Integer(src);
 }

 value_to_generic((argtype), (src), (&generic_args[i]));
 args.values[i] = (void *)&generic_args[i];
    }
    args.values[argc] = ((void*)0);
    args.fn = ((void*)(rb_num2ulong_inline(cfunc)));

    (void)rb_thread_call_without_gvl(nogvl_ffi_call, &args, 0, 0);

    rb_funcall(mFiddle, (__builtin_constant_p("last_error=") ?  ({ static ID rb_intern_id_cache; if (!rb_intern_id_cache) rb_intern_id_cache = rb_intern2((("last_error=")), (long)strlen(("last_error="))); (ID) rb_intern_id_cache; }) : rb_intern("last_error=")), 1, (((VALUE)((int)((*__errno_location ()))))<<1 | RUBY_FIXNUM_FLAG));




    rb_free_tmp_buffer(&(alloc_buffer));

    return generic_to_value((rb_iv_get(self, "@return_type")), (args.retval));
}
...
Init_fiddle_function(void)
{
    ...
    rb_define_const(cFiddleFunction, "DEFAULT", (((VALUE)((int)(FFI_DEFAULT_ABI)))<<1 | RUBY_FIXNUM_FLAG));
