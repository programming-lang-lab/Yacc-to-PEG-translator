...
int main (void)
{
  ...
  arg_types[2] = ((void*)0);
  (void)(!(ffi_prep_cif_var(&cif, FFI_DEFAULT_ABI, 2, 2, &ffi_type_double, arg_types) == FFI_OK) ? (abort(), 1) : 0);
  ...
  values[0] = &firstarg;
  values[1] = &doubles[0];
  ffi_call(&cif, ((void (*)(void))float_va_fn), &resfp, values);
  ...
  arg_types[4] = ((void*)0);
  (void)(!(ffi_prep_cif_var(&cif, FFI_DEFAULT_ABI, 2, 4, &ffi_type_double, arg_types) == FFI_OK) ? (abort(), 1) : 0);
