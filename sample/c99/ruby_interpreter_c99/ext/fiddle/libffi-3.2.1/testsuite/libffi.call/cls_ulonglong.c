...
static void cls_ret_ulonglong_fn(ffi_cif* cif , void* resp,
...
int main (void)
{
  ...
  cl_arg_types[1] = ((void*)0);


  (void)(!(ffi_prep_cif(&cif, FFI_DEFAULT_ABI, 1, &ffi_type_uint64, cl_arg_types) == FFI_OK) ? (abort(), 1) : 0);

  (void)(!(ffi_prep_closure_loc(pcl, &cif, cls_ret_ulonglong_fn, ((void*)0), code) == FFI_OK) ? (abort(), 1) : 0);
