...
int main (void)
{
  ...
  (void)(!(ffi_prep_cif(&cif, FFI_DEFAULT_ABI, 1, &ffi_type_sint8, args) == FFI_OK) ? (abort(), 1) : 0);


  for (sc = (signed char) -127;
       sc < (signed char) 127; sc++)
    {
      ffi_call(&cif, ((void (*)(void))return_sc), &rint, values);
      (void)(!((signed char)rint == sc) ? (abort(), 1) : 0);
    }
  exit(0);
}
