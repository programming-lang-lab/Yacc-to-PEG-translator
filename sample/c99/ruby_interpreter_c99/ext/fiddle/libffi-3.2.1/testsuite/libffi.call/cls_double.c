...
int main (void)
{
  ...
  (void)(!(ffi_prep_cif(&cif, FFI_DEFAULT_ABI, 1, &ffi_type_double, cl_arg_types) == FFI_OK) ? (abort(), 1) : 0);


  (void)(!(ffi_prep_closure_loc(pcl, &cif, cls_ret_double_fn, ((void*)0), code) == FFI_OK) ? (abort(), 1) : 0);
