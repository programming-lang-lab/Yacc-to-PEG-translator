...
int main (void)
{
  ...
  (void)(!(ffi_prep_cif(&cif, FFI_DEFAULT_ABI, 4, &ffi_type_float, args) == FFI_OK) ? (abort(), 1) : 0);
  ...
  ffi_call(&cif, ((void (*)(void))return_fl), &rfl, values);
  printf ("%f vs %f\n", rfl, return_fl(fl1, fl2, in3, fl4));
  (void)(!(rfl == fl1 + fl2 + in3 + fl4) ? (abort(), 1) : 0);
  exit(0);
}
