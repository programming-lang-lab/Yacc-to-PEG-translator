...
int main (void)
{
  ...
  ts9_type_elements[1] = &ffi_type_sint32;
  ts9_type_elements[2] = ((void*)0);

  args[0] = &ts9_type;
  values[0] = &ts9_arg;


  (void)(!(ffi_prep_cif(&cif, FFI_DEFAULT_ABI, 1, &ts9_type, args) == FFI_OK) ? (abort(), 1) : 0);
  ...
  ffi_call(&cif, ((void (*)(void))struct9), ts9_result, values);

  printf ("%g\n", ts9_result->f);
  printf ("%d\n", ts9_result->i);

  (void)(!(ts9_result->f == 5.55f + 1) ? (abort(), 1) : 0);
  (void)(!(ts9_result->i == 5 + 1) ? (abort(), 1) : 0);
