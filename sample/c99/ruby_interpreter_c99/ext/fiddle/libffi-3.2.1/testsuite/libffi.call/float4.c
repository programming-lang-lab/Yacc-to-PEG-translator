...
int main (void)
{
  ...
  (void)(!(ffi_prep_cif(&cif, FFI_DEFAULT_ABI, 1, &ffi_type_double, args) == FFI_OK) ? (abort(), 1) : 0);


  d = 2.2250738585072014e-308 / 2;



  memset(result[1].c, 0xba, sizeof (double));

  ffi_call(&cif, ((void (*)(void))dblit), &result[0].d, values);





  (void)(!(result[0].d == dblit(d)) ? (abort(), 1) : 0);


  for (i = 0; i < sizeof (double); ++i)
    (void)(!(result[1].c[i] == 0xba) ? (abort(), 1) : 0);
