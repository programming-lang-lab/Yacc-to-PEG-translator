...
int main (void)
{
  ...
  (void)(!(ffi_prep_cif(&cif, FFI_DEFAULT_ABI, 1, &ffi_type_sint64, args) == FFI_OK) ? (abort(), 1) : 0);


  for (ll = 0LL; ll < 100LL; ll++)
    {
      ffi_call(&cif, ((void (*)(void))return_ll), &rlonglong, values);
      (void)(!(rlonglong == ll) ? (abort(), 1) : 0);
    }
