...
static void
dummy_fn(ffi_cif* cif , void* resp ,
...
int main (void)
{
 ...
 (void)(!(ffi_prep_cif(&cif, 255, 0, &ffi_type_void, arg_types) == FFI_BAD_ABI) ? (abort(), 1) : 0);


 (void)(!(ffi_prep_cif(&cif, FFI_DEFAULT_ABI, 0, &ffi_type_void, arg_types) == FFI_OK) ? (abort(), 1) : 0);


 cif.abi= 255;

 (void)(!(ffi_prep_closure_loc(pcl, &cif, dummy_fn, ((void*)0), code) == FFI_BAD_ABI) ? (abort(), 1) : 0);
