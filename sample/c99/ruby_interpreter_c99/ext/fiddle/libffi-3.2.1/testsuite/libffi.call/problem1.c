...
void stub(ffi_cif* cif , void* resp, void** args,
   void* userdata )
{
...
int main(void)
{
  ...
  my_ffi_struct_fields[3] = ((void*)0);

  arg_types[0] = &my_ffi_struct_type;
  arg_types[1] = &my_ffi_struct_type;
  arg_types[2] = ((void*)0);

  (void)(!(ffi_prep_cif(&cif, FFI_DEFAULT_ABI, 2, &my_ffi_struct_type, arg_types) == FFI_OK) ? (abort(), 1) : 0);


  args[0] = &g;
  args[1] = &f;
  args[2] = ((void*)0);
  ffi_call(&cif, ((void (*)(void))callee), &res, args);

  printf("res: %g %g %g\n", res.a, res.b, res.c);


  (void)(!(ffi_prep_closure_loc(pcl, &cif, stub, ((void*)0), code) == FFI_OK) ? (abort(), 1) : 0);
