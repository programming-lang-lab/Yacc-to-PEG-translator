...
int main (void)
{
  ...
  (void)(!(ffi_prep_cif(&cif, FFI_DEFAULT_ABI, 4, &ffi_type_double, args) == FFI_OK) ? (abort(), 1) : 0);
  ...
  ffi_call(&cif, ((void (*)(void))return_dbl), &rdbl, values);
  printf ("%f vs %f\n", rdbl, return_dbl(dbl1, dbl2, in3, dbl4));
  (void)(!(rdbl == dbl1 + dbl2 + in3 + dbl4) ? (abort(), 1) : 0);
  exit(0);
}
