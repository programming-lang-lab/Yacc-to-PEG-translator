...
int main (void)
{
  ...
  ts3_type_elements[0] = &ffi_type_sint32;
  ts3_type_elements[1] = ((void*)0);

  args[0] = &ts3_type;
  values[0] = &ts3_arg;


  (void)(!(ffi_prep_cif(&cif, FFI_DEFAULT_ABI, 1, &ts3_type, args) == FFI_OK) ? (abort(), 1) : 0);


  ts3_arg.si = -123;
  compare_value = ts3_arg.si;

  ffi_call(&cif, ((void (*)(void))struct3), ts3_result, values);

  printf ("%d %d\n", ts3_result->si, -(compare_value*2));

  (void)(!(ts3_result->si == -(compare_value*2)) ? (abort(), 1) : 0);
