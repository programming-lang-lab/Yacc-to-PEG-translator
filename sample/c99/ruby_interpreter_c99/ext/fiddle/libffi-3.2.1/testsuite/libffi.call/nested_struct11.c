...
int main (void)
{
  ...
  args[5] = &k;
  args[6] = &k;
  (void)(!(ffi_prep_cif (&cif, FFI_DEFAULT_ABI, 7, &struct_type_c, arg_types) == FFI_OK) ? (abort(), 1) : 0);


  ffi_call (&cif, ((void (*)(void))C_fn), &result, args);


  (void)(!(result.a.a_x == source.a.a_x) ? (abort(), 1) : 0);
  (void)(!(result.a.a_y == source.a.a_y) ? (abort(), 1) : 0);
  (void)(!(result.b.b_x == source.b.b_x) ? (abort(), 1) : 0);
  (void)(!(result.b.b_y == source.b.b_y) ? (abort(), 1) : 0);
  exit (0);
}
