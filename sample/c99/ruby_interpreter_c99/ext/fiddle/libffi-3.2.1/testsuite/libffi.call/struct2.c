...
int main (void)
{
  ...
  ts2_type_elements[2] = ((void*)0);

  args[0] = &ts2_type;
  values[0] = &ts2_arg;


  (void)(!(ffi_prep_cif(&cif, FFI_DEFAULT_ABI, 1, &ts2_type, args) == FFI_OK) ? (abort(), 1) : 0);
  ...
  ffi_call(&cif, ((void (*)(void))struct2), ts2_result, values);

  printf ("%g\n", ts2_result->d1);
  printf ("%g\n", ts2_result->d2);

  (void)(!(ts2_result->d1 == 5.55 - 1) ? (abort(), 1) : 0);
  (void)(!(ts2_result->d2 == 6.66 - 1) ? (abort(), 1) : 0);
