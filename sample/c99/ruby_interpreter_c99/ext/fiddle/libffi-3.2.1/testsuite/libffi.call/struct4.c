...
int main (void)
{
  ...
  ts4_type_elements[0] = &ffi_type_uint32;
  ts4_type_elements[1] = &ffi_type_uint32;
  ts4_type_elements[2] = &ffi_type_uint32;
  ts4_type_elements[3] = ((void*)0);

  args[0] = &ts4_type;
  values[0] = &ts4_arg;


  (void)(!(ffi_prep_cif(&cif, FFI_DEFAULT_ABI, 1, &ts4_type, args) == FFI_OK) ? (abort(), 1) : 0);

  ts4_arg.ui1 = 2;
  ts4_arg.ui2 = 3;
  ts4_arg.ui3 = 4;

  ffi_call (&cif, ((void (*)(void))struct4), ts4_result, values);

  (void)(!(ts4_result->ui3 == 2U * 3U * 4U) ? (abort(), 1) : 0);
