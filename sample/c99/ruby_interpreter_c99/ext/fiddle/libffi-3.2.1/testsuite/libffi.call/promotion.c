...
int main (void)
{
  ...
  args[1] = &ffi_type_sint16;
  args[2] = &ffi_type_uint8;
  args[3] = &ffi_type_uint16;
  values[0] = &sc;
  values[1] = &ss;
  values[2] = &uc;
  values[3] = &us;
  ...
  (void)(!(ffi_prep_cif(&cif, FFI_DEFAULT_ABI, 4, &ffi_type_sint32, args) == FFI_OK) ? (abort(), 1) : 0);
 ...
 for (us = 0; us <= 60000; us += 10000)
   {
     ul++;
     ffi_call(&cif, ((void (*)(void))promotion), &rint, values);
     (void)(!((int)rint == (signed char) sc + (signed short) ss + (unsigned char) uc + (unsigned short) us) ? (abort(), 1) : 0);
