...
int main (void)
{
  ...
  (void)(!(ffi_prep_cif(&cif, FFI_DEFAULT_ABI, 1, &ffi_type_uint8, args) == FFI_OK) ? (abort(), 1) : 0);


  for (uc = (unsigned char) '\x00';
       uc < (unsigned char) '\xff'; uc++)
    {
      ffi_call(&cif, ((void (*)(void))return_uc), &rint, values);
      (void)(!((unsigned char)rint == uc) ? (abort(), 1) : 0);
    }
  exit(0);
}
