...
int main (void)
{
  ...
  ts7_type_elements[3] = ((void*)0);

  args[0] = &ts7_type;
  values[0] = &ts7_arg;


  (void)(!(ffi_prep_cif(&cif, FFI_DEFAULT_ABI, 1, &ts7_type, args) == FFI_OK) ? (abort(), 1) : 0);
  ...
  ffi_call(&cif, ((void (*)(void))struct7), ts7_result, values);

  printf ("%g\n", ts7_result->f1);
  printf ("%g\n", ts7_result->f2);
  printf ("%g\n", ts7_result->d);

  (void)(!(ts7_result->f1 == 5.55f + 1) ? (abort(), 1) : 0);
  (void)(!(ts7_result->f2 == 55.5f + 1) ? (abort(), 1) : 0);
  (void)(!(ts7_result->d == 6.66 + 1) ? (abort(), 1) : 0);
