...
typedef int ( *closure_test_type0)(int, int, int, int);
...
int main (void)
{
  ...
  cl_arg_types[2] = &ffi_type_uint32;
  cl_arg_types[3] = &ffi_type_uint32;
  cl_arg_types[4] = ((void*)0);


  (void)(!(ffi_prep_cif(&cif, FFI_DEFAULT_ABI, 4, &ffi_type_sint32, cl_arg_types) == FFI_OK) ? (abort(), 1) : 0);


  (void)(!(ffi_prep_closure_loc(pcl, &cif, closure_test, (void *) 3 , code) == FFI_OK) ? (abort(), 1) : 0);
