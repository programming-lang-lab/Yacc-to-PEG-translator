...
void ffi_prep_args_pa32(UINT32 *stack, extended_cif *ecif, unsigned bytes)
{
  ...
  do { if (1 <= 0) { printf("%s: stack = %p, ecif = %p, bytes = %u\n", __FUNCTION__, stack, ecif, bytes); } } while (0);
 ...
 case 10:
 case 14:
   do { if (3 <= 0) { printf("Storing UINT32 %u in slot %u\n", *(UINT32 *)(*p_argv), slot); } } while (0);
 ...
 case 2:
   ...
   do { if (3 <= 0) { printf("Storing UINT32(float) in slot %u\n", slot); } } while (0);
   *(UINT32 *)(stack - slot) = *(UINT32 *)(*p_argv);
   switch (slot - 9)
     {
 ...
 case 3:
   ...
   slot += (slot & 1) ? 1 : 2;
   do { if (3 <= 0) { printf("Storing UINT64(double) at slot %u\n", slot); } } while (0);
   *(UINT64 *)(stack - slot) = *(UINT64 *)(*p_argv);
   switch (slot - 9)
     {
 ...
 default:
...

  {
    ...
    do { if (5 <= 0) { printf("Stack setup:\n"); } } while (0);
    for (n = 0; n < (bytes + 3) / 4; n++)
      {
 if ((n%4) == 0) { do { if (5 <= 0) { printf("\n%08x: ", (unsigned int)(stack - n)); } } while (0); }
 do { if (5 <= 0) { printf("%08x ", *(stack - n)); } } while (0);
      }
    do { if (5 <= 0) { printf("\n"); } } while (0);
  }
...
static void ffi_size_stack_pa32(ffi_cif *cif)
{
 ...
 default:
  ...
  if (z <= 6)
    ...
    cif->bytes = 64 + (((size_t)((z - 6) * sizeof(UINT32)) + (64) - 1) & ~((64) - 1));

  do { if (3 <= 0) { printf("Calculated stack size is %u bytes\n", cif->bytes); } } while (0);
}
...
void ffi_call(ffi_cif *cif, void (*fn)(void), void *rvalue, void **avalue)
{
  ...
  switch (cif->abi)
    {
    case FFI_PA32:
      do { if (3 <= 0) { printf("Calling ffi_call_pa32: ecif=%p, bytes=%u, flags=%u, rvalue=%p, fn=%p\n", &ecif, cif->bytes, cif->flags, ecif.rvalue, (void *)fn); } } while (0);
...
ffi_status ffi_closure_inner_pa32(ffi_closure *closure, UINT32 *stack)
{
  ...
  avalue = (void **)__builtin_alloca (cif->nargs * 8);
  avn = cif->nargs;
  p_arg = cif->arg_types;
 ...
 default:
  ...
  do { if (3 <= 0) { printf("after calling function, ret[0] = %08x, ret[1] = %08x\n", ret[0], ret[1]); } } while (0);
  ...
  switch (cif->flags)
    {
    case 5:
      *(stack - 9) = (UINT8)(ret[0] >> 24);
      break;
    case 6:
      *(stack - 9) = (SINT8)(ret[0] >> 24);
      break;
    case 7:
      *(stack - 9) = (UINT16)(ret[0] >> 16);
      break;
    case 8:
      *(stack - 9) = (SINT16)(ret[0] >> 16);
      break;
    case 1:
    case 10:
    case 9:
 ...
 *(stack - 9 - 1) = ret2[1];
    ...
    default:
      do { if (0 <= 0) { printf("assert with cif->flags: %d\n",cif->flags); } } while (0);
                   ;
      break;
    }
  return FFI_OK;
