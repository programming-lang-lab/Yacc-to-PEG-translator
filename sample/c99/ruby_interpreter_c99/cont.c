...
static machine_stack_cache_t machine_stack_cache[10];
...
cont_free(void *ptr)
{
 ...
 if(cont->machine.stack){ruby_xfree(cont->machine.stack);(cont->machine.stack)=((void*)0);};
    }
    else {
 ...
 if (fib->ss_sp != ((void*)0)) {
  ...
  rb_bug("Illegal root fiber parameter");
    ...
    if(cont->saved_vm_stack.ptr){ruby_xfree(cont->saved_vm_stack.ptr);(cont->saved_vm_stack.ptr)=((void*)0);};
...
fiber_memsize(const void *ptr)
{
 ...
 fib->cont.saved_ec.local_storage != ((void*)0)) {
...
cont_save_machine_stack(rb_thread_t *th, rb_context_t *cont)
{
 ...
 cont->machine.stack_src = th->ec->machine.stack_start;
    ...
    if (cont->machine.stack) {
 ((cont->machine.stack)=(VALUE*)ruby_xrealloc2((char*)(cont->machine.stack),(size_t)(size),sizeof(VALUE)));
    }
    else {
 cont->machine.stack = ((VALUE*)ruby_xmalloc2((size_t)(size),sizeof(VALUE)));
    }

    ((void)0);
    memcpy((cont->machine.stack), (cont->machine.stack_src), sizeof(VALUE)*(size_t)(size));
}
...
cont_init(rb_context_t *cont, rb_thread_t *th)
{
    ...
    cont->saved_ec.local_storage = ((void*)0);
    cont->saved_ec.local_storage_recursive_hash = ((VALUE)RUBY_Qnil);
    cont->saved_ec.local_storage_recursive_hash_for_trace = ((VALUE)RUBY_Qnil);
}
...
cont_new(VALUE klass)
{
    ...
    do { if (!(th)->ec->tag) rb_raise(rb_eThreadError, "not running thread"); } while (0);
    contval = ({ VALUE data_struct_obj = rb_data_typed_object_zalloc(klass, sizeof(rb_context_t), &cont_data_type); (void)((cont) = (rb_context_t *)(((struct RData*)(data_struct_obj))->data));; data_struct_obj; });
    cont->self = contval;
    cont_init(cont, th);
    return cont;
}
...
cont_capture(volatile int *volatile stat)
{
    ...
    cont->saved_vm_stack.ptr = ((VALUE*)ruby_xmalloc2((size_t)(cont->saved_vm_stack.slen + cont->saved_vm_stack.clen),sizeof(VALUE)));
    memcpy((cont->saved_vm_stack.ptr), (ec->vm_stack), sizeof(VALUE)*(size_t)(cont->saved_vm_stack.slen));


    memcpy((cont->saved_vm_stack.ptr + cont->saved_vm_stack.slen), ((VALUE*)ec->cfp), sizeof(VALUE)*(size_t)(cont->saved_vm_stack.clen));
    ...
    ec_set_vm_stack(&cont->saved_ec, ((void*)0), 0);
 ...
 for (p=th->ec->ensure_list; p; p=p->next)
     size++;
 entry = cont->ensure_array = ((rb_ensure_entry_t*)ruby_xmalloc2((size_t)(size+1),sizeof(rb_ensure_entry_t)));
 for (p=th->ec->ensure_list; p; p=p->next) {
     if (!p->entry.marker)
 ...
 entry->marker = 0;
    ...
    if (__builtin_setjmp((cont->jmpbuf))) {
 VALUE value;

 ((cont) = (*(int volatile *)&(cont)));
 value = cont->value;
 if (cont->argc == -1) rb_exc_raise(value);
 cont->value = ((VALUE)RUBY_Qnil);
 *stat = 1;
 return value;
    }
    else {
...
cont_restore_thread(rb_context_t *cont)
{
 ...
 rb_execution_context_t *sec = &cont->saved_ec;
 rb_fiber_t *fib = ((void*)0);

 if (sec->fiber_ptr != ((void*)0)) {
     fib = sec->fiber_ptr;
 }
 ...
 memcpy((th->ec->vm_stack), (cont->saved_vm_stack.ptr), sizeof(VALUE)*(size_t)(cont->saved_vm_stack.slen));


 memcpy((th->ec->vm_stack + th->ec->vm_stack_size - cont->saved_vm_stack.clen), (cont->saved_vm_stack.ptr + cont->saved_vm_stack.slen), sizeof(VALUE)*(size_t)(cont->saved_vm_stack.clen));
 ...
 if (th->ec->trace_arg != ((void*)0) && sec->trace_arg == ((void*)0)) {
     rb_current_vm()->trace_running--;
 }

 else if (th->ec->trace_arg == ((void*)0) && sec->trace_arg != ((void*)0)) {
     rb_current_vm()->trace_running++;
 }
...
fiber_machine_stack_alloc(size_t size)
{
 ...
 if (machine_stack_cache[machine_stack_cache_index - 1].size == (size / sizeof(VALUE))) {
     ...
     machine_stack_cache[machine_stack_cache_index].ptr = ((void*)0);
 ...
 (*__errno_location ()) = 0;
 ptr = mmap(((void*)0), size, 0x1 | 0x2, (0x02 | 0x20 | 0x20000), -1, 0);
 if (ptr == ((void *) -1)) {
     rb_raise(rb_eFiberError, "can't alloc machine stack to fiber: %s", strerror((*__errno_location ())));
 }


 page = ptr + ((0));
 if (mprotect(page, (pagesize), 0x0) < 0) {
...
fiber_initialize_machine_stack_context(rb_fiber_t *fib, size_t size)
{
    ...
    context->uc_link = ((void*)0);
    context->uc_stack.ss_sp = ptr;
    context->uc_stack.ss_size = size;
    fib->ss_sp = ptr;
    fib->ss_size = size;
    makecontext(context, rb_fiber_start, 0);
    sec->machine.stack_start = (VALUE*)(ptr + ((size)));
    sec->machine.stack_maxsize = size - (pagesize);
 ...
 static void fiber_setcontext(rb_fiber_t *newfib, rb_fiber_t *oldfib);
...
fiber_setcontext(rb_fiber_t *newfib, rb_fiber_t *oldfib)
{
 ...
 if (((1))) {
    ...
    oldfib->cont.saved_ec.machine.stack_end = ((void*)0);
...
cont_restore_1(rb_context_t *cont)
{
 ...
 ((void)0);
 memcpy((cont->machine.stack_src), (cont->machine.stack), sizeof(VALUE)*(size_t)(cont->machine.stack_size));

    }
    __builtin_longjmp(((cont->jmpbuf)),(1));
}

  static void cont_restore_0(rb_context_t *, VALUE *);
static void
cont_restore_0(rb_context_t *cont, VALUE *addr_in_prev_frame)
{
 ...
 VALUE space[1];
  ...
  volatile VALUE *sp = ((VALUE*)__builtin_alloca (sizeof(VALUE)*(&space[0] - end)));
  space[0] = *sp;
...
void
ruby_register_rollback_func_for_ensure(VALUE (*ensure_func)(), VALUE (*rollback_func)())
{
    st_table **table_p = &rb_current_vm()->ensure_rollback_table;
    if ((__builtin_expect(!!(*table_p == ((void*)0)), 0))) {
...
static inline VALUE
lookup_rollback_func(VALUE (*ensure_func)())
{
    st_table *table = rb_current_vm()->ensure_rollback_table;
    st_data_t val;
...
rollback_ensure_stack(VALUE self,rb_ensure_list_t *current,rb_ensure_entry_t *target)
{
 ...
 cur_size--;
    ...
    while (i--) {
 func = (VALUE (*)()) lookup_rollback_func(target[i].e_proc);
 if ((VALUE)func != ((VALUE)RUBY_Qundef)) {
     (*func)(target[i].data2);
 }
    }
}
...
rb_cont_call(int argc, VALUE *argv, VALUE contval)
{
    rb_context_t *cont;
    rb_thread_t *th = rb_current_thread();
    (((cont)) = (rb_context_t*)rb_check_typeddata(((contval)), (&cont_data_type)));
...
{
    return rb_data_typed_object_wrap((klass),(0),(&fiber_data_type));
}
...
fiber_t_alloc(VALUE fibval)
{
    ...
    if ((((struct RData*)(fibval))->data) != 0) {
 rb_raise(rb_eRuntimeError, "cannot initialize twice");
    }

    do { if (!(th)->ec->tag) rb_raise(rb_eThreadError, "not running thread"); } while (0);
    fib = (((rb_fiber_t*)ruby_xcalloc((size_t)(1),sizeof(rb_fiber_t))));
...
fiber_init(VALUE fibval, VALUE proc)
{
    ...
    rb_execution_context_t *sec = &cont->saved_ec;
    rb_thread_t *cth = rb_current_thread();
    size_t fib_stack_size = cth->vm->default_params.fiber_vm_stack_size / sizeof(VALUE);


    cont->saved_vm_stack.ptr = ((void*)0);
    ec_set_vm_stack(sec, ((void*)0), 0);

    ec_set_vm_stack(sec, ((VALUE*)ruby_xmalloc2((size_t)(fib_stack_size),sizeof(VALUE))), fib_stack_size);
    ...
    sec->tag = ((void*)0);
    sec->local_storage = ((void*)0);
    sec->local_storage_recursive_hash = ((VALUE)RUBY_Qnil);
    sec->local_storage_recursive_hash_for_trace = ((VALUE)RUBY_Qnil);
...
VALUE
rb_fiber_new(VALUE (*func)(), VALUE obj)
{
...
rb_fiber_start(void)
{
 ...
 rb_context_t *cont = &(*(int volatile *)&(fib))->cont;
 int argc;
 const VALUE *argv, args = cont->value;
 (((proc)) = (rb_proc_t*)(((struct RData*)(((fib->first_proc))))->data));
 argv = (argc = cont->argc) > 1 ? rb_array_const_ptr(args) : &args;
 cont->value = ((VALUE)RUBY_Qnil);
 th->ec->errinfo = ((VALUE)RUBY_Qnil);
 th->ec->root_lep = rb_vm_proc_local_ep(fib->first_proc);
 th->ec->root_svar = ((VALUE)RUBY_Qfalse);

 do { const rb_event_flag_t flag_arg_ = (0x1000); if ((__builtin_expect(!!(ruby_vm_event_flags & (flag_arg_)), 0))) { rb_exec_event_hook_orig(th->ec, flag_arg_, th->self, 0, 0, 0, ((VALUE)RUBY_Qnil), 0); } } while (0);
 cont->value = rb_vm_invoke_proc(th->ec, proc, argc, argv, 0);
    }
 ...
 if (state == RUBY_TAG_RAISE || state == RUBY_TAG_FATAL) {
...
rb_threadptr_root_fiber_setup(rb_thread_t *th)
{
    ...
    memset((fib), 0, sizeof(rb_fiber_t)*(size_t)(1));
...
rb_threadptr_root_fiber_release(rb_thread_t *th)
{
 ...
 if (th->ec == ruby_current_execution_context_ptr) {
     ruby_current_execution_context_ptr = ((void*)0);
 }
 th->ec = ((void*)0);
    }
}
...
fiber_store(rb_fiber_t *next_fib, rb_thread_t *th)
{
 ...
 fib = root_fiber_alloc(th);
    ...
    if (((next_fib)->status == FIBER_CREATED)) {
 fiber_initialize_machine_stack_context(next_fib, th->vm->default_params.fiber_machine_stack_size);
    }


    if (((fib)->status == FIBER_RESUMED)) fiber_status_set(fib, FIBER_SUSPENDED);
 ...
 if (machine_stack_cache_index < 10) {
 ...
 terminated_machine_stack.ptr = ((void*)0);
...
fiber_switch(rb_fiber_t *fib, int argc, const VALUE *argv, int is_resume)
{
    ...
    if (th->root_fiber == ((void*)0)) root_fiber_alloc(th);
 ...
 value =  ({ (__builtin_constant_p("dead fiber called")) ? rb_exc_new((rb_eFiberError), ("dead fiber called"), (long)strlen("dead fiber called")) : rb_exc_new_cstr((rb_eFiberError), ("dead fiber called")); });

 if (!((th->ec->fiber_ptr)->status == FIBER_TERMINATED)) {
     rb_exc_raise(value);
     __builtin_unreachable();
 }
 else {
...
{
    rb_fiber_t *fib;
    do { (((fib)) = (rb_fiber_t*)rb_check_typeddata(((fibval)), (&fiber_data_type))); if (!(fib)) rb_raise(rb_eFiberError, "uninitialized fiber"); } while (0);
...
rb_fiber_close(rb_fiber_t *fib)
{
 ...
 ruby_xfree(vm_stack);
    ...
    ec_set_vm_stack(&fib->cont.saved_ec, ((void*)0), 0);
...
rb_fiber_terminate(rb_fiber_t *fib, int need_interrupt)
{
    ...
    fib->ss_sp = ((void*)0);
    fib->context.uc_stack.ss_sp = ((void*)0);
    fib->cont.machine.stack = ((void*)0);
    fib->cont.machine.stack_size = 0;


    ret_fib = return_fiber();
    if (need_interrupt) __atomic_fetch_or(&((&ret_fib->cont.saved_ec)->interrupt_flag), (PENDING_INTERRUPT_MASK), 5);
...
rb_fiber_alive_p(VALUE fibval)
{
    ...
    do { (((fib)) = (rb_fiber_t*)rb_check_typeddata(((fibval)), (&fiber_data_type))); if (!(fib)) rb_raise(rb_eFiberError, "uninitialized fiber"); } while (0);
    return ((fib)->status == FIBER_TERMINATED) ? ((VALUE)RUBY_Qfalse) : ((VALUE)RUBY_Qtrue);
}
static VALUE
...
fiber_to_s(VALUE fibval)
{
    ...
    do { (((fib)) = (rb_fiber_t*)rb_check_typeddata(((fibval)), (&fiber_data_type))); if (!(fib)) rb_raise(rb_eFiberError, "uninitialized fiber"); } while (0);
    ruby_snprintf(status_info, 0x10, " (%s)", fiber_status_name(fib->status));
    if (!rb_obj_is_proc(fib->first_proc)) {
 VALUE str = rb_any_to_s(fibval);
 strlcat(status_info, ">", sizeof(status_info));
 rb_str_set_len(str, (!(((struct RBasic*)(str))->flags & RSTRING_NOEMBED) ? (long)((((struct RBasic*)(str))->flags >> RSTRING_EMBED_LEN_SHIFT) & (RSTRING_EMBED_LEN_MASK >> RSTRING_EMBED_LEN_SHIFT)) : ((struct RString*)(str))->as.heap.len)-1);
  ({ (__builtin_constant_p(status_info)) ? rb_str_cat((str), (status_info), (long)strlen(status_info)) : rb_str_cat_cstr((str), (status_info)); });
 return str;
    }
    (((proc)) = (rb_proc_t*)(((struct RData*)(((fib->first_proc))))->data));
...
ruby_Init_Continuation_body(void)
{
    ...
    rb_undef_method(rb_class_of((VALUE)(rb_cContinuation)), "new");
