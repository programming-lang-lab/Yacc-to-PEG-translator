...
rb_safe_level(void)
{
    return rb_current_execution_context()->safe_level;
}

void
rb_set_safe_level_force(int safe)
{
    rb_current_execution_context()->safe_level = safe;
}
...
safe_getter(void)
{
    return (((VALUE)((int)(rb_safe_level())))<<1 | RUBY_FIXNUM_FLAG);
}
...
safe_setter(VALUE val)
{
    ...
    int level = rb_num2int_inline(val);
...
rb_secure(int level)
{
 ...
 if (caller_name) {
     rb_raise(rb_eSecurityError, "Insecure operation `%li\v' at level %d",
...
rb_insecure_operation(void)
{
    ID caller_name = rb_frame_callee();
    if (caller_name) {
 rb_raise(rb_eSecurityError, "Insecure operation - %li\v",
...
{
    if (rb_safe_level() > 0 && (!!((!(((VALUE)((x)) & RUBY_IMMEDIATE_MASK) || !!(((VALUE)((x)) & (VALUE)~((VALUE)RUBY_Qnil)) == 0)) && (int)(((struct RBasic*)((x)))->flags & RUBY_T_MASK) != RUBY_T_NODE)?(((struct RBasic*)(((x))))->flags&((RUBY_FL_TAINT))):0))) {
